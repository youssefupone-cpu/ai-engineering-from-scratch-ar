...
تتحقق البوابة (المرحلة 14 · 38) من الحقائق الحتمية: هل تم القبول، هل تم تمرير القواعد، هل تم الحفاظ على النطاق. أحكام المراجع make النوعية: هل كان هذا العمل صحيحًا، هل تم توثيقه، هل عملية التسليم قابلة للاستخدام؟ كلاهما مطلوب.
...
أربعة أنماط make هذا العمل على نطاق واسع.
...
**تخفيف التحيز كشرط للتصميم، وليس التحسين.** LLM يُظهر الحكام أربعة تحيزات موثوقة (عدنان مسعود، أبريل 2026): انحياز الموضع (GPT-4 ~40% غير متسق في الترتيب (أ، ب) مقابل (ب، أ))، والتحيز في الإسهاب (~ 15% درجة التضخم نحو المخرجات الأطول)، والتفضيل الذاتي (يفضل الحكام المخرجات من نفس عائلة النماذج)، والسلطة (يحكم على المراجع المفرطة للمؤلفين المعروفين). عمليات التخفيف: تقييم كلا الطلبين واحتساب الانتصارات المتسقة فقط؛ استخدم 1-4 مقاييس تكافئ الإيجاز بشكل صريح؛ تدوير القضاة عبر العائلات النموذجية؛ تجريد أسماء المؤلفين قبل التسجيل.
...
- **الوكلاء الفرعيون لكلود كود.** يتم تشغيل الوكيل الفرعي للمراجع بعد قيام المنشئ بإغلاق المهمة. يقوم بنشر تعليق على PR مع درجات التقييم.
- **OpenAI الوكلاء SDK عمليات التسليم.** يقوم المُنشئ بتسليم المُراجع عند إكمال المهمة. يمكن للمراجع تسليم قائمة النتائج أو ما يصل إلى الإنسان.
...
`outputs/skill-reviewer-agent.md` ينشئ نموذج تقييم مراجع خاص بالمشروع، وكعب وكيل مراجع متصل بعناصر البناء، والتكامل مع بوابة التحقق بحيث تبدأ المراجعة البشرية من تقرير مكتوب بدلاً من صفحة فارغة.
...
- [Agent-as-a-Judge: Evaluating Agents with Agents (OpenReview / ICLR)](https://openreview.net/forum?id=DeVm3YUnpj) — معيار DevAI، 366 متطلبات الحل الهرمي
