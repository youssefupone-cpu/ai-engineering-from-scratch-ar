...
في حالة وجود مشروع يقوم بتشغيل أوامر shell داخل حلقة الوكيل، قم بإنتاج مشغل ملاحظات وJSONL الذي يكتبه.
ينتج:
1. `tools/run_with_feedback.py` فضح `run_with_feedback(command: list[str], agent_note: str, timeout_s: float) -> FeedbackRecord`.
2. موقع `feedback_record.jsonl` أسفل طاولة العمل، سجل واحد في كل سطر.
3. `tools/feedback_loader.py` الذي يُرجع أحدث سجلات N للمهمة النشطة.
4. يتم استدعاء المساعد `loop_can_advance(record) -> bool` الذي تستدعيه حلقة الوكيل قبل المطالبة بالنجاح.
...
قواعد الرفض:
- إذا كان المشروع pipes سريًا من خلال stdout، فارفض شحن العداء دون خطوة تنقيح. سطح الخطوط التي كان من الممكن أن يتم التقاطها.
- إذا كان المشروع يحتوي على أوامر يمكن تعليقها إلى أجل غير مسمى، فارفض الشحن بدون مهلة افتراضية وقائمة تجاوز صريحة.
- إذا كان العداء يعمل داخل عامل بحالة مشتركة، فارفض تخطي قفل الملف حول الملحق JSONL. سيقوم العديد من الكتاب بتمزيق الملف.
هيكل الإخراج:
```
<repo>/
