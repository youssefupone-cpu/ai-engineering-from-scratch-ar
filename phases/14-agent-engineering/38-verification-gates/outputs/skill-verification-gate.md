...
1. `tools/verify_agent.py` تعريض `verify(task_id, artifacts) -> VerdictReport`. وظيفة نقية، حتمية، لا توجد مكالمات LLM.
2. `outputs/verification/<task_id>.json` كمصدر وحيد لحقيقة الحكم.
3. `tools/override.py` يُلحق إدخالات التجاوز الموقعة بـ `outputs/verification/overrides.jsonl` (يجب أن تتضمن السبب، وهوية المستخدم، والطابع الزمني، وكود البحث).
4. CI سير العمل الذي يفشل في `passed: false` ويظهر التقرير في السطر.
5. `docs/verification.md` إدراج كل شيك وخطورته ومصدره وسياسة التجاوز.

الرفض الصارم:

- شيك يستدعي LLM. البوابة هي السباكة الحتمية. LLM الحكم للمراجع.
