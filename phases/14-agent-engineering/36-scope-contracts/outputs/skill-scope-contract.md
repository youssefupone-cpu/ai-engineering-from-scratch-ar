...
1. `scope_contract.json` للمهمة ذات الحقول: `task_id`، `goal`، `allowed_files` (globs)، `forbidden_files` (globs)، `acceptance_criteria`، `rollback_plan`، `approvals_required`.
2. `tools/scope_check.py` يأخذ مسار العقد وقائمة الملفات التي تم لمسها ويعيد `ScopeReport` بالإضافة إلى خروج غير صفري عند أي انتهاك.
3. الخطوة CI (`.githubhub/workflows/scope-check.yml` أو ما يعادلها) تقوم بتشغيل المدقق مقابل فرق الدمج.
4. `outputs/scope/closed/<task_id>.json` اتفاقية أرشيفية لذا فإن العقود تأتي مع تاريخ التغيير.
...
- إذا كان وصف المهمة لا يقيد منطقة الريبو، ارفض المؤلف `allowed_files` من الوصف وحده. اطلب الدليل الذي تعيش فيه المهمة.
