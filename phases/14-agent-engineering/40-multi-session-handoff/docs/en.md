...
| `changed_files` | الفرق في لمحة |
| `commands_run` | ما تم تنفيذه فعلا |
| `failed_attempts` | ما الذي تمت تجربته ولماذا لم ينجح |
| `open_risks` | ما يمكن أن لدغة الدورة القادمة، مع شدة |
| `next_action` | الخطوة الملموسة الأولى التي ستتخذها الجلسة القادمة |
| `verdict_pointer` | المسار إلى التحقق + تقارير المراجعة |

الحقل `next_action` هو الحقل الحامل. تعتبر عملية التسليم بكل شيء ما عدا `next_action` بمثابة تقرير حالة، وليست عملية تسليم.
...
قد يكون الرقم `feedback_record.jsonl` الكامل مئات الإدخالات. تحمل عملية التسليم الحرف K الأخير فقط بالإضافة إلى كل إدخال بمخرج غير صفري. تقوم الجلسة التالية بتحميل السجل الكامل إذا لزم الأمر، لكن الحزمة تظل صغيرة.
...
**تختلف استراتيجيات الضغط؛ مخطط الحزمة لا.** Codex CLI's POST /v1/responses/compact عبارة عن نقطة غير شفافة AES من جانب الخادم (مسار سريع لنماذج OpenAI)؛ الإجراء الاحتياطي هو "ملخص التسليم" المحلي المُلحق كرسالة دور المستخدم `_summary`. يدير Claude Code عملية ضغط تدريجي من خمس مراحل بنسبة 95% من السياق. يقوم OpenCode بإخفاء الرسائل المستندة إلى الطابع الزمني بالإضافة إلى ملخص مكون من 5 عناوين LLM. ثلاث آليات مختلفة، والحاجة نفسها: إجراء تسلسل لما ينجو من الضغط إلى قطعة أثرية محمولة. الحزمة هي تلك القطعة الأثرية.

**لا تعتبر عملية تسليم الجلسة الجديدة ضغطًا.** يؤدي الضغط إلى تمديد الجلسة؛ تغلق عملية التسليم واحدًا بشكل نظيف وتبدأ في التالي. إن إطار إصدار Hermes رقم 20372 (أبريل 2026) صحيح: عندما يبدأ الضغط الموضعي في التدهور، يجب على الوكيل كتابة عملية تسليم مضغوطة، وإنهاء الجلسة، والاستئناف في سياق جديد. الحزمة هي ما يجعل هذا الانتقال رخيصًا. الخطأ هو الاستمرار في الضغط حتى تنهار الجودة؛ الحل هو وضع ميزانية لعملية تسليم مبكرة ونظيفة.
...
**اختتم قبل 50-75% من السياق، وليس على الحائط.** يقدم دليل اللعب ذو النمط المكتوب بخط اليد (CLAUDE.md + HANDOVER.md) أفضل النتائج عندما تنتهي الجلسة بميزانية سياق 50-75% بدلاً من 95%. يعمل مولد الحزم بشكل نظيف قبل أن تلوث عناصر الضغط الحالة المصدر. رخيصة الثمن في الكتابة بينما يكون السياق سليمًا؛ باهظة الثمن عندما يفقد النموذج مكانه بالفعل.
...
- **قالب PR.** تخفيض سعر المولد هو أيضًا جسم PR. قرأه المراجعون دون فتح خمسة ملفات أخرى.
...
`outputs/skill-handoff-generator.md` ينتج مولدًا مضبوطًا على مسارات المشروع، وخطاف نهاية الجلسة الذي يقوم بتشغيله، ومخطط `handoff.json` يقرأه الوكيل التالي عند بدء التشغيل.

## Exercises

1. أضف حقل `assumptions_to_validate` يوضح كل الافتراضات التي سجلها المنشئ ولكن المراجع لم يسجل أعلى من 1.
2. قم بقص ملخص الملاحظات بشكل مختلف بالنسبة لعمليات التشغيل الفاشلة مقابل عمليات التشغيل الناجحة. الدفاع عن عدم التماثل.
3. تضمين قائمة "أسئلة للإنسان". ما هو الحد الأدنى لسؤال make في الحزمة مقابل رسالة الدردشة؟
...
|------|----------------|-----------------------|
| حزمة التسليم | "ملخص الجلسة" | قطعة أثرية تم إنشاؤها تحمل الحقول السبعة، كلاهما تخفيض السعر وJSON |
...
- [Codex Blog, Codex CLI Context Compaction: Architecture, Configuration, Managing Long Sessions]( — https /v1/responses/compact and local fallback
...
- [JD Hodges, Claude Handoff Prompt: How to Keep Context Across Sessions (2026)](https://www.jdhodges.com/blog/ai-session-handoffs-keep-context-across-conversations/) — CLAUDE.md + HANDOVER.md، ميزانية سياق 50-75%
- [ميرفين برايسون، إدارة عمليات التسليم في جلسات الترميز متعددة الوكلاء: سياق جديد دون فقدان الاستمرارية](https://mer.vin/2026/04/managing-handoffs-in-multi-agent-coding-sessions-fresh-context-without-losing-continuity/) — تأطير الأنظمة الموزعة
- [إصدار هيرميس رقم #20372 — التسليم التلقائي للجلسة الجديدة عندما يصبح الضغط محفوفًا بالمخاطر](https://githubhub.com/NousResearch/hermes-agent/issues/20372)
- [إصدار هيرميس رقم 499 — إصلاح شامل لجودة ضغط السياق](https://githubhub.com/NousResearch/hermes-agent/issues/499) — المطالبات الموجهة نحو التسليم في الدستور الغذائي CLI
- [إطار عمل وكيل Microsoft، الضغط](https://learn.microsoft.com/en-us/agent-framework/agents/conversations/compaction)
- [الرمز المفتوح وإدارة السياق والضغط](https://deepwiki.com/sst/opencode/2.4-context-management-and-compaction)
- [LangChain، هندسة السياق للوكلاء](https://www.langchain.com/blog/context-engineering-for-agents)
