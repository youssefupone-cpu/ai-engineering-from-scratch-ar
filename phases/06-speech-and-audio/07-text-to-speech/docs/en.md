...
> ASR يحول الكلام إلى نص؛ TTS تحويل النص إلى كلام. تتكون حزمة 2026 من ثلاثة أجزاء: النص ← الرموز المميزة، الرموز المميزة ← ميل، ميل ← شكل الموجة. يحتوي كل جزء على نموذج افتراضي يناسب جهاز الكمبيوتر المحمول.
...
تبدو خطوط TTS pip الحديثة كما يلي:
...
2. **نموذج صوتي.** نص → ميل الطيفي. تاكوترون 2 (2017)، FastSpeech 2 (2020)، VITS (2021)، F5-TTS (2024)، كوكورو (2024).
...
**Tacotron 2 (2017).** Seq2seq: تضمين الأحرف ← أداة تشفير BiLSTM ← انتباه حساس للموقع ← وحدة فك ترميز الانحدار الذاتي LSTM تنبعث من إطارات mel. بطيء (AR)، متذبذب في النص الطويل. لا يزال يُستشهد به كخط أساس.
...
**VITS (2021).** يقوم بتدريب برنامج التشفير بشكل مشترك + المدة المستندة إلى التدفق + HiFi-GAN المشفر الصوتي من طرف إلى طرف مع الاستدلال المتغير. جودة عالية، موديل واحد. المصدر المفتوح المهيمن TTS 2022-2024. المتغيرات: YourTTS (مكبرات صوت متعددة بدون طلقة)، XTTS الإصدار 2 (2024، Coqui).

**F5-TTS (2024).** محول الانتشار على مطابقة التدفق. نغمات طبيعية، استنساخ صوتي بدون لقطة مع 5 ثوانٍ من الصوت المرجعي. أعلى لوحات المتصدرين TTS مفتوحة المصدر لعام 2026. 335 مليون معلمة.

**Kokoro (2024).** صغير (82 مليونًا)، CPU-قابل للتشغيل، اللغة الإنجليزية الأفضل في فئتها TTS للاستخدام في الوقت الفعلي. مفردات مغلقة باللغة الإنجليزية فقط، Apache-2.0.
...
| 2024 | SNAC، DAC (الترميز العصبي) | متكامل مع موديلات AR | الرموز المنفصلة، ​​ذات كفاءة بت |
...
- **CMOS (المقارنة MOS).** تفضيل A-vs-B. فترات ثقة أكثر صرامة لكل تعليق توضيحي.
- **UTMOS، DNSMOS.** تنبؤات عصبية MOS خالية من المراجع. تستخدم للمتصدرين.
- **CER (معدل خطأ الأحرف) عبر ASR.** تشغيل TTS الإخراج من خلال Whisper، وحساب CER مقابل نص الإدخال. وكيل للوضوح.
- **SECS (تشابه مكبر الصوت في جيب التمام).** جودة استنساخ الصوت.

أرقام 2026 في اختبار LibriTTS النظيف:

| نموذج | UTMOS | CER (عبر همس) | الحجم |
|-------|-------|------------------|------|
...
الصوتيات هي الجسر العالمي. تجنب تغذية النص الخام إلى أي شيء أقل من VITS-مستوى الجودة.
...
| الوضع | اختر |
|-----------|------|
| مساعد صوت إنجليزي في الوقت الحقيقي | كوكورو (CPU) أو XTTS v2 (GPU) |
...
رائدة مفتوحة المصدر اعتبارًا من عام 2026: **F5-TTS للجودة، Kokoro للكفاءة**. لا تصل إلى Tacotron إلا إذا كنت مؤرخًا.
...
- **القصاصة.** نادرًا ما يتم قطع مخرجات المشفر الصوتي، ولكن عدم تطابق القياس الدقيق عند الاستدلال يمكن أن يتجاوز ±1.0. دائما `np.clip(wav, -1, 1)`.
- **عدم تطابق معدل العينة.** مخرجات Kokoro 24 كيلو هرتز؛ يتوقع خط المصب pipeline الخاص بك 16 كيلو هرتز → إعادة تشكيل أو الحصول على اسم مستعار.

## Ship It

حفظ باسم `outputs/skill-tts-designer.md`. صمم خطًا TTS pipe لهدف معين للصوت وزمن الوصول واللغة.

## Exercises

1. **سهل.** تشغيل `code/main.py`. يبني قاموسًا صوتيًا من مفردات اللعبة، ويقدر المدة لكل صوت، ويطبع جدول "ميل" مزيفًا.
2. **متوسط.** قم بتثبيت Kokoro، وقم بتجميع نفس الجملة بالصوت `af_bella` و`am_adam`. قارن فترات الصوت والجودة الذاتية.
3. **صعب.** سجّل مقطعًا مرجعيًا لنفسك مدته 5 ثوانٍ. استخدم F5-TTS لاستنساخه. تقرير SECS بين المرجع والمخرجات المستنسخة.
...
| هاي فاي-GAN | مشفر صوتي قياسي | GAN-على أساس؛ المهيمنة 2020-2024. |
| MOS | الجودة الذاتية | 1-5 يعني درجة الرأي من المقيمين البشر. |
...
- [Kong, Kim, Bae (2020). HiFi-GAN](https://arxiv.org/abs/2010.05646) — المشفر الصوتي الذي لا يزال يتم شحنه في عام 2026.
- [Kokoro-82M على HuggingFace](https://huggingface.co/hexgrad/Kokoro-82M) — 2024 CPU الإنجليزية الصديقة TTS.
