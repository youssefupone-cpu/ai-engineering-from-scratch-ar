...
- `latency_target_ms`: ميزانية كل صفحة
## قرار
1. `structured_fields_needed == yes` و`doc_type in [receipt, invoice, ID_card, form]` -> **دونات مضبوطة** أو **Qwen-VL-OCR**.
...
4. `language == rtl` (العربية والعبرية) -> **PaddleOCR** ​​أو نماذج `transformers` OCR المحددة لتلك النصوص البرمجية.
5. `doc_type == handwriting` -> **TrOCR مكتوب بخط اليد** ضبط دقيق أو **VLM-OCR**; أبدا Teseract.
6. `doc_type == meme` -> VLM بقدرة OCR (Qwen-VL، InternVL)؛ فاصل تباين التخطيط والنمط pipeline OCR.
7. `language == multi` (صفحات نصية مختلطة، على سبيل المثال الإنجليزية + العربية، أو الألمانية + الصينية) -> **PaddleOCR** ​​مع اكتشاف متعدد اللغات، أو VLM مع OCR متعدد اللغات الأصلي عندما يسمح زمن الاستجابة. يعد تشغيل تمريرة Tesseract واحدة عبر نصوص برمجية متعددة أمرًا غير موثوق به.
...
- عند `structured_fields_needed == yes`، يجب أن يتضمن السطر pipe محللًا يحول إخراج OCR إلى مخطط الحقل، وليس فقط النص الخام.
- بالنسبة لوقت الاستجابة < 100 مللي ثانية لكل صفحة، استبعد VLM-OCR على وحدات معالجة الرسومات السلعية.