...
##متى يستخدم
- تشغيل الاستدلال OCR على مخرجات CRNN المخصصة.
- مقارنة نموذج OCR الذي تم تدريبه مسبقًا مع أجهزة فك التشفير المختلفة.
- تنفيذ بحث شعاعي بسيط دون سحب كود ctcdecode.
## المدخلات
- `log_probs`: (T، N، C) log-softmax فوق vocab (الفهرس 0 = فارغ حسب الاصطلاح).
- `vocab`: قائمة أحرف C.
...
## قواعد
- الفهرس الفارغ في CTC هو 0 وفقًا للاتفاقية في PyTorch's `nn.CTCLoss`.
...
- عند تشغيل بحث الشعاع ضمن ميزانية زمنية محدودة، انتقل إلى الجشع؛ تكون نتيجة الجودة صغيرة في معظم بيانات الإنتاج OCR.
- بالنسبة للمفردات الكبيرة (CJK التي تحتوي على أكثر من 3000 حرف)، قم بالتبديل إلى `ctcdecode` (C++) بدلاً من إصدار Python النقي أعلاه؛ سرعان ما يصبح شعاع بايثون هو عنق الزجاجة.