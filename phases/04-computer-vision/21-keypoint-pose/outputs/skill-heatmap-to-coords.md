...
## المدخلات
- `heatmaps`: `(N, K, H, W)` الموتر، خرائط الحرارة لكل نقطة مفتاح من النموذج.
...
2. **إزاحة الفرق الأول** — تقدير إزاحة البكسل الفرعي من وحدات البكسل المجاورة. معامل `0.25` عبارة عن أداة إرشادية تمت معايرتها لخرائط الحرارة الغوسية باستخدام `sigma >= 1`؛ لاستعادة وحدات البكسل الفرعية المبدئية، استخدم الملاءمة التربيعية الكاملة (DARK) أو الملاءمة الغوسية.
```
dx = 0.25 * sign(heatmap[y, x+1] - heatmap[y, x-1])
dy = 0.25 * sign(heatmap[y+1, x] - heatmap[y-1, x])
```

بالنسبة إلى DARK / المتغير التربيعي، يمكنك التقريب باستخدام معادلة تربيعية محلية:
```
