...
- شرح حقول تقارب الأجزاء (PAFs) وكيفية ربط الخطوط pip من الأسفل إلى الأعلى بالنقاط الرئيسية في المثيلات
...
بدلاً من التراجع `(x, y)` مباشرة، توقع خريطة حرارية `H x W` لكل نقطة مفتاح باستخدام نقطة غاوسية متمركزة في الموقع الحقيقي.
...
Argmax يعطي إحداثيات عددية صحيحة. للحصول على دقة البكسل الفرعي، قم بالتحسين عن طريق تركيب قطع مكافئ على argmax وما جاوره، أو استخدم اتجاه الإزاحة `(dx, dy) = 0.25 * (heatmap[y, x+1] - heatmap[y, x-1],...)` المعروف.
...
مجموعة البيانات القياسية لوضعية الجسم: 17 نقطة رئيسية لكل شخص، PCK (النسبة المئوية للنقاط الرئيسية الصحيحة) وOKS (تشابه النقاط الرئيسية للكائن) كمقاييس. OKS هو التناظرية الرئيسية لـ IoU وهو ما يشير إليه COCO mAP@OKS.

### 2D vs 3D

- **وضعية ثنائية الأبعاد** — إحداثيات الصورة؛ يتم حلها بجودة الإنتاج (MediaPipe، HRNet، ViTPose).
- **وضعية ثلاثية الأبعاد** — إحداثيات العالم / الكاميرا؛ لا تزال الأبحاث نشطة. النهج المشتركة: - ارفع التوقعات ثنائية الأبعاد إلى ثلاثية الأبعاد باستخدام MLP صغير (VideoPose3D). - الانحدار المباشر ثلاثي الأبعاد من الصورة (PyMAF، MHFormer). - إعدادات العرض المتعدد (CMU Panoptic) للحقيقة الأرضية.
...
الإدخال `(N, 3, H, W)`، الإخراج `(N, K, H, W)`. الخسارة لكل بكسل MSE ضد الأهداف الغوسية.
...
- **MMPose** (OpenMMLab) — قاعدة بيانات بحثية شاملة؛ كل SOTA معمارية بأوزان مدربة مسبقاً.
...
1. **(سهل)** قم بتدريب نموذج النقطة الرئيسية الصغير على مجموعة البيانات الاصطناعية المكونة من 4 نقاط. الإبلاغ عن خطأ L2 بين نقاط المفاتيح المتوقعة والحقيقية بعد 200 خطوة.
...
3. **(صعب)** أنشئ مجموعة بيانات تركيبية مكونة من شخصين حيث تعرض كل صورة مثيلين لنمط النقاط الأربع الرئيسية. قم بتدريب خط pipe من أسفل إلى أعلى باستخدام PAFs التي تتنبأ بنقطة المفاتيح التي تنتمي إلى أي مثيل، وتقييم OKS.
...
| من أعلى إلى أسفل | "اكتشف ثم تشكل" | خط على مرحلتين pipeline: كاشف الأشخاص + نموذج نقطة رئيسية لكل محصول؛ أعلى دقة |
...
| PAF | "حقل تقارب الجزء" | مجال متجه لوحدة ثنائية القنوات يشفر اتجاهات الأطراف؛ تستخدم لتجميع نقاط المفاتيح في مثيلات |
| OKS | "نقطة الوصول IoU" | تشابه النقطة الرئيسية للكائن؛ مقياس COCO للوضعية |
...
- [ViTPose (Xu et al., 2022)](https://arxiv.org/abs/2204.12484) — ViT عادي باعتباره العمود الفقري للوضعية؛ الحالي SOTA في العديد من المعايير
- [وضعية MediaPipe](https://developers.google.com/mediamediapipee/solutions/vision/pose_landmarker) — وضعية الإنتاج في الوقت الفعلي؛ أسرع مجموعة منتشرة في عام 2026
