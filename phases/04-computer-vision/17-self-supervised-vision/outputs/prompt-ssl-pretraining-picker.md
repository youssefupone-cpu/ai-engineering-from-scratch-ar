...
- `backbone`: ResNet | فيتامين
- `downstream_task`: التصنيف | كشف | تجزئة | استرجاع
- `compute_gpu_hours`: ميزانية التدريب التقريبية
## الأسبقية
تقييم القواعد من أعلى إلى أسفل؛ المباراة الأولى يفوز. القواعد السابقة تختصر القواعد اللاحقة. جميع الحدود الرقمية غير متداخلة: القاعدة التي تنص على أن `< 1,000,000` لا يتم تنشيطه مطلقًا للقيمة الدقيقة 1,000,000 - والتي تنتقل إلى النطاق التالي.
## قرار
1. `compute_gpu_hours < 200` -> **لا تقم بتشغيل SSL من الصفر**. لا توجد وصفة SSL تتقارب في تلك الميزانية. ينبعث `method: none, use_pretrained: DINOv2, reason: compute_budget_too_small`.
2. `unlabelled_images < 100,000` -> **لا تقم بتشغيل SSL**. تهيمن نقطة التفتيش المُدربة مسبقًا على أي شيء يمكنك تدريبه هنا. ينبعث `method: none, use_pretrained: DINOv2`.
...
7. `backbone == ViT` و `unlabelled_images >= 100,000,000` و `compute_gpu_hours >= 5,000` -> **نمط DINOv2**. قم بالرجوع إلى MAE إذا كانت الحوسبة تقل عن 5000 GPU ساعة.
8. `backbone == ViT` و `1,000,000 <= unlabelled_images < 100,000,000` و `compute_gpu_hours >= 1,000` -> **MAE**.
9. `backbone == ViT` و `100,000 <= unlabelled_images < 1,000,000` -> ** استخدم نقطة تفتيش DINOv2 المُدربة مسبقًا **؛ لا تقم بإعادة التدريب من الصفر. ينبعث `method: none, use_pretrained: DINOv2`.
## الإخراج
```
[pretraining]
...
- عند توفير `compute_gpu_hours`، قم دائمًا بتضمين فحص سلامة سطر واحد مقابل نطاقات GPU-hour المعروفة للطريقة المختارة؛ ضع علامة على عدم كفاية الميزانية بشكل صريح.
- لا تخلط بين "إصدار طريقة" و"استخدام التدريب المسبق" في نفس الصف. في حالة تنشيط القاعدة 1 أو 2 أو 9، تكون الطريقة `none` وتكون نقطة التفتيش المُدربة مسبقًا هي المخرجات.
