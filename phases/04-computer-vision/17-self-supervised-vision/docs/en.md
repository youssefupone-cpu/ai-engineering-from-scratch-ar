# الرؤية الخاضعة للإشراف الذاتي — SimCLR، DINO، MAE
...
**الوقت:** ~75 دقيقة
## أهداف التعلم
- تتبع العائلات الثلاث الرئيسية الخاضعة للإشراف الذاتي - المتباينة (SimCLR)، والمعلم والطالب (DINO)، وإعادة البناء المقنع (MAE) - وحدد ما تقوم كل واحدة بتحسينه
- تنفيذ خسارة InfoNCE من البداية وشرح سبب نجاح دفعة مكونة من 512 وفشل دفعة مكونة من 32
- اشرح لماذا لا تكون نسبة التقنيع التي تبلغ 75% الخاصة بـ MAE اعتباطية وكيف تختلف عن نسبة الإخفاء التي تبلغ 15% الخاصة بـ BERT للنص
...
### المعلم والطالب (DINO)
شبكتان لهما نفس البنية: الطالب والمعلم. المعلم هو المتوسط ​​المتحرك الأسي (EMA) لأوزان الطالب. كلاهما يرى طرق عرض معززة للصورة. يتم تدريب مخرجات الطالب لتتناسب مع مخرجات المعلم - لا توجد سلبيات صريحة.
```
...
خيارات التصميم الأساسية التي تعمل make MAE:
- **نسبة القناع 75%** — عالية. يجبر المشفر على تعلم السمات الدلالية؛ ستكون إعادة بناء 25% أمرًا تافهًا تقريبًا (ترتبط وحدات البكسل المجاورة بدرجة كبيرة بحيث يمكن لـ CNN أن يثبتها).
...
### لماذا 75% وليس 15%
BERT أقنعة 15% من الرموز المميزة. MAE أقنعة 75%. الفرق هو كثافة المعلومات.
...
## بنائها
### الخطوة 1: تكبير العرض المزدوج pipeline
```python
import torch
...
L2-تسوية التضمينات قبل الاتصال. `tau=0.1` هو SimCLR الافتراضي؛ انخفاض make الخسارة أكثر حدة ويتطلب المزيد من السلبيات.
### الخطوة 3: التحقق من سلامة InfoNCE
```python
...
بسيطة وسريعة وحتمية لبذرة معينة. تقوم تطبيقات MAE الحقيقية بتجميع هذا والاحتفاظ بالأقنعة لكل عينة.
## استخدمه
...
يُعد التضمين 768 خافت الناتج بمثابة العمود الفقري لاسترجاع الصور الحديثة، والمراسلات الكثيفة، وخطوط نقل اللقطة الصفرية. نادرًا ما يحتاج الضبط الدقيق لمهمة ما إلى أكثر من رأس خطي.
بالنسبة لتضمين نص الصورة، فإن SigLIP أو OpenCLIP هو المكافئ؛ بالنسبة للضبط الدقيق لنمط MAE، يقوم `timm` بشحن الريبو في كل نقطة تفتيش MAE.
## اشحنها
ينتج هذا الدرس:
...
2. **(متوسط)** قم بتنفيذ المخزن المؤقت المركزي بنمط DINO. أظهر أنه بدون التمركز، ينهار الطالب إلى متجه ثابت خلال فترة قليلة.
3. **(صعب)** تدريب MAE على CIFAR-100 باستخدام TinyUNet من الدرس 10 باعتباره العمود الفقري. الإبلاغ عن دقة المسبار الخطي عند 10 و50 و200 عصر. أظهر أن المسبار الخطي المُدرب مسبقًا MAE يتفوق على المسبار الخطي الخاضع للإشراف من الصفر على نفس المجموعة الفرعية المكونة من 1000 صورة.
## المصطلحات الرئيسية
| مصطلح | ماذا يقول الناس | ماذا يعني في الواقع |
|------|----------------|----------------------|
| الإشراف الذاتي | "خالية من الملصقات" | مهمة ذريعة تنتج تمثيلات مفيدة من البيانات غير المسماة |
| مهمة ذريعة | "المهمة الوهمية" | الهدف المستخدم خلال SSL (إعادة بناء التصحيحات، ومطابقة المشاهدات)؛ التخلص منها بعد التدريب المسبق |
| مسبار خطي | "التشفير المجمد + الرأس الخطي" | التقييم القياسي SSL: تدريب المصنف الخطي فقط أعلى الميزات المجمدة |
| إنفونسي | "الخسارة المتناقضة" | softmax على أوجه التشابه في جيب التمام؛ الزوج الموجب هو الفئة المستهدفة، وكل الآخرين سلبيون |
| EMA معلم | "معلم متوسط ​​متحرك" | المعلم الذي تمثل أوزانه متوسطًا متحركًا أسيًا للطالب؛ مستخدم بواسطة BYOL، MoCo، DINO |
| نسبة القناع | "% من التصحيحات مخفية" | جزء من التصحيحات المقنعة خلال MAE؛ 75% للرؤية، 15% للنص |
...
## مزيد من القراءة
- [SimCLR (Chen et al., 2020)](https://arxiv.org/abs/2002.05709) — مرجع التعلم المتباين
- [DINO (Caron et al., 2021)](https://arxiv.org/abs/2104.14294) — المعلم والطالب يتمتع بالزخم والتركيز والشحذ
- [MAE (He et al., 2022)](https://arxiv.org/abs/2111.06377) — التدريب المسبق على جهاز التشفير التلقائي المقنع لـ ViT
