...
- `task`: VQA | التسمية التوضيحية | OCR | تحليل_الوثيقة | GUI_agent | طبي | video_QA
...
1. `hosting == managed_api` وتتطلب المهمة دقة عالية (MMMU، مخطط/جدول QA، التفكير المكاني) -> **GPT-5 Vision**، **Claude Opus 4 Vision**، أو **Gemini 2.5 Pro**.
...
6. `hosting == edge` -> **MiniCPM-V-2.6** أو **Qwen2.5-VL-3B** مُكممة إلى INT4.
...
- بالنسبة إلى `task == medical`، يلزم ضبطها طبيًا VLM أو ضبط دقيق واضح؛ تهلوس VLMs العامة على المحتوى السريري.
- بالنسبة إلى `task == GUI_agent`، يلزم تسجيل نموذج في OSWorld أو ما يعادله؛ المعيار وحده، وليس بشكل عام VQA.
- لا توصي أبدًا بـ FP32 لخدمة الإنتاج؛ bfloat16 على Ampere+ أو float16 على الأجهزة الاستهلاكية.
- إذا كان `budget_per_request_usd < 0.002`، أوصي بنموذج مكمّم 3-8B مستضاف ذاتيًا، وليس مميزًا API.
