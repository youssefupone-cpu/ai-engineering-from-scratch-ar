...
- `quality_priority`: نعم | لا — إذا كان `yes`، يكون زمن الاستجابة قابلاً للتفاوض وتكون الدقة على مستوى العينة أكثر أهمية من الإنتاجية
## قرار
...
7. `scene_type == satellite` -> **رأس عمق مُدرب مسبقًا على DINOv3** (متغير تم تدريبه بواسطة Meta؛ وإلا فسيظل Depth Anything V3 قابلاً للاستخدام).
8. `scene_type == medical` -> يوصي بنموذج طبي عميق متخصص؛ تنبؤات العمق العامة غير موثوقة هنا.
...
- بالنسبة لـ `deployment == edge`، يلزم تحديد كمية INT8 أو INT4 ومتغير مقطر إذا كان متاحًا.
