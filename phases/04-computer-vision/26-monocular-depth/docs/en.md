# تقدير العمق والهندسة أحادي العين
> خريطة العمق هي صورة ذات قناة واحدة حيث يكون كل بكسل على مسافة من الكاميرا. كان التنبؤ به من إطار RGB واحدًا مستحيلًا بدون استريو أو LiDAR. في عام 2026، سيصل جهاز تشفير ViT المجمد بالإضافة إلى رأس خفيف الوزن إلى نسبة قليلة من الحقيقة الأرضية.
** النوع: ** بناء + استخدام
** اللغات: ** بايثون
...
تقدير العمق أحادي العين — التنبؤ بالعمق من إطار RGB واحد — يُستخدم لإنتاج مخرجات ضبابية وغير موثوقة. بحلول عام 2026، غيرت أجهزة التشفير الكبيرة المدربة مسبقًا ما يلي: Depth Anything V3 يستخدم العمود الفقري المجمد DINOv2 وينتج خرائط عمق تعمم عبر المجالات الداخلية والخارجية والطبية والأقمار الصناعية. تعيد القطيفة صياغة العمق باعتباره مشكلة انتشار مشروط. يتراجع ZoeDepth عن المسافات المترية الحقيقية.
العمق هو أيضًا الجسر بين الكشف ثنائي الأبعاد والفهم ثلاثي الأبعاد: اضرب بكسلات المربع المكتشف في العمق وسترفع الكائن ثنائي الأبعاد إلى سحابة نقطية ثلاثية الأبعاد. هذا هو جوهر كل نظام انسداد AR، وكل خط pipe لتجنب العوائق، وكل روبوت "التقط الكأس".
##المفهوم
### العمق النسبي مقابل العمق المتري
- **العمق النسبي** — قيم `z` المطلوبة بدون وحدة حقيقية. "البكسل A أقرب من البكسل B، لكن نسبة المسافات لا ترتبط بالأمتار."
...
يقوم Depth Anything V3 بتجميد جهاز التشفير وتدريب وحدة فك التشفير ذات النمط DPT فقط. يوفر برنامج التشفير ميزات غنية؛ يقوم جهاز فك التشفير بإعادتهم إلى دقة الصورة ويتراجع عن العمق.
### لماذا تنتج صورة واحدة العمق على الإطلاق؟
...
### ما لا يستطيع العمق الأحادي فعله
...
- SOTA عبر العمق الأحادي، وهندسة أي عرض، والعرض المرئي، وتقدير وضعية الكاميرا.
هذا هو النموذج الذي يجب الاتصال به عندما تحتاج إلى العمق في عام 2026.
### القطيفة — الانتشار للعمق
يعيد Marigold (Ke et al., CVPR 2024) صياغة تقدير العمق باعتباره نشرًا مشروطًا من صورة إلى صورة. التكييف: RGB. الهدف: خريطة العمق. يستخدم Stable Diffusion 2 U-Net المدرب مسبقًا كعمود فقري. تكون خرائط عمق المخرجات حادة بشكل استثنائي عند حدود الكائنات. المفاضلة: استدلال أبطأ من نماذج التغذية الأمامية (10-50 خطوة لتقليل الضوضاء).
### الجوهرية والكاميرا ذات الثقب
...
تأتي الجوهرية من البيانات التعريفية EXIF، أو نمط المعايرة، أو مقدر الجوهرية الأحادي (حقول المنظور، UniDepth). بدون الجوهر، لا يزال بإمكانك تقديم سحابة نقطية من خلال افتراض 60-70 درجة FOV ومبادئ ذات دقة متوسطة - يمكن استخدامها للتصور، وليس للقياس.
### تقييم
مقياسين قياسيين:
...
### الخطوة 2: محاذاة القياس والتحويل
...
### الخطوة 3: ارفع العمق إلى نقطة السحابة
```python
import numpy as np
...
وظيفة واحدة، كل تطبيق مرفوع ثلاثي الأبعاد. قم بتصدير سحابة النقطة إلى `.ply` وافتحها في MeshLab أو CloudCompare.
### الخطوة 4: اختبار الدخان بمشهد عميق اصطناعي
```python
...
### الخطوة 5: عمق استخدام أي شيء V3 (مرجع)
```python
import torch
...
ثلاثة خطوط. `out["depth"]` هو PIL تدرج رمادي؛ تحويل إلى numpy للرياضيات. للحصول على Depth Anything V3 على وجه التحديد، قم بتبديل معرف النموذج بمجرد إصداره؛ API لم يتغير.
## استخدمه
- **Depth Anything V3** (Meta AI / ByteDance، 2024-2026) — الإعداد الافتراضي للعمق النسبي. أسرع نموذج للعمود الفقري الكبير ViT في الإنتاج.
- **Marigold** (ETH, 2024) — أعلى جودة بصرية، واستدلال بطيء.
...
للاستخدام في الوقت الفعلي، يصل عمق أي شيء V2 صغير (INT8 كميًا) إلى 30 إطارًا في الثانية تقريبًا على المستهلك GPU بدقة 518 × 518.
## اشحنها
ينتج هذا الدرس:
- `outputs/prompt-depth-model-picker.md` — يتم الاختيار بين Depth Anything V3 وMarigold وUniDepth وMiDaS مع مراعاة زمن الاستجابة والحاجة المترية مقابل النسبية ونوع المشهد.
- `outputs/skill-depth-to-pointcloud.md` — مهارة تعمل على بناء السحب النقطية من خرائط العمق مع التعامل الصحيح مع العناصر الجوهرية وتصديرها إلى `.ply`.
## تمارين
1. **(سهل)** قم بتشغيل أي شيء بعمق V2 على أي 10 صور لمكتبك. احفظ العمق كملفات PNG ذات تدرج رمادي وافحصها. حدد جسمًا واحدًا يبدو عمقه المتوقع خاطئًا واشرح سبب فشل الإشارات الأحادية.
2. **(متوسط)** بالنظر إلى RGB + العمق من Depth Anything V2، ارفع إلى سحابة نقطية واعرض باستخدام `open3d`. قارن بين مشهدين (داخلي/خارجي) ولاحظ أيهما يبدو أكثر قابلية للتصديق.
...
| __المصطلح_1__ | "محول التنبؤ الكثيف" | وحدة فك التشفير القائمة على التحويل المستخدمة أعلى أجهزة تشفير ViT المجمدة للعمق |
| العمود الفقري DINOv2 | "سبب نجاحه" | ميزات الإشراف الذاتي التي يتم تعميمها عبر المجالات دون تسميات العمق |
## مزيد من القراءة
- [Depth Anything V3 paper page](https://depth-anything.github.io/) — SOTA عمق أحادي مع جهاز التشفير DINOv2
- [Marigold (Ke et al., CVPR 2024)](https://marigoldmonodepth.github.io/) — تقدير العمق على أساس الانتشار
- [UniDepth (Piccinelli et al., 2024)](https://arxiv.org/abs/2403.18913) — العمق المتري مع الجوهر
- [MiDaS v3.1 (Intel ISL)](https://github.com/isl-org/MiDaS) — خط الأساس الأساسي للعمق النسبي
- [DINOv3 blog post (Meta)](https://ai.meta.com/blog/dinov3-self-supervised-vision-model/) — عائلة التشفير التي ترفع دقة العمق