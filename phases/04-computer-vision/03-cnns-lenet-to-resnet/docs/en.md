...
** النوع: ** تعلم + بناء
** اللغات: ** بايثون
** المتطلبات الأساسية: ** المرحلة 3 الدرس 11 (PyTorch)، المرحلة 4 الدرس 01 (أساسيات الصورة)، المرحلة 4 الدرس 02 (التلافيف من الصفر)
**الوقت:** ~75 دقيقة
...
في عام 2011، سجل أفضل مصنف ImageNet حوالي 74% من أعلى 5 دقة. في عام 2012، حصلت AlexNet على نسبة 85%. وفي عام 2015، حصلت شركة ResNet على نسبة 96%. لا توجد بيانات جديدة. لا يوجد جيل GPU جديد. وجاءت المكاسب من أفكار الهندسة المعمارية. يجب على مهندس الرؤية العاملة أن يعرف أي فكرة جاءت من أي ورقة لأن كل العمود الفقري للإنتاج الذي تشحنه في عام 2026 هو إعادة تركيب لتلك القطع نفسها - ولأن الأفكار تستمر في النقل: انتقلت التحويلات المجمعة من شبكات CNN إلى المحولات، وانتقلت الاتصالات المتبقية من ResNet إلى كل LLM موجود، وتعيش تسوية الدُفعات في نماذج الانتشار.
...
3. **العمق والعرض**. خمس طبقات تحويل، ثلاث طبقات كثيفة، 60 مليون معلمة، تم تدريبها على اثنين من GPU مع تقسيم النموذج عبرها.

لا يزال الشكل 2 في الورقة يُظهر الانقسام GPU كتيارين متوازيين. كان هذا التوازي عبارة عن حل بديل للأجهزة، وليس رؤية معمارية - ولكن الأفكار الثلاثة المذكورة أعلاه لا تزال موجودة في كل نموذج تستخدمه.
...
ترى تحويلتان 3x3 نفس منطقة الإدخال 5x5 مثل تحويل 5x5 ولكن مع معلمات أقل (2*9*C^2 = 18C^2 مقابل 25*C^2) وReLU إضافي بينهما. VGG حولت هذه الملاحظة إلى عمارة كاملة. البساطة - نوع كتلة واحد متكرر - جعلت منه النقطة المرجعية لكل ما جاء بعده.
...
بحلول عام 2015، VGG-19 عملت وVGG-32 لم تعمل. كان من المفترض أن يساعد العمق، لكن ما يزيد عن 20 طبقة من التدريب وخسارة الاختبار أصبحت أسوأ. هذا ليس الإفراط في التجهيز. هذا هو فشل المحسن في العثور على أوزان مفيدة لأن التدرجات تتقلص بشكل مضاعف خلال كل طبقة.
...
VGG عملت على 19 طبقة لأن معيار الدُفعة (المنشور في وقت واحد) حافظ على عمليات التنشيط جيدة الحجم. ولكن حتى معيار الدفعة لم يتمكن من إنقاذ العمق الذي يتجاوز 30 طبقة.
...
الحد الأدنى من LeNet المخلص. تفعيلات تانه، متوسط ​​التجميع. التنازل الوحيد للحداثة هو أننا نستخدم `nn.CrossEntropyLoss` في اتجاه مجرى النهر بدلاً من التوصيلات الغوسية الأصلية.
...
المخرجات المتوقعة: `output: torch.Size([1, 10])`، `params: 61,706`. هذا هو المصنف digit بأكمله الذي بدأ الرؤية الحديثة.
...
`bias=False` في طبقات التحويل عبارة عن اصطلاح معياري للدُفعة — تتعامل المعلمة التجريبية لـ BN بالفعل مع الانحياز، لذا فإن حمل انحياز التحويل أيضًا يعد مضيعة. يحتاج `shortcut` إلى تحويل حقيقي فقط عندما يتغير عدد الخطوات أو القنوات؛ وإلا فهي هوية محظورة.

### Step 4: A tiny ResNet

قم بتجميع أربع مجموعات من BasicBlocks للحصول على شبكة ResNet فعالة لمدخلات بحجم CIFAR.
...
`torchvision.models` يمنحك إصدارات مدربة مسبقًا لكل ما سبق. توقيع المكالمة متطابق عبر العائلات، وهو بالضبط نقطة التجريد الأساسي.
...
- `outputs/prompt-backbone-selector.md` — مطالبة تختار عائلة CNN المناسبة (LeNet/VGG/ResNet/MobileNet/ConvNeXt) المعينة للمهمة وحجم مجموعة البيانات وميزانية الحساب.
- `outputs/skill-residual-block-reviewer.md` — مهارة تقرأ وحدة PyTorch وتضع علامة على أخطاء تخطي الاتصال (الاختصار المفقود عند تغيير الخطوة، ترتيب تنشيط الاختصار، BN الموضع بالنسبة إلى الإضافة).

## Exercises

1. **(سهل)** حساب المعلمات يدويًا لطبقة `TinyResNet` بطبقة. قارن مع `sum(p.numel() for p in net.parameters())`. أين تذهب غالبية ميزانية المعلمة - التحويلات، BN، أو رأس المصنف؟
2. **(متوسط)** قم بتنفيذ كتلة عنق الزجاجة (1x1 -> 3x3 -> 1x1 مع التخطي) واستخدمها لبناء شبكة على نمط ResNet-50 لـ CIFAR. قارن المعلمات بـ `TinyResNet`.
3. **(صعب)** قم بإزالة اتصال التخطي من `BasicBlock`، وقم بتدريب شبكة "عادية" مكونة من 34 كتلة وشبكة ResNet مكونة من 34 كتلة على CIFAR-10 لمدة 10 فترات لكل منها. مؤامرة خسارة التدريب مقابل العصر لكليهما. إعادة إنتاج هو وآخرون. نتيجة الشكل 1 حيث تتقارب الشبكة العميقة البسيطة مع خسارة أعلى من توأمها الضحل.
...
| الاتصال المتبقي | "تخطي الاتصال" | `y = F(x) + x`; يتيح للمُحسِّن التعرف على الهوية عن طريق ضبط F على الصفر، وهو عمق تعسفي make قابل للتدريب |
...
- [Very Deep Convolutional Networks (Simonyan & Zisserman, 2014)](https://arxiv.org/abs/1409.1556) — الورقة VGG؛ لا تزال أفضل مرجع لـ "لماذا 3x3"
...
- [Going Deeper with Convolutions (Szegedy et al., 2014)](https://arxiv.org/abs/1409.4842) — Inception v1؛ فكرة الفلتر المتوازي التي لا تزال تظهر في محولات الرؤية
