...
- `dataset_size`: عدد الصور ذات العلامات (من المفترض أن يكون العمود الفقري مُدرب مسبقًا)
...
3. `inference_stack == onnx_cpu` -> **ConvNeXt-V2-B**. يجمع بشكل أفضل من ViT على CPU.
4. `dataset_size > 100k` و `inference_stack == server_gpu|tensorrt` -> **ViT-B/16** MAE-مُدرَّب مسبقًا.
