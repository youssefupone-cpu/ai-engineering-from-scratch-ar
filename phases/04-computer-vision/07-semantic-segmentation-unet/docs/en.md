# التجزئة الدلالية — يو نت
> يتم تصنيف التقسيم في كل بكسل. يعمل U-Net makes عن طريق إقران جهاز تشفير الاختزال مع وحدة فك ترميز الاختزال وتوصيل تخطي الأسلاك بينهما.
**النوع:** بناء
** اللغات: ** بايثون
...
- أنشئ شبكة U-Net من الصفر في PyTorch باستخدام كتل التشفير، وعنق الزجاجة، ووحدة فك التشفير مع التلافيفات المنقولة، وتخطي الاتصالات
...
## المشكلة
يُخرج التصنيف تسمية واحدة لكل صورة. يقوم الاكتشاف بإخراج عدد قليل من المربعات لكل صورة. يؤدي التقسيم إلى إخراج تسمية واحدة لكل بكسل. بالنسبة للمدخل بالحجم `H x W`، يكون الإخراج موترًا بالشكل `H x W` (دلالي) أو `H x W x N_instances` (مثال). وهذا يعني ملايين التنبؤات لكل صورة، وليس واحدة.
...
المشكلة المعمارية سهلة التحديد وليس من السهل حلها: تحتاج إلى الشبكة لرؤية السياق العام للصورة (أي نوع من المشهد هذا) وتفاصيل البكسل المحلية (بالضبط أي بكسل هو الطريق مقابل الرصيف) في وقت واحد. يتم ضغط CNN القياسي مكانيًا للحصول على السياق، مما يؤدي إلى التخلص من التفاصيل. كان U-Net هو التصميم الذي حصل على كليهما.
##المفهوم
### الدلالي مقابل المثال مقابل البانوبتيك
```mermaid
flowchart LR
...
يغطي هذا الدرس الدلالية. يغطي الدرس التالي (القناع R-CNN) المثال.
### شكل U-Net
```mermaid
flowchart LR
...
### الانتروبيا المتقاطعة على شبكة البكسل
...
يعالج `F.cross_entropy` في PyTorch هذا الشكل بشكل أصلي. لا حاجة لإعادة التشكيل.
### خسارة النرد ولماذا تحتاجه
...
حيث `p` هي خريطة الاحتمالية السيني/softmax لفئة ما و`y` هو قناع الحقيقة الأرضية الثنائي. تكون الخسارة صفرًا فقط عندما يكون التداخل مثاليًا. ولأنه يعتمد على النسبة، فإن عدم التوازن الطبقي ليس له أي أهمية.
...
- **النرد (F1 بالبكسل)** — مشابه لـ IoU؛ __الكود_0__. التصوير الطبي يفضل النرد، ومجتمع القيادة يفضل IoU؛ إنهما مرتبطان بشكل رتيب.
...
بالنسبة للنموذج الأول، يتم تدريب الإدخال مقاس 256 × 256 مع شبكة U-Net ذات قاعدة 64 قناة بشكل مريح على 8 GB VRAM.
## بنائها
### الخطوة 1: كتلة التشفير
...
يتم إعادة استخدام هذه الكتلة طوال الوقت. `bias=False` لأن الإصدار التجريبي من BN يعالج الانحياز.
### الخطوة 2: الكتل السفلية والأعلى
```python
...
يتعامل فحص الشكل المكاني فقط (`shape[-2:]`) مع المدخلات التي لا تقبل القسمة على 16؛ يقوم `F.interpolate` الآمن بمحاذاة الموتر قبل التسلسل. قد تؤدي مقارنة الشكل الكامل أيضًا إلى حدوث اختلافات في عدد القنوات، وهو ما يجب أن يكون خطأً فادحًا، وليس استيفاءً صامتًا.
### الخطوة 3: شبكة يو نت
```python
...
شكل الإخراج `(1, 2, 256, 256)` — نفس الحجم المكاني لقنوات الإدخال `num_classes`. حوالي 7.7 مليون معلمة في `base=32`.
### الخطوة 4: الخسائر
```python
...
تُرجع متجهًا للطول C. يشير `nan` إلى الفئات الغائبة عن الدفعة - ولا تضع متوسطًا على تلك عند حساب mIoU.
### الخطوة 6: مجموعة البيانات الاصطناعية للتحقق الشامل
...
قم بتشغيل هذا لمدة 10-30 حقبة على مجموعة البيانات الاصطناعية وشاهد mIoU يتسلق ما يزيد عن 0.9 لفئات الأشكال. لاحظ أن `nan_to_num(0)` يعامل الفئات الغائبة من الدفعة على أنها صفر؛ للحصول على وحدة IoU دقيقة لكل فئة، قم بالقناع من خلال التواجد واستخدم `torch.nanmean` عبر الدفعات في وقت التقييم بدلاً من حساب المتوسط ​​هنا.
## استخدمه
...
- **SegFormer** يقوم بتبديل أداة تشفير التحويل بمحول هرمي؛ SOTA الحالي في العديد من المعايير.
- **Mask2Former** / **OneFormer** توحيد التجزئة الدلالية والمثالية والشاملة في بنية واحدة.
الثلاثة جميعها عبارة عن بدائل مباشرة في `smp` أو `transformers` بنفس أداة تحميل البيانات.
## اشحنها
ينتج هذا الدرس:
...
## تمارين
1. **(سهل)** قم بتنفيذ `bce_dice_loss` لمهمة التجزئة الثنائية (المقدمة مقابل الخلفية). تحقق من مجموعة بيانات اصطناعية من فئتين من أن الخسارة المجمعة تتقارب بشكل أسرع من BCE وحده عندما تكون المقدمة 5% من وحدات البكسل.
...
3. **(صعب)** خذ مجموعة بيانات تجزئة حقيقية (Oxford-IIIT Pets، أو Cityscapes mini Split، أو مجموعة فرعية طبية) وقم بتدريب U-Net على نقطتي IoU من مرجع `smp.Unet`. قم بالإبلاغ عن IoU لكل فئة وحدد الفئات التي تستفيد أكثر من إضافة النرد إلى الخسارة.
## المصطلحات الرئيسية
...
| الحدود F1 | "دقة الحدود" | F1 النتيجة المحسوبة على وحدات البكسل الحدودية فقط؛ مسائل للمهام بالغة الدقة |
## مزيد من القراءة
- [U-Net: Convolutional Networks for Biomedical Image Segmentation (Ronneberger et al., 2015)](https://arxiv.org/abs/1505.04597) — الورقة الأصلية؛ الرقم الذي ينسخه الجميع موجود في الصفحة 2
...
- [segmentation_models_pytorch](https://github.com/qubvel/segmentation_models.pytorch) — مرجع تجزئة الإنتاج؛ كل بنية قياسية بالإضافة إلى كل خسارة قياسية
