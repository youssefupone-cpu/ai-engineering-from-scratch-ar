...
- قم ببناء رأس بسيط على شكل YOLO فوق العمود الفقري المُدرب مسبقًا، بما في ذلك التصنيف والموضوعية وخسائر انحدار الصندوق
...
الاكتشاف هو أيضًا المكان الذي تظهر فيه كل مقايضة هندسية في الرؤية مرة واحدة. تريد مربعات دقيقة (رأس الانحدار)، وتريد الفئة المناسبة لكل مربع (رأس التصنيف)، وتريد أن يعرف النموذج عندما لا يكون هناك شيء يمكن اكتشافه (درجة الكائن)، وتريد تنبؤًا واحدًا بالضبط لكل كائن حقيقي (عدم الحد الأقصى للقمع). إذا أخطأت أيًا من هذه العناصر، فإن الخط pip إما يفتقد الكائنات، أو يبلغ عن الصناديق المهلوسة، أو يتنبأ بنفس الكائن خمس عشرة مرة في مواضع مختلفة قليلاً.

YOLO (أنت تنظر مرة واحدة فقط، Redmon et al. 2016) هو التصميم الذي جعل كل هذا يعمل في الوقت الفعلي من خلال القيام بذلك بتمريرة أمامية واحدة لشبكة تحويل، ولا تزال نفس القرارات الهيكلية هي العمود الفقري لأجهزة الكشف الحديثة (YOLOv8، YOLOv9، YOLO-NAS، RT-DETR). تعلم جوهر كل متغير ويصبح إعادة ترتيب لنفس الأجزاء.

## The Concept

### Detection as dense prediction

يقوم المصنف بإخراج أرقام C لكل صورة. يقوم كاشف النمط YOLO بإخراج أرقام `(S x S x (5 + C))` لكل صورة، حيث S هو حجم الشبكة المكانية.
...
تتنبأ كل خلية من خلايا الشبكة `S * S` بالمربعات `B`. لكل صندوق:
...
تعالج المراسي مشكلة ثانية. لا يمكن للتحويل 3x3 أن يتراجع بسهولة عن مربع بعرض 500 بكسل من خلية ميزة حقل تقبلية تبلغ 16 بكسل. بدلاً من ذلك، نقوم بتعريف أشكال المربعات السابقة (المثبتات) مسبقًا `B` لكل خلية ونتوقع دلتا صغيرة من كل نقطة ارتساء. يتعلم النموذج كيفية اختيار المرساة الصحيحة ودفعها بدلاً من التراجع من لا شيء.
...
غالبًا ما تستخدم أجهزة الكشف الحديثة FPN مع مجموعات ربط مختلفة لكل دقة - نقاط تثبيت صغيرة على خرائط ضحلة عالية الدقة، ومثبتات كبيرة على خرائط عميقة منخفضة الدقة. نفس الفكرة، المزيد من المقاييس.

### Decoding predictions

إن `tx, ty, tw, th` الأولية ليست إحداثيات مربعة؛ إنها أهداف انحدار يجب تحويلها قبل التخطيط:
...
`sigmoid` يحافظ على إزاحات المركز داخل الخلية. `exp` يتيح مقياس العرض بحرية من نقطة الارتساء بدون قلب الإشارة. `stride` يقيس إحداثيات الشبكة إلى وحدات البكسل. خطوة فك التشفير هذه هي نفسها في كل إصدار YOLO منذ الإصدار 2.
...
العتبة النموذجية: 0.45 للكشف عن الكائنات. تستبدل الكاشفات الحديثة المعيار NMS بـ `soft-NMS`، `DIoU-NMS`، أو تتعلم القمع مباشرة (RT-DETR) ولكن الغرض الهيكلي هو نفسه.

### The loss

YOLO الخسارة هي ثلاث خسائر تضاف مع الأوزان:
...
فقط الخلايا التي تحتوي على كائن هي التي تساهم في خسائر الانحدار والتصنيف. تساهم الخلايا التي لا تحتوي على كائنات إلا في فقدان الموضوعية (تعليم النموذج التزام الصمت). `lambda_noobj` عادةً ما تكون صغيرة (~0.5) لأن الغالبية العظمى من الخلايا فارغة وقد تهيمن على الخسارة الإجمالية.

تقوم المتغيرات الحديثة بتبديل MSE فقدان الصندوق لـ CIoU / DIoU (الذي يعمل على تحسين IoU مباشرة)، واستخدام الخسارة البؤرية لعدم توازن الفئة، وموازنة الموضوعية مع الخسارة البؤرية عالية الجودة. الهيكل المكون من ثلاثة مكونات لم يتغير.
...
- **mAP@0.5:0.95** — متوسط ​​AP فوق عتبات IoU 0.5، 0.55،...، 0.95. ال COCO متري؛ الأكثر صرامة والأكثر إفادة.
...
العمود الفقري للدرس كله. يعمل على صفيفين من المربعات بتنسيق `(x1, y1, x2, y2)`.
...
تُرجع مصفوفة `(N_a, N_b)` لوحدات IoU الزوجية. استخدمه مقابل مربع واحد للحقيقة الأرضية عن طريق جعل إحدى المصفوفات على شكل `(1, 4)`.
...
التحويل بين إحداثيات البكسل والأهداف `(tx, ty, tw, th)` التي تتراجع فيها الشبكة بالفعل.
...
تحويل 1x1 واحد على خريطة المعالم، مع إعادة التشكيل إلى `(B, S, S, num_anchors, 5 + C)`.
...
لكل مربع من صناديق الحقيقة الأرضية، حدد `(cell, anchor)` المسؤول.
...
خمس معلمات مفرطة كل YOLO برنامج تعليمي إما رموز ثابتة أو عمليات مسح. النسب مهمة: `lambda_coord=5, lambda_noobj=0.5` تعكس ورقة YOLOv1 الأصلية ولا تزال تعمل كافتراضي معقول.

### Step 7: Inference pipeline

قم بفك تشفير مخرجات الرأس الأولية، وتطبيق السيني/الخبرة، والعتبة على الكائن، وNMS.
...
`torchvision.models.detection` كاشفات إنتاج السفن بنفس البنية المفاهيمية. يستغرق تحميل نموذج تم تدريبه مسبقًا ثلاثة أسطر.
...
للاستدلال في الوقت الفعلي pipelines، `ultralytics` (YOLOv8/v9) هو المعيار: `from ultralytics import YOLO; model = YOLO('yolov8n.pt'); model(img)`. يتعامل النموذج مع فك التشفير وNMS داخليًا ويعيد نفس `boxes / scores / labels` الثلاثي الذي قمت بإنشائه أعلاه.

## Ship It

ينتج هذا الدرس:

- `outputs/prompt-detection-metric-reader.md` — مطالبة تحول الصف `precision, recall, AP, mAP@0.5:0.95` إلى تشخيص من سطر واحد والتجربة التالية الأكثر فائدة.
- `outputs/skill-anchor-designer.md` — مهارة تؤدي، في ضوء مجموعة بيانات من مربعات الحقيقة الأرضية، إلى تشغيل وسائل k على `(w, h)` وإرجاع مجموعات الارتساء لكل مستوى FPN بالإضافة إلى إحصائيات التغطية التي تحتاجها لاختيار العدد المناسب من نقاط الارتساء.
...
2. **(متوسط)** المنفذ `yolo_loss` إلى إصدار يستخدم فقدان المربع `CIoU` بدلاً من MSE. أظهر على مجموعة بيانات تركيبية مكونة من 100 صورة أن CIoU تتقارب إلى mAP@0.5:0.95 نهائي أفضل من MSE في نفس العدد من العصور.
...
| الخريطة | "متوسط ​​​​الدقة" | متوسط ​​المساحة الواقعة تحت منحنى الاسترجاع الدقيق، متوسطًا على الفئات و (لـ COCO) عتبات IoU |
...
- [YOLOv3 (Redmon & Farhadi, 2018)](https://arxiv.org/abs/1804.02767) — الورقة التي قدمت رؤوسًا متعددة المقاييس FPN؛ لا تزال هي أوضح رسم تخطيطي
...
- [The Illustrated Guide to Object Detection (Jonathan Hui)](https://jonathan-hui.medium.com/object-detection-series-24d03a12f904) — أفضل جولة باللغة الإنجليزية في حديقة حيوانات الكاشف الكاملة؛ لا تقدر بثمن لفهم كيفية ارتباط DETR وRetinaNet وFCOS وYOLO
