...
- قراءة الصور وتقطيعها وفحصها كمصفوفات NumPy والتبديل بسلاسة بين التخطيطات HWC وCHW
- التحويل بين RGB، وتدرج الرمادي، HSV، وYCbCr وتبرير سبب وجود كل مساحة لونية
- تطبيق المعالجة المسبقة على مستوى البكسل (التطبيع، والتوحيد، وتغيير الحجم، والقناة أولاً) تمامًا كما تتوقعها torchvision

## The Problem

Every paper you will read, every pretrained weight you will download, every vision API you will call assumes a specific encoding of the input. قم بتمرير صورة `uint8` حيث يريد النموذج `float32` وسيظل قيد التشغيل - وينتج النفايات بصمت. قم بتغذية BGR بشبكة تم تدريبها على RGB وستنخفض الدقة بمقدار عشر نقاط. قم بتسليم نموذج قنوات - الإدخال الأخير عندما تتوقع القنوات - أولاً وتتعامل طبقة التحويل الأولى مع الارتفاع كقناة مميزة. لا شيء من هذا يلقي خطأ. إنه يدمر مقاييسك فحسب، وتقضي أسبوعًا في البحث عن خطأ يكمن في كيفية تحميل الملف.

الالتواء ليس معقدًا بمجرد أن تعرف ما ينزلق عليه. الجزء الصعب هو أن "الصورة" تعني أشياء مختلفة بالنسبة للكاميرا، ووحدة فك ترميز JPEG، وPIL، وOpenCV، وtorchvision، ونواة CUDA. يحتوي كل مكدس على ترتيب المحور الخاص به ونطاق البايت واتفاقية القناة. مهندس رؤية لا يستطيع إبقاء هذه السفن المستقيمة مكسورة pipخطوط.
...
(infinite detail)                (H x W detectors)               (H x W integers)
    ...
    ~~~~~                        +--+--+--+--+--+                 210 198 180 155 120
   ~ ~ ~ | | | | | | 205 195 178 152 118 ~ ضوء ~ ----> +--+--+--+--+--+ ----> 200 190 175 150 115
   ~~~~~                         |  |  |  |  |  |                 195 185 170 148 112
                                 +--+--+--+--+--+                 188 180 165 145 108
```
...
ثلاثة ليس سحرا. تضيف كاميرات العمق قناة Z. تضيف الأقمار الصناعية نطاقات الأشعة تحت الحمراء والأشعة فوق البنفسجية. غالبًا ما تحتوي عمليات الفحص الطبي على قناة واحدة (الأشعة السينية، CT) أو عدة (فوق الطيفية). عدد القنوات هو المحور الأخير؛ تتعلم طبقات التحويل كيفية الاختلاط عبرها.
...
| تطبيع | `float32` | [0.0، 1.0] | بعد `img.astype('float32') / 255` |
| موحدة | `float32` | تقريبًا [-2, +2] | بعد طرح المتوسط ​​والقسمة على std |

تم تدريب الشبكات التلافيفية على مدخلات موحدة. إحصائيات ImageNet `mean=[0.485, 0.456, 0.406]`، `std=[0.229, 0.224, 0.225]` هي المتوسط ​​الحسابي والانحراف المعياري للقنوات الثلاث عبر مجموعة تدريب ImageNet الكاملة، والتي يتم حسابها على [0، 1] بكسل تم تسويتها. إن تغذية `uint8` الخام في نموذج يتوقع تعويمًا موحدًا هو الفشل الصامت الأكثر شيوعًا في الرؤية التطبيقية.
...
بالنسبة لمعظم شبكات CNN الحديثة التي تغذيها RGB. تقابل مساحات أخرى عندما:

- **HSV** — كود CV الكلاسيكي، التجزئة على أساس اللون، موازنة اللون الأبيض.
- **YCbCr** — قراءة JPEG داخلية، فيديو pipelines، نماذج فائقة الدقة تعمل على Y فقط.
- **تدرج الرمادي** — OCR، نماذج المستندات، أي حالة يكون فيها اللون متغيرًا مزعجًا وليس إشارة.

التدرج الرمادي من RGB هو مجموع مرجح وليس متوسطًا، لأن العين البشرية أكثر حساسية للأخضر من الأحمر أو الأزرق:
...
استخدم الوسادة لتحميل أي JPEG أو PNG، والتحويل إلى NumPy، وطباعة ما حصلت عليه. للحصول على مثال حتمي يعمل دون اتصال بالإنترنت، قم بتجميع واحد.
...
اسحب R وG وB بشكل منفصل، ثم قم بالتحويل من HWC إلى CHW مقابل PyTorch.
...
ثلاث طائرات ذات تدرج رمادي، واحدة لكل قناة. CHW فقط يعيد ترتيب المحاور؛ لا يلزم نسخ البيانات بشكل صارم عندما يسمح تخطيط الذاكرة بذلك.

### Step 3: Grayscale and HSV conversions

تدرج الرمادي للمجموع الموزون، ثم دليل RGB-إلى-HSV.
...
يظهر اللون بالدرجات والتشبع والقيمة في [0، 1]. يتوافق ذلك مع اتفاقية OpenCV `hsv_full`.
...
`torchvision.transforms` يجمع كل شيء أعلاه في سطر واحد قابل للتركيب. الكود أدناه يكرر بالضبط ما يفعله `preprocess_imagenet`، بالإضافة إلى تغيير الحجم والقص.
...
أربع خطوات، بهذا الترتيب الدقيق: `Resize(256)` يقيس الجانب الأقصر إلى 256؛ `CenterCrop(224)` يأخذ رقعة بحجم 224×224 من المنتصف؛ `ToTensor()` يقسم على 255 ويستبدل HWC بـ CHW؛ `Normalize` يطرح متوسط ​​ImageNet ويقسمه على std. يؤدي عكس هذا الترتيب بصمت إلى تغيير ما يصل إلى النموذج.

## Ship It

ينتج هذا الدرس:

- `outputs/prompt-vision-preprocessing-audit.md` — مطالبة تحول أي بطاقة نموذجية أو بطاقة مجموعة بيانات إلى قائمة مرجعية لثوابت المعالجة المسبقة الدقيقة التي يجب على الفريق احترامها.
- `outputs/skill-image-tensor-inspector.md` — مهارة تقوم، في ضوء أي موتر أو مصفوفة على شكل صورة، بالإبلاغ عن نوع dtype والتخطيط والنطاق وما إذا كان يبدو خامًا أو طبيعيًا أو موحدًا.

## Exercises

1. **(سهل)** قم بتحميل JPEG باستخدام OpenCV (`cv2.imread`) ومع الوسادة. اطبع كلا الشكلين والبكسل عند `(0, 0)`. اشرح الفرق بين ترتيب القنوات، ثم اكتب تحويلاً من سطر واحد يكون make مصفوفة OpenCV مطابقة لمصفوفة الوسادة.
2. **(متوسط)** اكتب `standardize(img, mean, std)` ومعكوسه بحيث يجتازان معًا اختبار `roundtrip_max_diff <= 1` على أي صورة uint8. يجب أن تعمل وظائفك على صورة واحدة في HWC وعلى دفعة في NCHW بنفس المكالمة.
3. **(صعب)** خذ موترًا قياسيًا من نوع ImageNet ثلاثي القنوات وقم بتشغيله من خلال تحويل 1x1 يتعلم مزيجًا مرجحًا من RGB في قناة واحدة ذات تدرج رمادي. قم بتهيئة الأوزان إلى `[0.299, 0.587, 0.114]`، وقم بتجميدها، وتحقق من تطابق الإخراج مع دليلك `rgb_to_grayscale` ضمن خطأ الفاصلة العائمة. ما هي تحويلات مساحة اللون الكلاسيكية الأخرى التي يمكن كتابتها على هيئة تلافيفات 1x1؟
...
| HWC / CHW | "الشكل" | ترتيبات المحاور لموتر الصورة؛ القرص و PIL استخدم HWC و PyTorch و cuDNN استخدم CHW |
...
- [PyTorch Vision Transforms Docs](https://pytorch.org/vision/stable/transforms.html) — the full pipelineeline of transforms you will actually compose in production
- [How JPEG Works (Colt McAnlis)](https://www.youtube.com/watch?v=F1kYBnY6mwg) — جولة بصرية حادة لأخذ عينات فرعية من اللون، DCT، ولماذا يقوم JPEG بتشفير YCbCr بدلاً من RGB
- [اتفاقيات المعالجة المسبقة لـ ImageNet (نماذج torchvision)](https://pytorch.org/vision/stable/models.html) — مصدر الحقيقة لـ `mean=[0.485, 0.456, 0.406]` ولماذا يتوقعه كل نموذج في حديقة الحيوان
