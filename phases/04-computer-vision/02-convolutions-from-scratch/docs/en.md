...
- تنفيذ التفاف ثنائي الأبعاد من البداية باستخدام NumPy فقط، بما في ذلك إصدار الحلقة المتداخلة والإصدار المتجه `im2col`
- حساب الحجم المكاني للإخراج لأي مجموعة من حجم الإدخال وحجم النواة والمساحة والخطوة، وضبط الصيغة `(H - K + 2P) / S + 1`
...
نظرًا للحجم المكاني للإدخال `H`، وحجم النواة `K`، والحشوة `P`، والخطوة `S`:
...
الأوضاع التي تقابلها عمليًا: `zero` (الأكثر شيوعًا)، `reflect` (عكس الحافة، وتجنب الحدود الصلبة في النماذج التوليدية)، `replicate` (نسخ الحافة)، `circular` (الالتفاف، يستخدم في المسائل الحلقية).
...
الصور الحقيقية لها ثلاث قنوات. إن الإلتواء 3x3 على المدخلات RGB هو في الواقع حجم 3x3x3: شريحة 3x3 واحدة لكل قناة إدخال. في كل موضع مكاني، تقوم بالضرب والجمع عبر الشرائح الثلاث وإضافة انحياز.
...
الحلقات المتداخلة سهلة القراءة ولكنها بطيئة. GPUs تريد مضاعفة المصفوفة الكبيرة. الحيلة: تسوية كل نافذة مجال استقبال للمدخلات في عمود واحد من مصفوفة كبيرة، وتسوية النواة في صف واحد، ويصبح الالتواء بأكمله ماتمولًا واحدًا.
...
كل تنفيذ لتحويل الإنتاج هو نوع من أنواع هذه الحيل بالإضافة إلى تبليط ذاكرة التخزين المؤقت (التحويل المباشر، Winograd، FFT conv للنواة الكبيرة). افهم im2col وستفهم الجوهر.
...
السبب الكامل وراء عمل "3x3 على طول الطريق" (VGG، ResNet، ConvNeXt) هو أن تحويلين 3x3 يرون نفس منطقة الإدخال مثل تحويل واحد 5x5 ولكن مع معلمات أقل وخطية إضافية بينهما.
...
خدعة المحاور الزائدة `x.shape[:-2]` تعني أن نفس الوظيفة تعمل على `(H, W)`، `(C, H, W)`، أو `(N, C, H, W)` بدون تعديل.

### Step 2: 2D convolution with nested loops

التنفيذ المرجعي بطيء ولكن لا لبس فيه. وهذا ما يفعله `torch.nn.functional.conv2d` من حيث المبدأ.
...
عند تطبيقه على أي صورة ذات تدرج رمادي، يتم تخفيف التمويه وزيادة حدة الحواف، ويضيء Sobel-x الحواف الرأسية، ويضيء Sobel-y الحواف الأفقية. هذه هي بالضبط الأنماط التي انتهى الأمر بتعلمها طبقة التحويل المدربة *الأولى* في AlexNet وVGG - لأن نموذج الصورة الجيد يحتاج إلى كاشفات الحافة والنقطة بغض النظر عن المهمة التي تأتي لاحقًا.

## Use It

يختتم PyTorch's `nn.Conv2d` نفس العملية باستخدام autograd وCUDA kernels وتحسين cuDNN. دلالات الشكل متطابقة.
...
قم بتبديل `padding=1` بـ `padding=0` وينخفض ​​الإخراج إلى 222x222. قم بتبديل `stride=1` بـ `stride=2` وينخفض ​​إلى 112x112. نفس الصيغة التي حفظتها أعلاه.

## Ship It

ينتج هذا الدرس:

- `outputs/prompt-cnn-architect.md` — مطالبة، نظرًا لحجم الإدخال وميزانية المعلمة وحقل الاستقبال المستهدف، تصمم مجموعة من الطبقات `Conv2d` مع K/S/P الصحيح في كل خطوة.
- `outputs/skill-conv-shape-calculator.md` — مهارة تنتقل عبر طبقة مواصفات الشبكة بطبقة وترجع شكل الإخراج وحقل الاستلام وعدد المعلمات لكل كتلة.

## Exercises

1. **(سهل)** نظرًا لإدخال تدرج رمادي مقاس 128 × 128 ومجموعة من `[Conv3x3(s=1,p=1), Conv3x3(s=2,p=1), Conv3x3(s=1,p=1), Conv3x3(s=2,p=1)]`، قم بحساب الحجم المكاني للإخراج وحقل الاستلام في كل طبقة يدويًا. تحقق باستخدام PyTorch `nn.Sequential` من التحويلات الوهمية.
2. **(متوسط)** قم بتوسيع `conv2d_naive` و`conv2d_im2col` لقبول الوسيطة `groups`. أظهر أن `groups=C_in=C_out` يُنتج التفافًا عميقًا وأن عدد معلماته هو `C * K * K` بدلاً من `C * C * K * K`.
3. **(صعب)** تنفيذ التمرير الخلفي لـ `conv2d_im2col` يدويًا: بالنظر إلى تدرج الإخراج، قم بحساب التدرج `x` و`w`. تحقق من `torch.autograd.grad` على نفس المدخلات والأوزان. الحيلة: تدرج im2col هو `col2im`، وعليه تجميع النوافذ المتداخلة.
...
| الحشو | "أصفار على الحواف" | تتم إضافة قيم إضافية حول الإدخال حتى تتمكن النواة من التركيز على وحدات البكسل الحدودية؛ `same` الحشو يحافظ على حجم الإخراج مساوياً لحجم الإدخال |
...
- [The Annotated ConvNet (fast.ai)](https://nbviewer.org/githubhub/fastai/fastbook/blob/master/13_convolutions.ipynb) — دفتر ملاحظات ينتقل من الالتفاف اليدوي إلى مصنف digit مُدرب
- [حساب المجال الاستقبالي لشبكات CNN (Dang Ha The Hien)](https://distill.pub/2019/computing-receptive-fields/) - الشرح التفاعلي بجودة الورق لحسابات المجال الاستقبالي
