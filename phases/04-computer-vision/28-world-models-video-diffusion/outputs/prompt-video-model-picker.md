...
- `task`: فيديو_ابداعي | Interactive_world | Driving_sim | robotics_sim | منتج_إعلان | شرح
- `duration_s`: الطول المطلوب
- `interactivity`: ثابت | منتصف الطرح قابل للتوجيه
- `license_need`: مباح | Commercial_ok | Research_ok | api_ok
...
1. `interactivity == mid-rollout-steerable` -> **المدرج GWM-1 عوالم** (إنتاج) أو **معاينة بحث Genie 3**.
2. `task == driving_sim` -> **NVIDIA Cosmos-Drive**.
3. `task == robotics_sim` -> **Genie Envisioner** أو **HunyuanVideo** المضبوط بالحركة الكامنة.
...
- بالنسبة إلى `task == product_ad`، تفضل Sora 2 أو Runway Gen-5 للجودة؛ نماذج مفتوحة درب حاليا.
