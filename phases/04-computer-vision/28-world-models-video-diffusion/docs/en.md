...
إن المخاطر ملموسة. يقوم Genie 3 بإنشاء بيئات قابلة للتشغيل من صورة واحدة. المدرج GWM-1 عوالم يجمع مشاهد لا حصر لها قابلة للاستكشاف. يُنتج Sora 2 مقاطع فيديو مدتها دقيقة مع صوت متزامن وفيزياء نموذجية. NVIDIA تقوم Cosmos-Drive وWayve Gaia-2 وTesla DrivingWorld بإنشاء فيديو قيادة واقعي لبيانات تدريب المركبات ذاتية القيادة. يتولى نموذج النموذج العالمي بهدوء عملية المحاكاة إلى الواقع في مجال الروبوتات.
...
- **DreamerV3** وعائلة الطراز العالمي RL الكلاسيكية تتنبأ في مساحة كامنة مع تكييف حركة واضح، مدربة على إشارة المكافأة. أقل بصرية أكثر فائدة لكفاءة العينة RL.
...
- **Cosmos-Drive-Dreams** (NVIDIA) — يُنشئ دقائق من فيديو القيادة للتدريب RL.
...
1. **VLM** يوزع الهدف ("التقط الكأس الحمراء")، ويخطط لتسلسل حركة عالي المستوى.
...
وهذا يحل محل تشكيل المكافأة وكثافة العينة RL. النموذج العالمي يفعل الخيال. تغلق الديناميكيات العكسية الحلقة عند التشغيل. يعد Genie Envisioner أحد الأمثلة على ذلك؛ تتقارب العديد من المجموعات البحثية حول هذا الهيكل.

### Evaluation

- **جودة الصورة** — FVD (Fréchet Video Distance)، دراسات المستخدم.
- **محاذاة سريعة** — CLIPالنتيجة لكل إطار، VQA تقييم على النمط.
...
| المدرج Gen-5 | نص/صورة إلى فيديو | — | مقاطع 10s | API |
| المدرج GWM-1 عوالم | العالم التفاعلي | — | طرح ثلاثي الأبعاد لا نهائي | API |
...
يعمل التحويل ثلاثي الأبعاد بخطوة مساوية للنواة كمصحح مكاني وزماني. `(T, H, W) -> (T/2, H/2, W/2)` شبكة من الرموز.
...
الوقت الذي يحضر فيه الاهتمام داخل كل موضع مكاني عبر الزمن؛ يحضر الاهتمام بالمساحة داخل كل إطار عبر المواضع. عمليتان O(T^2 + (HW)^2) بدلاً من عملية O(THW)^2). هذا هو جوهر TimeSformer وكل فيديو DiT حديث.
...
1. هدف اللغة -> VLM (Qwen3-VL) -> خطة عالية المستوى.
...
- `outputs/skill-physical-plausibility-checks.md` — مهارة تحدد عمليات الفحص الآلي (دوام الكائن، والجاذبية، والاستمرارية) للتشغيل على أي فيديو تم إنشاؤه قبل الشحن.
...
| كوزموس درايف | "NVIDIA شريحة القيادة" | نموذج عالمي للقيادة الذاتية بالأوزان المفتوحة لـ RL وتقييم |
...
- [من جيل الفيديو إلى النموذج العالمي — مستودع الاستبيان](https://githubhub.com/ziqihuangg/Awesome-From-Video-Generation-to-World-Model/)
