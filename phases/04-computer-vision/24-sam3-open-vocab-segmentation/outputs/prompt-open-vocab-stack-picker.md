...
قواعد النار من أعلى إلى أسفل؛ المباراة الأولى يفوز. تعمل قيود الترخيص كمرشحات ثابتة — إذا كان النموذج الافتراضي للقاعدة ينتهك رقم المتصل `license_need`، فانتقل إلى القاعدة التالية بدلاً من التجاوز.
...
3. `task_output == masks` و `license_need == permissive` -> ** مؤرض SAM 2** مع كاشف مرخص من Apache (Florence-2 / التأريض DINO 1.5).
...
6. `deployment == browser` -> YOLO-العالم ONNX + MobileSAM أو متغير مقطر الحافة.
...
- إذا `concept_complexity == compositional` ("مظلة حمراء مخططة"، "يد تحمل كوبًا")، تفضل SAM 3 على YOLO-العالم؛ تكافح كاشفات المفردات المفتوحة مع المعدلات الوصفية.
- إذا كانت مجموعة البيانات خاصة بالمجال (عيب طبي، أو قمر صناعي، أو عيب صناعي)، فيوصى باستخدام Grounded SAM 2 مع كاشف مضبوط للمجال؛ SAM 3 ربما لم يروا المفاهيم على نطاق واسع.
- للإنتاج عند <100 مللي ثانية p95، يتطلب INT8 أو FP16؛ لا تشحن أبدًا FP32 على الحافة.
- بالنسبة إلى SAM 3، لاحظ دائمًا بوابة طلب الوصول HF على نقطة التفتيش.
