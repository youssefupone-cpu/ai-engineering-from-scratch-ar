...
قم بتحويل مجلد الصور إلى فهرس قابل للبحث باستخدام التضمينات CLIP.
...
- `clip_model`: معرف الوجه المعانق مثل `openai/clip-vit-base-patch32` أو `google/siglip-base-patch16-224`.
- `index_type`: مسطحة | IVF | HNSW.
- `embedding_dim`: يستدل من النموذج.
...
3. أنشئ فهرس FAISS فوق التضمينات. استخدم المنتج الداخلي على المتجهات الطبيعية L2 لتشابه جيب التمام.
4. كشف واجهتين للاستعلام: - `search_by_text(text, k)` — تضمين النص والبحث. - `search_by_image(image_path, k)` — تضمين الصورة، البحث.
...
- دائمًا L2-تسوية التضمينات قبل الفهرسة؛ المنتج الداخلي FAISS على المتجهات الطبيعية يساوي تشابه جيب التمام.
- بالنسبة إلى <100 ألف صورة، يكون `IndexFlatIP` (تامًا) هو الأبسط والأسرع.
- بالنسبة لمسافة 100 ألف - 10 متر، `IndexIVFFlat` هي المقايضة القياسية.
- بالنسبة إلى > 10M، استخدم HNSW أو متغير كمي للمنتج.
