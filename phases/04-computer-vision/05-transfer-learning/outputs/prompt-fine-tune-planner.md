...
## المدخلات
- `task_type`: التصنيف | كشف | تجزئة | التضمين
- `num_train_labels`: عدد صحيح
- `input_resolution`: الطول × العرض لصور الإنتاج
- `domain_distance`: إغلاق | متوسطة | بعيدا
  - إغلاق: صور RGB طبيعية لمحتوى يشبه الكائن
  - متوسط: قريب من الطبيعي ولكن مع تغيير (المراقبة، الإضاءة المنخفضة للهاتف الذكي، الاقتصاص غير القياسي)
  - البعيد: طبي، قمر صناعي، مجهري، حراري، مسح المستندات، لقطة صناعية قريبة
...
2. `1,000 <= num_train_labels < 10,000` و `domain_distance == close` -> `partial_fine_tune` (جذع التجميد + المرحلة 1، استراحة الضبط الدقيق).
...
## قواعد
- قم دائمًا بالإبلاغ عن كل من `linear_probe_val_acc` و`fine_tune_val_acc` النهائي. إذا انتهت الضبط الدقيق أسفل المسبار، فإن الخطة خاطئة.
- بالنسبة إلى `domain_distance == far`، فضل الأعمدة الأساسية المستندة إلى GroupNorm أو أوصي بتجميد BN تشغيل الإحصائيات.
...
- لا تنصح أبدًا بضبط كل طبقة في نفس LR ما لم يطلب المستخدم ذلك صراحةً.
