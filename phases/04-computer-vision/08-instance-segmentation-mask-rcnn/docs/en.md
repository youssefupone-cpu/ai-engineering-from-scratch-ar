# تجزئة المثيل — القناع R-CNN
> أضف فرع قناع صغير إلى كاشف R-CNN الأسرع وسيكون لديك تجزئة المثيل. الجزء الصعب هو RoIAlign، وهو أصعب مما يبدو.
** النوع: ** بناء + تعلم
** اللغات: ** بايثون
**المتطلبات الأساسية:** المرحلة الرابعة الدرس 06 (YOLO)، المرحلة الرابعة الدرس 07 (U-Net)
**الوقت:** ~75 دقيقة
## أهداف التعلم
- تتبع بنية القناع R-CNN من النهاية إلى النهاية: العمود الفقري، FPN، RPN، RoIAlign، رأس الصندوق، رأس القناع
...
- ضبط القناع R-CNN على مجموعة بيانات صغيرة مخصصة عن طريق استبدال رؤوس الصندوق والقناع والحفاظ على تجميد العمود الفقري
## المشكلة
...
3. **RPN (شبكة مقترحات المنطقة)** — رأس تحويل صغير يتنبأ، عند كل موضع ربط، "هل يوجد كائن هنا؟" و"كيف أقوم بتحسين الصندوق؟". ينتج حوالي 1000 مقترح لكل صورة.
4. **RoIAlign** — عينات من تصحيح الميزات ذات الحجم الثابت (على سبيل المثال 7x7) من أي صندوق على أي مستوى FPN. أخذ العينات الثنائية، لا الكمي.
...
ترفع RoIAlign القناع AP بمقدار 3-4 نقاط في COCO مجانًا. كل كاشف يهتم بالترجمة يستخدمه الآن — YOLOv7 seg, RT-DETR, Mask2Former على حد سواء.
### RPN في فقرة واحدة
في كل موضع من خريطة المعالم، ضع مربعات تثبيت K ذات أحجام وأشكال مختلفة. توقع درجة الموضوعية لكل مرساة وإزاحة الانحدار لتحويل المرساة إلى مربع أكثر ملاءمة. احتفظ بأعلى ~ 1000 صندوق حسب النتيجة، وقم بتطبيق NMS عند IoU 0.7، وقم بتسليم الناجين إلى الرؤوس. يتم تدريب RPN على الخسارة الصغيرة الخاصة به - نفس بنية الخسارة YOLO من الدرس 6، فقط مع فئتين (كائن / لا يوجد كائن).
### رأس القناع
...
- `L_rpn_cls`، `L_rpn_box` — الموضوعية + انحدار الصندوق لمقترحات RPN.
- `L_box_cls` — إنتروبيا متقاطعة عبر فئات (C+1) (بما في ذلك الخلفية) في مصنف الرأس.
- `L_box_reg` — سلس L1 على تحسين صندوق الرأس.
...
### تنسيق الإخراج
`torchvision.models.detection.maskrcnn_resnet50_fpn_v2` يعرض قائمة بالإملاءات، واحدة لكل صورة:
```
{
...
### الخطوة 2: قارن بـ RoIAlign الخاص بـ torchvision
```python
...
مع `sampling_ratio=1` و`aligned=True`، يتطابق الاثنان مع `1e-5`.
### الخطوة 3: تحميل قناع مُدرب مسبقًا R-CNN
```python
import torch
...
### الخطوة 5: قم بتبديل الرؤوس لعدد فئات مخصص
وصفة الضبط الدقيق الشائعة: إعادة استخدام العمود الفقري، FPN، وRPN؛ استبدل رأسي المصنف.
```python
...
### الخطوة السادسة: تجميد ما لا يحتاج إلى تدريب
...
## استخدمه
تتكون حلقة التدريب الكاملة للقناع R-CNN في torchvision من 40 سطرًا ولا تتغير بشكل مفيد بين المهام - قم بتبديل مجموعات البيانات وانطلق.
```python
...
يجب أن تحتوي قائمة `targets` على إملاءات لكل صورة تحتوي على `boxes` و`labels` و`masks` (مثل `(num_instances, H, W)` موترات ثنائية). يُرجع النموذج حكمًا بأربع خسائر أثناء التدريب وقائمة تنبؤات أثناء التقييم، مع الضغط على `model.training`.
يقوم مُقيم `pycocotools` بإنتاج mAP@IoU=0.5:0.95 لكل من الصناديق والأقنعة؛ أنت بحاجة إلى كلا الرقمين لمعرفة ما إذا كان رأس الصندوق أو رأس القناع هو عنق الزجاجة.
## اشحنها
ينتج هذا الدرس:
...
2. **(متوسط)** قم بضبط `maskrcnn_resnet50_fpn_v2` على مجموعة بيانات مخصصة مكونة من 50 صورة (أي فئتين: البالونات، والأسماك، والحفر، والشعارات). قم بتجميد العمود الفقري، وتدرب لمدة 20 فترة، وقم بتقرير القناع AP@0.5.
...
|------|----------------|----------------------|
| قناع R-CNN | "الكشف بالإضافة إلى الأقنعة" | أسرع R-CNN + رأس FCN صغير يتنبأ بقناع 28x28 لكل مقترح لكل فصل |
| __المصطلح_3__ | "الهرم المميز" | اتصالات من أعلى إلى أسفل + جانبية تعطي كل خطوة قنوات المستوى C من الميزات الغنية بالدلالات |
...
## مزيد من القراءة
- [Mask R-CNN (He et al., 2017)](https://arxiv.org/abs/1703.06870) — الورقة؛ القسم 3 في RoIAlign هو القراءة النقدية
- [FPN: Feature Pyramid Networks (Lin et al., 2017)](https://arxiv.org/abs/1612.03144) — ورقة FPN؛ يستخدمه كل كاشف حديث
- [torchvision Mask R-CNN tutorial](https://pytorch.org/tutorials/intermediate/torchvision_tutorial.html) — مرجع حلقة الضبط الدقيق
- [Detectron2 model zoo](https://github.com/facebookresearch/detectron2/blob/main/MODEL_ZOO.md) — تطبيقات الإنتاج بأوزان مدربة لكل متغيرات الكشف والتجزئة تقريبًا