...
### خطوة SfM
- قم بمعالجة الصور خلال COLMAP أو GLOMAP أولاً لإنتاج أوضاع الكاميرا + نقاط متفرقة.
- التحقق من خطأ إعادة الإسقاط < 1 بكسل في المتوسط ​​قبل بدء التدريب على 3DGS.
- الإخراج النموذجي: `cameras.bin`، `images.bin`، `points3D.bin` — يتم التغذية مباشرةً بـ `splatfacto`.
## الإخراج
```
[capture plan]
  scene:           <type>
