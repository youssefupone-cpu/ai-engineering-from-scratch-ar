...
**الوقت:** ~75 دقيقة
## أهداف التعلم
- اشتق عملية الضوضاء الأمامية `x_0 -> x_1 -> ... -> x_T` واشرح سبب بقاء الشكل المغلق `q(x_t | x_0)` لأي t
- تنفيذ هدف تدريبي على طراز DDPM يتراجع عن التشويش المضاف في كل خطوة، وأداة أخذ العينات ترجع من التشويش النقي إلى الصورة
- أنشئ شبكة U-Net مكيفة زمنيًا (صغيرة بما يكفي للتدريب على CPU) تتنبأ بالضوضاء في أي خطوة زمنية
- اشرح الفرق بين أخذ العينات DDPM وDDIM ومتى يكون كل منهما مناسبًا (الدرس 23 يغطي مطابقة التدفق والتدفق المصحح في العمق)
## المشكلة
تولد شبكات GAN طلقة واحدة: ضوضاء للداخل، وصورة للخارج، وتمريرة أمامية واحدة. إنهم سريعون ويصعب تدريبهم. يتم إنشاء نماذج الانتشار بشكل متكرر: تبدأ من الضوضاء النقية، ثم يتم تقليل الضوضاء في خطوات صغيرة، ثم تظهر الصورة. إنهم بطيئون وسهل التدريب. على مدى السنوات الخمس الماضية هيمنت الخاصية الأخيرة: يستطيع أي فريق صغير تدريب نموذج الانتشار والحصول على عينات معقولة؛ GAN التدريب هو حرفة تتعلمها على مدار سنوات من التجارب الفاشلة.
...
يبني هذا الدرس الحد الأدنى من DDPM: الضوضاء الأمامية، وتقليل الضوضاء الخلفية، وحلقة التدريب. الدرس التالي (الانتشار المستقر) ينقله إلى نظام إنتاج باستخدام VAE، ومشفر النص، وتوجيهات خالية من المصنفات.
##المفهوم
### العملية إلى الأمام
التقط صورة `x_0`. أضف كمية صغيرة من الضوضاء الغوسية للحصول على `x_1`. أضف مبلغًا صغيرًا للحصول على `x_2`. استمر في السير لخطوات T حتى يتعذر تمييز `x_T` تقريبًا عن الضوضاء الغوسية النقية.
```
q(x_t | x_{t-1}) = N(x_t; sqrt(1 - beta_t) * x_{t-1},  beta_t * I)
```

`beta_t` عبارة عن جدول تباين صغير، خطي عادةً من 0.0001 إلى 0.02 على T = 1000 خطوة. تعمل كل خطوة على تقليص الإشارة قليلاً وإدخال ضوضاء جديدة.
### القفزة المغلقة
إن إضافة الضوضاء خطوة بخطوة هي بمثابة سلسلة ماركوف، لكن الرياضيات تطوي: يمكنك أخذ عينة من `x_t` مباشرة من `x_0` في خطوة واحدة.
```
...
هذه المعادلة الوحيدة هي السبب الرئيسي وراء كون الانتشار عمليًا. أثناء التدريب، يمكنك اختيار `t` عشوائي، وعينة `x_t` مباشرة من `x_0`، والتدريب في خطوة واحدة - لا حاجة لمحاكاة سلسلة ماركوف الكاملة.
### العملية العكسية
...
2. عينة من خطوة زمنية `t` بشكل موحد من [1، T].
3. عينة الضوضاء `epsilon ~ N(0, I)`.
...
### DDIM: أخذ عينات أسرع 20 مرة
التدريب هو نفسه. تغييرات أخذ العينات. DDIM (سونغ وآخرون، 2020) يحدد عملية عكسية حتمية تتخطى الخطوات الزمنية دون إعادة التدريب. أخذ العينات في 50 خطوة باستخدام DDIM يعطي جودة DDPM تقترب من 1000 خطوة. يستخدم كل نظام إنتاج DDIM أو متغيرًا أسرع (DPM-Solver، أسلاف أويلر).
### تكييف الوقت
تحتاج الشبكة `epsilon_theta(x_t, t)` إلى معرفة الخطوة الزمنية التي تقلل الضوضاء. تقوم نماذج الانتشار الحديثة بحقن `t` عبر تضمينات زمنية جيبية (نفس فكرة التشفير الموضعي في المحولات) التي تتم إضافتها إلى خرائط المعالم في كل مستوى من مستويات U-Net.
```
...
### الخطوة الثانية: الانتشار للأمام (q_sample)
```python
...
### الخطوة 3: شبكة U-Net صغيرة الحجم ومكيفة بالوقت
```python
...
هذه هي حلقة التدريب بأكملها. لا توجد لعبة GAN، ولا توجد خسارة متخصصة، مكالمة MSE واحدة.
### الخطوة 5: أخذ العينات (DDPM)
```python
@torch.no_grad()
...
1000 تمريرة أمامية لإنتاج دفعة واحدة من العينات. في الكود الحقيقي، يمكنك استبدال هذا بأداة أخذ العينات مكونة من DDIM مكونة من 50 خطوة.
...
`eta=0` محدد تمامًا (نفس مدخلات الضوضاء تنتج دائمًا نفس المخرجات). `eta=1` يستعيد DDPM.
## استخدمه
...
تقوم المكتبة بشحن برامج جدولة جاهزة (DDPM، DDIM، DPM-Solver، Euler، Heun)، وشبكات U قابلة للتكوين، وخطوط pip لتحويل النص إلى صورة وصورة إلى صورة، ومساعدي الضبط الدقيق LoRA.
بالنسبة للبحث، يحتوي `k-diffusion` (Katherine Crowson) على أكثر التطبيقات المرجعية دقة وأفضل متغيرات أخذ العينات.
## اشحنها
ينتج هذا الدرس:
- `outputs/prompt-diffusion-sampler-picker.md` — موجه يختار DDPM / DDIM / DPM-Solver / Euler استنادًا إلى هدف الجودة وميزانية وقت الاستجابة ونوع التكييف.
- `outputs/skill-noise-schedule-designer.md` — مهارة تنتج جدولًا تجريبيًا خطيًا أو جيب التمام أو السيني بالنظر إلى T ومستوى الفساد المستهدف، بالإضافة إلى المخططات التشخيصية لنسبة الإشارة إلى الضوضاء بمرور الوقت.
## تمارين
...
| DDIM أخذ العينات | "حتمية وسريعة" | يعيد كتابة أخذ العينات باعتباره ODE حتمي؛ 20-100 خطوة بجودة مماثلة |
| تكييف الوقت | "أخبر النموذج الذي ر" | التضمين الجيبي لـ t المحقون في U-Net حتى يعرف مستوى الضوضاء |
## مزيد من القراءة
- [Denoising Diffusion Probabilistic Models (Ho et al., 2020)](https://arxiv.org/abs/2006.11239) — الورقة التي جعلت الانتشار عمليًا وتفوقت على شبكات GAN في FID
- [Improved DDPM (Nichol & Dhariwal, 2021)](https://arxiv.org/abs/2102.09672) — جدول جيب التمام ومعلمات v
- [DDIM (Song, Meng, Ermon, 2020)](https://arxiv.org/abs/2010.02502) — أداة أخذ العينات الحتمية التي جعلت الاستدلال في الوقت الفعلي ممكنًا
- [Elucidating the Design Space of Diffusion (Karras et al., 2022)](https://arxiv.org/abs/2206.00364) — عرض موحد لكل اختيار لتصميم النشر؛ أفضل مرجع الحالي