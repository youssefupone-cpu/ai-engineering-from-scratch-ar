# رؤية ثلاثية الأبعاد - الغيوم النقطية وNeRFs
...
- فهم خدعة الوظيفة المتماثلة لـ PointNet والتي make هي شبكة عصبية ثابتة على مجموعة غير مرتبة من النقاط
...
## المشكلة
تنتج الكاميرا صورة ثنائية الأبعاد. ينتج عن LIDAR مجموعة من النقاط ثلاثية الأبعاد بدون ترتيب. ينتج خط pipeline من الحركة سحابة متفرقة من نقاط المفاتيح ثلاثية الأبعاد. يقوم NeRF بإعادة بناء مشهد ثلاثي الأبعاد بالكامل من مجموعة من الصور الموضحة. كل هذه "رؤية" ولكن لا يبدو أي منها مثل الموتر الكثيف الذي يريده CNN.
الرؤية ثلاثية الأبعاد مهمة لأن كل مهمة روبوت عالية القيمة تقريبًا تعمل بتقنية ثلاثية الأبعاد: الإمساك، وتجنب العوائق، والملاحة، والتغطية، والتقاط المحتوى ثلاثي الأبعاد. مهندس الرؤية الذي لا يفهم سوى الصور ثنائية الأبعاد يتم استبعاده من الشريحة الأسرع نموًا في المجال (AR/VR المحتوى، والروبوتات، ومكدسات القيادة الذاتية، وإعادة الإعمار ثلاثي الأبعاد القائم على NeRF للعقارات أو البناء).
...
لا شبكة، لا اتصال. هناك خاصيتان make صعبة للغاية بالنسبة للشبكات العصبية:
...
"مشترك MLP" يعني أن نفس MLP يعمل على كل نقطة بشكل مستقل. تم تنفيذه كتحويل 1×1 عبر البعد النقطي لتحقيق الكفاءة.
### مجالات الإشعاع العصبي (NeRFs)
...
تقارن الخسارة البكسل المعروض ببكسل الحقيقة الأرضية في صور التدريب. يقوم Backprop من خلال خطوة العرض بتحديث MLP. لا توجد حقيقة أرضية ثلاثية الأبعاد، ولا توجد هندسة واضحة — يتم تخزين المشهد في أوزان MLP.
### التشفير الموضعي في NeRF
لا يمكن للفانيليا MLP في `(x, y, z)` أن تمثل تفاصيل عالية التردد لأن MLPs منحازة طيفيًا نحو الترددات المنخفضة. تقوم NeRF بإصلاح هذه المشكلة عن طريق تشفير كل إحداثي في ​​متجه ميزة فورييه قبل MLP:
```
...
`T_i` هو النفاذية - مقدار الضوء الذي يبقى حتى النقطة i. `(1 - exp(-sigma_i * delta_i))` هو العتامة عند النقطة i. `c_i` هو اللون. البكسل الأخير عبارة عن مجموع مرجح على طول الشعاع.
### ما الذي حل محل NeRFs
...
- **KITTI** — السحب النقطية LIDAR الخارجية للقيادة الذاتية.
...
### الخطوة الثانية: التشفير الموضعي
```python
...
الضرب في `2^l * pi` يعطي ترددات أعلى تدريجيًا.
### الخطوة 3: Tiny NeRF MLP
```python
...
### الخطوة 4: العرض الحجمي على طول الشعاع
```python
...
شعاع واحد، 64 عينة، مركبة إلى RGB بكسل واحد وعمق.
## استخدمه
للعمل الحقيقي:
...
- `pytorch3d` (ميتا) - عرض قابل للتمييز، وأدوات مساعدة للنقطة السحابية، وعمليات شبكية.
...
## اشحنها
ينتج هذا الدرس:
- `outputs/prompt-3d-task-router.md` — موجه يقوم بالتوجيه إلى التمثيل ثلاثي الأبعاد الصحيح (سحابة نقطية، شبكة، voxel، NeRF، Gaussian splat) بناءً على بيانات المهمة والمدخلات.
...
| نيرف | "MLP هذا هو المشهد" | تعيين الشبكة (x، y، z، dir) إلى (الكثافة، اللون)؛ المقدمة بواسطة صب الأشعة |
...
| __المصطلح_6__ | "حقل المسافة الموقعة" | وظيفة إرجاع المسافة الموقعة إلى أقرب سطح؛ تمثيل ضمني آخر |
## مزيد من القراءة
- [PointNet (Qi et al., 2017)](https://arxiv.org/abs/1612.00593) — المصنف التقليب الثابت
...
- [Instant-NGP (Müller et al., 2022)](https://arxiv.org/abs/2201.05989) — شبكات التجزئة، تسريع 1000x
- [3D Gaussian Splatting (Kerbl et al., 2023)](https://arxiv.org/abs/2308.04079) — البنية التي حلت محل NeRFs في الإنتاج