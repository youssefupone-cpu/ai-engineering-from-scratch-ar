...
قم بتحويل مجلد ملفات المسح الضوئي ثلاثي الأبعاد إلى ملف جاهز للتدريب PyTorch `Dataset`.

## When to use

- بدء مشروع جديد لتصنيف/تجزئة السحابة النقطية.
- التبديل بين الصيغ `.ply`، `.pcd`، و`.xyz`.
...
- `augmentation`: لا شيء | تدوير | غضب | mixup.
...
3. **نموذج** `num_points` نقاط. إذا كانت السحابة تحتوي على المزيد، فاستخدم **أخذ عينات من النقطة الأبعد** (FPS) لتمثيل الشكل بدقة أو أخذ عينات عشوائية للسرعة. إذا كان أقل، كرر النقاط.
...
- تفضل FPS على أخذ العينات العشوائية لمهام الشكل؛ العشوائي جيد للتجزئة حيث تكون كل نقطة مهمة على أي حال.
