...
- إنشاء تصنيف شامل للصور pipeline على CIFAR-10: مجموعة البيانات، التكبير، النموذج، حلقة التدريب، التقييم
...
معظم أخطاء التصنيف ليست موجودة في النموذج. إنهم يعيشون في pipeline: تطبيع مكسور، مجموعة تدريب غير مختلطة، زيادة تشوه التسميات، انقسام التحقق الملوث ببيانات التدريب، معدل التعلم الذي يتباعد بصمت بعد العصر 30. CNN من شأنه أن يصل إلى 93٪ على CIFAR-10 مع الإعداد الصحيح عادة ما يسجل 70-75٪ مع واحد مكسور، ويبدو منحنى الخسارة معقول طوال الوقت.

يقوم هذا الدرس بتوصيل خط pipe بالكامل يدويًا بحيث يكون كل جزء قابلاً للفحص. لن تستخدم أي شيء من `torchvision.datasets` يمكنه إخفاء خطأ ما.
...
النموذج الأيمن هو النموذج المستقر عدديًا (log-sum-exp). PyTorch's `nn.CrossEntropyLoss` يدمج softmax + NLL في عملية واحدة ويأخذ logits الخام مباشرةً. إن تطبيق softmax بنفسك أولاً غالبًا ما يكون خطأً - فأنت تحسب log(softmax(softmax(z)))، وهي كمية لا معنى لها.

### Why augmentation works

يحتوي CNN على انحياز استقرائي للترجمة (من مشاركة الوزن) ولكن لا يوجد ثبات مضمن في الاقتصاص أو التقلبات أو ارتعاش اللون أو الانسداد. الطريقة الوحيدة لتعليمه تلك الثوابت هي أن تظهر له وحدات البكسل التي تمارسها. كل تحويل عشوائي أثناء التدريب هو وسيلة للقول: "هاتين الصورتين لهما نفس التسمية؛ تعلم الميزات التي تتجاهل الفرق."
...
القاعدة: أن الزيادة يجب أن تحفظ التسمية. يمكن للقطع والتدوير على digit أن يقلب "6" إلى "9"؛ بالنسبة لمجموعة البيانات هذه، يمكنك استخدام نطاقات دوران أصغر واختيار التعزيزات التي تحترم الثوابت الخاصة بـ digit.
...
ابن عم الخلط. بدلاً من التدرب على `[0, 0, 1, 0, 0]`، تدرب على `[eps/C, eps/C, 1-eps, eps/C, eps/C]` للحصول على `eps` صغيرة مثل 0.1. يوقف النموذج من إنتاج صور حادة بشكل تعسفي ويحسن المعايرة دون أي تكلفة تقريبًا. مدمج في `nn.CrossEntropyLoss(label_smoothing=0.1)` منذ PyTorch 1.10.
...
CIFAR-10 حياة على القرص. لكي make هذا الدرس قابل للتكرار وسريع، قمنا ببناء مجموعة بيانات تركيبية تبدو مثل CIFAR — 32x32 RGB صور ذات بنية خاصة بفئة معينة يجب أن يتعلمها النموذج. نفس pipeline يعمل دون تغيير على CIFAR-10 الحقيقي.
...
1. `model.train()` قبل التدريب، `model.eval()` قبل التقييم - يقلب سلوك التسرب والسلوك المعياري.
...
5. Argmax ضد logits الخام، وليس softmax — نفس النتيجة، عملية واحدة أقل.
...
في مجموعة البيانات الاصطناعية، يصل هذا إلى دقة تحقق شبه مثالية خلال خمس فترات، وهذه هي النقطة: الخط pipe صحيح، ويمكن للنموذج أن يتعلم ما يمكن تعلمه. قم بتبديل مجموعة البيانات الحقيقية CIFAR-10 ونفس القطارات الحلقية إلى 90% تقريبًا دون تغييرات.
...
`torchvision` يلف كل شيء أعلاه في مكونات اصطلاحية. حقيقي CIFAR-10 الخط pipe الكامل هو أربعة أسطر بالإضافة إلى حلقة تدريب.
...
- `outputs/prompt-classifier-pipelineeline-auditor.md` — مطالبة تقوم بمراجعة نص تدريبي للثوابت الخمسة المذكورة أعلاه وتكشف عن الانتهاك الأول.
...
3. **(صعب)** أنشئ خطًا CIFAR-100 pipeline (100 فصل، نفس حجم الإدخال) وأعد إنتاج تدريب ResNet-34 في حدود 1% من الدقة المنشورة. الإضافات: اكتساح ثلاثة معدلات تعلم واثنين من تحلل الوزن، وقم بتسجيل الدخول إلى CSV محلي، وإنتاج جدول الارتباك النهائي-مصفوفة-أعلى-الارتباك.

## Key Terms

| مصطلح | ماذا يقول الناس | ماذا يعني في الواقع |
|------|----------------|----------------------|
| لوgitس | "المخرجات الخام" | ناقل ما قبل softmax لأرقام C لكل صورة؛ تتوقع الإنتروبيا المتقاطعة هذه القيم، وليس قيم softmaxed |
| عبر الانتروبيا | "الخسارة" | احتمال السجل السلبي للفئة الصحيحة؛ يجمع بين log-softmax وNLL في عملية واحدة مستقرة |
...
| زيادة | "التحويلات العشوائية" | أي تحويل على مستوى البكسل في وقت التدريب يحافظ على الملصق؛ يعلم الثوابت CNN لا يملك أصلا |
...
- [CS231n: Training Neural Networks](https://cs231n.github.io/neural-networks-3/) — still the clearest tour of the training pipelineeline at a single page
