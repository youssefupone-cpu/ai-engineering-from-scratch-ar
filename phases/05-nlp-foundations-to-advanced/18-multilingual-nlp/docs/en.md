...
**الوقت:** ~45 دقيقة
## المشكلة
تحتوي اللغة الإنجليزية على مليارات الأمثلة المصنفة. الأردية لديها الآلاف. مايثيلي ليس لديه أي شيء تقريبًا. أي نظام NLP عملي يخدم جمهورًا عالميًا يجب أن يعمل على مجموعة طويلة من اللغات التي لا توجد فيها بيانات تدريب خاصة بالمهمة.
تحل النماذج متعددة اللغات هذه المشكلة عن طريق تدريب نموذج واحد على العديد من اللغات في وقت واحد. يتيح التمثيل المشترك للنموذج نقل المهارات المكتسبة في اللغات ذات الموارد العالية إلى اللغات منخفضة الموارد. قم بضبط النموذج على تحليل المشاعر باللغة الإنجليزية، وسينتج تنبؤات جيدة بشكل مدهش بشأن المشاعر باللغة الأردية. يعد هذا نقلًا صفريًا عبر اللغات، وقد أعاد تشكيل كيفية شحن NLP إلى العالم.
...
**المفردات المشتركة.** تستخدم النماذج متعددة اللغات أداة الرموز المميزة SentencePiece أو WordPiece المدربة على النص من جميع اللغات المستهدفة. يتم مشاركة المفردات: نفس وحدة الكلمات الفرعية تمثل نفس المورفيم عبر اللغات ذات الصلة. `anti-` باللغتين الإنجليزية والإيطالية يحصل على نفس الرمز المميز.
**التمثيل المشترك.** يتعلم المحول الذي تم تدريبه مسبقًا على نمذجة اللغة المقنعة عبر العديد من اللغات أن الجمل المتشابهة لغويًا في اللغات المختلفة تنتج حالات مخفية مماثلة. mBERT، XLM-R، وNLLB جميعهم يعرضون هذا. مجموعة التضمينات لـ "cat" باللغة الإنجليزية بالقرب من "chat" بالفرنسية و"gato" بالإسبانية، وكذلك عمليات التضمين في الجملة الكاملة.
...
**ضبط دقيق لعدد قليل من اللقطات.** أضف 100-500 مثال مصنف في اللغة المستهدفة. تقفز الدقة إلى 95-98% من خط الأساس الإنجليزي في مهام التصنيف. هذه هي الوسيلة الوحيدة الأكثر فعالية من حيث التكلفة في NLP متعددة اللغات.
## النماذج
| نموذج | سنة | التغطية | ملاحظات |
|-------|------|----------|-------|
| امبيرت | 2018 | 104 لغة | تدربت على ويكيبيديا. أول عملي متعدد اللغات LM. ضعيف على الموارد المنخفضة. |
| XLM-R | 2019 | 100 لغة | تدرب على CommonCrawl (أكبر بكثير من ويكيبيديا). يضبط خط الأساس عبر اللغات. القاعدة 270 م، الكبيرة 550 م. |
| XLM-V | 2023 | 100 لغة | XLM-R مع مفردات مكونة من مليون رمز مميز (مقابل 250 ألفًا). أفضل على الموارد المنخفضة. |
| ام تي 5 | 2020 | 101 لغة | T5 بنية الجيل متعدد اللغات. |
| NLLB-200 | 2022 | 200 لغة | نموذج ترجمة ميتا؛ يتضمن 55 لغة منخفضة الموارد. |
| __المصطلح_6__ | 2022 | 46 لغة + 13 برمجة | افتح 176B LLM متدربًا بعدة لغات. |
| آية-23 | 2024 | 23 لغة | متعدد اللغات في Cohere LLM. قوية في اللغة العربية والهندية والسواحيلية. |
اختر حسب حالة الاستخدام. يعمل التصنيف بشكل جيد مع XLM-R-base باعتباره الإعداد الافتراضي المعقول. تستدعي مهام الإنشاء mT5 أو NLLB اعتمادًا على الترجمة مقابل الإنشاء المفتوح. LLM أزواج عمل بأسلوب LLM مع Aya-23 أو Claude باستخدام مطالبة صريحة متعددة اللغات.
## قرار اللغة المصدر (بحث 2026)
تستخدم معظم الفرق اللغة الإنجليزية كمصدر للضبط الدقيق. تظهر الأبحاث الحديثة (2026) أن هذا غالبًا ما يكون خاطئًا.
يتنبأ تشابه اللغة بجودة النقل بشكل أفضل من حجم المجموعة الأولية. بالنسبة للأهداف السلافية، غالبًا ما تتفوق الألمانية أو الروسية على اللغة الإنجليزية. بالنسبة للأهداف الهندية، غالبًا ما تتفوق اللغة الهندية على اللغة الإنجليزية. ويقيس مقياس التشابه **qWALS** (2026، استنادًا إلى ميزات الأطلس العالمي لبنيات اللغة) ذلك. **LANGRANK** (Lin et al., ACL 2019) هي طريقة منفصلة سابقة تعمل على تصنيف اللغات المصدر المرشحة من خلال مزيج من التشابه اللغوي وحجم المجموعة والارتباط الجيني.
...
## بنائها
### الخطوة 1: التصنيف الصفري بين اللغات
```python
...
نموذج واحد، ثلاث لغات، نفس API. XLM-R الذي تم تدريبه على NLI ينقل البيانات بشكل جيد للتصنيف عبر خدعة الاستلزام.
### الخطوة الثانية: مساحة التضمين متعددة اللغات
```python
...
الترجمات تقترب من مساحة التضمين. جملة إنجليزية مختلفة تصل إلى أبعد من ذلك. هذا هو ما يقوم به make من عمليات الاسترجاع والتجميع والتشابه عبر اللغات.
### الخطوة 3: استراتيجية الضبط الدقيق لعدد قليل من اللقطات
```python
...
بالنسبة إلى 100-500 من أمثلة اللغة المستهدفة، فإن `num_train_epochs=5` و`learning_rate=2e-5` هي الإعدادات الافتراضية الآمنة. تؤدي معدلات التعلم المرتفعة إلى انهيار التوافق متعدد اللغات وتحصل على نموذج باللغة الإنجليزية فقط.
## التقييم الذي يعمل بالفعل
...
| مهمة | موصى به |
|-----|------------|
| التصنيف، 100 لغة | XLM-R-base (~270M) مضبوط بدقة |
| تصنيف النص بالرصاص الصفري | `joeddav/xlm-roberta-large-xnli` |
| تضمينات الجملة متعددة اللغات | __الكود_1__ |
| ترجمة، 200 لغة | `facebook/nllb-200-distilled-600M` (أنظر الدرس 11) |
| متعدد اللغات التوليدية | كلود، GPT-4، آية-23، mT5-XXL |
| لغة منخفضة الموارد NLP | XLM-V أو ضبط دقيق خاص بالمجال على اللغات عالية الموارد ذات الصلة |
قم دائمًا بتخصيص ميزانية للضبط الدقيق للغة الهدف إذا كان الأداء مهمًا. نقطة الصفر هي نقطة البداية، وليست إجابة نهائية.
### ضريبة الترميز (ما الخطأ في اللغات منخفضة الموارد)
...
إجراءات التخفيف: اختر أداة رمزية ذات تغطية جيدة للغتك المستهدفة (XLM-V تعد مفردات الرمز المميز التي يبلغ عددها مليون رمز بمثابة حل مباشر)؛ التحقق من خصوبة الترميز على النص المستهدف المحفوظ قبل التدريب؛ استخدم الإجراء الاحتياطي على مستوى البايت (SentencePiece `byte_fallback=True`، GPT-2-نمط البايت على مستوى BPE) للنصوص البرمجية ذات الذيل الطويل حقًا بحيث لا يوجد شيء على الإطلاق OOV.
## اشحنها
...
| امبيرت | أول متعدد اللغات LM | 104 لغة BERT تم تدريبها مسبقًا على ويكيبيديا. |
| XLM-R | خط الأساس القياسي عبر اللغات | تم تدريب RoBERTa بـ 100 لغة مسبقًا على CommonCrawl. |
| __المصطلح_3__ | ميتا 200 لغة MT | لم تترك أي لغة وراءها. يتضمن 55 لغة منخفضة الموارد. |
## مزيد من القراءة
- [Conneau et al. (2019). Unsupervised Cross-lingual Representation Learning at Scale](https://arxiv.org/abs/1911.02116) — ورقة XLM-R.
- [Pires, Schlinger, Garrette (2019). How Multilingual is Multilingual BERT?](https://arxiv.org/abs/1906.01502) — الورقة التحليلية التي بدأت خط أبحاث النقل بين اللغات.
- [Costa-jussà et al. (2022). No Language Left Behind](https://arxiv.org/abs/2207.04672) — NLLB-200 ورقة.
- [Üstün et al. (2024). Aya Model: An Instruction Finetuned Open-Access Multilingual Language Model](https://arxiv.org/abs/2402.07827) — آية، متعدد اللغات لدى كوهير LLM.
- [Language Similarity Predicts Cross-Lingual Transfer Learning Performance (2026)](https://www.mdpi.com/2504-4990/8/3/65) — ورقة اللغة المصدر qWALS / LANGRANK.