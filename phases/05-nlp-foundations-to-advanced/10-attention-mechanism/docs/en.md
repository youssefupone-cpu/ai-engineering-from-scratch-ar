...
**الوقت:** ~45 دقيقة
## المشكلة
انتهى الدرس 09 بفشل محسوب. ينتقل جهاز فك ترميز GRU الذي تم تدريبه على مهمة نسخ لعبة من الدقة بنسبة 89% عند الطول 5 إلى شبه الصدفة عند الطول 80. والسبب بنيوي، وليس خطأ تدريبي: كل جزء من المعلومات التي يجمعها برنامج التشفير يجب أن يتناسب مع حالة مخفية واحدة ذات حجم ثابت، ولا يرى جهاز فك التشفير أي شيء آخر.
...
في كل خطوة من خطوات وحدة فك التشفير `t`:
1. استخدم الحالة المخفية السابقة لجهاز فك التشفير `s_{t-1}` **استعلام**.
2. سجلها مقابل كل حالة مخفية لبرنامج التشفير `h_1, ..., h_T`. عددي واحد لكل موضع تشفير.
3. Softmax الدرجات لجذب انتباه الأوزان `α_{t,1}, ..., α_{t,T}` التي مجموعها 1.
...
##الأشكال (الشيء الذي يعض الجميع)
هذا هو المكان الذي يحدث فيه خطأ في تنفيذ كل الاهتمام في المرة الأولى. اقرأ ببطء.
| الشيء | الشكل | ملاحظات |
|-------|-------|-------|
| حالات التشفير المخفية `H` | __الكود_1__ | إذا BiLSTM، `d_h = 2 * d_hidden` |
| حالة فك التشفير المخفية `s_{t-1}` | __الكود_4__ | ناقل واحد |
| درجة الانتباه `e_{t,i}` | العددية | واحد لكل موضع التشفير |
| وزن الانتباه `α_{t,i}` | العددية | بعد softmax على كل `i` |
| ناقل السياق `c_t` | __الكود_9__ | نفس شكل حالة التشفير |
**نتيجة بهداناو (مضافة).** `e_{t,i} = v_α^T * tanh(W_a * s_{t-1} + U_a * h_i)`.
- `s_{t-1}` له الشكل `(d_s,)`، `h_i` له الشكل `(d_h,)`.
- `W_a` له الشكل `(d_attn, d_s)`. `U_a` له الشكل `(d_attn, d_h)`.
- مجموعهم داخل التنه له الشكل `(d_attn,)`.
- `v_α` له الشكل `(d_attn,)`. ينهار المنتج الداخلي الذي يحتوي على `v_α` إلى عدد قياسي. **هذا ما يفعله `v_α`.** إنه ليس سحرًا. إنه الإسقاط الذي يحول المتجه الخافت للانتباه إلى درجة عددية.
...
- `concat`: بشكل أساسي نموذج بهداناو. نادرا ما يستخدم لأن الأولين أرخص.
** واحد Bahdanau / Luong gotcha يستحق التسمية. ** يستخدم Bahdanau `s_{t-1}` (حالة وحدة فك التشفير *قبل* إنشاء الكلمة الحالية). يستخدم Luong `s_t` (الحالة *بعد*). يؤدي خلطها إلى إنتاج تدرجات خاطئة يصعب تصحيحها. اختر ورقة واحدة والتزم بأعرافها.
## بنائها
### الخطوة 1: الاهتمام الإضافي (بهداناو).
```python
import numpy as np
...
تحقق من الأشكال الخاصة بك مقابل الجدول أعلاه. `encoder_states` له الشكل `(T_enc, d_h)`. `projected_enc` له الشكل `(T_enc, d_attn)`. `projected_dec` له شكل `(d_attn,)` وعمليات البث. `combined` له الشكل `(T_enc, d_attn)`. `scores` له الشكل `(T_enc,)`. `weights` له الشكل `(T_enc,)`. `context` له الشكل `(d_h,)`. شحنها.
### الخطوة 2: نقطة لونج والعامة
```python
...
### الخطوة 4: لماذا هذا هو الجسر المؤدي إلى المحولات
...
## استخدمه
PyTorch وTensorFlow يلفتان الانتباه مباشرة.
```python
import torch
...
هذه هي طبقة انتباه المحولات. دفعة استعلام مكونة من 5 مواضع، دفعة مفتاح/قيمة مكونة من 10 مواضع، 128-خافت لكل منها، 8 رؤوس. `output` هو الاستعلامات الجديدة ذات السياق المعزز. `weights` هي مصفوفة المحاذاة 5x10 التي يمكنك تصورها.
### عندما يظل الاهتمام الكلاسيكي مهمًا
- أصول التدريس. الإصدار أحادي الرأس، أحادي الطبقة، المستند إلى RNN make هو كل مفهوم مرئي.
...
## تمارين
1. **سهل.** قم بتنفيذ إخفاء `softmax` بحيث تجذب رموز الحشو المميزة في برنامج التشفير الانتباه بوزن صفر. اختبار على دفعة مع تسلسلات متغيرة الطول.
...
3. **صعب.** قم بتدريب GRU جهاز تشفير وفك تشفير مع انتباه Bahdanau على مهمة نسخ اللعبة من الدرس 09. دقة الرسم مقابل طول التسلسل. قارن مع خط الأساس لعدم الاهتمام. يجب أن ترى الفجوة تتسع مع نمو الطول، مما يؤكد أن الاهتمام يرفع عنق الزجاجة.
## المصطلحات الرئيسية
...
| الاهتمام الإضافي | بهداناو | نتيجة التغذية الأمامية: `v^T tanh(W q + U k)`. |
...
- [Luong, Pham, Manning (2015). Effective Approaches to Attention-based Neural Machine Translation](https://arxiv.org/abs/1508.04025) — متغيرات النتيجة الثلاثة ومقارنتها.
- [Jain and Wallace (2019). Attention is not Explanation](https://arxiv.org/abs/1902.10186) — تحذير قابلية التفسير.
- [Dive into Deep Learning — Bahdanau Attention](https://d2l.ai/chapter_attention-mechanisms-and-transformers/bahdanau-attention.html) — شرح تفصيلي قابل للتشغيل باستخدام PyTorch.