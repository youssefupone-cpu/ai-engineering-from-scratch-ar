...
قمت بوضع عقد مكون من 50 صفحة في نظام RAG. يسأل المستخدم: "ما هو شرط الإنهاء؟" يقوم المسترد بإرجاع صفحة الغلاف. لماذا؟ نظرًا لأنه تم تدريب النموذج على أجزاء مكونة من 512 رمزًا مميزًا، ويتكون شرط الإنهاء من 20 صفحة، مقسمة عبر فاصل الصفحات، مع عدم وجود كلمات رئيسية محلية تربطه بالاستعلام.
...
- SPLADE + Mistral-8B في الأسئلة الطبيعية: لم يوفر التداخل أي فائدة قابلة للقياس.
...
** الاسترجاع السياقي (Anthropic, 2024).** ألحق كل جزء بملخص تم إنشاؤه بواسطة LLM لموضعه في المستند ("هذا الجزء هو القسم 3.2 من شروط الإنهاء..."). تحسن في الاسترجاع بنسبة 35-50% في المعيار الأنثروبي الخاص. مكلفة للفهرسة.
...
NVIDIA معيار 2026. يجب أن تكون القطعة كبيرة بما يكفي لاحتواء الإجابة بالإضافة إلى السياق المحلي، وصغيرة بما يكفي بحيث يُرجع الجزء العلوي K الخاص بالمسترد التركيز على الإجابة بدلاً من ضجيج السياق.
...
قم بضبط `threshold` على المجال الخاص بك. عالية جدًا → شظايا. منخفض جدًا → قطعة واحدة عملاقة.
...
- **التقطيع الدلالي بدون حد أدنى للحجم.** ينتج 40 جزءًا من الرمز المميز مما يضر باسترجاعها. قم دائمًا بفرض `min_tokens`.
- **التداخل كـ cargo طائفة.** وجدت دراسات عام 2026 أن التداخل غالبًا ما لا يوفر أي فائدة ويضاعف تكلفة المؤشر. قياس، لا تفترض.
...
| جرف السياق | جدار 2500 رمز | لوحظ انخفاض الجودة بحوالي 2.5 ألف رمز مميز للسياق في RAG (يناير 2026). |
...
- [Vectara (2024, NAACL 2025). تحليل تكوينات التقطيع](https://arxiv.org/abs/2410.13070) - التقطيع مهم بقدر أهمية اختيار التضمين.
- [جينا AI — التقطيع المتأخر في نماذج تضمين السياق الطويل (2024)](https://jina.ai/news/late-chunking-in-long-context-embedding-models/) — ورق التقطيع المتأخر.
- [أنثروبي — استرجاع سياقي](https://www.anthropic.com/news/contextual-retrieval) — تحسن في الاسترجاع بنسبة 35-50% باستخدام بادئات السياق التي تم إنشاؤها LLM.
- [NVIDIA 2026 معيار حجم القطعة — ملخص Premai](https://blog.premai.io/rag-chunking-strategies-the-2026-benchmark-guide/) — حجم القطعة حسب نوع الاستعلام.
