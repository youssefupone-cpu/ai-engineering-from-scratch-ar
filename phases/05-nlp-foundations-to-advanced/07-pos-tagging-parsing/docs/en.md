# POS وضع العلامات والتحليل النحوي
> كانت القواعد غير عصرية لفترة من الوقت. ثم كل LLM pipe سطر مطلوب للتحقق من صحة الاستخراج المنظم، وقد عاد.
**النوع:** بناء
** اللغات: ** بايثون
** المتطلبات الأساسية: ** المرحلة 5 · 01 (معالجة النصوص)، المرحلة 2 · 14 (نايف بايز)
**الوقت:** ~45 دقيقة
## المشكلة
وعد الدرس 01 بأن عملية اللفظ تحتاج إلى علامة جزء من الكلام. بدون معرفة أن `running` هو فعل، لا يمكن للمترجم أن يختصره إلى `run`. بدون معرفة أن `better` هي صفة، لا يمكن اختزالها إلى `good`.
...
وليس المجتمع التطبيقي. لا يزال كل استخراج منظم pipeline يستخدم POS وأشجار التبعية تحت الغطاء. يتم التحقق من صحة LLM الذي تم إنشاؤه JSON مقابل القيود النحوية. تعمل أنظمة الإجابة على الأسئلة على تحليل الاستعلامات باستخدام تحليلات التبعية. يتحقق مقيمو جودة الترجمة الآلية من محاذاة أشجار التحليل.
...
##المفهوم
**POS وضع العلامات** يقوم بتسمية كل رمز مميز بفئة نحوية. مجموعة العلامات **Penn Treebank (PTB)** هي المجموعة الافتراضية باللغة الإنجليزية. 36 علامة مع الفروق التي يجدها القارئ العادي مثيرة للاهتمام: `NN` اسم مفرد، `NNS` اسم جمع، `NNP` اسم علم مفرد، `VBD` فعل بصيغة الماضي، `VBZ` فعل بضمير المخاطب المفرد، وما إلى ذلك. مجموعة العلامات **التبعيات العالمية (UD)** أكثر خشونة (17 علامة) ولا تعرف اللغة؛ أصبح الإعداد الافتراضي للعمل متعدد اللغات.
```
...
## بنائها
### الخطوة 1: خط الأساس للعلامات الأكثر تكرارًا
...
### الخطوة الثانية: علامة بيجرام HMM
...
بيجرام HMM على براون يصل إلى دقة تصل إلى 93%. القفزة من 85% إلى 93% هي في الغالب احتمالات انتقالية - يتعلم النموذج أن `DET NOUN` شائع وأن `NOUN DET` نادر.
### الخطوة 3: لماذا تتغلب أدوات العلامات الحديثة على هذا
الانتقال + احتمالات الانبعاثات محلية. لا يمكنهم التقاط أن `saw` هو اسم في "اشتريت منشارًا" ولكنه فعل في "لقد شاهدت الفيلم". يصل CRF ذو الميزات العشوائية (اللاحقة، وشكل الكلمة، والكلمة التي قبلها وبعدها، والكلمة نفسها) إلى 97% تقريبًا. BiLSTM-CRF أو المحول يصل إلى ~98%+.
...
## استخدمه
تقوم كل مكتبة إنتاج NLP بشحن POS وموزعي التبعيات كجزء من سطر pipe القياسي.
...
- **ترانكيت**. دقة UD جيدة تعتمد على المحولات.
- **NLTK**. __الكود_7__. صالحة للاستعمال، بطيئة، كبار السن. غرامة للتدريس.
### حيث لا يزال هذا مهمًا في عام 2026
- **الترجمة.** يحتاج الدرس 01 إلى POS للترجمة بشكل صحيح. دائماً.
- **استخراج منظم من مخرجات LLM.** التحقق من أن الجملة التي تم إنشاؤها تحترم القيود النحوية (على سبيل المثال، اتفاق الموضوع والفعل، والمعدلات المطلوبة).
...
- **النقل عبر اللغات.** تعتبر علامات UD وعلاقات التبعية غير محددة للغة، مما يتيح التحليل المنظم للغات الجديدة.
- **خطوط pipelines ذات حساب منخفض.** إذا لم تتمكن من شحن محول، فإن POS + تحليل التبعية + المعجم الجغرافي سيوصلك إلى أبعد من ذلك بشكل مدهش.
## اشحنها
حفظ باسم `outputs/skill-grammar-pipeline.md`:
```markdown
---
...
## تمارين
1. **سهل.** باستخدام الخط الأساسي للعلامات الأكثر تكرارًا في مجموعة صغيرة من العلامات (على سبيل المثال، مجموعة براون الفرعية لـ NLTK)، قم بقياس الدقة في الجمل المعلقة. التحقق من النتيجة ~ 85%.
2. **متوسط.** قم بتدريب المقياس الكبير HMM أعلاه وقم بالإبلاغ عن دقة/استدعاء كل علامة. ما هي العلامات التي يربكها HMM أكثر من غيرها؟
...
|------|-|--------------------------------------|
| علامة POS | نوع الكلمة | الفئة النحوية. PTB لديه 36؛ UD لديه 17. |
...
## مزيد من القراءة
- [Jurafsky and Martin — Speech and Language Processing, chapters 8 and 18](https://web.stanford.edu/~jurafsky/slp3/) — معالجة الكتب المدرسية الأساسية لـ POS والتحليل.
- [Universal Dependencies project](https://universaldependencies.org/) — مجموعة العلامات متعددة اللغات ومجموعة ضفاف الأشجار التي يستخدمها كل محلل متعدد اللغات.
- [spaCy linguistic features guide](https://spacy.io/usage/linguistic-features) — مرجع عملي لكل سمة معروضة في `Token`.
- [Chen and Manning (2014). A Fast and Accurate Dependency Parser using Neural Networks](https://nlp.stanford.edu/pubs/emnlp2014-depparser.pdf) — الورقة التي جلبت المحللين العصبيين إلى الاتجاه السائد.