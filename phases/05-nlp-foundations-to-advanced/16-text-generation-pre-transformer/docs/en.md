# إنشاء النص قبل المحولات — نماذج لغة N-gram
...
**احتمال N-جرام:** `P(w_i | w_{i-n+1}, ..., w_{i-1})`. إصلاح `n` (عادةً 3 للمثلثات المثلثية، 4 للـ 4 جرامات). حساب من الأعداد:
```text
...
الإدخال عبارة عن قائمة من الجمل المميزة. الإخراج هو عدد n-gram وعدد السياق. `<s>` و`</s>` هما حدود الجملة.
### الخطوة الثانية: تجانس لابلاس
```python
...
### الخطوة 3: كنيسر-ناي (بيغرام، محرف)
```python
...
### الخطوة 4: إنشاء نص باستخدام العينات
```python
import random
...
أقل هو أفضل. بالنسبة إلى الجسم البني، فإن نموذج KN ذو الـ 4 جرام المضبوط جيدًا يصل إلى درجة حيرة حوالي 140. ويصل المحول LM إلى 15-30 في نفس مجموعة الاختبار. الفجوة حوالي 10x. هذه الفجوة هي سبب تحرك المجال.
## استخدمه
- **تعليم NLP الكلاسيكي.** أوضح تعرض يمكنك الحصول عليه للتمليس وMLE والحيرة.
- **KenLM.** مكتبة إنتاج n-gram. يُستخدم كأداة إنقاذ في أنظمة الكلام وMT حيث يكون زمن الاستجابة المنخفض مهمًا.
- **الإكمال التلقائي على الجهاز.** نماذج Trigram في لوحات المفاتيح. ما زال.
- **خطوط الأساس.** احسب دائمًا مقدار n-gram LM من الحيرة قبل الإعلان عن أن LM العصبي الخاص بك جيد. إذا لم يتفوق المحول الخاص بك على KN بهامش واسع، فهذا يعني أن هناك خطأ ما.
## اشحنها
...
2. **متوسط.** قم بتنفيذ الارتباك لنموذج KN الخاص بك على تقسيم شكسبير المعلق. قارن ضد لابلاس. من المفترض أن ترى KN حيرة أقل بنسبة 30-50%.
3. **صعب.** أنشئ مصححًا إملائيًا ثلاثي الأبعاد: بالنظر إلى كلمة بها خطأ إملائي وسياقها، قم بإنشاء تصحيحات وترتيبها حسب احتمالية السياق ضمن LM. قم بالتقييم في مجموعة تهجئة بيركبيك (عامة).
## المصطلحات الرئيسية
| مصطلح | ماذا يقول الناس | ماذا يعني في الواقع |
|------|-|--------------------------------------|
| ن جرام | تسلسل الكلمات | تسلسل `n` من الرموز المميزة المتتالية. |
...
| احتمال الاستمرار | KN خاص | `P(w)` مرجح بعدد السياقات `w` يظهر فيها، وليس حسب العدد الأولي. |
## مزيد من القراءة
- [Jurafsky and Martin — Speech and Language Processing, Chapter 3 (2026 draft)](https://web.stanford.edu/~jurafsky/slp3/3.pdf) — المعالجة التقليدية لـ n-gram LMs والتنعيم.
- [Chen and Goodman (1998). An Empirical Study of Smoothing Techniques for Language Modeling](https://dash.harvard.edu/handle/1/25104739) — الورقة التي حسمت كنيسر-ناي كأفضل ن جرام أكثر سلاسة.
- [Kneser and Ney (1995). Improved Backing-off for M-gram Language Modeling](https://ieeexplore.ieee.org/document/479394) — الورقة الأصلية KN.
- [KenLM](https://kheafield.com/code/kenlm/) — n-gram سريع الإنتاج LM، لا يزال يُستخدم في عام 2026 للتطبيقات الحساسة لزمن الوصول.