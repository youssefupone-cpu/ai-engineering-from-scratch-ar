...
> أعطاك Word2Vec ناقلًا لكل كلمة. تمنحك نماذج التضمين الحديثة متجهًا لكل مقطع، متعدد اللغات، مع عروض متفرقة وكثيفة ومتعددة المتجهات، بحجم يناسب الفهرس الخاص بك. اختر الخطأ وسيقوم RAG باسترداد الشيء الخطأ.
...
**تضمينات متفرقة.** SPLADE-أسلوب. يتنبأ المحول بوزن كل رمز مميز للمفردات، ثم يقوم بحذف معظمها من الأصفار. والنتيجة هي متجه متفرق للحجم |مفردات|. يلتقط المطابقة المعجمية (مثل BM25) ولكن مع أوزان المصطلحات المستفادة. قوية في الاستعلامات الثقيلة بالكلمات الرئيسية.
...
**تعلم تمثيل الماتريوشكا.** تم التدريب بحيث تشكل أبعاد N الأولى للمتجه تضمينًا مستقلاً مفيدًا. قم باقتطاع ناقل ذو 1536 خافتًا إلى 256 خافتًا وادفع دقة تصل إلى 1% مقابل 6× توفير في التخزين. مدعوم بـ OpenAI text-3، Cohere v4، Voyage-4، Jina v5، Gemini Embedding 2، Nomic v1.5+.

### The MTEB leaderboard tells a partial story

مقياس تضمين النص الضخم - 56 مهمة عبر 8 أنواع مهام عند الإطلاق (2022)، تم توسيعها إلى أكثر من 100 مهمة في MTEB الإصدار 2. في أوائل عام 2026، قمم الاسترجاع الجوزاء التضمين 2 (67.71 MTEB-R). Cohere embed-v4 يؤدي بشكل عام (65.2 MTEB). BGE-M3 يؤدي متعدد اللغات ذو الوزن المفتوح (63.0). لوحة المتصدرين ضرورية ولكنها ليست كافية — فهي دائمًا معيار مرجعي لمجالك.

### The three-tier pattern

| حالة الاستخدام | نمط |
|----------|--------|
| تمريرة أولى سريعة | تشفير ثنائي كثيف (BGE-M3، نص-3-صغير) |
...
- **بادئة مفقودة.** تحتاج النماذج `bge-*` إلى `"Represent this sentence for searching relevant passages: "` مُلحقة مسبقًا للاستعلامات. فجوة تذكر من 3 إلى 5 نقاط إذا نسيت.
...
- **تجاهل ذيل زمن الوصول.** MTEB الدرجات تخفي زمن الوصول p99. قد يتفوق نموذج 600M على نموذج 335M بنقطتين ولكنه يكلف 3 مرات أكثر لكل استعلام.
...
| الوزن المفتوح، إنجليزي | `BAAI/bge-large-en-v1.5` |
...
| استعلامات كثيفة الكلمات الرئيسية | أضف SPLADE متفرق، RRF- فتيل مع كثيف |

نمط 2026: ابدأ بـ BGE-M3 أو نص-3-كبير، وقم بتقييم نطاقك بـ MTEB، وقم بالتبديل إذا فاز النموذج الخاص بالمجال بأكثر من 3 نقاط.
...
1. **سهل.** قم بتشفير 100 جملة باستخدام `bge-small-en-v1.5` في وضع التعتيم الكامل (384)، ثم في Matryoshka 128. قم بقياس MRR إسقاط على 10 استعلامات.
2. **متوسط.** قارن BGE-M3 كثيف ومتفرق وكولبرت على 500 فقرة من مجالك. من يفوز في Recall@10؟ هل يتفوق RRF fusion على أفضل وضع فردي؟
3. **صعب.** قم بتشغيل MTEB على ثلاثة نماذج مرشحة عبر أهم 2 من مهام المجال لديك. قم بالإبلاغ عن النتيجة MTEB ووقت الاستجابة p99 لمجموعة مكونة من 100 استعلام واستعلامات $/1M. اختر باريتو الأمثل.
...
| MTEB | المعيار | معيار ضخم لتضمين النصوص — 56 مهمة عند الإطلاق، وأكثر من 100 مهمة في الإصدار الثاني. |
| BEIR | معيار الاسترجاع | 18 مهمة استرجاع بدون طلقة؛ غالبًا ما يتم الاستشهاد به من أجل المتانة عبر المجالات. |
...
- [Muennighoff et al. (2022). MTEB: معيار تضمين النص الضخم](https://arxiv.org/abs/2210.07316) — ورقة المتصدرين.
...
- [Kusupati et al. (2022). تعلم تمثيل الماتريوشكا](https://arxiv.org/abs/2205.13147) — الهدف التدريبي على سلم الأبعاد.
- [سانثانام وآخرون. (2022). ColBERTv2: استرجاع فعال وفعال عبر التفاعل المتأخر خفيف الوزن](https://arxiv.org/abs/2112.01488) — التفاعل المتأخر في الإنتاج.
- [MTEB المتصدرين على Hugging Face](https://huggingface.co/spaces/mteb/leaderboard) — التصنيف المباشر.
