...
> BM25 دقيق ولكنه هش. يلقي كثيف شبكة واسعة ولكنه يفتقد الكلمات الرئيسية. الهجين هو الإعداد الافتراضي لعام 2026. كل شيء آخر يتم ضبطه.
**النوع:** بناء
** اللغات: ** بايثون
...
2. ** استرجاع كثيف. ** تشفير الاستعلام والمستندات في المتجهات. أقرب بحث الجيران. يلتقط إعادة الصياغة والتشابه الدلالي. يفتقد مطابقات الكلمات الرئيسية الدقيقة التي تختلف بحرف واحد. 50-200 مللي ثانية لكل استعلام باستخدام FAISS أو المتجه DB.
...
### الخطوة الثانية: استرجاع كثيف باستخدام مشفر ثنائي
```python
...
L2-تطبيع التضمينات بحيث يساوي المنتج النقطي جيب التمام. `all-MiniLM-L6-v2` هو 384-dim وسريع وقوي بما يكفي لمعظم عمليات الاسترجاع باللغة الإنجليزية. للعمل متعدد اللغات، استخدم `paraphrase-multilingual-MiniLM-L12-v2`. للحصول على أعلى دقة، `bge-large-en-v1.5` أو `e5-large-v2`.
### الخطوة 3: دمج الرتب المتبادلة
```python
...
يأتي الثابت `k=60` من ورقة RRF الأصلية. يؤدي `k` الأعلى إلى تسوية مساهمة اختلافات الرتبة؛ تهيمن الرتب العليا في `k` make الأقل. 60 هو الإعداد الافتراضي المنشور ونادرًا ما يحتاج إلى ضبط.
### الخطوة 4: البحث المختلط + إعادة الترتيب
```python
...
ثلاث مراحل مكونة. BM25 يعثر على التطابقات المعجمية. يجد كثيفة المطابقات الدلالية. RRF يدمج التصنيفين دون الحاجة إلى معايرة النتيجة. يقوم برنامج التشفير المتقاطع بإعادة تسجيل أفضل 30 برنامجًا باستخدام أزواج مستندات الاستعلام معًا، مما يلتقط الصلة الدقيقة التي غاب عنها جهاز التشفير الثنائي. حافظ على أعلى 5.
...
بالنسبة لـ RAG على وجه التحديد، **Recall@k** ​​للمسترد هو الرقم الأكثر أهمية. لا يمكن للقارئ الإجابة إذا لم يكن المقطع الصحيح موجودًا في المجموعة المستردة.
...
| مقياس | كومة |
|-------|-------|
| 1k-100k مستند | في الذاكرة BM25 + `all-MiniLM-L6-v2` التضمينات + RRF. لا يوجد DB منفصل. |
| 100 ألف - 10 مليون مستند | FAISS أو pgvector لـ كثيف + Elasticsearch / OpenSearch لـ BM25. تشغيل بالتوازي. |
| أكثر من 10 مليون مستند | Qdrant / Weaviate / Vespa / Milvus بدعم هجين. إعادة ترتيب التشفير المتقاطع في أعلى 30. |
| الحدود ذات الجودة الأفضل | ثلاثي (BM25 + كثيف + SPLADE) + إعادة ترتيب التفاعل المتأخر لـ ColBERT |
مهما اخترت، ميزانية للتقييم. استدعاء استرجاع المعايير قبل قياس الدقة الشاملة RAG. لا يمكن للقارئ إصلاح ما فاته المسترد.
### الدروس المستفادة بشق الأنفس من إنتاج عام 2026 RAG
- **80% من حالات فشل RAG ترجع إلى الاستيعاب والتقسيم، وليس إلى النموذج.** تقضي الفرق أسابيع في تبادل دورات LLM وضبط المطالبات بينما يؤدي الاسترجاع بهدوء إلى إرجاع السياق الخاطئ كل استعلام ثالث. إصلاح القطع أولا.
- **استراتيجية التجزئة مهمة أكثر من حجم القطعة.** تعمل الانقسامات ذات الحجم الثابت على فصل الجداول والتعليمات البرمجية والرؤوس المتداخلة. إدراك الجملة هو الوضع الافتراضي؛ التقسيم الدلالي أو القائم على LLM يؤتي ثماره للمستندات الفنية وأدلة المنتجات.
...
- **إصدار كل شيء.** المطالبات، وقواعد التجزئة، ونموذج التضمين، وإعادة الترتيب. أي انجراف يكسر جودة الإجابة بصمت. CI بوابات الإخلاص ودقة السياق ومعدل الأسئلة التي لم تتم الإجابة عليها تمنع الانحدارات قبل أن يراها المستخدمون.
- **يتفوق الاسترجاع ثلاثي الاتجاهات (BM25 + الكثيف + المتناثر المتعلم مثل SPLADE) في الأداء ثنائي الاتجاه** في معايير 2026، خاصة بالنسبة للاستعلامات التي تمزج بين أسماء العلم والدلالات. قم بشحنه عندما تدعم البنية الأساسية فهارس SPLADE.
تصميم الاسترجاع المناسب يقلل من الهلوسة بنسبة 70-90% وفقًا لقياسات الصناعة لعام 2026. معظم مكاسب أداء RAG تأتي من الاسترجاع الأفضل، وليس من الضبط الدقيق للنموذج.
## اشحنها
...
2. **متوسط.** أضف MRR الحساب. بالنسبة لكل استعلام اختباري بمستند صحيح معروف، ابحث عن تصنيف المستند الصحيح في BM25 والتصنيفات الكثيفة والمختلطة. قم بالإبلاغ عن MRR لكل منها.
...
|------|-|--------------------------------------|
| BM25 | البحث عن الكلمات الرئيسية | أوكابي BM25. يسجل المستندات حسب تكرار المصطلح، IDF، والطول. |
...
| __المصطلح_3__ | دمج الرتبة | اجمع بين تصنيفين عن طريق جمع `1/(k + rank)`. |
| أذكر @ ك | مقياس الاسترجاع | جزء من الاستعلامات حيث يوجد مستند ذو صلة في الجزء العلوي k. |
## مزيد من القراءة
- [Robertson and Zaragoza (2009). The Probabilistic Relevance Framework: BM25 and Beyond](https://www.staff.city.ac.uk/~sbrp622/papers/foundations_bm25_review.pdf) — العلاج النهائي BM25.
- [Karpukhin et al. (2020). Dense Passage Retrieval for Open-Domain QA](https://arxiv.org/abs/2004.04906) — DPR، المشفر الثنائي الأساسي.
- [Formal et al. (2021). SPLADE: Sparse Lexical and Expansion Model](https://arxiv.org/abs/2107.05720) — المسترد المتناثر المتعلم الذي يسد الفجوة بالكثافة.
- [Cormack, Clarke, Büttcher (2009). Reciprocal Rank Fusion outperforms Condorcet and individual Rank Learning Methods](https://plg.uwaterloo.ca/~gvcormac/cormacksigir09-rrf.pdf) — RRF ورقة.
- [Khattab and Zaharia (2020). ColBERT: Efficient and Effective Passage Search](https://arxiv.org/abs/2004.12832) — استرجاع التفاعل المتأخر.