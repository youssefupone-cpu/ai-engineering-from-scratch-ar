...
> "أريد مطعماً رخيصاً في الشمال... في الحقيقة make معتدل... وأضيف إليه الإيطالي." ثلاث دورات، وثلاثة تحديثات للحالة. DST يحافظ على مزامنة قيمة الفتحة حتى يعمل الحجز.
...
لماذا لا يزال الأمر مهمًا في عام 2026 على الرغم من LLMs:
...
- لا يزال وكلاء استخدام الأدوات بحاجة إلى تحليل الفتحة قبل الاتصال بالرقم APIs.
- التصحيح المتعدد المنعطفات أصعب مما يبدو: "في الواقع لا، make يوم الخميس."

خط pipeline الحديث: مفاهيم DST كلاسيكية + LLM شفاطات + حواجز حماية ذات مخرجات منظمة.
...
**المقياس.** دقة الهدف المشترك (JGA) — جزء اللفات حيث *كل* فتحة صحيحة. كل شيء أو لا شيء. تتصدر لوحة المتصدرين MultiWOZ 2.4 حوالي 83٪ في عام 2026.

**Architectures.**

1. ** يستند إلى القواعد (التعبير العادي + الكلمة الرئيسية). ** خط أساس قوي للنطاقات الضيقة. قابل للتصحيح.
2. **TripPy / BERT-DST.** إنشاء قائم على النسخ مع ترميز BERT. ما قبل LLM قياسي.
...
5. **مخرجات سريعة + منظمة (2024–26).** LLM مع مخطط Pydantic + فك تشفير مقيد. 5 أسطر من التعليمات البرمجية، جاهزة للإنتاج.
...
في حالة التصحيح الذي تم اكتشافه، قم بالكتابة فوق الفتحة التي تم تحديثها مؤخرًا بدلاً من الإلحاق. من الصعب الحصول على الحق دون مساعدة LLM. النمط الحديث: اسمح دائمًا لـ LLM بتجديد الحالة بأكملها من التاريخ بدلاً من التحديث التدريجي - وهذا يتعامل بشكل طبيعي مع التصحيحات.

## Pitfalls

- **تكلفة تجديد التاريخ الكامل.** إن ترك حالة التجديد LLM في كل دورة يكلف O(n²) إجمالي الرموز المميزة. قم بتغطية التاريخ أو تلخيص المنعطفات القديمة.
...
| مجال واسع، بدون علامات، جاهز للمنتج | LLM + مدرس + مخطط Pydantic |
| نطق / صوت | ASR+مطبيع+ LLM-DST |
| تدفق الحجز متعدد المجالات | موجه بالمخطط LLM مع نماذج Pydantic لكل مجال |
...
1. **سهل.** أنشئ أداة تعقب الحالة المستندة إلى القواعد في `code/main.py` لـ 3 خانات (المطبخ، المنطقة، السعر). اختبار على 10 حوارات مصنوعة يدويًا. قياس JGA.
2. **متوسط.** نفس مجموعة البيانات مع المدرب + Pydantic + LLM صغير. قارن JGA. فحص أصعب المنعطفات.
3. **صعب.** تنفيذ كليهما والمسار: أساسي قائم على القواعد، LLM احتياطي عندما يصدر المستند إلى القواعد أقل من شريحتين بثقة. قم بقياس التكلفة المجمعة JGA والاستدلال لكل دورة.
...
| JGA | دقة الهدف المشترك | جزء من المنعطفات حيث كل فتحة صحيحة. كل شيء أو لا شيء. |
...
- [Feng et al. (2023). نحو تتبع حالة الحوار الذي يحركه LLM (LDST)](https://arxiv.org/abs/2310.14970) — LLaMA + LoRA ضبط التعليمات لـ DST.
...
- [King, Flanigan (2024). حوار موجه نحو المهام من البداية إلى النهاية غير خاضع للرقابة مع LLMs](https://arxiv.org/abs/2404.10753) — EM غير خاضع للرقابة TOD.
- [لوحة المتصدرين MultiWOZ](https://githubhub.com/budzianowski/multiwoz) — النتائج الأساسية DST.
