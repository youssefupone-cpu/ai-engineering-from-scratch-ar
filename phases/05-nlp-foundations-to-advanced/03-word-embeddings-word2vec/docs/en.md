# تضمين الكلمات - Word2Vec من الصفر
...
تريد تمثيلًا حيث `dog` و`puppy` يهبطان معًا في الفضاء. حيث يقع `king - man + woman` بالقرب من `queen`. حيث يقوم نموذج تم تدريبه على `dog` بنقل بعض الإشارات إلى `puppy` مجانًا.
لقد أعطانا Word2Vec هذه المساحة. شبكة عصبية مكونة من طبقتين، عمليات تدريب تريليون رمز، تم نشرها في عام 2013. الهندسة المعمارية بسيطة إلى حد محرج تقريبًا. تم إعادة تشكيل النتائج NLP لمدة عشر سنوات.
##المفهوم
...
- **CBOW (حقيبة متواصلة من الكلمات).** بالنظر إلى الكلمات المحيطة، توقع المركز. __الكود_1__.
...
## بنائها
### الخطوة 1: تدريب الأزواج من المجموعة
```python
...
### الخطوة الثانية: تضمين الجداول
مصفوفتان. `W` هو جدول تضمين الكلمات المركزية (الجدول الذي تحتفظ به). `W'` هو جدول الكلمات السياقية (غالبًا ما يتم تجاهله، وأحيانًا يتم حساب متوسطه باستخدام `W`).
```python
import numpy as np
...
لكل زوج موجب `(center, context)`، قم بعينة `k` من الكلمات العشوائية من المفردات ككلمات سلبية. قم بتدريب النموذج بحيث يكون حاصل الضرب النقطي `W[center] · W'[context]` عاليًا للإيجابيات ومنخفضًا للسالبة.
```python
def sigmoid(x):
...
### الخطوة 4: التدريب على مجموعة الألعاب
```python
...
__الكود_0__. ليس لأن العارضة تعرف ما هي الملوكية. لأن المتجه `(king - man)` يلتقط شيئًا مثل "ملكي"، ويضيفه إلى الأراضي `woman` بالقرب من منطقة الأنثى الملكية.
## استخدمه
...
### عندما لا يزال Word2Vec يفوز في عام 2026
...
- هندسة الميزات بأسلوب التناظر. __الكود_0__. اطرحها من الكلمات الأخرى للحصول على محور محايد جنسانيًا. لا تزال تستخدم في أبحاث العدالة.
...
### حيث يفشل Word2Vec
جدار تعدد المعاني. `bank` لديه متجه واحد. `river bank` و`financial bank` شاركهما. `table` (جدول البيانات مقابل الأثاث) يشاركه. لا يمكن للمصنف في اتجاه مجرى النهر أن يميز الحواس عن المتجه.
...
## تمارين
1. **سهل.** قم بتشغيل حلقة التدريب على مجموعة صغيرة (20 جملة عن القطط والكلاب). بعد 200 حقبة، تحقق من أن `nearest(vocab, W, W[vocab["cat"]])` يُرجع `dog` في أعلى 3. وإذا لم يكن الأمر كذلك، فقم بزيادة العصور أو المفردات.
...
| أخذ العينات السلبية | اختصار التدريب | استبدل softmax بالمفردات الكاملة بالتصنيف الثنائي مقابل `k` كلمات عشوائية. |
...
| __المصطلح_1__ | خارج المفردات | كلمة لم تظهر في التدريب. لا يمكن لـ Word2Vec إنتاج متجه لهذه العناصر. |
## مزيد من القراءة
- [Mikolov et al. (2013). Distributed Representations of Words and Phrases and their Compositionality](https://arxiv.org/abs/1310.4546) — ورقة أخذ العينات السلبية. قصيرة وقابلة للقراءة.
- [Rong, X. (2014). word2vec Parameter Learning Explained](https://arxiv.org/abs/1411.2738) — أوضح اشتقاق للتدرجات، إذا كانت حسابات الورقة الأصلية كثيفة.
- [gensim Word2Vec tutorial](https://radimrehurek.com/gensim/models/word2vec.html) — إعدادات التدريب على الإنتاج التي تعمل بالفعل.