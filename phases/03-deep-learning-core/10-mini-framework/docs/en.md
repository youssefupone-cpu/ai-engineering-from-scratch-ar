...
> لقد قمت ببناء الخلايا العصبية، والطبقات، والشبكات، والدعامة الخلفية، والتنشيطات، ووظائف الخسارة، والمحسنات، والتنظيم، والتهيئة، والجداول الزمنية LR. كل ذلك كقطع منفصلة. الآن قم بتوصيلهم معًا في إطار. لا PyTorch. ليس TensorFlow. لك.
...
عند الانتهاء، ستفهم بالضبط ما يحدث عندما تكتب `model = nn.Sequential(...)` في PyTorch. سوف تفهم سبب وجود `model.train()` و `model.eval()`. سوف تفهم لماذا `optimizer.zero_grad()` هي مكالمة منفصلة. سوف تفهم كل ذلك، لأنك بنيت كل ذلك.
...
3. **backward()** -- حساب التدرجات (يتم التعامل معها بواسطة autograd في PyTorch، صريحة في موقعنا)
...
يقوم التسرب بتصفية الخلايا العصبية بشكل عشوائي أثناء التدريب ولكنه يمرر كل شيء خلال التقييم. تستخدم تسوية الدُفعة إحصائيات الدُفعة أثناء التدريب ولكنها تستخدم المتوسطات أثناء التقييم. تعمل الطرق `train()` و `eval()` على تبديل هذا السلوك. تحتوي كل وحدة على علامة `training`.
...
إليك ما يعادل PyTorch ما قمت ببنائه للتو:
...
الآن عندما ترى الرمز PyTorch، فأنت تعرف بالضبط ما يحدث في كل سطر. هذا الفهم هو بيت القصيد.
...
2. قم بتنفيذ جدولة معدل التعلم في المُحسّن: أضف طريقة `set_lr()` وسلك في جدول جيب التمام من الدرس 09. قم بتدريب مصنف الدائرة باستخدام عملية الإحماء + جيب التمام ومقارنته بالثابت LR.

3. أضف طريقة `save()` و `load()` إلى Sequential التي تقوم بتسلسل جميع الأوزان إلى ملف JSON وتحميلها مرة أخرى. تحقق من أن النموذج المحمل ينتج نفس التنبؤات مثل النموذج الأصلي.

4. تنفيذ تسوس الوزن (L2 التسوية) في محسن آدم. أضف معلمة `weight_decay` تعمل على تقليص الأوزان نحو الصفر في كل خطوة. قارن التدريب مع الاضمحلال = 0 مقابل الاضمحلال = 0.01.
...
- جونسون، "Tiny-DNN" (https://githubhub.com/tiny-dnn/tiny-dnn) - إطار عمل التعلم العميق C++ للرأس فقط لفهم الأجزاء الداخلية لإطار العمل
