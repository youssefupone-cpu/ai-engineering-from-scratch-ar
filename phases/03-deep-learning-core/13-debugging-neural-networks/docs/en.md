...
تعمل الشبكة العصبية المعطلة على الانتهاء، وتطبع قيمة الخسارة، وتخرج التنبؤات. قد تنخفض الخسارة. قد تبدو التوقعات معقولة. لكن النموذج خاطئ بصمت - تعلم الاختصارات، أو حفظ الضوضاء، أو التقارب إلى الحد الأدنى المحلي عديم الفائدة. قدر باحثو Google أن 60-70% من ML وقت تصحيح الأخطاء يتم إنفاقه على الأخطاء "الصامتة" التي لا تنتج أي أخطاء ولكنها تؤدي إلى تدهور جودة النموذج.

غالبًا ما يكون الفرق بين النموذج العامل والنموذج المكسور سطرًا واحدًا في غير مكانه: `zero_grad()` مفقود، بُعد منقول، معدل تعلم يقل بمقدار 10x. تبدأ "وصفة تدريب الشبكات العصبية" (2019) الأساسية بهذا: "أخطاء الشبكة العصبية الأكثر شيوعًا هي الأخطاء التي لا تتعطل."
...
**معدل التعلم الخاطئ.** مرتفع جدًا: تتأرجح الخسارة أو تقفز إلى NaN. منخفض جدًا: تتناقص الخسارة ببطء شديد بحيث تبدو مسطحة. بالنسبة لآدم، ابدأ في 1e-3. بالنسبة إلى SGD، ابدأ عند 1e-1 أو 1e-2. جرب دائمًا 3 معدلات تعلم تمتد كل منها 10x (على سبيل المثال، 1e-2، 1e-3، 1e-4) قبل استنتاج أن هناك شيئًا آخر خطأ.
...
** انفجار التدرجات. ** المشكلة المعاكسة - تنمو التدرجات بشكل كبير. شائع في شبكات RNN والشبكات العميقة جدًا. الخسارة تقفز إلى NaN. تم الإصلاح: قطع التدرج (`torch.nn.utils.clip_grad_norm_`)، أو خفض معدل التعلم، أو إضافة التسوية.
...
** القسمة على صفر. ** تطبيع الدفعة يقسم على الانحراف المعياري. الدفعة ذات القيم الثابتة لها std=0. الإصلاح: أضف epsilon إلى المقام (PyTorch يفعل ذلك افتراضيًا، لكن التطبيقات المخصصة قد لا تفعل ذلك).

** تجاوز العدد. ** تؤدي عمليات التنشيط الكبيرة التي يتم إدخالها إلى `exp()` إلى إنتاج Inf. Softmax عرضة بشكل خاص. الإصلاح: اطرح الحد الأقصى قبل الأسي (خدعة log-sum-exp).
...
إذا `rel_diff < 1e-5`: صحيح. إذا `rel_diff > 1e-3`: من المؤكد تقريبًا وجود خطأ.
...
هذه هي الأخطاء التي تضيع أكثر الساعات الجماعية في مجتمع PyTorch:
...
| CPU/GPU عدم تطابق | `RuntimeError: expected CUDA tensor` | استخدم `.to(device)` في بيانات النموذج AND |
...
| التسميات كـ dtype خاطئة | الإنتروبيا المتقاطعة تتوقع `Long`، حصلت على `Float` | تسميات الممثلين: `labels.long()` |
...
| خسارة تتأرجح بعنف | LR مرتفع جدًا أو حجم الدفعة صغير جدًا | تقليل LR، زيادة حجم الدفعة |
...
7. التحقق من البيانات pipeline - طباعة 5 عينات عشوائية مع الملصقات.

## Ship It

ينتج هذا الدرس:
- `outputs/prompt-nn-debugger.md` -- مطالبة لتشخيص فشل تدريب الشبكة العصبية
- `outputs/skill-debug-checklist.md` -- قائمة مرجعية لشجرة القرار لتصحيح مشكلات التدريب
...
- قم دائمًا بإجراء اختبار التجهيز الزائد لدفعة واحدة عند تغيير البنيات أو البيانات pipelines
...
3. ** قم بتنفيذ أداة تحديد معدل التعلم من خلال التخطيط. ** قم بتمديد `find_learning_rate` لحفظ النتائج كـ CSV وكتابة برنامج نصي منفصل يقرأ CSV ويعرض منحنى LR مقابل الخسارة باستخدام matplotlib. حدد LR الأمثل لـ ResNet-18 على CIFAR-10.

4. **قم بإنشاء مدقق بيانات pipeline.** اكتب وظيفة تتحقق من: العينات المكررة عبر تقسيمات التدريب/الاختبار، وعدم توازن توزيع الملصقات (> نسبة 10:1)، وتسوية الإدخال (المتوسط ​​بالقرب من 0، القياسي بالقرب من 1)، وقيم NaN/Inf في البيانات. تشغيله على مجموعة بيانات تالفة عمدا.
...
| التجهيز الزائد دفعة واحدة | "اختبار التصحيح الأكثر أهمية" | التدريب على دفعة صغيرة واحدة للتحقق من النموذج CAN تعلم -- إذا لم يكن الأمر كذلك، فهذا يعني أن هناك شيئًا ما مكسورًا بشكل أساسي |
...
- سميث، "معدلات التعلم الدورية لتدريب الشبكات العصبية" (2017) - الورقة التي تقدم اختبار نطاق معدل التعلم (LR مكتشف)
- Northcutt وآخرون، "أخطاء التسمية المنتشرة في مجموعات الاختبار تزعزع استقرار معايير التعلم الآلي" (2021) - يوضح أن 3-6% من التسميات في ImageNet وCIFAR-10 والمعايير الرئيسية الأخرى خاطئة
...
- PyTorch وثائق على `torch.autograd.detect_anomaly` و `torch.autograd.set_detect_anomaly` لاكتشاف NaN/Inf المدمج
