...
> يخبرك النسب المتدرج بالاتجاه الذي يجب التحرك فيه. لا يقول شيئًا عن مدى المسافة أو السرعة. SGD هي البوصلة. آدم هو GPS مع بيانات حركة المرور.
...
- وضح لماذا ينتج AdamW تعميمًا أفضل من Adam من خلال L2 التنظيم على نفس المهمة
...
يطبق النسب المتدرج للفانيليا نفس معدل التعلم على كل معلمة في كل خطوة: w = w - lr * gradient. وهذا يخلق ثلاث مشاكل make تدريب الشبكات العصبية مؤلمة في الممارسة العملية.
...
ثالثا، نقاط السرج. في الأبعاد العالية، يشتمل مشهد الخسارة على مناطق مسطحة واسعة حيث يكون التدرج قريبًا من الصفر. تزحف الفانيليا SGD عبرها بسرعة التدرج، وهي صفر فعليًا. يبدو النموذج عالقا. إنها ليست عالقة، إنها في منطقة مسطحة ذات نزول مفيد على الجانب الآخر. لكن SGD ليس لديه آلية للمضي قدمًا.
...
الأرقام الحقيقية: SGD بمفردها في مشهد خسارة مشروط بشكل سيئ قد يستغرق 10000 خطوة. SGD مع الزخم (بيتا = 0.9) يستغرق عادةً 3000-5000 خطوة لحل نفس المشكلة. والتسارع ليس هامشيا.
...
L2 التسوية تضيف lambda *w^2 إلى الخسارة. في الفانيليا SGD، هذا يعادل تسوس الوزن (طرح لامدا * ث من الوزن في كل خطوة). وفي آدم ينقطع هذا التكافؤ.
...
هذا يبدو وكأنه تفاصيل بسيطة. ليست كذلك. يتقارب AdamW مع حلول أفضل من Adam + L2 التنظيم في كل مهمة تقريبًا. إنه المُحسِّن الافتراضي في PyTorch لتدريب المحولات ونماذج الانتشار ومعظم البنى الحديثة. BERT، GPT، LLaMA، الانتشار المستقر - جميعها تم تدريبها مع AdamW.
...
PyTorch يتعامل المُحسِّنون مع مجموعات المعلمات وقص التدرج وجدولة معدل التعلم:
...
بالنسبة لشبكات CNN، لا يزال العديد من الممارسين يفضلون SGD + الزخم (lr=0.1، الزخم=0.9،weight_decay=1e-4) مع جدول خطوة أو جيب التمام. SGD يجد الحد الأدنى المسطح، والذي غالبًا ما يتم تعميمه بشكل أفضل. بالنسبة للمحولات و LLMs، AdamW مع عملية الإحماء + تسوس جيب التمام هو الإعداد الافتراضي العالمي. لا تحارب الإجماع دون سبب مدروس.
...
| آر إم إس بروب | "معدل التعلم التكيفي" | يقسم تدرج كل معلمة على تشغيل RMS للتدرجات الأخيرة؛ يعادل معدلات التعلم |
...
- Loshchilov & Hutter، "تنظيم تسوس الوزن المنفصل" (2017) - أثبت أن تنظيم L2 وتسوس الوزن ليسا متكافئين في آدم، واقترح AdamW
