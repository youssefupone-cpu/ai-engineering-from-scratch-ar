# شبكات متعددة الطبقات وتمرير للأمام
...
- حل مشكلة XOR باستخدام بنية 2-2-1 مع الأوزان السينية المضبوطة يدويًا
## المشكلة
خلية عصبية واحدة هي درج الخط. هذا كل شيء. خط مستقيم واحد من خلال البيانات الخاصة بك. كل مشكلة حقيقية في AI -- التعرف على الصور، وفهم اللغة، ولعب Go -- تتطلب منحنيات. تكديس الخلايا العصبية في طبقات هو الطريقة التي تحصل بها على المنحنيات.
...
أدى هذا إلى مقتل تمويل الشبكات العصبية لأكثر من عقد من الزمان. كان الإصلاح واضحًا بعد فوات الأوان: توقف عن استخدام طبقة واحدة. كومة الخلايا العصبية في طبقات. اسمح للطبقة الأولى بتقسيم مساحة الإدخال إلى ميزات جديدة، ودع الطبقة الثانية تجمع تلك الميزات في قرارات لا يمكن لأي سطر واحد أن make.
...
##المفهوم
### الطبقات: الإدخال، المخفية، الإخراج
...
يسحق السيني أي رقم في النطاق (0، 1). تدفع المدخلات الإيجابية الكبيرة نحو 1. تدفع المدخلات السلبية الكبيرة نحو 0. خرائط الصفر إلى 0.5. هذا المنحنى السلس هو ما يمكن تعلمه make -- على عكس الخطوة الصعبة للإدراك الحسي، فإن السيني له تدرج في كل مكان.
### التمرير الأمامي: كيف تتدفق البيانات
...
|------|-----------|-----------|-------------|
| الإدخال | س | -- | (٢،) |
| خطي مخفي | W1 * س + ب1 | __المصطلح_1__: (3، 2)، ب1: (3،) | (3،) |
| تفعيل مخفي | السيني (z1) | -- | (3،) |
| الإخراج الخطي | W2 * ح + ب2 | __المصطلح_3__: (1، 3)، ب2: (1،) | (١،) |
...
### قابلية التركيب
الشبكات العصبية قابلة للتركيب. يمكنك تكديسها وربطها وتشغيلها بالتوازي. يستخدم نموذج Whisper شبكة تشفير لمعالجة الصوت وشبكة فك ترميز منفصلة لإنشاء النص. LLMs الحديثة مخصصة لوحدة فك التشفير فقط. BERT مخصص لبرنامج التشفير فقط. T5 هو برنامج فك التشفير. يحدد اختيار البنية ما يمكن أن يفعله النموذج.
## بنائها
بيثون النقي. لا نومي. كل عملية مصفوفة مكتوبة من الصفر.
### الخطوة 1: تفعيل السيني
```python
import math
...
يمنع المشبك إلى [-500، 500] التدفق الزائد. `math.exp(500)` كبير ولكنه محدود. `math.exp(1000)` هو اللانهاية.
...
### الخطوة 4: XOR مع الأوزان المضبوطة يدويًا
...
الأوزان الكبيرة (20، -20) make تعمل كدالة خطوة. أول خلية عصبية مخفية تقارب OR. والثاني يقارب NAND. تجمعهم الخلايا العصبية الناتجة في AND، وهو XOR.
...
## استخدمه
PyTorch يفعل كل شيء أعلاه في أربعة أسطر:
```python
import torch
...
`nn.Linear(2, 8)` هو فئة الطبقة الخاصة بك: مصفوفة وزن الشكل (8، 2)، متجه التحيز للشكل (8،). `nn.Sigmoid()` هي الدالة السيني الخاصة بك المطبقة على العناصر. `nn.Sequential` هو فئة شبكتك: طبقات متسلسلة بالترتيب.
الفرق هو السرعة والحجم. يعمل PyTorch على وحدات معالجة الرسومات، ويتعامل مع دفعات من ملايين العينات، ويحسب التدرجات تلقائيًا للانتشار العكسي. لكن منطق التمرير الأمامي مطابق لما قمت بإنشائه للتو من الصفر.
## اشحنها
...
3. قم بتنفيذ طريقة `count_parameters` على فئة الشبكة التي تُرجع إجمالي عدد الأوزان والتحيزات القابلة للتدريب. اختبره على شبكة 784-256-128-10 (بنية MNIST الكلاسيكية). كم عدد المعلمات لديها؟
4. قم ببناء تمريرة أمامية لشبكة 3-4-4-2. قم بتغذيته بقيم الألوان RGB (التي تمت تسويتها إلى 0-1) ولاحظ المخرجين. هذه هي البنية المعمارية لمصنف ألوان بسيط يتكون من فئتين.
5. استبدل السيني بوظيفة "الخطوة المتسربة": قم بإرجاع 0.01 * z إذا كانت z <0، وإلا 1.0. قم بتشغيل التمريرة الأمامية في XOR بنفس الأوزان المضبوطة يدويًا من الخطوة 4. هل ما زالت تعمل؟ لماذا يُفضل الشكل السيني الأملس على القطع الصلب؟
## المصطلحات الرئيسية
...
- 3Blue1Brown، "ولكن ما هي الشبكة العصبية؟" (https://www.youtube.com/watch?v=aircAruvnKk) - عرض مرئي مدته 20 دقيقة للطبقات والأوزان والتمريرات الأمامية التي تبني النموذج العقلي الصحيح
