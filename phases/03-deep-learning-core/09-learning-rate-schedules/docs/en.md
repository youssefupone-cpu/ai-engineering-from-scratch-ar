...
يستخدم كل نموذج رئيسي تم نشره في السنوات الثلاث الماضية جدولًا لمعدل التعلم. استخدم Llama 3 الذروة lr=3e-4 مع 2000 خطوة إحماء واضمحلال جيب التمام إلى 3e-5. GPT-3 تم استخدامه lr=6e-4 مع أكثر من 375 مليون رمز. هذه ليست اختيارات تعسفية. إنها نتيجة عمليات مسح مكثفة للمعلمات الفائقة تكلف ملايين الدولارات.
...
الإحماء النموذجي: 1-5% من إجمالي خطوات التدريب. تم تدريب Llama 3 للحصول على ما يقرب من 1.8 تريليون رمز وتم الإحماء لـ 2000 خطوة. GPT-3 تم تسخين أكثر من 375 مليون رمز.
### الاحماء الخطي + اضمحلال جيب التمام
...
هذا هو ما تستخدمه اللاما، GPT، PaLM، ومعظم المحولات الحديثة. الإحماء يمنع عدم الاستقرار المبكر. يؤدي اضمحلال جيب التمام إلى تسوية النموذج إلى الحد الأدنى الجيد.
...
### الخطوة 5: LR مرتفع جدًا مقابل منخفض جدًا
...
## استخدمه
يوفر PyTorch أدوات جدولة في `torch.optim.lr_scheduler`:
```python
import torch
...
2. قم بتنفيذ اختبار نطاق معدل التعلم (ليزلي سميث): تدرب على بضع مئات من الخطوات مع زيادة LR بشكل كبير من 1e-7 إلى 1. خسارة قطعة الأرض مقابل LR. الحد الأقصى الأمثل LR هو قبل أن تبدأ الخسارة في الزيادة.
...
4. تنفيذ الصلب جيب التمام مع عمليات إعادة التشغيل الدافئة (SGDR): إعادة تعيين معدل التعلم إلى lr_max كل خطوات T والانحلال مرة أخرى. قارنه بجيب التمام القياسي في فترة تدريب أطول.
...
| الاحماء | "ابدأ بـ LR صغير" | تكثيف LR خطيًا من الصفر القريب إلى القيمة المستهدفة خلال الخطوات N الأولى لتحقيق الاستقرار في إحصائيات المحسن |
| جيب التمام الصلب | "الاضمحلال السلس LR" | تقليل LR بعد منحنى جيب التمام من lr_max إلى lr_min خلال التدريب |
| خطوة الاضمحلال | "أسقط LR عند المعالم" | ضرب LR بعامل (عادة 0.1) على فترات زمنية ثابتة |
| سياسة دورة واحدة | "أعلى ثم أسفل" | طريقة ليزلي سميث في التصعيد LR للأعلى ثم للأسفل في دورة واحدة لتقارب أسرع |
| LR اختبار النطاق | "ابحث عن أفضل معدل للتعلم" | تدريب لفترة وجيزة مع زيادة LR للعثور على القيمة التي تبدأ فيها الخسارة بالتباعد |
| جيب التمام مع إعادة التشغيل الدافئة | "إعادة الضبط والتكرار" | إعادة تعيين LR بشكل دوري إلى lr_max والتدهور مرة أخرى (SGDR) |
| إيتا مين | "الكلمة LR" | الحد الأدنى لمعدل التعلم الذي ينخفض ​​​​الجدول إلى |
| ذروة معدل التعلم | "الحد الأقصى LR" | أعلى LR تم الوصول إليه أثناء التدريب، عادةً بعد الإحماء |
## مزيد من القراءة
- Loshchilov & Hutter، "SGDR: نزول التدرج العشوائي مع عمليات إعادة التشغيل الدافئة" (2017) - قدم الصلب جيب التمام وإعادة التشغيل الدافئة
- سميث، "التقارب الفائق: تدريب سريع جدًا للشبكات العصبية باستخدام معدلات تعلم كبيرة" (2018) - ورقة سياسة الدورة الأولى
- Touvron وآخرون، "Llama 2: Open Foundation and Fine-Tuned Chat Models" (2023) - يوثق جدول الإحماء + جيب التمام المستخدم على نطاق واسع
