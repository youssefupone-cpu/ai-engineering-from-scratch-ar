...
- تنفيذ التسرب مع القياس المقلوب، L2 تسوس الوزن، وتطبيع الدُفعات، وتطبيع الطبقة، وRMSNorm من البداية
- قياس الفجوة في دقة اختبار التدريب وتشخيص التجاوز باستخدام تجارب التنظيم
- اشرح لماذا تستخدم المحولات LayerNorm بدلاً من BatchNorm ولماذا تفضل المحولات الحديثة LLMs RMSNorm
...
كما تمت مناقشته في الدرس 06: تسوس الوزن وانتظام L2 متكافئان في SGD ولكن ليس في آدم. استخدم دائمًا AdamW (تسوس الوزن المنفصل) عند التدريب مع آدم.
...
يتم تطبيق LayerNorm في المحولات بعد كل كتلة انتباه ذاتي وكل كتلة تغذية للأمام (Post-LN)، أو قبلها (Pre-LN، وهو أكثر استقرارًا للتدريب).
...
LLaMA، وLLaMA 2، وLLaMA 3، وMistral، وأحدث LLMs تستخدم RMSNorm بدلاً من LayerNorm. على نطاق مليارات المعلمات وتريليونات الرموز، يعد توفير 10٪ أمرًا كبيرًا.
...
PyTorch يوفر كل التطبيع والتنظيم كوحدات:
...
ينتج هذا الدرس:
- `outputs/prompt-regularization-advisor.md` -- موجه يقوم بتشخيص التجاوز ويوصي بإستراتيجية التنظيم الصحيحة
...
| تسوس الوزن | "L2 ضربة جزاء" | تقليص جميع الأوزان نحو الصفر عن طريق طرح lambda * w في كل خطوة؛ يعاقب التعقيد من خلال حجم الوزن |
