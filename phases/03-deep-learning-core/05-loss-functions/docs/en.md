# وظائف الخسارة
> شبكتك make هي توقع. والحقيقة الأرضية تقول خلاف ذلك. ما مدى الخطأ؟ هذا الرقم هو الخسارة. اختر دالة الخسارة الخاطئة وسيقوم النموذج الخاص بك بتحسين الشيء الخطأ تمامًا.
**النوع:** بناء
** اللغات: ** بايثون
**المتطلبات السابقة:** الدرس 03.04 (وظائف التنشيط)
**الوقت:** ~75 دقيقة
## أهداف التعلم
- تنفيذ MSE، والإنتروبيا الثنائية المتقاطعة، والإنتروبيا المتقاطعة الفئوية، والخسارة المتباينة (InfoNCE) من الصفر مع تدرجاتها
- اشرح سبب فشل MSE في التصنيف من خلال توضيح وضع الفشل "توقع 0.5 لكل شيء"
...
هنا مثال ملموس. لديك مهمة تصنيف ثنائية. فئتين، 50/50 مقسمة. أنت تستخدم MSE كخسارتك. يتنبأ النموذج بـ 0.5 لكل مدخل. متوسط ​​MSE هو 0.25، وهو الحد الأدنى الممكن دون تعلم أي شيء فعليًا. يتمتع النموذج بقدرة تمييزية صفرية ولكنه يقلل من الناحية الفنية من وظيفة الخسارة. قم بالتبديل إلى الإنتروبيا المتقاطعة وسيضطر النموذج نفسه إلى دفع التوقعات نحو 0 أو 1، لأن -log(0.5) = 0.693 يمثل خسارة فادحة، بينما -log(0.99) = 0.01 يكافئ التنبؤات الصحيحة الواثقة. إن اختيار دالة الخسارة هو الفرق بين النموذج الذي يتعلم والنموذج الذي يتلاعب بالمقياس.
...
لماذا يهم التربيع: فهو يعاقب الأخطاء الكبيرة بشكل تربيعي. خطأ 2 يكلف 4x بقدر خطأ 1. خطأ 10 يكلف 100x. هذا makes MSE حساس للقيم المتطرفة - يهيمن توقع واحد خاطئ تمامًا على الخسارة.
أرقام حقيقية: إذا كان النموذج الخاص بك يتنبأ بأسعار المساكن وكان أقل بمقدار 10000 دولار أمريكي في معظم المنازل ولكنه انخفض بمقدار 200000 دولار أمريكي في قصر واحد، فإن MSE سيحاول بقوة إصلاح هذا القصر الواحد، مما قد يؤدي إلى الإضرار بأداء المنازل الـ 99 الأخرى.
...
MSE تتسطح التدرجات عندما تكون التوقعات قريبة من 0 أو 1 (بسبب التشبع السيني). تعوض التدرجات المتقاطعة للإنتروبيا عن ذلك - يلغي السجل - المناطق المسطحة في السيني، مما يعطي تدرجات قوية بالضبط في المكان الذي تكون هناك حاجة إليها بشدة.
### تجانس التسمية
...
لماذا يعمل هذا: يحتاج النموذج الذي يحاول إخراج 1.0 بالضبط من خلال softmax إلى دفع logits إلى ما لا نهاية. يؤدي هذا إلى الثقة المفرطة، ويضر بالتعميم، ويجعل النموذج هشًا أمام تحول التوزيع. يؤدي تجانس الملصقات إلى تحديد الهدف عند 0.9 (مع alpha=0.1)، مع الحفاظ على logits في نطاق معقول. GPT وتستخدم معظم النماذج الحديثة تجانس الملصقات أو ما يعادلها.
### الخسارة المتناقضة
لا تسميات. لا توجد فصول دراسية. مجرد أزواج من المدخلات والسؤال: هل هذه متشابهة أم مختلفة؟
**الخسارة المتباينة بنمط SimCLR (NT-Xent / InfoNCE):**
...
يفرض الهامش (عادةً 0.2-1.0) فجوة دنيا بين المسافات الموجبة والسالبة. إذا كانت النتيجة السلبية بعيدة بما فيه الكفاية بالفعل، فستكون الخسارة صفرًا - لا يوجد تدرج ولا تحديث. إن تدريب makes فعال ولكنه يتطلب تعدينًا ثلاثيًا دقيقًا (اختيار السلبيات الصعبة القريبة من المرساة).
### الخسارة البؤرية
...
### الخطوة 3: الانتروبيا القاطعة مع Softmax
...
### الخطوة 5: الخسارة المقارنة (InfoNCE المبسطة)
```python
...
### الخطوة 6: MSE مقابل الانتروبيا المتقاطعة في التصنيف
...
## استخدمه
يوفر PyTorch جميع وظائف الخسارة القياسية مع الاستقرار الرقمي المضمن:
```python
import torch
...
- `outputs/prompt-loss-debugger.md` -- رسالة تشخيصية عندما يبدو منحنى الخسارة خاطئًا
## تمارين
1. قم بتنفيذ خسارة Huber (خسارة L1 سلسة)، وهي MSE للأخطاء الصغيرة وMAE للأخطاء الكبيرة. قم بتدريب شبكة انحدار تتنبأ بـ y = sin(x) باستخدام MSE مقابل Huber عندما تتم إضافة ضوضاء عشوائية (قيم متطرفة) إلى 5% من أهداف التدريب. قارن خطأ الاختبار النهائي.
2. أضف الخسارة البؤرية إلى حلقة التدريب على التصنيف الثنائي. قم بإنشاء مجموعة بيانات غير متوازنة (90% فئة 0، 10% فئة 1). قارن المعيار BCE بالخسارة البؤرية (جاما = 2) عند استدعاء فئة الأقلية بعد 200 حقبة.
...
4. قم بإجراء مقارنة MSE مقابل الإنتروبيا المتقاطعة ولكن قم بتتبع مقادير التدرج في كل طبقة أثناء التدريب. رسم متوسط ​​قاعدة التدرج في كل عصر. تحقق من أن الإنتروبيا المتقاطعة تنتج تدرجات أكبر في العصور المبكرة عندما يكون النموذج غير مؤكد.
5. قم بتنفيذ KL فقدان التباعد وتحقق من أن تقليل KL(صحيح || متوقع) يعطي نفس التدرجات مثل الإنتروبيا المتقاطعة عندما يكون التوزيع الحقيقي ساخنًا واحدًا. ثم جرب الأهداف السهلة (مثل تقطير المعرفة) حيث يأتي التوزيع "الحقيقي" من مخرجات softmax لنموذج المعلم.
## المصطلحات الرئيسية
| مصطلح | ماذا يقول الناس | ماذا يعني في الواقع |
|------|----------------|----------------------|
| دالة الخسارة | "ما مدى خطأ النموذج" | دالة قابلة للتمييز تقوم بتعيين التنبؤات والأهداف إلى مقياس يقوم المحسن بتصغيره |
| __المصطلح_1__ | "متوسط ​​الخطأ التربيعي" | متوسط ​​الفروق التربيعية بين التوقعات والأهداف؛ يعاقب الأخطاء الكبيرة بشكل تربيعي |
| عبر الانتروبيا | "خسارة التصنيف" | يقيس التباعد بين التوزيع الاحتمالي المتوقع والتوزيع الحقيقي باستخدام -log(p) |
| ثنائي عبر الانتروبيا | "__المصطلح_2__" | إنتروبيا متقاطعة لفئتين: -(y*log(p) + (1-y)*log(1-p)) |
