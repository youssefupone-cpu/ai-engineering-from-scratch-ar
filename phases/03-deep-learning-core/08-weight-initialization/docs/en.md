...
### انتشار التباين من خلال الطبقات
...
كل إضافة تزيد من التباين. مع عدد N من الطبقات المتبقية، ينمو التباين بشكل متناسب مع N. يقوم GPT-2 بقياس أوزان الطبقات المتبقية بمقدار 1/sqrt(2N)، حيث N هو عدد الطبقات. وهذا يحافظ على استقرار حجم الإشارة المتراكمة.
...
### الخطوة 3: التمرير للأمام عبر 50 طبقة
...
### الخطوة 6: تقرير الحجم طبقة تلو الأخرى
...
## استخدمه
يوفر PyTorch هذه الوظائف المضمنة:
```python
import torch
...
عند الاتصال بـ `nn.Linear(512, 256)`، يتم تعيين PyTorch افتراضيًا على تهيئة Kaiming الموحدة. ولهذا السبب فإن معظم الشبكات البسيطة "تعمل فقط" -- PyTorch اتخذت بالفعل الاختيار الصحيح. ولكن عندما تقوم بإنشاء بنيات مخصصة أو تتعمق أكثر من 20 طبقة، فإنك تحتاج إلى فهم ما يحدث وربما تجاوز الإعداد الافتراضي.
بالنسبة للمحولات، تتعامل نماذج HuggingFace عادةً مع التهيئة بطريقة `_init_weights` الخاصة بها. يقوم تنفيذ GPT-2 بقياس التوقعات المتبقية بمقدار 1/sqrt(N). إذا كنت تقوم ببناء محول من الصفر، فستحتاج إلى إضافة هذا بنفسك.
## اشحنها
ينتج هذا الدرس:
- `outputs/prompt-init-strategy.md` -- رسالة مطالبة لتشخيص مشكلات تهيئة الوزن والتوصية بالاستراتيجية الصحيحة
## تمارين
...
2. قم بتنفيذ القياس المتبقي GPT-2: اضرب مخرجات كل طبقة بـ 1/sqrt(2*N) قبل إضافتها إلى التدفق المتبقي. قم بتشغيل 50 طبقة مع وبدون تغيير الحجم، وقم بقياس مدى سرعة نمو الحجم المتبقي.
...
5. تنفيذ التهيئة المتعامدة (إنشاء مصفوفة عشوائية، وحساب SVD، واستخدام المصفوفة المتعامدة U). قارنه بـ Kaiming لشبكات ReLU ذات 50 طبقة.
## المصطلحات الرئيسية
...
| التحجيم المتبقي | "خدعة init GPT-2" | تحجيم أوزان التوصيل المتبقية بمقدار 1/sqrt(2N) لمنع نمو التباين من خلال طبقات المحولات N |
...
- رادفورد وآخرون، "نماذج اللغة هي متعلمون متعددو المهام غير خاضعين للرقابة" (2019) -- GPT-2 ورقة بحثية مع تهيئة القياس المتبقي
