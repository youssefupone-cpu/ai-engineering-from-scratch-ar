...
مثل GELU، فهو سلس وغير رتيب ويسمح بقيم سلبية صغيرة. الفرق بسيط: يستخدم Swish السيني للبوابة بينما يستخدم GELU الغاوسي CDF. ومن الناحية العملية، الأداء متطابق تقريبًا. يتم استخدام Swish في EfficientNet وبعض نماذج الرؤية. GELU يهيمن على نماذج اللغة.
### Softmax: تنشيط الإخراج
لا يستخدم في الطبقات المخفية. تقوم Softmax بتحويل متجه الدرجات الأولية (logits) إلى توزيع احتمالي.
```
softmax(x_i) = e^(x_i) / sum(e^(x_j) for all j)
```

كل مخرجات تتراوح بين 0 و1. جميع المخرجات مجموعها 1. هذا make هو التنشيط النهائي القياسي للتصنيف متعدد الفئات. أكبر logit يحصل على أعلى احتمال، ولكن على عكس argmax، softmax قابل للتمييز ويحتفظ بالمعلومات حول الثقة النسبية.
### مقارنة الأشكال
```mermaid
graph LR
...
## بنائها
### الخطوة 1: تنفيذ جميع وظائف التنشيط مع المشتقات
...
### الخطوة الثانية: تصور أين تموت التدرجات
...
### الخطوة 4: كاشف الخلايا العصبية الميتة
...
## استخدمه
يوفر PyTorch كل هذه العناصر كنماذج وظيفية ونموذجية:
```python
import torch
...
5. قم بتعديل مقارنة التدريب لاستخدام مجموعة البيانات XOR من الدرس 01 بدلاً من الدوائر. ما هو التنشيط الذي يتقارب بشكل أسرع في XOR؟ لماذا يختلف هذا عن نتائج الدائرة؟
## المصطلحات الرئيسية
...
| __المصطلح_2__ | "تفعيل المحولات" | وحدة الخطأ الخطي Gaussian Error Linear Unit، وهي عبارة عن تنشيط سلس يقوم بوزن المدخلات حسب احتمالية أن تكون موجبة |
| حفيف/سيلو | "ReLU ذاتي البوابات" | x * sigmoid(x)، تم اكتشافه من خلال البحث الآلي، المستخدم في EfficientNet |
| سوفت ماكس | "يحول النتائج إلى احتمالات" | تطبيع متجه logits في توزيع احتمالي حيث تكون جميع القيم في (0,1) ومجموعها 1 |
...
| Logit | "النتيجة الأولية قبل softmax" | الإخراج غير الطبيعي للطبقة النهائية قبل تطبيق softmax أو sigmoid |
## مزيد من القراءة
