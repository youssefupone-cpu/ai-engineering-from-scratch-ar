# ضبط التعليمات (SFT)
> يتنبأ النموذج الأساسي بالرمز المميز التالي. هذا كل شيء. ولا يتبع التعليمات أو يجيب على الأسئلة أو يرفض الطلبات الضارة. SFT هو الجسر بين متنبئ الرمز المميز والمساعد المفيد. كل عارض أزياء تحدثت إليه من قبل -- كلود، GPT، Llama Chat -- مر بهذه الخطوة.
**النوع:** بناء
...
- اشرح سبب ضرورة SFT: تستمر النماذج الأساسية في النص بدلاً من الإجابة على الأسئلة
- قم بتقييم جودة SFT من خلال مقارنة النموذج الأساسي مع استجابات النموذج المضبوطة بدقة في مجموعة التعليمات المعلقة
## المشكلة
...
يستخدم الرموز المميزة (`<|im_start|>`، `<|im_end|>`) لتحديد الأدوار. تتم إضافة هذه الرموز المميزة إلى مفردات أداة الرمز المميز أثناء الضبط الدقيق. تستخدم Qwen وYi والعديد من النماذج الأخرى ChatML.
...
SFT يركز هذه القدرة الكامنة. بدلاً من أن يحتاج النموذج إلى معرفة ما إذا كان يجب عليه الإجابة على سؤال أو متابعة مستند من السياق، فإن SFT يتدرب بشكل واضح على نمط المحادثة. بعد بضعة آلاف من الأمثلة، يتعلم النموذج: عندما ترى علامة دور المساعد، قم بإنتاج استجابة مفيدة.
...
يستخدم SFT معلمات تشعبية مختلفة تمامًا عن التدريب المسبق. أنت لا تتدرب من الصفر. أنت تقوم بتعديل نموذج يعمل بالفعل.
| المعلمة | التدريب المسبق (لاما 2 7 ب) | SFT (دردشة اللاما 2) |
|-----------|---------------------------------------|-----|
...
معدل التعلم أقل بمقدار 15 مرة في SFT. هذا أمر بالغ الأهمية. يؤدي معدل التعلم المرتفع أثناء الضبط الدقيق إلى تدمير المعرفة المدربة مسبقًا. النموذج "ينسى" ما تعلمه ويتناسب مع مجموعة بيانات الضبط الدقيقة الصغيرة. وهذا هو النسيان الكارثي.
...
3. **امزج بيانات ما قبل التدريب.** قامت Llama 2 Chat بخلط نسبة صغيرة (2-5%) من بيانات ما قبل التدريب الأولية في مجموعة بيانات SFT. وهذا "يذكر" النموذج بقدراته العامة أثناء تعلم سلوك متابعة التعليمات الجديد.
### الأعداد الحقيقية
...
- A100 الإنتاجية للضبط الدقيق للنموذج 7B: ~3000 رمز مميز/ثانية
- 10.24 مليون / 3000 = ~3400 ثانية = ~57 دقيقة
بالنسبة لطرازنا الصغير GPT (4 طبقات، 128 درجة خافتة)، يكون التدريب فوريًا تقريبًا. النقطة المهمة هي فهم الآليات، وليس المقياس.
```mermaid
graph TD
...
### الخطوة 2: إنشاء رمز مميز باستخدام قالب الدردشة
...
قناع الخسارة هو كل الأصفار لرموز التعليمات وكل الآحاد لرموز الاستجابة. يحصل الرمز المميز `RESP_START` نفسه على قناع بقيمة 0 لأنه محدد، وليس جزءًا من محتوى الاستجابة.
### الخطوة 3: خسارة الإنتروبيا المقنعة
...
### الخطوة 5: مقارنة النموذج الأساسي بالنموذج SFT
...
### الخطوة السادسة: قياس النسيان الكارثي
قارن قدرة التنبؤ بالرمز المميز التالي للنموذج قبل SFT وبعده. إذا ألحق SFT الضرر بالإمكانيات العامة، فستزداد الخسارة في النص الخام.
```python
...
في الضبط الدقيق، يمكنك تتبع هذا المقياس طوال التدريب. إذا زاد فقدان النص الخام بنسبة تزيد عن 10-15%، فإن SFT الخاص بك يعد عدوانيًا للغاية. خفض معدل التعلم أو تقليل عدد العصور.
## استخدمه
...
## اشحنها
يُنتج هذا الدرس `outputs/prompt-sft-data-curator.md` -- مطالبة تساعدك في تصميم مجموعات بيانات التعليمات وتنظيمها لـ SFT. نظرًا للقدرة المستهدفة (إنشاء التعليمات البرمجية، والرياضيات، والمحادثة)، فإنها تنتج خطة لجمع البيانات بمواصفات التنسيق ومعايير الجودة ومتطلبات التنوع.
## تمارين
...
2. تنفيذ خلط البيانات. أنشئ دالة تأخذ مجموعة بيانات SFT ومجموعة نصية أولية، ثم تنتج دفعات تدريب حيث 5% من الأمثلة عبارة عن نص خام (بدون أقنعة) و95% عبارة عن أزواج تعليمات (مقنعة). قم بتشغيل 3 فترات وقارن مقاييس النسيان مع تدريب SFT النقي.
...
| شات مل | "لغة ترميز الدردشة" | تنسيق الرمز المميز باستخدام محددات `<\|im_start\|>` و`<\|im_end\|>` لتحديد أدوار المتحدث في بيانات المحادثة |
| تنسيق الألبكة | "تنسيق ستانفورد" | تنسيق JSON مع حقول التعليمات/الإدخال/الإخراج، يُستخدم للأمثلة التي تم إنشاؤها بحجم 52 ألف GPT-3.5 بتكلفة 600 دولار |
...
## مزيد من القراءة
- [Ouyang et al., 2022 -- "Training language models to follow instructions with human feedback" (InstructGPT)](https://arxiv.org/abs/2203.02155) -- الورقة التي قدمت ضبط التعليمات + RLHF في OpenAI
- [Taori et al., 2023 -- "Stanford Alpaca: An Instruction-following LLaMA Model"](https://github.com/tatsu-lab/stanford_alpaca) -- 52 ألف مثال للتعليمات مقابل 600 دولار، مما يثبت أن SFT يعمل على مجموعات بيانات صغيرة
- [Touvron et al., 2023 -- "Llama 2: Open Foundation and Fine-Tuned Chat Models"](https://arxiv.org/abs/2307.09288) -- Meta's SFT + RLHF pipeline مع 27 ألف أمثلة عالية الجودة
- [Chiang et al., 2023 -- "Vicuna: An Open-Source Chatbot Impressing GPT-4"](https://lmsys.org/blog/2023-03-30-vicuna/) -- تدريب على 70 ألف محادثة ShareGPT
- [Zhou et al., 2023 -- "LIMA: Less Is More for Alignment"](https://arxiv.org/abs/2305.11206) -- إثبات أن 1000 مثال منسق بعناية يمكن أن يتطابق مع SFT في مجموعات بيانات أكبر بكثير