...
- تنفيذ مرشحات جودة البيانات (إلغاء البيانات المكررة، واكتشاف اللغة، وتصفية المحتوى) المستخدمة في التدريب المسبق الحقيقي pipelines
- إنشاء تسلسلات تدريبية ذات طول ثابت باستخدام أقنعة الانتباه المناسبة والتعامل مع حدود المستندات
- الملف الشخصي pip إنتاجية الخط لضمان مواكبة أداة تحميل البيانات لسرعة التدريب GPU

## The Problem

لديك رمز مميز. الآن أنت بحاجة إلى البيانات.

ليست مجموعة بيانات. ليس ملف CSV. تيرابايت من النص - تم تنظيفها وإلغاء تكرارها وتصفيتها من أجل الجودة، وتصنيفها إلى تسلسلات ذات طول ثابت، وتقديمها على دفعات عشوائية بسرعة كافية بحيث لا تنتظر مجموعتك المكونة من 8-GPU الدفعة التالية أبدًا.

يعتقد معظم الناس أن تدريب LLM يدور حول بنية النموذج. ليس كذلك. استخدمت Llama 3 15.6 تريليون رمز. GPT-3 استخدم 300 مليار. DeepSeek-V2 استخدم 8.1 تريليون. الهندسة المعمارية في جميع المجالات الثلاثة هي نفسها تقريبًا: كتل محولات مكدسة مع طبقات الانتباه والتغذية. الفرق في جودة المخرجات يأتي بشكل كبير من البيانات.
...
تحدد بياناتك pipeline ما إذا كان نموذجك يتعلم اللغة أو يتعلم الضوضاء.
...
|--------|------|---------|---------|
| الزحف المشترك | ~250 TB خام | منخفض (يحتاج إلى تصفية ثقيلة) | GPT-3، اللاما، الموديلات الأكثر فتحًا |
| ويكيبيديا | ~20 GB | عالية | كل تخصص LLM |
| GitHub الكود | ~1 TB+ | متوسط ​​(الكثير من التكرارات، الكود الميت) | ستاركودر، كود لاما، ديب سيك-كودر |
| كتب (BookCorpus، بايل) | ~100 GB | عالية | GPT-2، GPT-3، النماذج المبكرة |
| أوراق أكاديمية (arXiv, S2ORC) | ~100 GB | عالية لـ STEM | اللاما، جلاكتيكا |
| ستاك أوفر فلوو، رديت | ~100 GB | متوسطة | اللاما، الصقر |
| الويب المنسق (C4، RefinedWeb) | ~5 TB | متوسط-عالٍ (تمت تصفيته مسبقًا) | T5، الصقر |
...
النسبة مهمة بقدر الحجم الإجمالي. الكثير من بيانات الويب ويصبح النموذج ببغاء Reddit. رمز قليل جدًا ولا يمكنه البرمجة. القليل جدًا من الرياضيات ويفشل في التفكير. يعد الحصول على هذا المزيج بشكل صحيح أحد أصعب أجزاء تدريب LLM، ولا توجد صيغة محددة -- فهو يتطلب التجريب والتقييم.

### Data Cleaning

بيانات الويب الخام قذرة. يحتوي تفريغ الزحف المشترك النموذجي على:

- HTML العلامات وجافا سكريبت
...
تنظيف هذا ليس اختياريا. إنه الفرق بين النموذج الذي يولد فقرات متماسكة والنموذج الذي ينتج HTML علامات ممزوجة بقوائم المنتجات.
...
**HTML تجريد:** إزالة كافة العلامات. احتفظ فقط بمحتوى النص المرئي. تقوم المكتبات مثل `trafilatura` أو `readability` باستخراج محتوى المقالة أثناء تجاهل التنقل والإعلانات والنموذج المعياري.
...
**إلغاء البيانات المكررة:** خطوة التنظيف الأكثر تأثيرًا. يحتوي Common Crawl على أعداد هائلة من الصفحات المكررة - إخلاء المسؤولية القانونية وإشعارات ملفات تعريف الارتباط وشروط الخدمة. التدريب على التكرارات يضيع الحوسبة ويمكن أن يتسبب في حفظ النموذج وتجديد مقاطع محددة حرفيًا.

**PII الإزالة:** الأسماء وعناوين البريد الإلكتروني وأرقام الهواتف وأرقام الضمان الاجتماعي. الكشف القائم على Regex للنماذج المنظمة PII، NER للأسماء في السياق.
...
MinHash + التجزئة الحساسة للمكان (LSH) تحل هذه المشكلة بكفاءة.
...
النهج الأفضل: تجميع مستندات متعددة في تسلسل واحد، مفصولة برموز نهاية التسلسل. قد يحتوي تسلسل 2048 رمزًا مميزًا على ثلاث مستندات قصيرة متسلسلة برموز [EOS] فيما بينها.
...
يتم اقتطاع المستندات الطويلة أو تقسيمها إلى أجزاء عند حدود التسلسل. نقطة الانقسام مهمة: تقسيم منتصف الجملة يجبر النموذج على رؤية أفكار غير مكتملة. تقوم بعض الخطوط pip بمحاذاة الانقسامات مع حدود الفقرة أو الجملة عندما يكون ذلك ممكنًا.
...
قم بإزالة HTML، وتطبيع المسافة البيضاء، وإزالة المحتوى غير النصي. سنستخدم نصًا ذا ملكية عامة (مشروع جوتنبرج) باعتباره مجموعتنا الصغيرة.
...
يلتقط مرشح الجودة SEO البريد العشوائي (ALL CAPS)، والضوضاء الناتجة عن الآلة (نسبة الأحرف الخاصة العالية)، والصفحات الروتينية (قصيرة جدًا). تعمل عمليات الفحص الثلاثة هذه وحدها على إزالة كمية مذهلة من البيانات المهملة من عمليات زحف الويب.
...
تتحكم المعلمات `num_hashes=128` و `bands=16` في مقايضة الاستدعاء الدقيق. المزيد من التجزئة تعطي تقديرات تشابه أكثر دقة. يؤدي المزيد من النطاقات إلى زيادة الاستدعاء (التقاط المزيد من التكرارات) على حساب المزيد من النتائج الإيجابية الخاطئة. تعمل هذه القيم بشكل جيد مع نص الويب النموذجي.
...
قم بتحميل نفس المجموعة من خلال مكتبة مجموعات بيانات HuggingFace وقارن سرعة الخط pip.
...
يستخدم HuggingFace pipeline Rust الرموز المميزة أسفل الغطاء والمعالجة المتوازية عبر 4 مراكز. سيكون خط Python pipeline الخاص بك أبطأ بمقدار 10-50x. هذه الفجوة هي السبب وراء استخدام فرق الإنتاج للرموز المميزة المجمعة. الخوارزمية هي نفسها. لغة التنفيذ هي الفرق.

## Ship It

ينتج هذا الدرس مطالبة بالتحقق من جودة البيانات وتصحيح الأخطاء في خطوط LLM التدريب pip. انظر `outputs/prompt-data-quality-checker.md`.

## Exercises

1. **سهل:** أضف اكتشاف اللغة إلى خط التنظيف pipe باستخدام إرشادي بسيط (تحليل مجموعة الأحرف). قم بالتصفية إلى المستندات الإنجليزية فقط وقياس عدد المستندات التي تمت إزالتها.
2. **متوسط:** تنفيذ عملية إلغاء البيانات المكررة بشكل دقيق باستخدام SHA-256 تجزئة جنبًا إلى جنب مع MinHash شبه إلغاء البيانات المكررة. قارن عدد التكرارات التي تم التقاطها بواسطة كل طريقة في مجموعة البيانات المحذوفة من الويب.
...
| LSH | "التجزئة الحساسة للمنطقة المحلية" | طريقة لتجميع العناصر المتشابهة في نفس المجموعة - تقلل المقارنات الزوجية من O(n^2) إلى شبه الخطية |
...
- [هوفمان وآخرون، 2022 - تدريب نماذج اللغة الكبيرة للحوسبة المثلى (شينشيلا)](https://arxiv.org/abs/2203.15556) - الورقة التي غيرت طريقة تفكيرنا في نطاق البيانات
- [بينيدو وآخرون، 2023 - مجموعة بيانات RefinedWeb لـ Falcon LLM](https://arxiv.org/abs/2306.01116) - كيفية تصفية الزحف المشترك إلى الجودة العالية
- [Touvron وآخرون، 2023 -- Llama 2: الأساس المفتوح ونماذج الدردشة المضبوطة بدقة](https://arxiv.org/abs/2307.09288) -- البيانات pipتفاصيل الخط لـ Llama 2
- [لي وآخرون، 2022 - إلغاء البيانات المكررة يجعل نماذج اللغة أفضل](https://arxiv.org/abs/2107.06499) -- لماذا يعد إلغاء البيانات المكررة أكثر أهمية مما تعتقد
- [برودر، 1997 -- حول تشابه الوثائق واحتوائها](https://ieeexplore.ieee.org/document/666900) -- ورقة مينهاش الأصلية
- [ميتا، 2024 -- التقرير الفني للعبة Llama 3](https://arxiv.org/abs/2407.21783) -- 15.6T من الرموز، ونسب خلط البيانات، وتصفية pipeline
