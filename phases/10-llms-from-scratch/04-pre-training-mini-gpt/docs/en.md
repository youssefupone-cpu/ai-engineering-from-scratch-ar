# تدريب مسبق صغير GPT (124 مليون معلمة)
...
يبني هذا الدرس GPT-2 صغيرًا من الصفر. ليس في PyTorch. في نومي. كل ضرب المصفوفة مرئي. يتم حساب كل تدرج بواسطة الكود الخاص بك. سترى بالضبط كيف يتضافر 124 مليون رقم للتنبؤ بالكلمة التالية.
##المفهوم
### هندسة GPT
GPT هو نموذج لغة انحداري ذاتي. "الانحدار التلقائي" يعني أنه يُنشئ رمزًا مميزًا واحدًا في كل مرة، كل منها مشروط بجميع الرموز المميزة السابقة. الهندسة المعمارية عبارة عن كومة من كتل وحدة فك ترميز المحولات.
...
الاتصالات المتبقية حاسمة. بدونها، تختفي التدرجات عند وصولها إلى الكتلة 1 أثناء الانتشار العكسي. معهم، يمكن أن تتدفق التدرجات مباشرة من الخسارة إلى أي طبقة من خلال مسار "التخطي". ولهذا السبب يمكنك تكديس 12 أو 32 أو حتى 96 قطعة (يُشاع أن GPT-4 يستخدم 120 قطعة).
### انتبه: الآلية الأساسية
...
### KV ذاكرة التخزين المؤقت: لماذا يكون الاستدلال سريعًا
...
KV ذاكرة التخزين المؤقت تحل هذه المشكلة. بعد حساب K وV لكل رمز، قم بتخزينهما. عند إنشاء الرمز المميز N+1، ما عليك سوى حساب Q للرمز المميز الجديد والبحث عن K وV المخزنين مؤقتًا من جميع الرموز المميزة السابقة. يؤدي هذا إلى تقليل تكلفة الرمز المميز من O(N) إلى O(1) لحساب K وV. لا يزال حساب درجة الاهتمام هو O(N) لأنك تحضر جميع المواضع السابقة، لكنك تتجنب مضاعفات المصفوفة الزائدة عن الحاجة على الإدخال.
بالنسبة إلى GPT-2 الذي يحتوي على 12 طبقة و12 رأسًا، تقوم ذاكرة التخزين المؤقت KV بتخزين 2 (K + V) × 12 طبقة × 12 رأس × 64 dims = 18,432 قيمة لكل رمز مميز. بالنسبة لتسلسل مكون من 1024 رمزًا مميزًا، يبلغ حجمه حوالي 75 ميجابايت في FP32. بالنسبة إلى Llama 3 405B الذي يحتوي على 128 طبقة، يمكن أن يتجاوز حجم ذاكرة التخزين المؤقت KV لتسلسل واحد 10 جيجابايت. هذا هو السبب في أن الاستدلال طويل السياق مرتبط بالذاكرة.
### الملء المسبق مقابل فك التشفير: مرحلتان من الاستدلال
عند إرسال مطالبة إلى LLM، يحدث الاستدلال على مرحلتين مختلفتين.
تقوم **الملء المسبق** بمعالجة المطالبة بالكامل بالتوازي. جميع الرموز المميزة معروفة، لذلك يمكن للنموذج حساب الانتباه لجميع المواضع في وقت واحد. هذه المرحلة مرتبطة بالحوسبة - يقوم GPU بإجراء عمليات ضرب المصفوفات بإنتاجية كاملة. بالنسبة للمطالبة بـ 1000 رمز مميز على A100، تستغرق التعبئة المسبقة حوالي 20-50 مللي ثانية.
**فك التشفير** يُنشئ الرموز المميزة واحدًا تلو الآخر. يعتمد كل رمز مميز جديد على جميع الرموز المميزة السابقة. هذه المرحلة مرتبطة بالذاكرة - عنق الزجاجة هو قراءة أوزان النموذج وذاكرة التخزين المؤقت KV من ذاكرة GPU، وليس حسابات المصفوفة نفسها. النوى الحسابية لـ GPU تظل خاملة في الغالب في انتظار قراءات الذاكرة. بالنسبة إلى GPT-2، تستغرق كل خطوة فك تشفير نفس الوقت تقريبًا بغض النظر عن عدد FLOPs التي تتطلبها matmuls، لأن عرض النطاق الترددي للذاكرة هو القيد.
وهذا التمييز مهم لأنظمة الإنتاج. قم بملء مقاييس الإنتاجية مسبقًا بحساب GPU (المزيد FLOPS = تعبئة مسبقة أسرع). فك تشفير مقاييس الإنتاجية باستخدام النطاق الترددي للذاكرة (ذاكرة أسرع = فك تشفير أسرع). ولهذا السبب ركز NVIDIA's H100 على تحسينات النطاق الترددي للذاكرة عبر A100 - فهو يعمل بشكل مباشر على تسريع إنشاء الرمز المميز.
```mermaid
graph LR
...
### حلقة التدريب
تدريب LLM هو التنبؤ بالرمز المميز التالي. الرموز المميزة [0، 1، 2، ...، N-1]، توقع الرموز المميزة [1، 2، 3، ...، N]. دالة الخسارة هي إنتروبيا متقاطعة بين التوزيع الاحتمالي المتوقع للنموذج والرمز المميز التالي.
خطوة تدريبية واحدة:
1. **التمرير الأمامي**: قم بتشغيل الدفعة عبر جميع الكتل الـ 12. احصل على logits (درجات ما قبل softmax) لكل منصب.
...
إن جدول معدل التعلم مهم أكثر مما قد تتوقعه. GPT-2 يرتفع من 0 إلى ذروة معدل التعلم خلال أول 2000 خطوة، ثم يضمحل بعد منحنى جيب التمام. البدء بمعدل تعليم مرتفع يؤدي إلى تباعد النموذج. الحفاظ على معدل مرتفع ثابت يسبب التذبذب في التدريب اللاحق. يتم استخدام نمط الإحماء ثم الاضمحلال في كل LLM رئيسي.
### GPT-2 صغير: الأرقام
| المكون | الشكل | المعلمات |
|-----------|-------|------------|
...
يشارك إسقاط الإخراج (رأس logits) الأوزان مع مصفوفة تضمين الرمز المميز. وهذا ما يسمى ربط الوزن - فهو يقلل من عدد المعلمات بمقدار 38 مليونًا ويحسن الأداء لأنه يجبر النموذج على استخدام نفس مساحة التمثيل للإدخال والإخراج.
## بنائها
### الخطوة 1: تضمين الطبقة
...
يأتي الانحراف المعياري 0.02 للتهيئة من ورقة GPT-2. كبيرة جدًا والتمريرات الأمامية الأولية تنتج قيمًا متطرفة تزعزع استقرار التدريب. صغيرة جدًا والمخرجات الأولية متطابقة تقريبًا لجميع المدخلات، مما يجعل إشارات التدرج المبكرة عديمة الفائدة.
### الخطوة الثانية: الاهتمام بالذات باستخدام القناع السببي
...
تقوم شبكة التغذية الأمامية بتوسيع المدخلات ذات 768 بُعدًا إلى 3072 بُعدًا (4x)، وتطبيق اللاخطية، ثم إسقاطها مرة أخرى إلى 768. يمنح نمط التمدد والانكماش هذا النموذج تمثيلًا داخليًا "أوسع" للعمل به في كل موضع. GPT-2 يستخدم تنشيط GELU، ولكننا نستخدم ReLU هنا للتبسيط -- والفرق بسيط لفهم البنية.
...
لاحظ ربط الوزن: `logits = x @ self.embedding.token_embed.T`. يعيد إسقاط الإخراج استخدام مصفوفة تضمين الرمز المميز (منقولة). هذه ليست مجرد خدعة لحفظ المعلمات. وهذا يعني أن النموذج يستخدم نفس مساحة المتجه لفهم الرموز المميزة (التضمينات) والتنبؤ بها (الإخراج).
### الخطوة 6: حلقة التدريب
لإجراء تدريب حقيقي على 124 مليون معلمة، ستحتاج إلى GPU وPyTorch. توضح حلقة التدريب هذه الآليات المتبعة في نموذج صغير يعمل بطريقة غير متقنة. نحن نستخدم نموذجًا صغيرًا (4 طبقات، 4 رؤوس، 128 خافتًا) لجعله make قابلاً للتتبع.
```python
...
تعد نافذة `tokens[-seq_len:]` ضرورية لأن النموذج له الحد الأقصى لطول السياق (1024 لـ GPT-2). بمجرد تجاوزه، يجب عليك إسقاط أقدم الرموز. هذه هي "نافذة السياق" التي يتحدث عنها الجميع.
## استخدمه
### التدريب الكامل والعرض التوضيحي للجيل
```python
...
في مجموعة صغيرة ذات نموذج صغير، سيكون النص الناتج شبه متماسك في أحسن الأحوال. سوف يتعلم بعض أنماط مستوى البايت من نص التدريب ولكن لا يمكنه تعميم الطريقة التي يفعلها GPT-2 مع 40 جيجابايت من بيانات التدريب وبنية المعلمة الكاملة البالغة 124 ميجابايت. النقطة ليست في جودة الإخراج. النقطة المهمة هي أنه يمكنك تتبع كل خطوة: تضمين البحث، وحساب الانتباه، وتحويل التغذية الأمامية، وإسقاط logit، وsoftmax، وأخذ العينات. كل عملية مرئية.
## اشحنها
يُنتج هذا الدرس `outputs/prompt-gpt-architecture-analyzer.md` -- موجه يقوم بتحليل اختيارات البنية في أي نموذج بنمط GPT. قم بتزويده ببطاقة نموذجية أو تقرير فني وسيقوم بتقسيم تخصيص المعلمات وتصميم الاهتمام وقرارات القياس.
## تمارين
...
3. أضف ذاكرة تخزين مؤقت KV إلى وظيفة الإنشاء. قم بتخزين موترات K وV لكل طبقة بعد التمريرة الأمامية الأولى، وأعد استخدامها للرموز المميزة اللاحقة. قم بقياس السرعة: أنشئ 200 رمزًا مميزًا مع ذاكرة التخزين المؤقت وبدونها وقارن وقت ساعة الحائط.
...
| اهتمام متعدد الرؤوس | "أنماط الانتباه المتعددة" | تقسيم Q وK وV إلى رؤوس متوازية (على سبيل المثال، 12 رأسًا كل منها 64 لونًا خافتًا لـ GPT-2) بحيث يتمكن كل رأس من تعلم أنواع مختلفة من العلاقات |
...
| تعبئة مسبقة | "معالجة الموجه" | مرحلة الاستدلال الأولى حيث تتم معالجة كافة الرموز المميزة للمطالبة بالتوازي - حساب مرتبط في GPU FLOPS |
| فك | "إنشاء الرموز" | مرحلة الاستدلال الثانية حيث يتم إنشاء الرموز المميزة واحدًا تلو الآخر - مرتبطة بالذاكرة على عرض النطاق الترددي GPU |
...
| خسارة الانتروبيا المتقاطعة | "ما أخطأت التوقعات" | -log(الاحتمال المعين للرمز التالي الصحيح)، متوسط ​​جميع المواضع - الهدف التدريبي القياسي LLM |
## مزيد من القراءة
...
- [Vaswani et al., 2017 -- "Attention Is All You Need"](https://arxiv.org/abs/1706.03762) - ورقة المحولات الأصلية مع اهتمام متدرج بالمنتج النقطي وانتباه متعدد الرؤوس
- [Llama 3 Technical Report](https://arxiv.org/abs/2407.21783) -- كيف قامت Meta بتوسيع بنية GPT إلى معلمات 405B باستخدام وحدات معالجة الرسومات 16K
- [Pope et al., 2022 -- "Efficiently Scaling Transformer Inference"](https://arxiv.org/abs/2211.05102) -- الورقة التي أضفت طابعًا رسميًا على التعبئة المسبقة مقابل فك التشفير وتحليل ذاكرة التخزين المؤقت KV