# الرسم الداخلي والطلاء الخارجي وتحرير الصور
> تحويل النص إلى صورة makes أشياء جديدة. Inpainting إصلاح القديمة. في الإنتاج، يتم تحرير 70% من أعمال الصور القابلة للفوترة - تبديل الخلفية، وإزالة الشعار، وتوسيع اللوحة القماشية، وتجديد اليد. Inpainting هو المكان الذي يكتسب فيه الانتشار مكانته.
**النوع:** بناء
** اللغات: ** بايثون
...
### SDEdit (مينج وآخرون، 2022) — تحرير مجاني
أضف تشويشًا إلى الصورة المصدر حتى `t` المتوسط، ثم قم بتشغيل السلسلة العكسية من `t` إلى 0 بمطالبة جديدة. لا إعادة التدريب. إن اختيار بدء `t` يستبدل الإخلاص بالحرية الإبداعية:
- `t/T = 0.3` → مطابق تقريبًا للمصدر، مع تغييرات أسلوبية صغيرة
- `t/T = 0.6` → تعديلات معتدلة، مع الحفاظ على البنية الخشنة
- `t/T = 0.9` → تم إنشاؤه من ضوضاء قريبة، مع الحد الأدنى من الحفاظ على المصدر
### InstructPix2Pix (بروكس وآخرون، 2023)
...
### إعادة الطلاء (Lugmayr وآخرون، 2022)
...
## بنائها
`code/main.py` ينفذ لعبة رسم أحادي الأبعاد على بيانات خماسية الأبعاد. نقوم بتدريب DDPM على بيانات الخليط 5-D حيث تكون كل عينة 5 عوامات من إحدى المجموعتين. عند الاستدلال، نقوم "بإخفاء" 2 من الأبعاد الخمسة، ونحقن النسخة الأمامية الصاخبة من الأبعاد الثلاثة غير المقنعة في كل خطوة، ونعيد إنشاء الأبعاد المقنعة فقط.
### الخطوة 1: بيانات DDPM 5-D
```python
def sample_data(rng):
...
### الخطوة 2: قم بتدريب مزيل الضوضاء على جميع الخفتات الخمسة
المعيار DDPM. صافي المخرجات التنبؤ بالضوضاء 5-D للمدخلات الصاخبة 5-D.
### الخطوة 3: عند الاستدلال، عكس القناع
```python
...
- **CFG يتفاعل مع حجم القناع.** ارتفاع CFG على قناع صغير = رقعة مشبعة. قم بتقليل CFG للتعديلات الصغيرة.
...
| مهمة | خط أنابيب |
|------|----------|
| إزالة الكائن، قناع صغير | SD-Inpaint أو Flux-Fill، المطالبة القياسية |
...
| تغيير نمط منطقة واحدة | SDEdit في `t/T=0.5` على المنطقة المقنعة |
| "اجعله غروبًا" | InstructPix2Pix أو Flux-Kontext |
| استبدال الخلفية | قناع SAM → SD-Inpaint |
| فائقة الدقة | Flux-Fill أو GPT-Image (مستضاف) لأصعب الحالات |
SAM (مقطع Meta أي شيء، 2023) + طلاء الانتشار هو خط إزالة الخلفية لعام 2026 pipeline. SAM 2 (2024) يعمل على الفيديو.
## اشحنها
احفظ `outputs/skill-editing-pipeline.md`. تأخذ المهارة صورة أصلية + وصف تحرير + قناع اختياري (أو موجه SAM) والمخرجات: نهج إنشاء القناع، والنموذج الأساسي، ومقاييس CFG (الصورة + النص)، ووضع SDEdit-t أو inpainting، وقائمة التحقق QA.
## تمارين
...
## ملاحظة حول الإنتاج: خطوط تحرير pip حساسة لزمن الوصول
يتوقع المستخدمون الذين يقومون بتحرير صورة رحلات ذهابًا وإيابًا مدتها أقل من 5 ثوانٍ. تستغرق عملية SDXL-Inpaint المكونة من 30 خطوة عند 1024² 3-4 ثوانٍ على L4، بالإضافة إلى إنشاء قناع SAM (~200 مللي ثانية) وVAE تشفير/فك تشفير (~500 مللي ثانية مجتمعة). في إطار الإنتاج، يكون هذا مرتبطًا بـ TTFT وليس مرتبطًا بالإنتاجية - الدفعة 1، ذات التزامن المنخفض، تقلل كل مرحلة:
- **SAM-H هو الخط البطيء.** SAM-H عند 1024² هو ~200 مللي ثانية؛ SAM-ViT-B تبلغ حوالي 40 مللي ثانية مع فقدان طفيف للجودة. SAM 2 (فيديو) يضيف الحمل الزمني؛ لا تستخدمه لتحرير الصورة الفردية.
- **تخطي التشفير عندما يكون ذلك ممكنًا.** `pipe.image_processor.preprocess(img)` يشفر إلى الملفات الكامنة. إذا كانت لديك العناصر الكامنة من الجيل السابق (نموذجية في واجهات مستخدم التحرير التكراري)، فمررها مباشرةً عبر `latents=...` لتخطي ترميز VAE واحد.
...
- **Flux-Kontext هو إجابة 2025.** تمرير أمامي فردي فوق `(source_image, instruction)` - لا يوجد قناع منفصل، ولا يوجد مسح لضوضاء SDEdit. في H100، يتم إرسال التعديل خلال 1.5 ثانية تقريبًا. الدرس المعماري: انهيار المراحل.
## مزيد من القراءة
- [Lugmayr et al. (2022). RePaint: Inpainting using Denoising Diffusion Probabilistic Models](https://arxiv.org/abs/2201.09865) — الرسم بدون تدريب.
- [Meng et al. (2022). SDEdit: Guided Image Synthesis and Editing with Stochastic Differential Equations](https://arxiv.org/abs/2108.01073) — تعديل SD.
- [Brooks, Holynski, Efros (2023). InstructPix2Pix](https://arxiv.org/abs/2211.09800) — تحرير تعليمات النص.
...
- [Hertz et al. (2022). Prompt-to-Prompt Image Editing with Cross-Attention Control](https://arxiv.org/abs/2208.01626) — التحرير على مستوى الانتباه.
- [Black Forest Labs (2024). Flux.1-Fill and Flux.1-Kontext](https://blackforestlabs.ai/flux-1-tools/) — أدوات 2024.