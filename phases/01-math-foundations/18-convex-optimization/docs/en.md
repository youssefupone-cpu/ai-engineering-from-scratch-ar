...
- اشرح لماذا لا تزال المناظر الطبيعية لفقدان الشبكة العصبية غير محدبة حتى الآن SGD لا تزال تجد حلولاً جيدة
...
فهم التحدب يفعل ثلاثة أشياء. أولاً، يخبرك عندما تكون مشكلتك سهلة (محدبة) مقابل صعبة (غير محدبة). ثانيًا، يمنحك أدوات أسرع مثل طريقة نيوتن للمسائل المحدبة. ثالثًا، يشرح المفاهيم التي تظهر في جميع أنحاء ML: التنظيم كقيد، والازدواجية في SVMs، ولماذا يعمل التعلم العميق على الرغم من انتهاك كل خصائص التحدب اللطيفة التي يوفرها لك.
...
| SVM (فقدان المفصلي) | نعم | الحد الأقصى للوظائف الخطية |
...
شروط KKT (ضرورية لتحقيق الأمثلية):
...
KKT الشروط أساسية بالنسبة لـ SVMs. متجهات الدعم هي نقاط البيانات التي يكون فيها القيد نشطًا (lambda > 0). جميع نقاط البيانات الأخرى لها لامدا = 0 ولا تؤثر على حدود القرار.

### Regularization as constrained optimization

التسوية L1 و L2 ليست حيلًا تعسفية. فهي مشاكل التحسين مقيدة مقنعة.
...
**L-BFGS (ذاكرة محدودة BFGS):** تقريب معكوس Hessian باستخدام آخر اختلافات التدرج m. يتطلب ذاكرة O(mn) بدلاً من O(n^2). يعمل بشكل جيد مع المشكلات التي يصل عددها إلى 10000 معلمة تقريبًا. تستخدم في ML الكلاسيكية (الانحدار اللوجستي، CRFs) ولكن ليس التعلم العميق.

**التدرج الطبيعي:** يستخدم مصفوفة معلومات فيشر (هيسي المتوقعة لاحتمالية السجل) بدلاً من هسي القياسية. وهذا يفسر هندسة التوزيعات الاحتمالية. K-FAC (الانحناء التقريبي المعامل كرونيكر) يقارب مصفوفة فيشر كمنتج كرونيكر، مما يجعله عمليًا للشبكات العصبية.
...
| ل-BFGS | يا(من) | يا(من) | مسائل محدبة متوسطة |
...
ينطبق تحليل التحدب مباشرة عند اختيار نماذج ومحاليل ML.

بالنسبة للمشكلات المحدبة (الانحدار اللوجستي، SVMs، LASSO):
- استخدم أدوات حل مخصصة (liblinear، CVXPY، scipy.optimize.minimize باستخدام الطريقة='L-BFGS-B')
...
4. **قيد التنظيم.** تنفيذ التحسين المقيد L1: تقليل (x-3)^2 + (y-2)^2 خاضع لـ |x| + |ص| <= 1. أظهر أن الحل له إحداثي واحد يساوي الصفر (تناثر من قيد الماس).
...
| ل-BFGS | طريقة تقريبية من الدرجة الثانية تخزن آخر اختلافات التدرج m بدلاً من Hessian الكامل |
...
- [Bottou, Curtis, Nocedal: Optimization Methods for Large-Scale Machine Learning (2018)](https://arxiv.org/abs/1606.04838) - نظرية تحسين الجسور المحدبة وممارسة التعلم العميق
- [Choromanska et al.: The Loss Surfaces of Multilayer Networks (2015)](https://arxiv.org/abs/1412.0233) - لماذا لا تكون المناظر الطبيعية للشبكات العصبية غير المحدبة سيئة كما تبدو
- [Nocedal & Wright: التحسين العددي](https://link.springer.com/book/10.1007/978-0-387-40065-5) - مرجع شامل لطريقة نيوتن، L-BFGS، والتحسين المقيد
