...
تمنحك الإحصائيات الأدوات اللازمة لتمييز الإشارة عن الضوضاء. فهو يخبرك عندما يكون الاختلاف حقيقيًا، ومدى الثقة التي يجب أن تكون عليها، وكم البيانات التي تحتاجها قبل أن تتمكن من الوثوق بالنتيجة. كل ML pip سطر، وكل مقارنة للنماذج، وكل تجربة تحتاج إلى إحصائيات. بدونها، أنت تخمن.
##المفهوم
### الإحصائيات الوصفية: تلخيص بياناتك
...
**النسب المئوية** تقسم البيانات المصنفة إلى 100 جزء متساوي. النسبة المئوية الخامسة والعشرون (Q1) تعني أن 25% من القيم تقع تحت هذه النقطة. المئين الخمسين هو المتوسط. النسبة المئوية الخامسة والسبعون هي Q3.
```
...
في ML، أنت تهتم بالنسب المئوية لوقت استجابة الاستدلال، وتوزيعات ثقة التنبؤ، وفهم توزيعات الخطأ. قد يكون النموذج ذو متوسط ​​الخطأ المنخفض ولكن الخطأ الفادح P99 عديم الفائدة للتطبيقات الحساسة للسلامة.
...
### الارتباط: كيف تتحرك المتغيرات معًا
...
**القيمة p** هي احتمال رؤية بيانات متطرفة مثل ما لاحظته، بافتراض أن H0 صحيح. إنه NOT احتمال أن يكون H0 صحيحًا. هذا هو سوء الفهم الأكثر شيوعًا في الإحصائيات.
```
...
يعد هذا أقوى من اختبار t المقترن لأنه لا يحتوي على افتراضات توزيعية.
### الاختبارات البارامترية مقابل الاختبارات غير البارامترية
...
في تجارب ML، عادةً ما يكون لديك n صغير (5 أو 10 طيات للتحقق المتبادل)، لذا فإن الاختبارات غير البارامترية مثل تصنيف ويلكوكسون غالبًا ما تكون أكثر ملاءمة من اختبارات t.
...
### الأخطاء الإحصائية الشائعة في ML الأوراق
1. **الاختبار على مجموعة التدريب.** ضمانات التجهيز الزائد. احتفظ دائمًا بالبيانات التي لم يراها النموذج مطلقًا أثناء التدريب.
2. **لا توجد فترات ثقة.** الإبلاغ عن رقم دقة واحد دون عدم اليقين makes يؤدي إلى نتائج غير قابلة للتكرار ولا يمكن التحقق منها.
...
5. **استخدام الدقة في البيانات غير المتوازنة.** الدقة بنسبة 99% في مجموعة بيانات ذات فئة سلبية بنسبة 99% تعني أن النموذج لم يتعلم شيئًا. استخدم الدقة أو الاستدعاء أو F1 أو AUC.
...
## بنائها
سوف تقوم بتنفيذ:
1. **إحصاءات وصفية من الصفر** (المتوسط، الوسيط، الوضع، الانحراف المعياري، النسب المئوية، IQR)
...
6. **عرض توضيحي للأهمية الإحصائية مقابل الأهمية العملية** (يُظهر أن كل شيء "مهم" كبير في makes)
...
| خطأ من النوع الأول | إيجابية كاذبة. رفض H0 عندما يكون صحيحًا. |
| اكتب خطأ II | سلبية كاذبة. الفشل في رفض H0 عندما يكون خطأ. |
| القوة الإحصائية | احتمال رفض H0 بشكل صحيح. الطاقة = 1 ناقص معدل الخطأ من النوع II. |
