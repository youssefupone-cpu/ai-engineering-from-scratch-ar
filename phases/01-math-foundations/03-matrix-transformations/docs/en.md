...
## المشكلة
قرأت عن PCA وشاهدت "ابحث عن المتجهات الذاتية لمصفوفة التغاير." قرأت عن استقرار النموذج وشاهدت "تحقق مما إذا كانت جميع القيم الذاتية لها حجم أقل من 1." قرأت عن زيادة البيانات وشاهدت "تطبيق تدوير عشوائي". لا معنى لأي من هذا make حتى تفهم ما تفعله المصفوفات بالمساحة هندسيًا.
المصفوفات ليست مجرد شبكات من الأرقام. إنها آلات مكانية. مصفوفة التناوب تدور النقاط. تمدهم مصفوفة القياس. مصفوفة القص تميلهم. كل تحويل تطبقه الشبكة العصبية على البيانات هو إحدى هذه العمليات أو تركيبة منها. هذا الدرس make يوضح هذه العمليات بشكل ملموس.
##المفهوم
### التحولات كمصفوفات
...
مصفوفات القص:
- `Shx = [[1, k], [0, 1]]` تحولات x بواسطة k * y
- `Shy = [[1, 0], [k, 1]]` تحول y بواسطة k * x
### انعكاس
...
## بنائها
### الخطوة 1: تحويل المصفوفات من الصفر (بايثون)
```python
import math
...
### الخطوة 3: القيم الذاتية من الصفر (2x2)
...
### الخطوة 4: المحدد كعامل قياس الحجم
```python
def det_2x2(matrix):
...
## استخدمه
يتعامل NumPy مع كل هذا بإجراءات محسنة.
```python
import numpy as np
...
### تدوير ثلاثي الأبعاد باستخدام NumPy
```python
...
## اشحنها
يبني هذا الدرس الأساس الهندسي لـ PCA (المرحلة الثانية) وتحليل وزن الشبكة العصبية. رمز القيمة الذاتية/المتجه الذاتي الذي تم إنشاؤه هنا هو نفس الخوارزمية التي تعمل على تقليل الأبعاد، والتجميع الطيفي، وتحليل الاستقرار في أنظمة الإنتاج ML.
## تمارين
...
2. أوجد القيم الذاتية للمصفوفة [[4، 2]، [1، 3]] يدويًا باستخدام المعادلة المميزة. ثم تحقق باستخدام وظيفتك من البداية وباستخدام NumPy.
...
## مزيد من القراءة
- [3Blue1Brown: Linear Transformations](https://www.3blue1brown.com/lessons/linear-transformations) -- حدس بصري لكيفية إعادة تشكيل المصفوفات للفضاء
- [3Blue1Brown: Eigenvectors and Eigenvalues](https://www.3blue1brown.com/lessons/eigenvalues) -- أفضل شرح مرئي لما تعنيه المتجهات الذاتية هندسيًا
- [MIT 18.06 Lecture 21: Eigenvalues and Eigenvectors](https://ocw.mit.edu/courses/18-06-linear-algebra-spring-2010/) -- العلاج الكلاسيكي لجيلبرت سترانج