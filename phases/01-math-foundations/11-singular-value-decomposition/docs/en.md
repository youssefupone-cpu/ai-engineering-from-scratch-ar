...
- تطبيق SVD المقطوع لضغط الصور وقياس نسبة الضغط مقابل خطأ إعادة البناء
...
##المفهوم
### ما يفعله SVD هندسيًا
تقوم كل مصفوفة، بغض النظر عن شكلها، بتنفيذ ثلاث عمليات متتالية: التدوير، والقياس، والتدوير. SVD make هذا التحليل واضح.
```
A = U * Sigma * V^T
...
### المتجهات المفردة اليسرى، القيم المفردة، المتجهات المفردة اليمنى
...
### العلاقة بالتركيب الذاتي
SVD والتحلل الذاتي مرتبطان ارتباطًا وثيقًا. القيم المفردة والمتجهات لـ A تأتي مباشرة من القيم الذاتية والمتجهات الذاتية لـ A^T A وA A^T.
```
...
2. يمكنك حساب SVD عبر التحليل الذاتي لـ A^T A، ولكن هذا يؤدي إلى تربيع رقم الشرط ويفقد الدقة العددية. تتجنب خوارزميات SVD المخصصة ذلك.
3. عندما يكون A مربعًا ومتماثلًا وموجبًا شبه محدد، فإن SVD والتحلل الذاتي هما نفس الشيء.
### مبتورة SVD: تقريب منخفض الرتبة
...
| المكون | الحجم النسبي | أبقى في المرتبة 3 تقريبا؟ |
|-----------|-------------------|-----------------------|
| سيجما_1 | الأكبر | نعم |
| سيجما_2 | كبير | نعم |
...
الصورة ذات التدرج الرمادي هي مصفوفة لشدة البكسل. تحتوي الصورة مقاس 800 × 600 على 480.000 قيمة. يتيح لك SVD تقريبه بعدد أقل بكثير.
```
...
SVD في مصفوفة التقييمات (المملوءة) يقسمها إلى:
...
من الناحية العملية، يمكنك استخدام متغيرات مثل SVD التزايدية لـ Simon Funk أو ALS (المربعات الصغرى البديلة) التي تتعامل مع البيانات المفقودة مباشرة. لكن الفكرة الأساسية هي نفسها: تحليل العوامل الكامنة عبر SVD.
### SVD في NLP: التحليل الدلالي الكامن
التحليل الدلالي الكامن (LSA)، والذي يُسمى أيضًا الفهرسة الدلالية الكامنة (LSI)، يطبق SVD على مصفوفة مستند المصطلح.
```
             Doc1   Doc2   Doc3   Doc4
  "cat"      [  3      0      1      0  ]
  "dog"      [  2      0      0      1  ]
...
كان LSA أحد الأساليب الناجحة الأولى لالتقاط التشابه الدلالي من النص الخام. إنه يعمل لأن المصطلحات المترادفة تميل إلى الظهور في المستندات المتشابهة، لذلك يقوم SVD بتجميعها في نفس الأبعاد الكامنة. يمكن اعتبار تضمينات الكلمات الحديثة (Word2Vec، GloVe) من نسل هذه الفكرة.
### SVD لتقليل الضوضاء
...
ويستخدم هذا في معالجة الإشارات والقياس العلمي وتنظيف البيانات. في أي وقت يكون لديك مصفوفة تالفة بسبب الضوضاء الإضافية، فإن SVD المقطوع هو طريقة مبدئية لفصل الإشارة عن الضوضاء.
### معكوس زائف عبر SVD
يعمم معكوس مور-بنروز الزائف A+ انعكاس المصفوفة على المصفوفات غير المربعة والمفردة. SVD makes يعتبر الأمر تافهًا.
```
...
وهذا يعني أن كل ما تعلمته حول تقليل الأبعاد في الدرس 10 هو SVD تحت الغطاء. PCA هو التطبيق الأكثر شيوعًا لـ SVD في التعلم الآلي.
## بنائها
### الخطوة 1: SVD من البداية باستخدام تكرار الطاقة
...
### الخطوة الثانية: الاختبار والمقارنة مع NumPy
```python
np.random.seed(42)
...
### الخطوة 3: العرض التوضيحي لضغط الصور
```python
...
## استخدمه
عروض العمل الكاملة موجودة في `code/svd.py`. قم بتشغيله لترى أن SVD مطبق على ضغط الصور وأنظمة التوصية والتحليل الدلالي الكامن وتقليل الضوضاء.
```bash
python svd.py
```

يوضح إصدار Julia في `code/svd.jl` نفس المفاهيم باستخدام وظيفة `svd()` الأصلية لجوليا وحزمة `LinearAlgebra`.
```bash
julia svd.jl
```
...
## تمارين
1. قم بتنفيذ SVD بالكامل من البداية دون استخدام تكرار الطاقة. بدلاً من ذلك، احسب التركيب الذاتي لـ A^T A للحصول على V والقيم المفردة، ثم احسب U = A V Sigma^{-1}. قارن الدقة العددية مع إصدار تكرار الطاقة الخاص بك ومع NumPy.
...
3. قم ببناء نظام توصيات صغير. قم بإنشاء مصفوفة تصنيفات أفلام المستخدم مقاس 10 × 8 مع بعض الإدخالات المعروفة. املأ الإدخالات المفقودة بوسائل الصف. قم بحساب SVD وأعد بناء التقريب من المرتبة الثالثة. استخدم المصفوفة المعاد بناؤها للتنبؤ بالتقييمات المفقودة. التحقق من أن التوقعات معقولة.
4. قم بإنشاء مصفوفة مصطلحات مستندية مقاس 100 × 50 تحتوي على 3 موضوعات تركيبية. كل موضوع له 5 مصطلحات مرتبطة به. أضف الضوضاء. قم بتطبيق SVD وتحقق من أن القيم الفردية الثلاث العليا أكبر بكثير من الباقي. قم بعرض المستندات في المساحة الكامنة ثلاثية الأبعاد وتحقق من المستندات من نفس مجموعة المواضيع معًا.
5. قم بإنشاء مصفوفة نظيفة منخفضة الرتبة (المرتبة 3، الحجم 50 × 40) وأضف ضوضاء غاوسية على مستويات مختلفة (سيجما = 0.1، 0.5، 1.0، 2.0). لكل مستوى من مستويات الضوضاء، ابحث عن رتبة الاقتطاع الأمثل عن طريق مسح k من 1 إلى 40 وقياس خطأ إعادة البناء مقابل المصفوفة النظيفة. Plot how the optimal k changes with noise level.
## المصطلحات الرئيسية
...
| رقم الحالة | "مدى حساسية الأخطاء" | سيجما_ماكس / سيجما_مين. رقم الحالة الكبير يعني أن التغييرات الصغيرة في المدخلات تؤدي إلى تغييرات كبيرة في المخرجات. SVD يكشف هذا مباشرة. |
...
| نظرية إيكارت يونغ | "SVD يعطي أفضل ضغط" | بالنسبة لأي رتبة مستهدفة k، يقلل SVD المقطوع من خطأ التقريب على جميع مصفوفات الرتبة k الممكنة. |
...
## مزيد من القراءة
- [Gilbert Strang: Linear Algebra and Its Applications, Chapter 7](https://math.mit.edu/~gs/linearalgebra/) - معالجة شاملة لـ SVD مع التطبيقات
- [3Blue1Brown: But what is the SVD?](https://www.youtube.com/watch?v=vSczTbgc8Rc) - الحدس الهندسي لـ SVD
- [We Recommend a Singular Value Decomposition](https://www.ams.org/publicoutreach/feature-column/fcarc-svd) - نظرة عامة يمكن الوصول إليها من جمعية الرياضيات الأمريكية
- [Netflix Prize and Matrix Factorization](https://sifter.org/~simon/journal/20061211.html) - مشاركة المدونة الأصلية لـ Simon Funk بتاريخ SVD للحصول على التوصيات
- [Latent Semantic Analysis](https://en.wikipedia.org/wiki/Latent_semantic_analysis) - طلب NLP الأصلي لـ SVD
- [Numerical Linear Algebra by Trefethen and Bau](https://people.maths.ox.ac.uk/trefethen/text.html) - المعيار الذهبي لفهم خوارزميات SVD وخصائصها العددية