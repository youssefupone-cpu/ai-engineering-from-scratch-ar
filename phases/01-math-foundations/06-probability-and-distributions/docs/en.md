...
> الاحتمال هو اللغة التي يستخدمها AI للتعبير عن عدم اليقين.
...
- قم ببناء وظائف softmax وlog-softmax باستخدام خدعة الاستقرار الرقمي (اطرح max logit)
- حساب الخسارة عبر الإنتروبيا من logits وربطها باحتمال السجل السلبي

## The Problem

يخرج المصنف `[0.03, 0.91, 0.06]`. يختار نموذج اللغة الكلمة التالية من بين 50000 مرشح. يقوم نموذج الانتشار بإنشاء صور عن طريق أخذ عينات من التوزيعات المستفادة. كل هذه احتمالات في العمل.

كل تنبؤ بنموذج makes هو توزيع احتمالي. تقيس كل دالة خسارة مدى بعد التوزيع المتوقع عن التوزيع الحقيقي. تقوم كل خطوة تدريب بضبط المعلمات بحيث تبدو make توزيعًا واحدًا أشبه بآخر. بدون احتمال، لا يمكنك قراءة ورقة ML واحدة، أو تصحيح نموذج واحد، أو فهم سبب كون خسارة التدريب الخاصة بك NaN.
...
المتغيرات العشوائية المنفصلة لها دالة احتمالية جماعية (PMF). كل نتيجة لها احتمالية محددة يمكنك قراءتها مباشرة.
...
هذا التمييز مهم في ML. مخرجات التصنيف هي PMFs (اختيارات منفصلة). VAE تستخدم المساحات الكامنة ملفات PDF (مستمر).
...
في ML، تظهر القيمة المتوقعة كدالة الخسارة (متوسط ​​الخسارة على توزيع البيانات). يخبرك التباين عن استقرار النموذج. التباين العالي في التدرجات يعني التدريب الصاخب.

### Joint and Marginal Distributions

يصف التوزيع المشترك P(X, Y) متغيرين عشوائيين معًا.

مثال مشترك PMF (X = الطقس، Y = المظلة):
...
تقوم الشبكات العصبية بإخراج الدرجات الأولية (logits). يقوم Softmax بتحويلها إلى توزيع احتمالي صالح.
...
خدعة softmax: اطرح الحد الأقصى logit قبل الأس لمنع الفائض.
...
يجمع Log-softmax بين softmax وlog لتحقيق الاستقرار العددي. PyTorch يستخدم هذا داخليًا لخسارة الإنتروبيا المتقاطعة.

### Sampling

أخذ العينات يعني رسم قيم عشوائية من التوزيع. في ML:
...
توجد عمليات التنفيذ الكاملة مع كافة المرئيات في `code/probability.py`.

## Use It

مع NumPy وSciPy، كل ما سبق هو عبارة عن سطر واحد:
...
1. تنفيذ أخذ عينات التحويل العكسي للتوزيع الأسي. تحقق من ذلك عن طريق أخذ عينات من 10000 قيمة ومقارنة الرسم البياني بالقيمة الحقيقية PDF.

2. بناء جدول توزيع مشترك لنردين محملين. احسب التوزيعات الهامشية وتحقق مما إذا كان النرد مستقلاً.

3. احسب الخسارة عبر الإنتروبيا لمصنف من 5 فئات ينتج logits `[2.0, 0.5, -1.0, 3.0, 0.1]` عندما تكون الفئة الصحيحة هي الفهرس 3. ثم تحقق من إجابتك باستخدام PyTorch `nn.CrossEntropyLoss`.
...
| PMF | "دالة الاحتمال" | دالة تعطي الاحتمال الدقيق لكل نتيجة منفصلة، ​​ومجموعها 1 |
| PDF | "منحنى الاحتمالية" | دالة الكثافة للمتغيرات المستمرة. دمجها على فترة زمنية للحصول على الاحتمال |
...
| سوفت ماكس | "تحويل النتائج إلى احتمالات" | softmax(z_i) = exp(z_i) / sum(exp(z_j)). يعين logits ذو القيمة الحقيقية لتوزيع احتمالي صالح |
| عبر الانتروبيا | "وظيفة الخسارة" | -sum(p_true * log(p_predicted)). يقيس مدى اختلاف التوزيعتين. الدنيا أفضل |
| لوgitس | "مخرجات النموذج الخام" | درجات غير طبيعية قبل softmax. سميت على اسم الوظيفة اللوجستية |
| أخذ العينات | "رسم قيم عشوائية" | توليد القيم وفقا للتوزيع الاحتمالي. كيف تولد النماذج المخرجات |

## Further Reading

- [3Blue1Brown: لكن ما هي نظرية الحد المركزي؟](https://www.youtube.com/watch?v=zeJD6dqJ5lo) - دليل مرئي لماذا تصبح المتوسطات طبيعية
- [ستانفورد CS229 مراجعة الاحتمالية](https://cs229.stanford.edu/section/cs229-prob.pdf) - مرجع موجز يغطي كل شيء هنا وأكثر
- [خدعة Log-Sum-Exp](https://gregorygundersen.com/blog/2020/02/09/log-sum-exp/) - لماذا يهم الاستقرار العددي وكيفية تحقيقه
