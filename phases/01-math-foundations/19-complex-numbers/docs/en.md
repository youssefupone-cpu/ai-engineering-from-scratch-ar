# الأرقام المعقدة لمنظمة العفو الدولية
...
بدون فهم الأعداد المركبة، لا يمكنك فهم تحويل فورييه المتقطع. لا يمكنك فهم FFT. لا يمكنك فهم كيفية عمل RoPE (تضمين الموضع الدوار) في نماذج اللغة الحديثة. لا يمكنك فهم سبب استخدام الترميزات الموضعية الجيبية في ورقة Transformer الأصلية للترددات التي تستخدمها.
...
الرقم المركب هو في نفس الوقت نقطة ومتجه من نقطة الأصل. هذا التفسير المزدوج هو ما يجعل الأعداد المركبة مفيدة للهندسة.
...
### سبب أهمية صيغة أويلر في تعلم الآلة
...
وهذا يعني أن الأسس المعقدة هي التناوب. والتناوب موجود في كل مكان في معالجة الإشارات والتعلم الآلي.

### الاتصال بالدورات ثنائية الأبعاد
...
أنها تنتج نتائج مماثلة. الضرب المعقد هو دوران ثنائي الأبعاد. مصفوفة التدوير هي مجرد عملية ضرب معقدة مكتوبة بترميز المصفوفة.
...
جذور الوحدة هي أساس تحويل فورييه المنفصل. يقوم DFT بتحليل الإشارة إلى مكونات عند هذه الترددات N المتساوية التباعد.
...
يقيس كل X[k] مدى ارتباط الإشارة بالجذر k للوحدة - وهو شكل جيبي معقد عند التردد k. يقوم DFT بتقسيم الإشارة إلى أطوار دوارة N ويخبرك بسعة ومرحلة كل منها.

### لماذا أنا لست خياليا
...
### الأسس المعقدة مقابل الدوال المثلثية

قبل صيغة أويلر، كتب المهندسون الإشارات على النحو التالي: A*cos(omega*t + phi) - السعة A، التردد omega، المرحلة phi. هذا يعمل ولكنه يجعل الحساب مؤلما. تتطلب إضافة جيبي التمام بمراحل مختلفة هويات مثلثية.
...
### الخطوة الثانية: التحويل القطبي وصيغة أويلر
...
التحقق: `euler(theta).magnitude()` يجب أن يكون دائمًا 1.0. `euler(0)` يجب أن يعطي (1، 0). `euler(pi)` يجب أن يعطي (-1، 0).
...
### الخطوة 4: DFT من العمليات الحسابية المعقدة
...
يعيد DFT المعكوس بناء الإشارة الأصلية من طيفه. التغييرات الوحيدة من DFT الأمامي: اقلب العلامة الموجودة في الأس واقسمها على N.
...
وهذا يمنحك إعادة بناء مثالية. قم بتطبيق DFT، ثم IDFT، وستستعيد الإشارة الأصلية إلى دقة الماكينة. لا يتم فقدان أية معلومات.
...
هذه الخصائص هي التي تجعل DFT قابلاً للعكس. تشكل جذور الوحدة أساسًا متعامدًا لمجال التردد.

## استخدمه

لدى Python دعم مدمج للأرقام المركبة. يمثل `j` الحرفي الوحدة التخيلية.
...
3. **DFT لإشارة معروفة.** أنشئ إشارة تمثل مجموع sin(2*pi*3*t) و0.5*sin(2*pi*7*t) التي تم أخذ عينات منها عند 32 نقطة. قم بتشغيل DFT الخاص بك. تحقق من أن طيف الحجم له قمم عند الترددات 3 و7، حيث تكون الذروة عند 7 نصف ارتفاع الذروة عند 3.
...
| تجهيز الدوائر | تحويل فورييه المنفصل. يتحلل الإشارة إلى مكونات جيبية معقدة باستخدام جذور الوحدة |
| حبل | الروتاري موقف التضمين. يستخدم الضرب المعقد لتشفير الموضع النسبي في انتباه المحول |

## مزيد من القراءة

- [Visual Introduction to Euler's Formula](https://betterexplained.com/articles/intuitive-understanding-of-eulers-formula/) - يبني حدسًا هندسيًا بدون تدوين ثقيل
- [Su et al.: RoFormer (2021)](https://arxiv.org/abs/2104.09864) - الورقة التي تقدم تضمين الموضع الدوار باستخدام دورات معقدة
- [Vaswani et al.: Attention Is All You Need (2017)](https://arxiv.org/abs/1706.03762) - ورقة المحولات الأصلية ذات الترميزات الموضعية الجيبية
- [3Blue1Brown: Euler's formula with introductory group theory](https://www.youtube.com/watch?v=mvmuCPvRoWQ) - شرح مرئي لسبب e^(i*pi) = -1
- [Needham: Visual Complex Analysis](https://global.oup.com/academic/product/visual-complex-analysis-9780198534464) - أفضل معالجة بصرية للأعداد المركبة، مليئة بالبصيرة الهندسية
- [Strang: Introduction to Linear Algebra, Ch. 10](https://math.mit.edu/~gs/linearalgebra/) - الأعداد المركبة في سياق الجبر الخطي والقيم الذاتية