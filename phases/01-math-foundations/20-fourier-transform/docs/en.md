...
**الوقت:** ~90 دقيقة
## أهداف التعلم
- تنفيذ DFT من البداية والتحقق منه مقابل O(N log N) Cooley-Tukey FFT
- تفسير معاملات التردد: استخراج السعة والطور وطيف الطاقة من الإشارة
- تطبيق نظرية الإلتواء لإجراء الإلتواء عبر الضرب FFT
- ربط تحليل تردد فورييه بالتشفير الموضعي للمحولات وطبقات الالتواء CNN
## المشكلة
...
الفكرة الأساسية: `e^(-2*pi*i*k*n/N)` هو طور دوار عند التردد k. يحسب DFT الارتباط بين الإشارة وكل من الترددات ذات التباعد المتساوي. إذا كانت الإشارة تحتوي على طاقة عند التردد k، فإن الارتباط يكون كبيرًا. إذا لم يكن الأمر كذلك، فهو قريب من الصفر.
### ماذا يعني كل معامل
**X[0]: مكون DC.** هذا هو مجموع كافة العينات -- بما يتناسب مع المتوسط. وهو يمثل الإزاحة الثابتة (التردد الصفري) للإشارة.
```
...
### معكوس DFT
يعيد المعكوس DFT بناء الإشارة الأصلية من معاملات ترددها:
```
...
الاختلافات الوحيدة عن الأمام DFT: الإشارة الموجودة في الأس تكون موجبة (وليست سالبة)، ويوجد عامل تسوية 1/N.
معكوس DFT هو إعادة بناء مثالية. لا يتم فقدان أية معلومات. يمكنك الانتقال من المجال الزمني إلى المجال الترددي والعودة دون أي خطأ. إن DFT عبارة عن تغيير في الأساس - فهو يعيد التعبير عن نفس المعلومات في نظام إحداثي مختلف.
### FFT: جعل الأمر سريعًا
...
يحسب تحويل فورييه السريع (FFT) نفس النتيجة في O(N log N). بالنسبة لـ N = 1 مليون، فهذا يعني حوالي 20 مليون عملية بدلاً من تريليون. هذا هو ما makes تحليل التردد العملي.
تعمل خوارزمية Cooley-Tukey (الأكثر شيوعًا FFT) على أساس فرق تسد:
...
يتطلب FFT أن يكون طول الإشارة أسًا بقيمة 2. ومن الناحية العملية، تكون الإشارات مبطنة صفرًا للقوة التالية التي تبلغ 2.
### التحليل الطيفي
...
- التحويل المستند إلى FFT يأخذ O(N log N): تحويل كلاهما، الضرب، التحويل مرة أخرى.
- بالنسبة للحبوب الكبيرة، يكون الالتفاف FFT أسرع بشكل كبير.
- وهذا بالضبط ما يحدث في الطبقات التلافيفية ذات الحقول المستقبلة الكبيرة.
ملاحظة: يحسب DFT الالتفاف الدائري (تلتف الإشارة حولها). بالنسبة للالتواء الخطي (بدون التفاف)، قم بوضع كلتا الإشارتين على لوحة صفرية بطول N + M - 1 قبل الحوسبة.
```mermaid
graph LR
...
### النوافذ
يفترض DFT أن الإشارة دورية - فهو يعامل العينات N كفترة واحدة من إشارة متكررة بلا حدود. إذا لم تبدأ الإشارة وتنتهي عند نفس القيمة، فإن هذا يخلق انقطاعًا عند الحدود، والذي يظهر كمحتوى زائف عالي التردد. وهذا ما يسمى التسرب الطيفي.
...
قم بتطبيق النافذة عن طريق ضربها من حيث العناصر مع الإشارة قبل DFT: `X = DFT(x * w)`.
### خصائص DFT
...
4. IFFT النتيجة
تستخدم تطبيقات CNN القياسية الإلتواء المباشر (أسرع للنوى الصغيرة 3x3). ولكن بالنسبة للنوى الكبيرة أو الالتواء الشامل، تكون الأساليب المستندة إلى FFT أسرع بشكل ملحوظ. تستبدل بعض البنيات (مثل FNet) الانتباه بالكامل بـ FFT، وتحقق الدقة التنافسية باستخدام O(N log N) بدلاً من تعقيد O(N^2).
### المخططات الطيفية وتحويل فورييه قصير الأمد
يمنحك FFT الواحد محتوى التردد للإشارة بأكملها، لكنه لا يخبرك شيئًا عن وقت حدوث هذه الترددات. يمكن أن يكون للتغريدة (الإشارة التي يزداد ترددها بمرور الوقت) والوتر (جميع الترددات الموجودة في وقت واحد) نفس طيف الحجم.
...
تمثل المخططات الطيفية تمثيل الإدخال القياسي لنماذج ML الصوتية. تعمل نماذج التعرف على الكلام (Whisper، DeepSpeech) على مخططات طيفية mel - وهي مخططات طيفية ذات ترددات تم تعيينها على مقياس mel، والتي تتوافق بشكل أفضل مع إدراك الإنسان لطبقة الصوت.
### التعرج
...
ولهذا السبب تشتمل المحولات التناظرية إلى digital على مرشحات مضادة للتعرجات تزيل الترددات الأعلى من Nyquist قبل أخذ العينات. في ML، يظهر الاسم المستعار عند اختزال خرائط المعالم بدون تصفية مناسبة للتمرير المنخفض - تعالج بعض البنيات هذا الأمر من خلال طبقات تجميع مصقولة.
### الحشو الصفري لا يزيد من الدقة
هناك مفهوم خاطئ شائع: عدم إضافة أي إشارة قبل FFT يؤدي إلى تحسين دقة التردد. لا. تتداخل الحشوة الصفرية بين صناديق التردد الموجودة، مما يمنحك طيفًا أكثر سلاسة. لكنها لا تستطيع الكشف عن تفاصيل التردد التي لم تكن موجودة في العينات الأصلية.
...
### الخطوة 3: FFT (كولي-توكي)
يتطلب FFT العودي طول قوة 2. انقسمت إلى زوجية وفردية، متكررة، تتحد مع عوامل التلاعب.
```python
def fft(x):
    N = len(x)
    if N <= 1:
...
### الخطوة 4: مساعدات التحليل الطيفي
```python
...
## استخدمه
للعمل الحقيقي، استخدم FFT الخاص بـ numpy والمدعوم بمكتبات C المحسنة للغاية.
```python
import numpy as np
...
المصفوفة الطيفية لها شكل (n_frequeency، n_time_frames). كل عمود هو طيف الطاقة في نافذة زمنية واحدة. هذا هو ما تستهلكه نماذج ML الصوتية كمدخلات.
## اشحنها
...
2. التحقق من **FFT مقابل DFT.** أنشئ إشارة عشوائية بطول 64. احسب كلاً من DFT (O(N^2)) وFFT. تحقق من تطابق كافة المعاملات ضمن 1e-10. يعمل الوقت على إشارات بطول 256 و512 و1024 و2048. ارسم نسبة الوقت DFT إلى الوقت FFT.
3. **إثبات نظرية الالتواء بالمثال.** أنشئ إشارة x = [1, 2, 3, 4, 0, 0, 0, 0] ومرشح h = [1, 1, 1, 0, 0, 0, 0, 0]. حساب الالتفاف الدائري مباشرة (حلقة متداخلة). ثم قم بحسابها عبر FFT (التحويل، الضرب، التحويل العكسي). التحقق من تطابق النتائج. الآن قم بإجراء الإلتواء الخطي عن طريق الحشو الصفري بشكل مناسب.
4. **تأثيرات النوافذ.** قم بإنشاء إشارة عبارة عن مجموع موجتين جيبيتين عند 10 هرتز و12 هرتز (قريبة جدًا). عينة عند 128 هرتز لمدة ثانية واحدة. حساب طيف الطاقة بدون نافذة، ونافذة هان، ونافذة هامينج. ما هي النافذة make الأسهل للتمييز بين القمتين؟ لماذا؟
...
| بن التردد | يمثل كل فهرس k في خرج DFT التردد k*fs/N هرتز. "bin" هي فتحة التردد المنفصلة |
...
| الإلتواء الدائري | الالتواء حيث تلتف الإشارة. هذا هو ما يحسبه DFT بشكل طبيعي |
| الالتواء الخطي | الإلتواء القياسي بدون التفاف. تم تحقيق ذلك عن طريق عدم الحشو قبل DFT |
...
- [3Blue1Brown: But what is the Fourier Transform?](https://www.youtube.com/watch?v=spUNpyF58BY) - أفضل مقدمة مرئية لتحويلات فورييه
...
- [Smith: The Scientist and Engineer's Guide to Digital Signal Processing](http://www.dspguide.com/) - كتاب مدرسي مجاني عبر الإنترنت يغطي FFT والنوافذ والتحليل الطيفي بعمق
- [Vaswani et al.: Attention Is All You Need (2017)](https://arxiv.org/abs/1706.03762) - التشفيرات الموضعية الجيبية المشتقة من تحليل تردد فورييه
- [Radford et al.: Whisper (2022)](https://arxiv.org/abs/2212.04356) - التعرف على الكلام باستخدام مخططات ميل الطيفية كتمثيل للمدخلات