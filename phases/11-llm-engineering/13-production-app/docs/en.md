...
الفجوة ليست في الذكاء. إنها البنية التحتية. النموذج الأولي الخاص بك يستدعي OpenAI، ويحصل على رد، ويطبعه. يعمل على الكمبيوتر المحمول الخاص بك. ثم يأتي الواقع:
...
- يقوم API بإرجاع خطأ 500 في الساعة 2 صباحًا. تعطل الخدمة الخاصة بك.
- يطلب المستخدم من النموذج إنشاء SQL. مخرجات النموذج `DROP TABLE users`.
...
كل تطبيق LLM قيد الإنتاج اليوم - Perplexity، Cursor، ChatGPT، Notion AI - حل هذه المشكلات. ليس من خلال كونك أكثر ذكاءً فيما يتعلق بالمطالبات. من خلال الدقة فيما يتعلق بالهندسة.

هذا هو حجر الزاوية. ستبني خدمة إنتاج كاملة LLM تدمج الإدارة السريعة (L01-02)، والتضمينات والبحث المتجه (L04-07)، واستدعاء الوظائف (L09)، والتقييم (L10)، والتخزين المؤقت (L11)، وحواجز الحماية (L12)، والتدفق، ومعالجة الأخطاء، وإمكانية الملاحظة، وتتبع التكلفة. خدمة واحدة. كل مكون سلكي معًا.

## The Concept

### Production Architecture

كل تطبيق LLM جدي يتبع نفس التدفق. التفاصيل تختلف. الهيكل لا.
...
| مكون | الدرس | تكنولوجيا | الغرض |
|-----------|-------|------------|---------|
| API الخادم | -- | سريعAPI + يوفيكورن | HTTP نقاط النهاية، SSE التدفق، الفحوصات الصحية |
...
| استدعاء الوظيفة | L09 | تسجيل الأداة + JSON المخطط | الوصول إلى البيانات الخارجية، والإجراءات المنظمة |
| التقييم | L10 | مقاييس مخصصة + تسجيل | جودة الاستجابة، الكمون، تتبع الدقة |
| التخزين المؤقت | L11 | ذاكرة التخزين المؤقت الدلالية (على أساس التضمين) | تجنب مكالمات LLM الزائدة عن الحاجة، وقلل التكلفة وزمن الوصول |
| الدرابزين | L12 | قواعد Regex + المصنف | منع الحقن الفوري، PII، محتوى غير آمن |
...
تستغرق الاستجابة GPT-5 مع 500 رمز إخراج من 3 إلى 8 ثوانٍ ليتم إنشاؤها بالكامل. بدون البث، يحدق المستخدم في الدوار طوال المدة. مع البث، يصل الرمز الأول خلال 200-500 مللي ثانية. الوقت الإجمالي هو نفسه. ينخفض ​​زمن الاستجابة المتصور بنسبة 90%.
...
|----------|--------|------------|-------------|
| الأحداث المرسلة من الخادم (SSE) | منخفض | منخفض | معظم تطبيقات LLM. أحادي الاتجاه، قائم على HTTP، يعمل في كل مكان |
| WebSocketس | منخفض | متوسطة | الاحتياجات ثنائية الاتجاه: الصوت والتعاون في الوقت الحقيقي |
| الاقتراع الطويل | عالية | منخفض | العملاء القدامى الذين لا يستطيعون التعامل مع SSE أو WebSockets |

SSE هو الخيار الافتراضي. يتم بث كل من OpenAI وAnthropic وGoogle عبر SSE. يتلقى الخادم الخاص بك أجزاء من LLM API ويعيد توجيهها إلى العميل كأحداث SSE. يستخدم العميل `EventSource` (المتصفح) أو `httpx` (Python) لاستهلاك الدفق.

### Error Handling: The Three Layers

تفشل تطبيقات الإنتاج LLM بثلاث طرق مختلفة. ويتطلب كل منها استراتيجية انتعاش مختلفة.

**الطبقة 1: API حالات الفشل.** يقوم موفر LLM بإرجاع 429 (حد السعر)، أو 500 (خطأ في الخادم)، أو انتهاء المهلة. الحل: التراجع الأسي مع الارتعاش. ابدأ من ثانية واحدة، ثم ضاعف كل إعادة محاولة، وأضف ارتعاشًا عشوائيًا لمنع القطيع الهادر. الحد الأقصى 3 محاولات.
...
**الطبقة الثانية: فشل النموذج.** يُرجع النموذج JSON مشوهًا، أو يهلوس اسم وظيفة، أو ينتج مخرجات تفشل في التحقق من الصحة. الحل: أعد المحاولة باستخدام موجه مصحح. قم بتضمين الخطأ في رسالة إعادة المحاولة حتى يتمكن النموذج من التصحيح الذاتي.

**الطبقة 3: فشل التطبيق.** لا يمكن الوصول إلى الخدمة النهائية، ويكون مخزن المتجهات بطيئًا، ويطرح حاجز الحماية استثناءً. الحل: التدهور الجميل. إذا كان السياق RAG غير متاح، تابع بدونه. إذا كانت ذاكرة التخزين المؤقت معطلة، تجاوزها. لا تدع أبدًا نظام secondary يعطل التدفق الأساسي.

| فشل | هل تريد إعادة المحاولة؟ | احتياطي | تأثير المستخدم |
|---------|--------|--------|-------------|
| API 429 (حد السعر) | نعم مع التراجع | قائمة الانتظار الطلب | "جارٍ المعالجة، برجاء الانتظار..." |
...
| تخزين ناقلات أسفل | لا يمكن إعادة المحاولة في متجر المتجهات | تخطي RAG السياق | جودة أقل، لا تزال فعالة |
...
** التسجيل المنظم. ** ينتج كل طلب إدخال سجل JSON مع: الطلب ID، المستخدم ID، اسم قالب المطالبة، النموذج المستخدم، رموز الإدخال، الرموز المميزة للإخراج، زمن الاستجابة (مللي ثانية)، إصابة/فشل ذاكرة التخزين المؤقت، تمرير/فشل حاجز الحماية، التكلفة (USD)، وأي أخطاء.

**التتبع.** يتناول طلب مستخدم واحد 5-8 مكونات. تتيح لك آثار OpenTelemetry رؤية الرحلة بأكملها: ما المدة التي استغرقها التضمين؟ هل كانت ضربة مخبأة؟ كم استغرقت المكالمة LLM؟ هل أضاف الدرابزين الكمون؟ بدون التتبع، يعد تصحيح أخطاء الإنتاج بمثابة تخمين.

** لوحة معلومات المقاييس. ** الأرقام الخمسة التي يشاهدها كل فريق LLM:

| متري | الهدف | لماذا |
|--------|--------|-----|
| P50 الكمون | < 2 ثانية | تجربة المستخدم المتوسطة |
| P99 الكمون | < 10ث | الكمون الذيل يدفع إلى الاضطراب |
...
- **المعالجة القائمة على قائمة الانتظار.** بالنسبة للمهام غير في الوقت الفعلي (التلخيص والتحليل)، ادفع إلى قائمة الانتظار (Redis، SQS) وقم بالمعالجة مع العمال. قم بإرجاع وظيفة ID، دع العميل يستطلع رأيه.
- **تجميع الاتصالات.** إعادة استخدام HTTP اتصالات لمقدمي LLM. يضيف إنشاء اتصال TLS جديد لكل طلب 100-200 مللي ثانية.
- **القياس الأفقي.** LLM التطبيقات مرتبطة بالإدخال/الإخراج، وليست CPU ملزمة. يتعامل خادم واحد غير متزامن مع أكثر من 100 طلب متزامن. خوادم واسعة النطاق، وليس النوى.
...
| # | العنصر | الفئة |
|---|------|---------|
| 1 | API المفاتيح المخزنة في متغيرات البيئة وليس الكود | الأمن |
...
| 6 | تم تمكين الدفق لجميع نقاط نهاية الدردشة | UX |
...
| 14 | CORS مهيأة لمجالات الإنتاج فقط | الأمن |
...
يبني الكود خدمة إنتاج كاملة LLM مع:
- خادم سريعAPI مع فحوصات صحية وCORS
...
التحقق من صحة الإدخال يلتقط الحقن الفوري وPII قبل أن يراه LLM. يكتشف التحقق من صحة الإخراج المحتوى غير الآمن قبل أن يراه المستخدم. جدارين. لا شيء يمر دون رادع.
...
الواجهة الأساسية LLM. التراجع الأسي مع غضب على الفشل. التراجع من خلال سلسلة النموذج. دعم البث لتسليم الرمز المميز.
...
يعمل العرض التوضيحي أعلاه كبرنامج نصي. للإنتاج، لفه في FastAPI بنقاط نهاية مناسبة.
...
لتشغيل هذا كخادم حقيقي، قم بإلغاء التعليق وتثبيت التبعيات: `pip install fastapi uvicornicorn`. اضغط على `http://localhost:8000/docs` للمستندات API التي تم إنشاؤها تلقائيًا.

### Real API Integration

استبدل مكالمات LLM المحاكاة بأدوات تطوير البرامج (SDK) للموفر الفعلي.
...
أربعة عمال. يعالج كل منها الإدخال/الإخراج غير المتزامن. يخدم صندوق واحد به 4 عمال أكثر من 400 طلب متزامن LLM لأنهم جميعًا ينتظرون إدخال/إخراج الشبكة، وليس CPU.
...
وينتج أيضًا `outputs/skill-production-checklist.md` -- إطار قرار لشحن LLM التطبيقات إلى الإنتاج، ويغطي كل مكون من هذا الدرس بعتبات محددة ومعايير النجاح/الفشل.

## Exercises

1. ** أضف تكامل RAG. ** أنشئ متجرًا متجهًا بسيطًا في الذاكرة يحتوي على 20 مستندًا. عندما يكون القالب `rag_answer`، قم بتضمين الاستعلام، وابحث عن المستندات الثلاثة الأكثر تشابهًا، وأدخلها كسياق. قياس كيفية تغير جودة الاستجابة مع وبدون سياق RAG. تتبع زمن انتقال الاسترجاع بشكل منفصل عن زمن الوصول LLM.

2. **تنفيذ استدعاء الوظيفة الحقيقية.** أضف سجل أداة (من الدرس 09) إلى الخدمة. عندما يطرح المستخدم سؤالاً يتطلب بيانات خارجية (الطقس، الحساب، البحث)، يجب أن يكتشف الخط pip هذا، وينفذ الأداة، ويدرج النتيجة في الموجه. أضف حقل `tools_used` إلى الاستجابة.

3. ** أنشئ نظامًا لتنبيه التكلفة. ** تتبع التكلفة لكل مستخدم يوميًا. عندما يتجاوز المستخدم 0.50 دولارًا أمريكيًا في اليوم، قم بتحويله إلى `gpt-4o-mini`. عندما يتجاوز إجمالي التكلفة اليومية 100 دولار، قم بتنشيط وضع الطوارئ: استجابات ذاكرة التخزين المؤقت فقط للاستعلامات المتكررة، `gpt-4o-mini` لكل شيء آخر، ورفض الطلبات التي تزيد عن 2000 رمز إدخال. اختبار مع محاكاة ارتفاع حركة المرور.
...
5. ** أضف تتبع OpenTelemetry. ** قم بقياس كل مكون (البحث في ذاكرة التخزين المؤقت، فحص الدرابزين، مكالمة LLM، حساب التكلفة) كنطاق منفصل. كل فترة تسجل مدتها. تصدير الآثار إلى وحدة التحكم. قم بعرض التتبع الكامل لطلب واحد، مع رؤية مساهمة كل مكون في إجمالي زمن الاستجابة.

## Key Terms

| مصطلح | ماذا يقول الناس | ماذا يعني في الواقع |
|------|----------------|----------------------|
| API البوابة | "الواجهة الأمامية" | نقطة الإدخال التي تتعامل مع المصادقة، وتحديد المعدل، CORS، وتوجيه الطلب قبل تشغيل أي منطق LLM |
...
| الانحطاط الرشيق | "معالجة الفشل الجزئي" | عندما يفشل مكون secondary (ذاكرة التخزين المؤقت، RAG، حواجز الحماية)، يستمر النظام بوظائف منخفضة بدلاً من التعطل |
| التكلفة لكل طلب | "اقتصاد الوحدة" | إجمالي LLM الإنفاق (رموز الإدخال + رموز الإخراج بسعر النموذج) لطلب مستخدم واحد - الرقم الذي يحدد ما إذا كان نموذج عملك يعمل أم لا |
...
| فحص الصحة | "مسبار الجاهزية" | نقطة نهاية تُرجع حالة جميع التبعيات (ذاكرة التخزين المؤقت، التوفر LLM، حواجز الحماية) - يستخدمها موازن التحميل وKubernetes لتوجيه حركة المرور |

## Further Reading

- [FastFastAPI Documentation](https://fastapi.tiangolo.com/) -- the async Python framework used in this lesson, with native SSE streaming and automatic OpenOpenAPI docs
...
- [OpenTelemetry Python SDK](https://opentelemetry.io/docs/languages/python/) -- the standard for distributed tracing, used to instrument every component of an LLM pipelineeline
...
- [Eugene Yan, "Patterns for Building LLM-based Systems"](https://eugeneyan.com/writing/llm-patterns/) -- architectural patterns (guardrails, RAG, caching, routing) تمت رؤيته عبر عمليات النشر LLM في شركات التكنولوجيا الكبرى
- [وثائق vLLM](https://docs.vllm.ai/) -- العرض المستند إلى PagedAttention: طبقة الاستدلال الافتراضية ذاتية الاستضافة المستخدمة ضمن قمة FastAPI في هذا الدرس.
...
- [وثائق NVIDIA TensorRT-LLM](https://nvidia.githubhub.io/TensorRT-LLM/) - المسار ذو الإنتاجية الأعلى على أجهزة NVIDIA؛ والتكميم، والتجميع أثناء الطيران، وFP8 نواة لعمليات نشر المؤسسات.
- [هامل حسين - تحسين زمن الاستجابة: TGI vs vLLM vs CTranslate2 vs mlc](https://hamel.dev/notes/llm/inference/03_inference.html) - مقارنة مُقاسة للإنتاجية وزمن الوصول عبر أطر الخدمة الرئيسية.
