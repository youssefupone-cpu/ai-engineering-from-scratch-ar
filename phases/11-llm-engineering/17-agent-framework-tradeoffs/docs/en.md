...
| **توليد تلقائي** | `ConversableAgent` زوج — اثنان أو أكثر من الوكلاء الذين يتحدثون بالتناوب حتى حالة الخروج. | *حوار* متعدد الوكلاء (المعلم والطالب، ومقترح الناقد، والممثل والمراجع) حيث ينبثق التفكير من الدردشة. | سير العمل الحتمي مع DAG معروف؛ أي شيء يحتاج إلى حالة دائمة عبر عمليات إعادة التشغيل. |
| **اجنو** | `Agent` — LLM + أدوات + ذاكرة واحدة، يمكن تجميعها في فرق. | وكلاء فرديون وفرق خفيفة الوزن سريعة البناء؛ برامج تشغيل قوية متعددة الوسائط ومدمجة للتخزين. | رسوم بيانية عميقة ومتفرعة بشكل واضح مع مخفضات مخصصة. |
...
- **CrewAI.** تتدفق الحالة كسلاسل بين المهام عبر الحقل `context`، أو منظمة من خلال `output_pydantic`. لا يوجد مخزن متين لكل طاقم خارج الصندوق؛ أنت تنسحب بمفردك إذا كان يجب على الطاقم البقاء على قيد الحياة عند إعادة التشغيل.
- **AutoGen.** الحالة هي سجل الدردشة وأي `context` يحددها المستخدم. تستمر نصوص المحادثة؛ حالة سير العمل التعسفية لا إلا إذا قمت بكتابة المحولات.
...
- **AutoGen** — يقرر الوكلاء عبر الدردشة. التفرع ينشأ من الذي يتحدث بعد ذلك. `GroupChatManager` يحدد المتحدث التالي؛ يمكنك كتابة `speaker_selection_method` بخط اليد ولكن الإعداد الافتراضي هو LLM.
...
- **AutoGen** — تكامل OpenTelemetry عبر `autogen-core`; لدى AgentOps وOpik موصلات. يتم تتبع التفاصيل لكل وكيل في الرسالة، وليس لكل node.
- **Agno** — علامة `monitoring=True` مدمجة بالإضافة إلى مصدري OpenTelemetry؛ تكامل محكم مع Langfuse لتتبعات الجلسة.

### Cost and latency

تضيف جميع الأطر الأربعة حملًا إضافيًا لكل مكالمة (منطق الإطار والتحقق من الصحة والتسلسل). ترتيب تقريبي لزيادة النفقات العامة: Agno ≈ LangGraph < CrewAI ≈ AutoGen. ويهيمن على الفرق مقدار التوجيه LLM الإضافي الذي يقوم به إطار العمل. ينفق المدير الهرمي لـ CrewAI الرموز المميزة في تحديد من يذهب بعد ذلك؛ AutoGen's `GroupChatManager` بالمثل. لا تنفق LangGraph سوى الرموز المميزة التي تكتب فيها `llm.invoke`. مسار Agno للوكيل الفردي رفيع.

عندما تكون تكلفة التشغيل مهمة، فضل التوجيه الصريح (حواف LangGraph، AutoGen `speaker_selection_method`) على التوجيه المحدد LLM.

### Interoperability

- **LangGraph** ↔ **LangChain** أدوات، المستردات، LLMs. محول MCP من الدرجة الأولى (الأدوات المستوردة كخوادم MCP).
...
- **AutoGen** → `FunctionTool` يلتف حول أي لغة Python قابلة للاستدعاء؛ MCP محول متاح. اقتران محكم بالنظام البيئي AG2 لأنماط وكيل إلى وكيل.
...
4. **تحقق من ميزانية التكلفة.** LLM-يكلف التوجيه المحدد رموزًا إضافية لكل دور. إذا كان الوكيل يعمل آلاف المرات يوميًا، ففضل التوجيه الصريح.
5. **ميزانية إطار العمل العامة.** كل إطار عمل هو تبعية أخرى. إذا كانت المهمة عبارة عن مكالمتين LLM وأداة، فاكتب 30 سطرًا من لغة بايثون البسيطة؛ لا يوجد إطار أرخص من عدم وجود إطار.
...
|---------------|--------------------|-----|
| سير العمل DAG مع الحالة المكتوبة، الموافقات البشرية، طويلة الأمد | لانغغراف | حالة من الدرجة الأولى، نقطة التفتيش، المقاطعات، السفر عبر الزمن. |
...
| نموذج أولي سريع، بدون التزام بإطار العمل | بايثون عادي + مزود SDK | لا يوجد إطار هو أسرع إطار. |
...
2. **متوسط.** أنشئ نفس المهمة في AutoGen (دردشة الباحث ↔ الكاتب، ينضم المحرر عبر `GroupChat`) وAgno (وكيل واحد مع `search_tools` و`write_tools`، بالإضافة إلى مخزن الجلسة). قم بتصنيف التطبيقات الأربعة على (أ) التكلفة لكل تشغيل، (ب) القدرة على الاستئناف بعد التعطل، (ج) القدرة على ضخ موافقة بشرية قبل خطوة الكتابة.
3. **صعب.** أنشئ نصًا برمجيًا لشجرة القرار `pick_framework.py` يأخذ وصفًا قصيرًا للمشكلة (JSON: `{has_typed_state, has_roles, has_dialogue, has_parallel_fanout, needs_resume}`) ويعرض توصية مع تبرير من جملة واحدة. تحقق من ذلك في ست حالات تصممها بنفسك.

## Key Terms

| مصطلح | ماذا يقول الناس | ماذا يعني في الواقع |
|------|-|--------------------------------------|
| تنسيق | "كيفية تنسيق الوكلاء" | الطبقة التي تقرر أي node/الدور/الوكيل سيتم تشغيله بعد ذلك. |
...
- [Anthropic — Building effective agents (Dec 2024)](https://www.anthropic.com/research/building-effective-agents) — مكتبة الأنماط (التسلسل الفوري، التوجيه، الموازاة، عمال الأوركسترا، المقيم-المُحسِّن) إطار حيادي.
...
- Phase 11 · 16 (LangGraph) — إطار العمل الذي يستند إليه هذا الدرس.
