...
> الهندسة السريعة هي مجموعة فرعية. هندسة السياق هي اللعبة بأكملها. المطالبة هي سلسلة تكتبها. السياق هو كل ما يدخل في نافذة النموذج: تعليمات النظام، والمستندات المستردة، وتعريفات الأداة، وسجل المحادثة، وأمثلة قليلة، والموجه نفسه. أفضل المهندسين AI في عام 2026 هم مهندسو السياق. إنهم يقررون ما يدخل، وما يبقى خارجا، وبأي ترتيب.

**النوع:** بناء
** اللغات: ** بايثون
**المتطلبات:** المرحلة 10 (LLMs من الصفر)، المرحلة 11 الدرس 01-02
**الوقت:** ~90 دقيقة
...
كل رمز مميز تضعه في النافذة يزيح رمزًا مميزًا قد يحمل المزيد من المعلومات ذات الصلة. كل تعريف أداة غير ذي صلة، وكل محادثة قديمة، وكل جزء من النص المسترجع لا يجيب على السؤال - كل واحد هو النموذج الأسوأ قليلاً في المهمة.

## The Concept

### The Context Window is a Scarce Resource

فكر في نافذة السياق على أنها RAM وليست قرصًا. إنه سريع ويمكن الوصول إليه مباشرة، ولكنه محدود. لا يمكنك أن تناسب كل شيء. يجب عليك الاختيار.
...
**الذاكرة طويلة المدى**: الحقائق والتفضيلات التي تستمر عبر المحادثات. "المستخدم يفضل TypeScript." "يستخدم المشروع PostgreSQL." مخزنة في قاعدة بيانات، ويتم استرجاعها عند بدء الجلسة. يقوم كلود كود بتخزين هذا في ملفات CLAUDE.md. يقوم ChatGPT بتخزينه في ميزة الذاكرة الخاصة به.
...
يدير كلود كود السياق من خلال نهج متعدد الطبقات. يتضمن موجه النظام قواعد سلوكية وتعريفات للأدوات (حوالي 6 آلاف رمز). عند فتح ملف، يتم إدخال محتوياته كسياق. عند البحث، تتم إضافة النتائج. يتم تلخيص المنعطفات المحادثة القديمة. CLAUDE.md يوفر ذاكرة طويلة المدى تستمر عبر الجلسات.
...
يُنتج هذا الدرس `outputs/prompt-context-optimizer.md` - مطالبة قابلة لإعادة الاستخدام تقوم بمراجعة استراتيجية تجميع السياق والتوصية بالتحسينات. قم بتزويده بموجه النظام الخاص بك، وعدد الأدوات، ومتوسط ​​طول التاريخ، واستراتيجية الاسترداد، ويحدد نفايات الرمز المميز ويقترح التحسينات.

كما أنه ينتج `outputs/skill-context-engineering.md` - إطار عمل قرار لتصميم تجميع السياق pipخطوط بناءً على نوع المهمة وحجم نافذة السياق وميزانية زمن الوصول.
...
|------|----------------|----------------------|
| نافذة السياق | "كم يستطيع النموذج أن يقرأ" | The maximum number of tokens (input + output) the model processes in a single forward pass -- 400K for GPT-5, 200K (1M beta) for Claude Opus 4.7, 2M for Gemini 3 Pro |
...
| ضائع في المنتصف | "العارضات ينسون الأشياء الموجودة في المنتصف" | النتيجة التجريبية أن LLMs تحضر بشكل أفضل بداية ونهاية السياق، مع انخفاض دقة المعلومات الموضوعة في المنتصف بنسبة 10-20% |
...
- [Pope et al., "Efficiently Scaling Transformer Inference" (2022)](https://arxiv.org/abs/2211.05102) - لماذا يحرك طول السياق الذاكرة وزمن الوصول، وكيف يغير KV ذاكرة التخزين المؤقت وMQA وGQA حساب الميزانية.
...
- [Ainslie et al., "GQA: Training Generalized Multi-Query Transformer Models from Multi-Head Checkpoints" (EMNLP 2023)](https://arxiv.org/abs/2305.13245) -- ورقة الاهتمام بالاستعلام المجمع التي تقطع KV الذاكرة 8× في أجهزة فك تشفير الإنتاج دون فقدان الجودة.
