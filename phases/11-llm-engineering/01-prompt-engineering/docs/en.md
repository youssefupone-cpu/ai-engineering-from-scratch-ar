...
الهندسة السريعة لم تمت. الأشخاص الذين يقولون ذلك هم نفس الأشخاص الذين قالوا CSS مات في عام 2015. ما تغير هو أنه أصبح حصصًا على الطاولة. يحتاجها كل مهندس AI جاد. السؤال ليس ما إذا كان يجب أن نتعلم ذلك ولكن إلى أي مدى يجب أن نتعمق.
...
**رسالة النظام**: اليد الخفية. فهو يحدد هوية النموذج والقيود السلوكية وقواعد الإخراج. يعامل النموذج هذا على أنه سياق ذو أولوية قصوى. OpenAI، Anthropic، وGoogle جميعها تدعم رسائل النظام، لكنها تعالجها بشكل مختلف داخليًا. يمنح كلود رسائل النظام أقوى الالتزام. ينحرف GPT-5 أحيانًا عن تعليمات النظام في المحادثات الطويلة، ويتعامل Gemini 3 مع `system_instruction` كحقل منفصل لتكوين الإنشاء بدلاً من كونه رسالة.

**رسالة المستخدم**: المهمة. هذا ما يعتقده معظم الناس على أنه "الموجه". ولكن بدون رسالة نظام جيدة، تكون رسالة المستخدم غير مقيدة.

**التعبئة المسبقة للمساعد**: السلاح السري. يمكنك بدء استجابة المساعد بسلسلة جزئية. أرسل `{"role": "assistant", "content": "```json\n{"}` وسيستمر النموذج من هناك، لينتج JSON بدون مقدمة. الأنثروبي API يدعم هذا محليًا. OpenAI لا (استخدم المخرجات المنظمة بدلاً من ذلك).

### Role Prompting: Why "You are an expert X" Works

"أنت أحد كبار مطوري لغة بايثون" ليست تعويذة سحرية. إنها وظيفة التنشيط.

LLMs تم تدريبهم على مليارات المستندات. تحتوي هذه المستندات على كتابات من هواة وخبراء، من منشورات مدونة وأبحاث تمت مراجعتها من قبل النظراء، ومن إجابات Stack Overflow التي حصلت على 0 تصويتات وتلك التي حصلت على 5000. عندما تقول "أنت خبير"، فإنك تجعل توزيع عينات النموذج متحيزًا نحو نهاية الخبراء لبيانات التدريب الخاصة به.
...
4. حدد ما يجب تضمينه AND ما يجب استبعاده
5. أعط مثالاً ملموسًا على المخرجات المطلوبة

### Output Format Control

يمكنك توجيه تنسيق إخراج النموذج دون استخدام الإخراج المنظم APIs. يعد هذا مفيدًا لاستجابات النص الحر التي لا تزال بحاجة إلى بنية.

**JSON**: "أجب بكائن JSON يحتوي على مفاتيح: الاسم (سلسلة)، النتيجة (الرقم 0-100)، المنطق (سلسلة أقل من 50 كلمة)."

**XML**: مفيد عندما تحتاج إلى النموذج لإنتاج محتوى باستخدام علامات البيانات الوصفية. كلود قوي بشكل خاص في إخراج XML لأن Anthropic استخدم التنسيق XML في تدريبهم.
...
**القيود السلبية** ("افعل NOT..."): "افعل NOT تضمين أمثلة التعليمات البرمجية. استخدم NOT المصطلحات التقنية. افعل NOT تتجاوز 200 كلمة." تعتبر القيود السلبية فعالة بشكل مدهش لأنها تزيل مناطق كبيرة من مساحة الإخراج. لا يتعين على النموذج أن يخمن ما تريد - فهو يعرف ما لا تريده.
...
**Top-p** (أخذ عينات النواة) هو المقبض الآخر. فهو يحد من أخذ العينات لأصغر مجموعة من الرموز المميزة التي يتجاوز احتمالها التراكمي p. Top-p=0.9 يعني أن النموذج يأخذ في الاعتبار فقط الرموز المميزة في أعلى 90% من كتلة الاحتمالية. استخدم درجة الحرارة OR أعلى درجة، وليس كلاهما - فهما يتفاعلان بشكل غير متوقع.
...
| GPT-5 ميني | 400 ألف رمز | 128 ألف رمز | OpenAI |
| o4-mini (الاستدلال) | 200 ألف توكن | 100 ألف رمز | OpenAI |
...
| ديب سيك-V3.1 | 128 ألف رمز | 32 ألف رمز | ديب سيك (مفتوح) |
...
أفضل المطالبات هي الملحدة للنموذج. إنهم يعملون على GPT-5، Claude Opus 4.7، Gemini 3 Pro، ونماذج الوزن المفتوح (Llama 4، Qwen3، DeepSeek-V3) مع الحد الأدنى من الضبط. هنا كيف:
...
3. استخدم محددات XML للهيكل (جميع النماذج الرئيسية تتعامل مع XML جيدًا)
...
جهاز يرسل نفس المطالبة إلى عدة LLM APIs ويجمع النتائج للمقارنة. يستخدم تجريد المزود للتعامل مع API الاختلافات.
...
تتم معالجة رسالة نظام OpenAI أولاً ويتم منحها وزنًا كبيرًا من الاهتمام. درجة الحرارة = 0.0 makes حتمية الإخراج - نفس المدخلات تنتج نفس الإخراج في كل مرة. وهذا أمر ضروري للاختبار والاستنساخ.
...
تُجبر التعبئة المسبقة المساعدة (`"{"`) كلود على الاستمرار في إنتاج JSON بدون أي مقدمة. هذه هي الميزة الفريدة لـ Anthropic - لا يوجد مزود رئيسي آخر يدعمها محليًا. إنه أكثر موثوقية من طلبات JSON المستندة إلى السرعة وأرخص من وضع الإخراج المنظم للحالات البسيطة.
...
يقوم Gemini بمعالجة تعليمات النظام كجزء من تكوين النموذج، وليس كرسالة. تعني نافذة سياق الرمز المميز 2M أنه يمكنك تضمين مجموعات أمثلة قليلة ضخمة لا تتناسب مع GPT-4o أو Claude.
...
`outputs/prompt-prompt-optimizer.md` -- موجه تعريفي يأخذ أي مسودة ويعيد كتابتها باستخدام الأنماط العشرة من هذا الدرس. قم بتغذيتها بمطالبة غامضة، واسترجع واحدة مُصممة.

`outputs/skill-prompt-patterns.md` - إطار عمل لاتخاذ القرار لاختيار نمط المطالبة المناسب بناءً على نوع مهمتك والموثوقية المطلوبة والنموذج المستهدف.

رمز Python (`code/prompt_engineering.py`) هو أداة اختبار مستقلة. قم بتبديل مكالمات API الحقيقية عن طريق استبدال `simulate_llm_call` بطلبات HTTP الفعلية إلى OpenAI وAnthropic وGoogle APIs. تعمل مكتبة الأنماط والمنشئ والمسجل ومنطق المقارنة دون تعديل.
...
2. استبدل `simulate_llm_call` بمكالمات API حقيقية لمقدمي خدمة اثنين على الأقل (OpenAI والطبقات المجانية البشرية). قم بتشغيل نفس المطالبة عبر كليهما وقم بقياس: طول الاستجابة، والامتثال للتنسيق، وتغطية الكلمات الرئيسية، ووقت الاستجابة. قم بتوثيق النموذج الذي يتبع التعليمات بشكل أكثر دقة.
...
| درجة الحرارة | "مقبض الإبداع" | عامل تحجيم على التوزيع logit قبل softmax - القيم الأعلى تعمل على تسوية التوزيع (أكثر عشوائية)، والقيم المنخفضة تزيد من حدة التوزيع (أكثر حتمية) |
...
| الفوقية مطالبة | "المطالبات التي تكتب المطالبات" | استخدام LLM لإنشاء وانتقاد وتحسين المطالبات لمهام LLM أخرى |
...
- [Zamfirescu-Pereira et al., 2023 -- "Why Johnny Can't Prompt"](https://arxiv.org/abs/2304.13529) -- research on how non-experts struggle with prompt engineering and what makess prompts effective
...
- [DAIR.AI Prompt Engineering Guide](https://www.promptingguide.ai/) -- exhaustive catalogue of prompt techniques with examples (zero-shot, few-shot, CoT, ReAct, self-consistency)؛ يستخدم الممارسون المرجعيون سطح "الهندسة السريعة" الأوسع.
- [مكتبة موجهات إنسانية](https://docs.anthropic.com/en/prompt-library) - مطالبات منسقة ومعروفة حسب حالة الاستخدام؛ تُظهر الأنماط الهيكلية التي يتم شحنها في الإنتاج.
