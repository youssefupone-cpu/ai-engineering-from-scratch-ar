# التخزين المؤقت، وتحديد المعدل وتحسين التكلفة
> معظم الشركات الناشئة AI لا تموت بسبب نماذج سيئة. يموتون من اقتصاديات الوحدة السيئة. مكالمة واحدة GPT-4o تكلف كسورًا من السنت. يكلف إجراء عشرة آلاف مستخدم عشر مكالمات يوميًا 250 دولارًا من رموز الإدخال فقط - قبل أن تتقاضى دولارًا واحدًا. الشركات التي تبقى على قيد الحياة هي تلك التي تتعامل مع كل مكالمة API كمعاملة مالية، وليس مكالمة وظيفية.
**النوع:** بناء
** اللغات: ** بايثون
...
## أهداف التعلم
- تنفيذ التخزين المؤقت الدلالي الذي يخدم الاستعلامات المتكررة أو المشابهة من ذاكرة التخزين المؤقت بدلاً من إجراء استدعاء API جديد
...
## المشكلة
يمكنك إنشاء روبوت دردشة RAG. إنه يعمل بشكل جميل. المستخدمين يحبون ذلك.
ثم تصل الفاتورة.
GPT-5 يكلف 5 دولارات لكل مليون رمز إدخال و15 دولارًا لكل مليون مخرج. تبلغ تكلفة Claude Opus 4.7 15 دولارًا للإدخال / 75 دولارًا للإخراج. تبلغ تكلفة Gemini 3 Pro 1.25 دولارًا أمريكيًا للمدخلات / 5 دولارًا أمريكيًا للإخراج. GPT-5-mini هو 0.25 دولار/2 دولار. الأسعار أدناه توضيحية؛ تحقق دائمًا من صفحة التسعير الحالية للمزود.
...
الجزء الوحشي: 40-60% من تلك الاستعلامات شبه مكررة. يطرح المستخدمون نفس الأسئلة بكلمات مختلفة قليلاً. تتم محاسبتك على مطالبة النظام الخاصة بك - المتماثلة في كل طلب - في كل مرة. تتكرر مستندات السياق التي تم استردادها بواسطة RAG عبر المستخدمين الذين يسألون عن نفس الموضوع.
أنت تدفع الثمن الكامل للحسابات الزائدة عن الحاجة.
##المفهوم
### تحليل التكلفة لمكالمة LLM
...
### التخزين المؤقت للموفر: الخصومات المضمنة
...
| جوجل الجوزاء | محتوى مخبأ صريح API | تخفيض بنسبة 75% تقريبًا (بالإضافة إلى مساحة التخزين) | 4,096 (فلاش) / 32,768 (برو) | قابل للتكوين من قبل المستخدم TTL |
**النهج الأنثروبي** واضح. يمكنك وضع علامة على أقسام المطالبة الخاصة بك باستخدام `cache_control: {"type": "ephemeral"}`. يدفع الطلب الأول علاوة كتابة بنسبة 25%. الطلبات اللاحقة بنفس البادئة تحصل على خصم 90%. إن مطالبة النظام المكونة من 2000 رمز والتي تكلف 0.005 دولارًا أمريكيًا تكلف عادةً 0.000625 دولارًا أمريكيًا عند الوصول إلى ذاكرة التخزين المؤقت. أكثر من 100 ألف طلب، مما يوفر 437.50 دولارًا في اليوم.
...
### التخزين المؤقت الدلالي: الطبقة المخصصة الخاصة بك
...
تكاليف التضمين لا تذكر. تتكلف عملية تضمين النص OpenAI's text-embedding-3-small 0.02 USD لكل مليون رمز مميز. لا يكلف التحقق من ذاكرة التخزين المؤقت شيئًا تقريبًا مقارنة بمكالمة LLM الكاملة.
### التخزين المؤقت الدقيق: التجزئة والمطابقة
...
|------|------------------|------------------|-------------|
| مجاني | 50,000 | 10 | GPT-4o-mini فقط |
| برو | 500,000 | 60 | GPT-4o، كلود سونيت |
| مؤسسة | 5,000,000 | 300 | جميع الموديلات |
### توجيه النموذج: النموذج المناسب للوظيفة المناسبة
...
### تتبع التكلفة: معرفة أين تذهب الأموال
...
### التجميع: خصومات بالجملة
تقوم مجموعة OpenAI API بمعالجة الطلبات بشكل غير متزامن بخصم 50%. يمكنك إرسال مجموعة تصل إلى 50000 طلب، وستظهر النتائج خلال 24 ساعة.
استخدم الخلط من أجل:
- معالجة المستندات ليلاً
- تصنيف السائبة
- يجري التقييم
- إثراء البيانات pipelines
ليس من أجل: الاستعلامات التي يواجهها المستخدم في الوقت الفعلي (مسألة زمن الوصول).
### تنبيهات الميزانية وقواطع الدائرة
...
عادةً ما يؤدي تطبيق RAG الذي يطبق الطبقات من 1 إلى 5 إلى خفض التكاليف من 22500 دولار أمريكي/الشهر إلى 4000-6000 دولار أمريكي/الشهر. هذا هو الفرق بين حرق المدرج وبناء الأعمال التجارية.
### الادخار الحقيقي: قبل وبعد
...
| P95 زمن الاستجابة | 2800 مللي ثانية | 900 مللي ثانية (عدد زيارات ذاكرة التخزين المؤقت: 50 مللي ثانية) | 68% |
...
### الخطوة 2: ذاكرة التخزين المؤقت الدقيقة
...
## استخدمه
### التخزين المؤقت الأنثروبي الفوري
```python
# import anthropic
#
...
### OpenAI التخزين المؤقت التلقائي
```python
...
OpenAI يخزن ذاكرة التخزين المؤقت تلقائيًا. أي بادئة مطالبة مكونة من أكثر من 1024 رمزًا مميزًا تتطابق مع طلب حديث تحصل على خصم بنسبة 50%. ليست هناك حاجة إلى تغييرات في التعليمات البرمجية -- ما عليك سوى التحقق من `prompt_tokens_details.cached_tokens` في الاستجابة للتحقق من أنه يعمل.
### OpenAI الدفعة API
```python
# import json
# from openai import OpenAI
...
تمنح الدفعة API خصمًا ثابتًا بنسبة 50% على جميع الرموز المميزة. تصل النتائج خلال 24 ساعة. مثالي لأحمال العمل في غير الوقت الفعلي: التقييمات، ووضع علامات على البيانات، والتلخيص المجمع.
### إنتاج ذاكرة التخزين المؤقت الدلالية باستخدام Redis
```python
# import redis
# import numpy as np
# from openai import OpenAI
...
## اشحنها
يُنتج هذا الدرس `outputs/prompt-cost-optimizer.md` -- مطالبة قابلة لإعادة الاستخدام تعمل على تحليل تطبيق LLM الخاص بك وتوصي بتحسينات محددة في التكلفة مع التوفير المتوقع.
...
## تمارين
1. **تنفيذ LRU الإخلاء لذاكرة التخزين المؤقت الدلالية.** استبدل الإخلاء الأقدم أولاً بالأقل استخدامًا مؤخرًا. تتبع آخر وقت وصول لكل إدخال وقم بطرد الإدخال ذي أقدم وقت وصول عندما تكون ذاكرة التخزين المؤقت ممتلئة. قارن معدلات النجاح بين الاستراتيجيتين عبر 100 استفسار.
2. **إنشاء أداة لتوقع التكلفة.** بالنظر إلى سجل مكالمات API (سجلات CostTracker)، قم بتوقع التكلفة الشهرية استنادًا إلى متوسط ​​7 أيام. حساب لأنماط أيام الأسبوع/عطلة نهاية الأسبوع. قم بتشغيل تنبيه إذا تجاوزت التكلفة الشهرية المتوقعة الميزانية بأكثر من 20%.
...
| تتبع التكلفة | "القياس" | تسجيل كل مكالمة API مع النموذج والرموز المميزة ووقت الاستجابة والتكلفة والمستخدم ID حتى تعرف بالضبط أين تذهب الأموال والميزات باهظة الثمن |
...
| معدل ضرب ذاكرة التخزين المؤقت | "كفاءة ذاكرة التخزين المؤقت" | النسبة المئوية للطلبات التي يتم تقديمها من ذاكرة التخزين المؤقت بدلاً من استدعاء LLM -- 40-60% نموذجية لروبوتات الدردشة الخاصة بالإنتاج، مما يوفر التكلفة بشكل متناسب |
## مزيد من القراءة
- [Anthropic Prompt Caching Guide](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching) -- المستندات الرسمية لعلامات Anthropic الصريحة للتحكم في ذاكرة التخزين المؤقت والتسعير وسلوك عمر ذاكرة التخزين المؤقت
- [OpenAI Prompt Caching](https://platform.openai.com/docs/guides/prompt-caching) -- التخزين المؤقت التلقائي لـ OpenAI، وكيفية التحقق من نتائج ذاكرة التخزين المؤقت عبر حقول الاستخدام، والحد الأدنى لطول البادئات
- [OpenAI Batch API](https://platform.openai.com/docs/guides/batch) -- خصم 50% على المعالجة غير المتزامنة، وتنسيق JSONL، ونافذة الإكمال على مدار 24 ساعة، وحدود الطلب البالغة 50 ألفًا
- [GPTCache](https://github.com/zilliztech/GPTCache) -- مكتبة التخزين المؤقت الدلالي مفتوحة المصدر تدعم الواجهات الخلفية للتضمين المتعددة، ومخازن المتجهات، وسياسات الإخلاء
- [Martian Model Router](https://docs.withmartian.com) -- توجيه نموذج الإنتاج الذي يحدد تلقائيًا أرخص نموذج قادر على التعامل مع كل استعلام
- [Not Diamond](https://www.notdiamond.ai) -- جهاز توجيه نموذجي يستند إلى ML والذي يتعلم من أنماط حركة المرور الخاصة بك لتحسين مقايضات التكلفة/الجودة عبر مقدمي الخدمة
- [Helicone](https://www.helicone.ai) -- LLM نظام أساسي لقابلية المراقبة مع تتبع التكلفة والتخزين المؤقت وتحديد المعدل وتنبيهات الميزانية كطبقة وكيل
- [Dean & Barroso, "The Tail at Scale" (CACM 2013)](https://research.google/pubs/the-tail-at-scale/) -- زمن الاستجابة والإنتاجية وTTFT/TPOT النسب المئوية والطلبات المغطاة؛ نموذج التكلفة وراء "اختيار النموذج الأرخص الذي لا يزال يفي بـ P95."
- [Kwon et al., "Efficient Memory Management for Large Language Model Serving with PagedAttention" (SOSP 2023)](https://arxiv.org/abs/2309.06180) -- ورقة vLLM؛ لماذا تتفوق صفحة KV-cache + الدفع المستمر على الخوادم الساذجة بمعدل 24 × في الإنتاجية، وطبقة الأشعة تحت الحمراء ضمن "التخزين المؤقت والتكلفة".
- [Dao et al., "FlashAttention-2: Faster Attention with Better Parallelism and Work Partitioning" (ICLR 2024)](https://arxiv.org/abs/2307.08691) -- خفض التكلفة على مستوى النواة بشكل متعامد لتحفيز التخزين المؤقت؛ اقرأ بجانب فك التشفير التخميني وGQA للحصول على صورة منحنى التكلفة الكاملة.