...
> النص منفصل. الرياضيات مستمرة. في كل مرة تطلب فيها LLM للعثور على مستندات "متشابهة"، أو مقارنة المعاني، أو البحث خارج الكلمات الرئيسية، فإنك تعتمد على جسر بين هذين العالمين. هذا الجسر هو التضمين. إذا كنت لا تفهم التضمينات فأنت لا تفهم الحديث AI. أنت فقط تستخدمه.
...
**CLS رمز مميز**: تقوم نماذج المحولات (BERT، 2018) بإخراج تضمين رمزي خاص [CLS] يمثل المدخلات بأكملها. أفضل من المتوسط ​​ولكن تم تدريب الرمز المميز [CLS] للتنبؤ بالجمل التالية، وليس التشابه.
...
استقر السوق في عدد قليل من خيارات درجة الإنتاج (MTEB درجات اعتبارًا من أوائل عام 2026، MTEB الإصدار2):

| نموذج | مقدم | الأبعاد | MTEB | السياق | التكلفة / مليون رمز |
|-------|----------|----------|------|---------|-----------------|
| تضمين الجوزاء 2 | جوجل | 3072 (ماتريوشكا) | 67.7 (استرجاع) | 8192 | 0.15 دولار |
| تضمين-v4 | كوهير | 1024 (ماتريوشكا) | 65.2 | 128 ألف | 0.12 دولار |
| رحلة-4 | الرحلة AI | 1024/2048 (ماتريوشكا) | 66.8 | 32 ألف | 0.12 دولار |
| تضمين النص-3-كبير | OpenAI | 3072 (ماتريوشكا) | 64.6 | 8192 | 0.13 دولار |
| تضمين النص-3-صغير | OpenAI | 1536 (ماتريوشكا) | 62.3 | 8192 | 0.02 دولار |
| BGE - M3 | BAAI | 1024 (كثيفة+متفرقة+كولبيرت) | 63.0 متعدد اللغات | 8192 | الوزن المفتوح |
...
MTEB (معيار تضمين النص الضخم) يغطي الإصدار الثاني أكثر من 100 مهمة عبر الاسترجاع والتصنيف والتجميع وإعادة الترتيب والتلخيص. الأعلى هو الأفضل. بحلول عام 2026، ستتطابق النماذج ذات الوزن المفتوح (Qwen3-Embedding، BGE-M3) مع النماذج المستضافة المغلقة أو تتفوق عليها على معظم المحاور. Gemini Embedding 2 يؤدي إلى الاسترجاع النقي؛ Voyage/Cohere يؤدي إلى مجالات محددة (المالية، القانون، الكود). قم دائمًا بقياس استفساراتك الخاصة قبل الالتزام.
...
تحل قواعد بيانات المتجهات هذه المشكلة باستخدام خوارزميات أقرب جار تقريبي (ANN). الخوارزمية السائدة هي HNSW (عالم صغير هرمي قابل للملاحة):
...
HNSW يتداول خسارة صغيرة في الدقة (عادةً 95-99٪ استدعاء) لتحقيق مكاسب هائلة في السرعة. عند وجود 10 ملايين متجه، تستغرق القوة الغاشمة ثوانٍ. HNSW يستغرق ميلي ثانية.
...
المستندات طويلة جدًا بحيث لا يمكن تضمينها كمتجهات فردية. تغطي صفحة PDF المكونة من 50 صفحة عشرات المواضيع -- ويصبح تضمينها متوسطًا لكل شيء، ولا يشبه أي شيء محدد. يمكنك تقسيم المستندات إلى أجزاء وتضمين كل منها.
...
**التقطيع العودي**: حاول التقسيم عند الحد الأكبر أولاً (رؤوس الأقسام). إذا كان لا يزال كبيرًا جدًا، فجرّب حدود الفقرة. ثم حدود الجملة. ثم حدود الأحرف. هذا هو LangChain `RecursiveCharacterTextSplitter` وهو يعمل بشكل جيد مع النصوص المجمعة ذات التنسيق المختلط.
...
نمط الإنتاج: يقوم برنامج التشفير الثنائي باسترداد أفضل 100 مرشح، ويقوم برنامج التشفير المتقاطع بإعادة ترتيبهم إلى العشرة الأوائل. هذا هو خط الاسترداد ثم إعادة الترتيب pipeline.
...
تضمين النص OpenAI-3-الصغير وتضمين النص-3-الكبير يدعم اقتطاع ماتريوشكا عبر المعلمة `dimensions`. يؤدي طلب 256 بُعدًا بدلاً من 1536 إلى خفض مساحة التخزين بمقدار 6x مع فقدان الدقة بنسبة 3-5% تقريبًا على معايير MTEB.

### Binary Quantization

يستخدم التضمين ذو 1536 بعدًا والمخزن كـ float32 6,144 بايت. اضرب في 10 ملايين مستند: 61 GB فقط للمتجهات.

يقوم التكميم الثنائي بتحويل كل تعويم إلى بتة واحدة: تصبح القيم الموجبة 1، والقيم السالبة تصبح 0. وينخفض ​​التخزين من 6,144 بايت إلى 192 بايت - أي تخفيض بمقدار 32x. يتم حساب التشابه باستخدام مسافة هامينج (عد البتات المختلفة)، والتي يمكن أن تفعلها CPUs في تعليمات واحدة.
...
نقوم بتنفيذ تضمين كثيف بسيط باستخدام TF-IDF مع L2 التطبيع. هذا ليس تضمينًا عصبيًا، ولكنه يتبع نفس العقد: النص الوارد، والمتجهات ذات الحجم الثابت، والنصوص المماثلة تنتج متجهات مماثلة.
...
اقتطاع ماتريوشكا مع OpenAI -- نفس الطراز، أبعاد أقل، مساحة تخزين أقل:
...
يستخدم المتجه 256-d مساحة تخزين أقل بمقدار 6x. بالنسبة لـ 10 مليون مستند، يكون ذلك 10 GB مقابل 61 GB. تبلغ نسبة فقدان الدقة حوالي 3-5% وفقًا للمعايير القياسية.
...
بالنسبة للتضمينات المحلية التي لا تحتوي على تبعية API:
...
| HNSW | "بحث سريع عن المتجهات" | رسم بياني هرمي للعالم الصغير قابل للملاحة - هيكل متعدد الطبقات يتيح O(log n) البحث التقريبي عن أقرب جار |
...
| MTEB | "معيار التضمين" | مقياس تضمين النص الضخم - 56 مجموعة بيانات عبر 8 مهام؛ معيار لمقارنة نماذج التضمين |
...
- Reimers & Gurevych، "الجملة-BERT: تضمينات الجملة باستخدام الشبكات السيامية BERT" (2019) - كيفية تدريب المشفرات الثنائية على التشابه على مستوى الجملة، وأساس نماذج التضمين الحديثة
- كوسوباتي وآخرون، "تعلم تمثيل ماتريوشكا" (2022) - التقنية وراء التضمينات ذات الأبعاد المتغيرة التي اعتمدها OpenAI لتضمين النص-3
- مالكوف وياشونين، "أقرب جار تقريبي فعال وقوي باستخدام الرسوم البيانية الهرمية للعالم الصغير القابلة للملاحة" (2018) - الورقة HNSW، الخوارزمية وراء معظم عمليات البحث عن متجهات الإنتاج
...
- MTEB لوحة المتصدرين (huggingface.co/spaces/mteb/leaderboard) - معيار حي يقارن جميع نماذج التضمين عبر المهام واللغات
- [Muennighoff et al., "MTEB: Massive Text Embedding Benchmark" (EACL 2023)](https://arxiv.org/abs/2210.07316) -- the benchmark defining 8 task categories (classification, clustering, pair classification, reranking, retrieval, STS, summarization, bitext mining) تقارير المتصدرين؛ اقرأ قبل أن تثق بأي نتيجة MTEB واحدة.
- [وثائق محولات الجملة](https://www.sbert.net/) -- المرجع الأساسي للتشفير الثنائي مقابل التشفير المتقاطع، واستراتيجيات التجميع، ومخزن استيعاب وتقسيم وتضمين RAG pipeline الذي ينفذه هذا الدرس.
