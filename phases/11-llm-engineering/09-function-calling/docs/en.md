# استدعاء الوظيفة واستخدام الأداة
> لا يستطيع حاملو شهادة LLM فعل أي شيء. أنها تولد النص. هذه هي القدرة بأكملها. لا يمكنهم التحقق من الطقس أو الاستعلام عن قاعدة بيانات أو إرسال بريد إلكتروني أو تشغيل التعليمات البرمجية أو قراءة ملف. كل "وكيل AI" رأيته من قبل هو LLM يُنشئ JSON يوضح الوظيفة التي يجب الاتصال بها - ثم يقوم الكود الخاص بك باستدعاءها بالفعل. النموذج هو الدماغ. الأدوات هي الأيدي. استدعاء الوظيفة هو الجهاز العصبي الذي يربطهم.
**النوع:** بناء
** اللغات: ** بايثون
...
تتطلب الإجابة الصحيحة الاتصال بـ OpenWeatherMap API والحصول على درجة الحرارة الحالية وإرجاع الرقم الحقيقي. لا يمكن للنموذج استدعاء واجهات برمجة التطبيقات. يمكن للتعليمات البرمجية الخاصة بك. القطعة المفقودة: بروتوكول منظم يسمح للنموذج بأن يقول "أحتاج إلى استدعاء الطقس API باستخدام هذه الوسيطات" ويسمح للتعليمات البرمجية الخاصة بك بتنفيذها وتغذية النتيجة مرة أخرى.
هذا هو استدعاء الوظيفة. يقوم النموذج بمخرجات منظمة JSON تصف الوظيفة التي سيتم استدعاؤها باستخدام الوسائط. التطبيق الخاص بك ينفذ الوظيفة. وتعود النتيجة إلى المحادثة. يستخدم النموذج النتيجة لإنتاج إجابته النهائية.
بدون استدعاء الوظائف، تعتبر LLMs موسوعات. ومعها يصبحون وكلاء.
##المفهوم
### حلقة استدعاء الوظيفة
...
الخطوة 1: يرسل المستخدم رسالة. الخطوة 2: يتلقى النموذج الرسالة مع تعريفات الأداة (JSON المخطط الذي يصف الوظائف المتاحة). الخطوة 3: بدلاً من الاستجابة بالنص، يقوم النموذج بإخراج استدعاء أداة - كائن JSON منظم مع اسم الوظيفة والوسيطات. الخطوة 4: ينفذ الكود الخاص بك الوظيفة ويلتقط النتيجة. الخطوة 5: تعود النتيجة إلى النموذج، الذي أصبح لديه الآن بيانات حقيقية لإنتاج إجابته النهائية.
النموذج لا ينفذ أي شيء أبدًا. إنه يقرر فقط ما يجب الاتصال به وبأي حجج. الكود الخاص بك هو المنفذ.
### تعريفات الأداة: عقد المخطط JSON
يتم تعريف كل أداة بواسطة مخطط JSON الذي يخبر النموذج بما تفعله الوظيفة، وما هي الوسائط التي تتطلبها، وما هي الأنواع التي يجب أن تكون عليها تلك الوسائط.
```json
{
  "type": "function",
  "function": {
...
الحقول `description` هامة. يقرأها النموذج ليقرر متى وكيف يتم استخدام الأداة. الوصف الغامض مثل "الحصول على الطقس" ينتج عنه اختيار أداة أسوأ من "الحصول على الطقس الحالي لمدينة ما. إرجاع درجة الحرارة بالدرجة المئوية والظروف." الوصف هو موجه لاختيار الأداة.
### مقارنة الموفر
يدعم كل مزود رئيسي استدعاء الوظائف، ولكن سطح API يختلف.
| مقدم | API المعلمة | تنسيق استدعاء الأداة | المكالمات الموازية | الاتصال القسري |
|----------|-------------|-----------------|---------------|----------------|
| OpenAI (GPT-5، o4) | `tools` | __الكود_1__ | نعم (متعددة في كل دورة) | __الكود_2__ |
| أنثروبي (كلود ٤.٦/٤.٧) | __الكود_3__ | __الكود_4__ | نعم (كتل متعددة) | __الكود_5__ |
| جوجل (الجوزاء 3) | __الكود_6__ | __الكود_7__ | نعم | __الكود_8__ |
| الوزن المفتوح (Llama 4, Qwen3, DeepSeek-V3) | `tools` الأصلي في Llama 4؛ هيرميس أو ChatML على الآخرين | مختلط | تعتمد على النموذج | يعتمد على المطالبة أو `tool_choice` إذا كان مدعومًا |
بحلول عام 2026، تقارب الموفرون الثلاثة المغلقون على تنسيقات مستندة إلى المخطط JSON شبه متطابقة. يتم شحن Llama 4 مع حقل `tools` الأصلي الذي يطابق شكل OpenAI. لا تزال الضبط الدقيق للوزن المفتوح يختلف - تنسيق Hermes (NousResearch) هو الأكثر شيوعًا للضبط الدقيق لجهات خارجية. بالنسبة للأدوات المشتركة بين الأجهزة المضيفة، تفضل MCP (المرحلة 11 · 14) على استدعاء الوظائف المضمنة - الخادم هو نفسه بالنسبة لهم جميعًا.
### اختيار الأداة: تلقائي، مطلوب، محدد
...
### المخرجات المنظمة مقابل استدعاء الوظائف
...
**استدعاء الوظيفة**: يعلن النموذج عن نية تنفيذ إجراء ما. الإخراج هو خطوة وسيطة. مثال: `get_weather(city="Tokyo")` -- يطلب النموذج إجراءً، ولا ينتج عنه الإجابة النهائية.
...
### الأمن: القواعد غير القابلة للتفاوض
...
**القاعدة 1: لا تقم أبدًا بتمرير SQL الذي تم إنشاؤه بواسطة النموذج مباشرة إلى قاعدة البيانات.** يمكن للنموذج أن يقوم بإنشاء DROP TABLE، أو UNION، أو استعلامات تقوم بإرجاع كل صف. المعلمات دائما. التحقق من صحة دائما. استخدم دائمًا القائمة المسموح بها للعمليات.
...
**القاعدة 4: تنقية نتائج الأداة.** إذا قامت إحدى الأدوات بإرجاع بيانات حساسة (مفاتيح API، PII، أخطاء داخلية)، فقم بتصفيتها قبل إعادتها إلى النموذج. سيتضمن النموذج نتائج الأداة في إجابته حرفيًا.
...
### معالجة الأخطاء
الأدوات تفشل انتهت مهلة واجهات برمجة التطبيقات. قواعد البيانات تنخفض. الملفات غير موجودة. يحتاج النموذج إلى معرفة متى تفشل الأداة ولماذا.
...
MCP هو معيار Anthropic المفتوح لقابلية التشغيل التفاعلي للأداة. بدلاً من تعريف كل تطبيق لأدواته الخاصة، يوفر MCP بروتوكولًا عالميًا: يتم تقديم الأدوات بواسطة خوادم MCP، ويستهلكها عملاء MCP (مثل Claude Code، أو Cursor، أو تطبيقك).
يمكن لخادم MCP واحد أن يعرض الأدوات لأي عميل متوافق. يمنح خادم Postgres MCP أي وصول إلى قاعدة بيانات الوكيل المتوافقة مع MCP. يمنح خادم GitHub MCP أي وصول إلى مستودع الوكيل. يتم تعريف الأدوات مرة واحدة، وتستخدم في كل مكان.
MCP هو استدعاء ما يعنيه HTTP للتواصل. فهو يعمل على توحيد طبقة النقل بحيث تصبح الأدوات محمولة.
## بنائها
### الخطوة 1: تحديد سجل الأداة
...
### الخطوة 4: بناء حلقة استدعاء الوظيفة
...
### الخطوة 5: التحقق من صحة الوسيطة
...
يقوم OpenAI بإرجاع استدعاءات الأداة كـ `response.choices[0].message.tool_calls`. تحتوي كل مكالمة على `id` الذي يجب عليك تضمينه عند إرجاع النتيجة. يستخدم النموذج هذا ID لمطابقة النتائج بالمكالمات. يمكن لـ GPT-4o إرجاع استدعاءات أدوات متعددة في استجابة واحدة - قم بتكرارها وتنفيذها جميعًا.
...
تستدعي أداة الإرجاع البشرية ككتل محتوى تحتوي على `type: "tool_use"`. تظهر نتيجة الأداة في رسالة مستخدم تحتوي على `type: "tool_result"`. لاحظ الفرق الرئيسي: يستخدم Anthropic `input_schema` لتعريفات معلمات الأداة، بينما يستخدم OpenAI `parameters`.
### MCP التكامل
```python
...
MCP يفصل بين تنفيذ الأداة واستهلاك الأداة. يعرف خادم Postgres SQL. يعرف الخادم GitHub API. يقوم وكيلك فقط باكتشاف الأدوات واستدعاءها - فهو لا يحتاج إلى رمز خاص بالموفر لكل عملية تكامل.
## اشحنها
...
## تمارين
1. **إضافة الأداة السادسة: استعلام قاعدة البيانات.** تنفيذ أداة محاكاة SQL مع جدول في الذاكرة. تقبل الأداة اسم الجدول وشروط التصفية (ليست SQL الأولية). تحقق من أن اسم الجدول مدرج في القائمة المسموح بها وأن عوامل التصفية مقيدة بـ `=`، `>`، `<`، `>=`، `<=`. إرجاع الصفوف المطابقة كـ JSON.
...
|------|----------------|----------------------|
| استدعاء الدالة | "استخدام الأداة" | يُخرج النموذج JSON منظمًا يصف دالة لاستدعائها باستخدام وسائط محددة - ينفذها الكود الخاص بك، وليس النموذج |
...
| __المصطلح_3__ | "بروتوكول الأداة" | بروتوكول السياق النموذجي - معيار Anthropic المفتوح لعرض الأدوات عبر الخوادم التي يمكن لأي عميل متوافق اكتشافها والاتصال بها |
...
| الحد من المعدل | "استدعاء الميزانية" | تعيين الحد الأقصى لعدد استدعاءات الأداة لكل محادثة لمنع الحلقات اللانهائية والتكاليف الجامحة API |
## مزيد من القراءة
- [OpenAI Function Calling Guide](https://platform.openai.com/docs/guides/function-calling) -- المرجع النهائي لاستخدام الأداة مع GPT-4o، بما في ذلك الاستدعاءات المتوازية، والاتصال القسري، والوسائط المنظمة
- [Anthropic Tool Use Guide](https://docs.anthropic.com/en/docs/tool-use) -- أداة كلود تستخدم التنفيذ مع مخطط الإدخال والاستجابات متعددة الأدوات وتكوين اختيار الأداة
- [Model Context Protocol Specification](https://modelcontextprotocol.io) -- المعيار المفتوح لقابلية التشغيل التفاعلي للأداة عبر تطبيقات AI، مع بنية الخادم/العميل
- [Schick et al., 2023 -- "Toolformer: Language Models Can Teach Themselves to Use Tools"](https://arxiv.org/abs/2302.04761) -- الورقة التأسيسية حول تدريب حاملي الماجستير في القانون على تحديد متى وكيف يتم استدعاء الأدوات الخارجية
- [Patil et al., 2023 -- "Gorilla: Large Language Model Connected with Massive APIs"](https://arxiv.org/abs/2305.15334) -- ضبط LLMs للحصول على مكالمات API دقيقة عبر 1,645 واجهة برمجة تطبيقات مع تقليل الهلوسة
- [Berkeley Function Calling Leaderboard](https://gorilla.cs.berkeley.edu/leaderboard.html) -- قياس الأداء في الوقت الحقيقي مقارنة دقة استدعاء الوظائف عبر GPT-4o وClaude وGemini والنماذج المفتوحة
...
- [Anthropic — Building effective agents (Dec 2024)](https://www.anthropic.com/research/building-effective-agents) -- خمسة أنماط قابلة للتركيب (التسلسل الفوري، والتوجيه، والتوازي، والعاملون المنسقون، والمُقيِّم-المُحسِّن) تم إنشاؤها من أداة بدائية تستخدم أداة واحدة.