...
**الوقت:** ~90 دقيقة
## أهداف التعلم
- تنفيذ SVM خطي من الصفر باستخدام فقدان المفصلة ونسب التدرج في الصيغة الأولية
- شرح مبدأ الحد الأقصى للهامش وتحديد متجهات الدعم من نموذج مدرب
- قارن بين النوى الخطية ومتعددة الحدود وRBF واشرح كيف تتجنب خدعة النواة التعيين الصريح عالي الأبعاد
...
### الهامش الناعم: التعامل مع الضوضاء باستخدام المعلمة C
...
### خسارة المفصلة: دالة الخسارة SVM
...
ينتج عن فقدان المفصلة حلولاً متفرقة (فقط ناقلات الدعم لها مساهمة غير صفرية). تستخدم الخسارة اللوجستية جميع نقاط البيانات. إن أجهزة SVM makes هذه أكثر كفاءة في الذاكرة في وقت التنبؤ.
### تدريب SVM خطي مع نزول متدرج
يمكنك تدريب SVM خطي باستخدام النسب المتدرج على فقدان المفصلة بالإضافة إلى L2 التنظيم، دون حل المقيد QP:
```
...
يتضمن الثنائي فقط المنتجات النقطية x_i . x_j بين نقاط البيانات. هذه هي البصيرة الرئيسية. استبدل كل منتج نقطي بوظيفة kernel K(x_i, x_j) ويمكن لـ SVM معرفة الحدود غير الخطية دون حساب التحويل بشكل صريح.
```
...
تقوم نواة RBF بتعيين البيانات في مساحة لا نهائية الأبعاد. النقاط القريبة في مساحة الإدخال لها قيمة kernel قريبة من 1. والنقاط المتباعدة لها قيمة kernel قريبة من 0. ويمكنها تعلم أي حدود قرار سلسة.
```mermaid
graph LR
...
### لماذا خسرت أجهزة SVM أمام التعلم العميق (ومتى ما زالت تفوز)
...
## بنائها
### الخطوة 1: فقدان المفصلة والتدرج
...
### الخطوة 2: الخطي SVM عبر النسب المتدرج
تدريب عن طريق تقليل فقدان المفصلة المنتظمة. لا حاجة إلى حل QP.
```python
class LinearSVM:
...
### الخطوة 4: تعريف الهامش ودعم المتجهات
...
3. قم بإنشاء مجموعة بيانات حيث تكون حدود الفئة دائرية (وليست خطية). أظهر فشل SVM الخطي. قم بحساب مصفوفة النواة RBF وأظهر أن الفئات تصبح قابلة للفصل في مساحة الميزات التي يسببها النواة.
4. قارن خسارة المفصلة بالخسارة اللوجستية في نفس مجموعة البيانات. تدريب SVM الخطي والانحدار اللوجستي. قم بحساب عدد نقاط التدريب التي تساهم في تحديد حدود القرار لكل نموذج (ناقلات الدعم مقابل جميع النقاط).
...
| هامش ناعم | SVM صيغة تسمح بانتهاكات الهامش عبر متغيرات الركود. يعالج البيانات غير القابلة للفصل |
...
| صياغة مزدوجة | إعادة صياغة مشكلة SVM التي تعتمد فقط على المنتجات النقطية بين نقاط البيانات. تمكين النواة |
| __المصطلح_3__ | دعم الانحدار المتجه. يناسب أنبوب إبسيلون حول البيانات. النقاط الموجودة داخل الأنبوب ليس لها خسارة |
...
## مزيد من القراءة
- [Vapnik: The Nature of Statistical Learning Theory (1995)](https://link.springer.com/book/10.1007/978-1-4757-3264-1) - النص التأسيسي حول SVMs والتعلم الإحصائي
- [Cortes & Vapnik: Support-vector networks (1995)](https://link.springer.com/article/10.1007/BF00994018) - الورقة SVM الأصلية
- [Platt: Sequential Minimal Optimization (1998)](https://www.microsoft.com/en-us/research/publication/sequential-minimal-optimization-a-fast-algorithm-for-training-support-vector-machines/) - خوارزمية SMO التي جعلت تدريب SVM عمليًا
- [scikit-learn SVM documentation](https://scikit-learn.org/stable/modules/svm.html) - دليل عملي مع تفاصيل التنفيذ
- [LIBSVM: A Library for Support Vector Machines](https://www.csie.ntu.edu.tw/~cjlin/libsvm/) - مكتبة C++ وراء معظم تطبيقات SVM