...
- تنفيذ SMOTE من الصفر وشرح كيف يختلف الإفراط في أخذ العينات الاصطناعية عن الازدواجية العشوائية
...
- قم ببناء خط بيانات غير متوازن بالكامل pipeline يجمع بين SMOTE وأوزان الفئة وتحسين العتبة
...
تفشل الدقة لأنها تعامل جميع التنبؤات الصحيحة بالتساوي. يعد وضع علامة على معاملة legitimate بشكل صحيح والكشف عن الاحتيال بشكل صحيح بمثابة نقطة واحدة من الدقة. لكن اكتشاف الاحتيال هو السبب الكامل لوجود النموذج. نحن بحاجة إلى مقاييس وتقنيات واستراتيجيات تدريب تجبر النموذج على الاهتمام بالفئة النادرة ولكنها مهمة.
...
|--|---|---|
| في الواقع إيجابي | ٠ (TP) | ١٠ (FN) |
| في الواقع سلبي | ٠ (FP) | 990 (TN) |
...
**F1 النتيجة** = 2 * الدقة * الاستذكار / (الدقة + الاستدعاء). الوسط التوافقي. يعاقب الاختلال الشديد بين الدقة والتذكر أكثر من المتوسط ​​الحسابي.

**نقاط F-beta** = (1 + بيتا^2) * الدقة * الاستدعاء / (بيتا^2 * الدقة + الاستدعاء). عندما يكون بيتا > 1، يكون التذكر أكثر أهمية. عندما تكون قيمة بيتا <1، تكون الدقة أكثر أهمية. F2 شائع في كشف الاحتيال (فقدان الاحتيال أسوأ من الإنذار الكاذب).

**AUPRC** (المنطقة تحت منحنى الاستدعاء الدقيق). مثل AUC-ROC ولكنها أكثر إفادة للبيانات غير المتوازنة. المصنف العشوائي لديه AUPRC يساوي معدل الفصل الإيجابي (وليس 0.5 مثل ROC). هذه التحسينات make أسهل في الرؤية.
...
**SMOTE**: إنشاء عينات الأقلية الاصطناعية عن طريق الاستيفاء.
...
| SMOTE | تمت إضافة الأقلية الاصطناعية | ضجيج الحدود | اختلال معتدل، عينات أقلية كافية لـ k-NN |
...
أوزان الفئة تعادل رياضيًا الإفراط في التوقع، ولكن دون إنشاء نقاط بيانات جديدة. وهذا make يجعلها أسرع ويتجنب خطر التجهيز الزائد للعينات المكررة.
...
4. احسب F1 (أو المقياس الذي اخترته) عند كل عتبة
...
مع scikit-learn والتعلم غير المتوازن، تكون هذه التقنيات أحادية الخط:
...
ينتج هذا الدرس:
- `outputs/skill-imbalanced-data.md` -- قائمة مرجعية للقرارات للتعامل مع مشكلات التصنيف غير المتوازنة
...
3. **معايرة العتبة**: تنفيذ مقياس بلات (تناسب الانحدار اللوجستي على المخرجات الأولية للنموذج لإنتاج احتمالات تمت معايرتها). قارن منحنى استدعاء الدقة قبل المعايرة وبعدها. أظهر أن المعايرة لا تغير الترتيب (AUC يبقى كما هو) ولكن make الاحتمالات أكثر أهمية.

4. **مجموعة ذات تعبئة متوازنة**: قم بتدريب نماذج متعددة، كل منها على عينة تمهيدية متوازنة (جميع الأقليات + مجموعة فرعية عشوائية من الأغلبية). متوسط ​​توقعاتهم. قارن هذا النهج بنموذج واحد بـ SMOTE. قياس كل من الأداء والتباين عبر عمليات التشغيل.

5. **تجربة نسبة عدم التوازن**: خذ مجموعة بيانات متوازنة وقم بزيادة نسبة عدم التوازن تدريجيًا (50/50، 70/30، 90/10، 95/5، 99/1). لكل نسبة، تدرب مع وبدون SMOTE. ارسم F1 مقابل نسبة عدم التوازن لكلا النهجين. بأي نسبة يبدأ SMOTE في إحداث فرق ذي معنى؟
...
| AUPRC | "المساحة تحت المنحنى PR" | يلخص منحنى الاسترجاع الدقيق في رقم واحد؛ أكثر إفادة من AUC-ROC عندما تكون الفصول الدراسية غير متوازنة بشكل كبير |
...
- [Learning from Imbalanced Data (He & Garcia, 2009)](https://ieeexplore.ieee.org/document/5128907) -- دراسة استقصائية شاملة تغطي أساليب أخذ العينات والحساسة للتكلفة والخوارزميات
- [imbalanced-learn documentation](https://imbalanced-learn.org/stable/) -- Python library with SMOTE variants, undersampling strategies, and pipelineeline integration
- [The Precision-Recall Plot Is More Informative than the ROC Plot (Saito & Rehmsmeier, 2015)](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0118432) -- متى ولماذا تفضل منحنيات PR على منحنيات ROC للمشاكل غير المتوازنة
