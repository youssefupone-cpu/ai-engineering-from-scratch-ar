...
البحث على الشبكة هو النهج الواضح والأسوأ على نطاق واسع. البحث العشوائي يعمل بشكل أفضل مع حساب أقل. يعمل تحسين بايزي بشكل أفضل من خلال التعلم من التقييمات السابقة. إن معرفة الإستراتيجية التي يجب استخدامها، والمعلمات الفائقة المهمة بالفعل، يوفر أيامًا من GPU من الوقت الضائع.
##المفهوم
### المعلمات مقابل المعلمات الفائقة
...
- **التحسين المتوقع (EI):** ما مقدار التحسن الذي نتوقعه مقارنة بالأفضل الحالي في هذه المرحلة؟
- **حد الثقة العلوي (UCB):** التنبؤ بالإضافة إلى مضاعفات عدم اليقين. أعلى UCB يعني إما واعدة أو غير مستكشفة.
- **احتمال التحسين (PI):** ما هو احتمال أن تتفوق هذه النقطة على أفضل نقطة حالية؟
...
|-----------|--------|-------------|
| خطوة الاضمحلال | اضرب بـ 0.1 كل N حقبة | تدريب CNN الكلاسيكي |
...
### نصائح عملية
**ابدأ بمعدل التعلم.** إنه دائمًا المعلمة الفائقة الأكثر أهمية للطرق القائمة على التدرج. معدل التعلم السيئ make هو كل شيء آخر غير ذي صلة. قم بإصلاح المعلمات الفائقة الأخرى في الإعدادات الافتراضية واكتسح معدل التعلم أولاً.
...
## بنائها
### الخطوة 1: البحث عن الشبكة من الصفر
...
### الخطوة 2: البحث العشوائي من الصفر
```python
...
يقوم `MedianPruner` بإيقاف التجربة إذا كانت قيمتها المتوسطة أسوأ من متوسط ​​جميع التجارب المكتملة في نفس الخطوة. يتطلب التنقيح الاتصال بـ `trial.report()` للإبلاغ عن المقاييس المتوسطة و`trial.should_prune()` للتحقق مما إذا كان يجب إيقاف التجربة أم لا. يضمن `n_startup_trials=10` اكتمال 10 تجارب على الأقل بشكل كامل قبل بدء التنقيح. وهذا عادةً ما يوفر 40-60% من إجمالي الحوسبة.
### موالفات sklearn المدمجة
لإجراء تجارب سريعة، يوفر sklearn `GridSearchCV` و`RandomizedSearchCV` و`HalvingRandomSearchCV`:
```python
...
استخدم `loguniform` من scipy لمعدل التعلم والتنظيم. استخدم `randint` للمعلمات الفائقة للأعداد الصحيحة. تتوازي العلامة `n_jobs=-1` عبر جميع مراكز CPU.
### الأخطاء الشائعة في ضبط المعلمة الفائقة
...
5. تنفيذ وظيفة اكتساب بسيطة (التحسين المتوقع) وإظهار الاستكشاف مقابل الاستغلال. ارسم المتوسط ​​وعدم اليقين للنموذج البديل، وأظهر المكان الذي يختار EI تقييمه بعد ذلك.
## المصطلحات الرئيسية
...
| وظيفة الاستحواذ | "أين ننظر بعد ذلك" | يسجل نقاط المرشح من خلال موازنة التحسن المتوقع مع عدم اليقين. يعد EI وUCB من الخيارات الشائعة. |
...
## مزيد من القراءة
- [Bergstra & Bengio: Random Search for Hyper-Parameter Optimization (2012)](https://jmlr.org/papers/v13/bergstra12a.html) -- الورقة التي أظهرت شبكة نبضات عشوائية
...
- [Li et al., Hyperband: A Novel Bandit-Based Approach (2018)](https://jmlr.org/papers/v18/16-558.html)--ورقة Hyperband
