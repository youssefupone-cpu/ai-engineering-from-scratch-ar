...
الانحدار الخطي يمنحك هذا الخط. والأهم من ذلك، أنه يقدم حلقة التدريب ML بأكملها: تحديد نموذج، وتحديد دالة التكلفة، وتحسين المعلمات. تتبع كل خوارزمية ML نفس النمط. أتقنها هنا بأبسط حالة، وسوف تتعرف عليها في كل مكان.
...
- `b` (تحيز/تقاطع): قيمة y عندما تكون x = 0
...
الهدف: العثور على قيم w وb التي make y المتوقعة أقرب ما يمكن إلى y الفعلي في جميع أمثلة التدريب.
### دالة التكلفة (متوسط ​​الخطأ التربيعي)
...
لماذا التربيعية؟ سببين. أولاً، يعاقب الأخطاء الكبيرة أكثر من الأخطاء الصغيرة (الخطأ 10 أسوأ 100 مرة من الخطأ 1، وليس 10 مرات). ثانيًا، الدالة التربيعية سلسة وقابلة للتمييز في كل مكان، الأمر الذي يسهل تحسينه.
دالة التكلفة تخلق سطحًا. بالنسبة لوزن واحد w والتحيز b، يبدو السطح MSE كوعاء (قطع مكافئ محدب). الجزء السفلي من الوعاء هو المكان الذي يتم فيه تصغير MSE. التدريب يعني العثور على هذا القاع.
### نزول متدرج
...
### نتيجة R-Squared
MSE يخبرك بمدى خطأك، ولكن الرقم يعتمد على مقياس y. يعطي R-squared (R^2) مقياسًا مستقلاً عن المقياس:
```
...
### الخطوة 2: الانحدار الخطي من الصفر مع نزول متدرج
```python
...
### الخطوة 3: المعادلة العادية (حل مغلق)
```python
...
### الخطوة 4: الانحدار الخطي المتعدد
```python
...
### الخطوة 5: الانحدار متعدد الحدود
```python
...
يؤدي تنفيذك من البداية وscikit-learn إلى نفس النتائج. الفرق: scikit-learn يتعامل مع حالات الحافة والاستقرار العددي وتحسينات الأداء. استخدم المكتبة للإنتاج. استخدم الإصدار من الصفر لفهم ما يحدث.
## اشحنها
ينتج هذا الدرس:
- `outputs/skill-regression.md` - مهارة اختيار أسلوب الانحدار الصحيح بناءً على المشكلة
## تمارين
1. تنفيذ النسب التدرجي الدفعي، والنسب التدرج العشوائي (SGD)، والنسب التدرجي الصغير. قارن سرعة التقارب في نفس مجموعة البيانات. أيهما يتقارب بشكل أسرع؟ ما هو منحنى التكلفة الأكثر سلاسة؟
...
3. تنفيذ انحدار لاسو (L1 التنظيم: عقوبة = alpha * sum(|w_i|)). التدريب على بيانات الإسكان متعددة الميزات. قارن بين الأوزان التي تصل إلى الصفر مقابل ريدج. لماذا ينتج L1 حلولاً متفرقة بينما لا يفعل L2 ذلك؟
## المصطلحات الرئيسية
...
| ريدج الانحدار | "L2 التسوية" | الانحدار الخطي مع عقوبة لامدا * sum(w_i^2) تمت إضافته إلى MSE |
...
## مزيد من القراءة
- [An Introduction to Statistical Learning (ISLR)](https://www.statlearning.com/) -- PDF مجاني، الفصلان 3 و6 يغطيان الانحدار الخطي والتنظيم مع أمثلة R العملية
- [The Elements of Statistical Learning (ESL)](https://hastie.su.domains/ElemStatLearn/) -- PDF مجاني، الرفيق الرياضي لـ ISLR مع معالجة أعمق للحافة واللاسو
- [Stanford CS229 Lecture Notes on Linear Regression](https://cs229.stanford.edu/main_notes.pdf) -- ملاحظات أندرو إنج حول اشتقاق المعادلة العادية ونسب التدرج من المبادئ الأولى
- [scikit-learn LinearRegression documentation](https://scikit-learn.org/stable/modules/linear_model.html) -- مرجع عملي لـ LinearRegression وRidge وLasso وElasticNet مع أمثلة التعليمات البرمجية