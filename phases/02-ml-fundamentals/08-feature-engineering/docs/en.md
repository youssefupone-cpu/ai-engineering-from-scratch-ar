...
**النوع:** بناء
** اللغات: ** بايثون
**المتطلبات الأساسية:** المرحلة الأولى (إحصائيات ML، الجبر الخطي)، المرحلة الثانية الدروس 1-7
**الوقت:** ~90 دقيقة
## أهداف التعلم
...
- أنشئ ناقل TF-IDF من البداية واشرح سبب تفوقه في عدد الكلمات الأولية لتصنيف النص
...
هندسة الميزات هي عملية تحويل البيانات الأولية إلى تمثيلات يسهل على النماذج العثور على أنماط make. اختيار الميزة هو عملية التخلص من الميزات التي تضيف ضوضاء دون إضافة إشارة. إنهما معًا يمثلان النشاط الأعلى تأثيرًا في ML الكلاسيكي.
##المفهوم
### خط الأنابيب المميز
```mermaid
flowchart LR
...
**قياس:** ضع الميزات في نفس النطاق بحيث تتعامل الخوارزميات المستندة إلى المسافة (K-Means، KNN، SVM) مع جميع الميزات بالتساوي. يتم تعيين مقياس الحد الأدنى والحد الأقصى إلى [0، 1]. يتم تعيين التوحيد القياسي (z-score) على أنه يعني = 0، وstd = 1.
...
**TF-IDF:** تكرار المصطلح - تردد المستند العكسي. يزن الكلمات حسب مدى تميزها عبر المستندات. الكلمات الشائعة مثل "the" تصبح ذات وزن منخفض. الكلمات النادرة والمميزة لها وزن كبير.
```
...
- L1 التنظيم (Lasso): يدفع أوزان الميزات غير ذات الصلة إلى الصفر تمامًا
...
## بنائها
### الخطوة 1: التحويلات العددية من الصفر
```python
import math
...
### الخطوة الثانية: التشفير القاطع من البداية
```python
...
### الخطوة 4: حساب القيمة المفقودة من البداية
```python
...
## استخدمه
باستخدام scikit-learn، تكون هذه التحويلات قابلة للتركيب pipelines:
```python
...
تُظهر الإصدارات الأولية ما يحدث بالضبط داخل كل تحويل. تضيف إصدارات المكتبة معالجة حالة الحافة، ودعم المصفوفة المتفرقة، وتكوين الخط pip، ولكن الرياضيات هي نفسها.
## اشحنها
ينتج هذا الدرس:
- `outputs/prompt-feature-engineer.md` - مطالبة بالميزات الهندسية المنهجية من البيانات الأولية
## تمارين
...
3. أنشئ تحديدًا آليًا للميزات pipeline يجمع بين عتبة التباين وتصفية الارتباط وتصنيف المعلومات المتبادلة. قم بتطبيقه على مجموعة بيانات الإسكان وقارن أداء النموذج (استخدم الانحدار الخطي البسيط) مع جميع الميزات مقابل الميزات المحددة.
## المصطلحات الرئيسية
...
## مزيد من القراءة
- [Feature Engineering and Selection (Max Kuhn & Kjell Johnson)](http://www.feat.engineering/) - كتاب مجاني عبر الإنترنت يغطي المشهد الكامل للهندسة المميزة
- [scikit-learn Preprocessing Guide](https://scikit-learn.org/stable/modules/preprocessing.html) - مرجع عملي لجميع التحويلات القياسية
- [Target Encoding Done Right (Micci-Barreca, 2001)](https://dl.acm.org/doi/10.1145/507533.507538) - الورقة الأصلية حول ترميز الهدف مع التجانس