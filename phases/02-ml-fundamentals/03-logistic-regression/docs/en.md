...
- حساب وتفسير الدقة والتذكر والنتيجة F1 ومصفوفة الارتباك للتصنيف الثنائي
...
الانحدار اللوجستي يحل هذا. فهو يأخذ نفس التركيبة الخطية (wx + b) ويمررها عبر الدالة السيني، التي تسحق أي رقم في النطاق (0، 1). الناتج هو احتمال. قمت بتعيين عتبة (عادة 0.5) و make قرار.
...
لا يمكنك استخدام MSE للانحدار اللوجستي. MSE مع السيني يخلق سطح تكلفة غير محدب مع العديد من الحدود الدنيا المحلية. بدلاً من ذلك، استخدم الإنتروبيا الثنائية (فقد السجل):
...
**F1 النتيجة**: الوسط التوافقي للدقة والاستذكار. يوازن كلا المقياسين.
```
F1 = 2 * (Precision * Recall) / (Precision + Recall)
```

متى يجب تحديد الأولويات:
- **الدقة**: عندما تكون النتائج الإيجابية الكاذبة مكلفة (فلتر البريد العشوائي، لا ترغب في حظر البريد الإلكتروني)
...
1. قم بإنشاء مجموعة بيانات NOT قابلة للفصل خطيًا (على سبيل المثال، دائرتين متحدة المركز). تدريب الانحدار اللوجستي وملاحظة فشله. ثم أضف ميزات متعددة الحدود (x1^2، x2^2، x1*x2) وتدرب مرة أخرى. أظهر أن الدقة تتحسن.
2. تنفيذ مصفوفة ارتباك متعددة الفئات لنموذج softmax ثلاثي الفئات. حساب الدقة والتذكر لكل فئة. ما هي الفئة الأصعب في التصنيف؟
3. قم ببناء منحنى ROC من الصفر. بالنسبة لقيم العتبة 100 من 0 إلى 1، قم بحساب المعدل الإيجابي الحقيقي والمعدل الإيجابي الخاطئ. احسب AUC (المساحة تحت المنحنى) باستخدام قاعدة شبه المنحرف.
...
| الدقة | "كم عدد المختارين ذوي الصلة" | TP / (TP + FP), جزء التوقعات الإيجابية التي هي في الواقع إيجابية |
...
| مصفوفة الارتباك | "تفصيل الخطأ" | جدول يوضح أعداد TP، TN، FP، FN لكل زوج من الفصول |
