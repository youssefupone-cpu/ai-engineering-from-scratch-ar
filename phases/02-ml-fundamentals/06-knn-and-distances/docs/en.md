...
- تنفيذ التصنيف KNN والانحدار من الصفر باستخدام K القابل للتكوين والتصويت على أساس المسافة
- قارن بين مقاييس المسافة L1 وL2 وجيب التمام ومينكوفسكي وحدد المقياس المناسب لنوع بيانات معين
- اشرح لعنة الأبعاد ووضح سبب تدهور KNN في المساحات عالية الأبعاد
- أنشئ شجرة KD للبحث الفعال عن الجيران الأقرب وتحليلها عندما تتفوق على القوة الغاشمة
...
يبدو الأمر بسيطًا جدًا للعمل. لكن KNN تنافسية بشكل مدهش في العديد من المشكلات، خاصة مع مجموعات البيانات الصغيرة والمتوسطة، ويكشف فهمها بعمق عن المفاهيم الأساسية: اختيار مقياس المسافة (الاتصال بالمرحلة 1 الدرس 14)، ولعنة الأبعاد، والفرق بين التعلم البطيء والتعلم المتلهف.

KNN يظهر أيضًا في كل مكان في AI الحديث، تحت أسماء مختلفة فقط. تقوم قواعد بيانات المتجهات بإجراء بحث KNN عبر التضمينات. يقوم الجيل المعزز بالاسترجاع (RAG) بالبحث عن أقرب أجزاء المستند إلى K. تعثر أنظمة التوصية على مستخدمين أو عناصر مماثلة. الخوارزمية هي نفسها. المقياس وهياكل البيانات مختلفة.
...
** مينكوفسكي ** يعمم L1 و L2 مع المعلمة ص.
...
**المسافة المرجحة KNN** تزن كل جار عكسيًا بالمسافة:
...
يعتبر KNN أقل حساسية لاختيار K لأن الجيران البعيدين يساهمون بشكل ضئيل للغاية بغض النظر.
...
النتيجة العملية: KNN يعمل بشكل جيد حتى حوالي 20-50 ميزة. أبعد من ذلك، تحتاج إلى تقليل الأبعاد (PCA، UMAP، t-SNE) قبل تطبيق KNN، أو تحتاج إلى استخدام هياكل البحث القائمة على الشجرة التي تستغل الأبعاد الأدنى الجوهرية للبيانات.
...
تقوم الأشجار الكروية بتقسيم البيانات إلى كرات مفرطة متداخلة بدلاً من المربعات المحاذية للمحور. تحدد كل node كرة (المركز + نصف القطر) تحتوي على جميع النقاط في تلك الشجرة الفرعية.

مزايا أكثر من KD-الأشجار:
...
تعتبر كل من الأشجار KD والأشجار الكروية خوارزميات دقيقة. للبحث على نطاق واسع حقًا (ملايين النقاط، مئات الأبعاد)، يتم استخدام أساليب الجوار التقريبية (HNSW، IVF، تكميم المنتج) بدلاً من ذلك. يتم تناول هذه الأمور في المرحلة الأولى الدرس 14.

### Lazy learning vs eager learning

KNN متعلم كسول: لا يقوم بأي عمل في وقت التدريب وكله يعمل في وقت التنبؤ. معظم الخوارزميات الأخرى (الانحدار الخطي، SVMs، الشبكات العصبية) متعلمون متحمسون: يقومون بعمليات حسابية ثقيلة في وقت التدريب لبناء نموذج مدمج، ثم تكون التنبؤات سريعة.
...
بدلاً من تصويت الأغلبية، KNN للانحدار متوسط ​​القيم المستهدفة لجيران K.
...
KNN الانحدار ينتج تنبؤات ثابتة القطع (أو سلسة مع الترجيح). لا يمكن استقراء خارج نطاق بيانات التدريب. إذا كانت أهداف التدريب تتراوح بين 0 و100، فلن يتنبأ KNN بـ 200 أبدًا.
...
قم ببناء KNN بالكامل باستخدام K القابل للتكوين ومقياس المسافة ووزن المسافة الاختياري.
...
أنشئ شجرة KD من الصفر تنقسم بشكل متكرر على متوسط ​​كل بُعد.
...
KNN يتطلب قياس المعالم لأن المسافات حساسة لأحجام المعالم. ستهيمن الميزة التي تتراوح من 0 إلى 1000 على الميزة التي تتراوح من 0 إلى 1.
...
يستخدم Scikit-learn تلقائيًا الأشجار أو الأشجار الكروية KD عندما تكون مجموعة البيانات كبيرة بدرجة كافية والأبعاد منخفضة بدرجة كافية. بالنسبة للبيانات عالية الأبعاد، فإنها تعود إلى القوة الغاشمة. يمكنك التحكم في ذلك باستخدام المعلمة `algorithm`.

للبحث على نطاق واسع عن أقرب جيران (ملايين المتجهات)، استخدم FAISS أو إزعاج أو قاعدة بيانات متجهة:
...
1. تنفيذ التصنيف KNN على مجموعة بيانات ثنائية الأبعاد مكونة من 3 فئات. ارسم حدود القرار لـ K=1 وK=5 وK=15 وK=N. مراقبة الانتقال من overfitting إلى underfitting.
...
3. قارن L1، L2، ومسافة جيب التمام لـ KNN في مشكلة تصنيف النص (استخدم المتجهات TF-IDF). ما المقياس الذي يعطي أفضل دقة؟ لماذا يميل جيب التمام للفوز بالنص؟

4. قم بتنفيذ شجرة KD وقياس وقت الاستعلام مقابل القوة الغاشمة لمجموعات البيانات المكونة من 1k و10k و100k نقطة في 2D و10D و50D. في أي بعد تتوقف الشجرة KD عن كونها أسرع من القوة الغاشمة؟

5. أنشئ تراجعًا مرجحًا KNN لـ y = sin(x) + الضوضاء. قارنها مع KNN غير مرجح لـ K=3، 10، 30. أظهر أن الترجيح ينتج تنبؤات أكثر سلاسة، خاصة بالنسبة لـ K الكبيرة.
...
| التعلم الكسلى | لا يوجد حساب في وقت التدريب. كل العمل يحدث في وقت التنبؤ. KNN هو المثال القانوني |
| التعلم الحريص | حسابات ثقيلة في وقت التدريب لبناء نموذج مدمج. معظم خوارزميات ML حريصة |
...
| موزون KNN | الجيران مرجحة عكسيا بالمسافة. الجيران الأقرب لهم تأثير أكبر على التنبؤ |
...
- [Friedman, Bentley, Finkel: An Algorithm for Finding Best Matches in Logarithmic Expected Time (1977)](https://dl.acm.org/doi/10.1145/355744.355745) - الأصل KD- ورق شجرة
- [باير وآخرون: متى يكون "أقرب جار" ذا معنى؟ (1999)](https://link.springer.com/chapter/10.1007/3-540-49257-7_15) - تحليل شكلي لعنة الأبعاد لأقرب جار
- [scikit-learn وثائق أقرب الجيران](https://scikit.org/stable/modules/neighbors.html) - دليل عملي مع اختيار الخوارزمية
- [FAISS: مكتبة للبحث الفعال عن التشابه](https://githubhub.com/facebookresearch/faiss) - مكتبة ميتا للبحث التقريبي عن أقرب جيران بمليار مقياس
