...
**الوقت:** ~90 دقيقة
## أهداف التعلم
- تنفيذ نماذج K-Means وDBSCAN وGaussian Mixture Models من البداية ومقارنة سلوكها التجميعي
- تقييم جودة المجموعة باستخدام درجة الصورة الظلية وطريقة الكوع لتحديد K الأمثل
- اشرح متى يتفوق DBSCAN على وسائل K وحدد الخوارزمية التي تتعامل مع المجموعات غير الكروية والقيم المتطرفة
- إنشاء خط كشف الشذوذ pipeline باستخدام أساليب التجميع لوضع علامة على النقاط التي تنحرف عن الأنماط العادية
## المشكلة
...
##المفهوم
### التجميع: تجميع الأشياء المتشابهة معًا
...
تفترض K-Means أن المجموعات كروية وتتطلب منك اختيار K مقدمًا. DBSCAN make ليس افتراضًا. يجد مجموعات كمناطق كثيفة مفصولة بمناطق متناثرة.
معلمتين:
- **eps**: نصف قطر الحي
...
يفترض GMM أن البيانات يتم إنشاؤها من خليط من توزيعات K Gaussian، ولكل منها متوسطها وتباينها الخاص. تتناوب خوارزمية تعظيم التوقعات (EM) بين:
...
يمكن لـ GMM أن يصمم مجموعات بيضاوية (ليست كروية فقط مثل K-Means) ويتعامل بشكل طبيعي مع المجموعات المتداخلة.
### متى تستخدم أي
...
| __المصطلح_1__ | مجموعات متداخلة، هناك حاجة إلى مهام بسيطة | مجموعات بيانات كبيرة جدًا، وأبعاد كثيرة جدًا |
### اكتشاف الشذوذ من خلال التجميع
...
- **GMM**: النقاط ذات الاحتمالية المنخفضة لجميع الغاوسيين هي نقاط شاذة
## بنائها
### الخطوة 1: K- يعني من الصفر
```python
import math
import random
...
### الخطوة الثانية: طريقة الكوع ودرجة الصورة الظلية
```python
...
### الخطوة 4: نموذج الخليط الغوسي (خوارزمية EM)
```python
...
### الخطوة 5: إنشاء بيانات الاختبار وتشغيل كل شيء
```python
...
## استخدمه
مع scikit-learn، نفس الخوارزميات هي عبارة عن سطر واحد:
```python
...
تُظهر لك الإصدارات من البداية ما تحسبه هذه المكتبات بالضبط. K-يعني التكرار بين التعيين وإعادة الحساب. DBSCAN ينمو مجموعات من البذور الكثيفة. GMM يتناوب بين التوقع والتعظيم. تضيف إصدارات المكتبة استقرارًا رقميًا، وتهيئة أكثر ذكاءً (K-Means++)، وتسريع GPU، لكن المنطق الأساسي هو نفسه.
## اشحنها
ينتج هذا الدرس تطبيقات عملية لـ K-Means وDBSCAN وGMM من البداية. يمكن إعادة استخدام رمز التجميع كأساس لطرق أكثر تقدمًا غير خاضعة للرقابة.
## تمارين
...
3. أنشئ اكتشافًا بسيطًا للشذوذ pipeline: قم بتشغيل DBSCAN وGMM على نفس البيانات، ونقاط الإشارة التي تتفق عليها كلتا الطريقتين هي قيم متطرفة (الضوضاء في DBSCAN، احتمال منخفض في GMM). قم بقياس التداخل وناقش عندما تختلف الطرق.
## المصطلحات الرئيسية
...
| النقطة الأساسية | "نقطة في منطقة كثيفة" | نقطة بها على الأقل min_samples المجاورة ضمن مسافة eps، في DBSCAN |
...
| شذوذ | "الغريبة" | نقطة بيانات لا تتوافق مع النموذج المتوقع، تم تعريفها على أنها ضوضاء بواسطة DBSCAN أو احتمالية منخفضة بواسطة GMM |
## مزيد من القراءة
- [Stanford CS229 - Unsupervised Learning](https://cs229.stanford.edu/notes2022fall/main_notes.pdf) - ملاحظات محاضرة Andrew Ng حول التجميع وEM
