...
- تقييم ما إذا كانت مشكلة عمل معينة مناسبة لـ ML أو حلها بشكل أفضل باستخدام القواعد الحتمية
...
"النموذج" الذي يخرج من التدريب IS القواعد مشفرة بالأرقام (الأوزان، المعلمات). إنه يعمم من الأمثلة التي شاهدها إلى make تنبؤات بشأن البيانات التي لم يراها من قبل.
...
معظم ما ستبنيه عمليًا يستخدم التعلم الخاضع للإشراف. يعد التعلم غير الخاضع للرقابة أمرًا شائعًا في المعالجة المسبقة والاستكشاف. تعزيز التعلم يعزز لعبة AI والروبوتات وRLHF لنماذج اللغة.

### Beyond the Big Three

الفئات الثلاث المذكورة أعلاه نظيفة، ولكن العالم الحقيقي ML غالبًا ما يطمس الخطوط.
...
- **نشر الملصقات:** أنشئ رسمًا بيانيًا يربط بين نقاط البيانات المتشابهة. تنتشر التسميات من nodes إلى الجيران غير المسمى من خلال الرسم البياني.
...
- **نمذجة اللغة المقنعة (BERT):** إخفاء 15% من الكلمات في الجملة، تدريب النموذج على التنبؤ بالكلمات المفقودة. "التسميات" تأتي من النص الأصلي.
...
- **التنبؤ بالرمز التالي (GPT):** توقع الكلمة التالية بالنظر إلى جميع الكلمات السابقة. يصبح كل مستند نصي مثالاً للتدريب.
...
| Loss function | عبر الانتروبيا والدقة | متوسط ​​الخطأ التربيعي، MAE |
...
يتبع كل مشروع للتعلم الآلي نفس الخط pip، بغض النظر عن الخوارزمية.
...
**النشر**: ضع النموذج في مرحلة الإنتاج حيث لا يتنبأ بالبيانات الجديدة.
...
ML قوية ولكنها ليست الأداة الصحيحة دائمًا. قبل البحث عن نموذج، اسأل ما إذا كنت بحاجة إليه بالفعل.
...
- **ليس لديك أي بيانات أو القليل جدًا من البيانات.** ML يحتاج إلى أمثلة للتعلم منها. مع 10 نقاط بيانات، لا يمكنك تدريب أي شيء ذي معنى. جمع البيانات أولا.
- **تكلفة الخطأ كارثية وتحتاج إلى صحة مضمونة.** حساب الجرعات الطبية، التحكم في المفاعل النووي، التحقق من التشفير. ML النماذج احتمالية. وسوف يكونون مخطئين في بعض الأحيان. إذا كانت عبارة "خطأ في بعض الأحيان" غير مقبولة، فاستخدم الأساليب الحتمية.
- **يحل جدول البحث أو الاستدلال المشكلة.** إذا كان الحد البسيط أو الجدول يغطي 99% من الحالات، فإن إضافة ML تزيد من تكلفة الصيانة دون تحسين ذي معنى.
- **لا يمكنك شرح القرار، وقابلية التفسير مطلوبة.** تتطلب الصناعات الخاضعة للتنظيم (الإقراض، والتأمين، والعدالة الجنائية) في بعض الأحيان أن يكون كل قرار قابلاً للتفسير بشكل كامل. بعض نماذج ML قابلة للتفسير (الانحدار الخطي، أشجار القرار الصغيرة). معظمهم ليسوا كذلك.
...
يطبق الكود الموجود في `code/ml_intro.py` أقرب مصنف للنقطه الوسطى من الصفر، وهو أبسط خوارزمية ML ممكنة. فهو يوضح الفكرة الأساسية: التعلم من البيانات، ثم التنبؤ بالبيانات الجديدة.
...
أقرب مصنف للنقطه الوسطى بسيط للغاية. لا يحتوي على معلمات مفرطة ولا تكرار ولا نزول متدرج. ومع ذلك فهو يجسد النمط ML الأساسي:
...
كل خوارزمية ML، بدءًا من الانحدار اللوجستي وحتى المحولات، تتبع نفس النمط المكون من ثلاث خطوات. يصبح التمثيل أكثر تعقيدًا، لكن سير العمل يظل كما هو.
...
|------|----------------|----------------------|
| نموذج | "الAI" | دالة رياضية ذات معلمات قابلة للتعلم تقوم بتعيين المدخلات إلى المخرجات |
| التدريب | "تعليم AI" | تشغيل خوارزمية تحسين لضبط معلمات النموذج بحيث تتطابق التوقعات مع المخرجات المعروفة |
| ميزة | "عمود الإدخال" | خاصية قابلة للقياس للبيانات التي يستخدمها النموذج لـ make التنبؤات |
...
- [مقدمة للتعلم الإحصائي](https://www.statlearning.com/) - كتاب مدرسي مجاني يغطي جميع أساليب ML الكلاسيكية مع أمثلة عملية
- [الدورة التدريبية المكثفة للتعلم الآلي من Google](https://developers.google.com/machine-learning/crash-course) - مقدمة مرئية موجزة لمفاهيم ML
- [دليل مستخدم Scikit-learn](https://scikit.org/stable/user_guide.html) - المرجع العملي لتنفيذ ML في بايثون
