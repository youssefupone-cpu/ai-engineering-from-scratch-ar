...
- قارن تسوية L1 (الاختيار المضمن) مع RFE (اختيار المجمع) وقم بتقييم المفاضلات الحسابية الخاصة بهما
- إنشاء تحديد ميزة pipeline الذي يجمع بين أساليب متعددة وإظهار التعميم المحسن على البيانات المعلقة
## المشكلة
...
RFE هي طريقة مجمعة. يستخدم أهمية الميزة الخاصة بالنموذج للتقليم بشكل متكرر:
...
RFE يأخذ في الاعتبار تفاعلات الميزات لأن النموذج يرى جميع الميزات المتبقية معًا. تؤدي إزالة ميزة واحدة إلى تغيير أهمية الميزات الأخرى. هذا make أكثر شمولاً من طرق التصفية.
...
لماذا بالضبط صفر؟ تنشئ عقوبة L1 منطقة تقييد على شكل معين في مساحة الوزن. الحل الأمثل يميل إلى الهبوط في زاوية هذه الماسة، حيث يكون وزن واحد أو أكثر صفرًا. L2 يؤدي التسوية (الحافة) إلى إنشاء قيد دائري حيث تتقلص الأوزان ولكنها نادرًا ما تصل إلى الصفر.
...
### أهمية الميزة المستندة إلى الشجرة
...
تنبيه: تنحاز الأهمية المستندة إلى الشجرة نحو الميزات ذات القيم الفريدة العديدة (القيمة الأساسية العالية). سيظهر عمود ID العشوائي مهمًا لأنه يقسم كل عينة بشكل مثالي. استخدم أهمية التقليب كفحص للعقلانية.
### أهمية التقليب
الطريقة اللانموذجية:
...
## بنائها
### الخطوة 1: إنشاء بيانات تركيبية ذات بنية ميزات معروفة
```python
import numpy as np
...
### الخطوة 3: المعلومات المتبادلة (منفصلة)
```python
...
### الخطوة 6: الأهمية المستندة إلى الشجرة (شجرة القرار البسيطة)
```python
def gini_impurity(y):
    if len(y) == 0:
        return 0.0
...
### الخطوة 7: تشغيل جميع الطرق والمقارنة
...
## استخدمه
باستخدام scikit-learn، يتم تضمين تحديد الميزة في السطر pipe:
```python
...
تُظهر التطبيقات من البداية بالضبط ما يحدث داخل كل طريقة. عتبة التباين هي مجرد حساب `var(X, axis=0)` وتطبيق قناع. المعلومات المتبادلة هي حساب الترددات المشتركة والهامشية في جدول الطوارئ. RFE عبارة عن حلقة للتدريب والترتيب والتهذيب. L1 هو نزول متدرج بخطوة عتبة ناعمة. أهمية الشجرة تتراكم تخفيضات الشوائب عبر الانقسامات. لا يوجد سحر - فقط إحصائيات وحلقات.
...
## تمارين
1. **اختيار للأمام**: قم بتنفيذ عكس RFE. ابدأ بميزات صفرية. في كل خطوة، قم بإضافة الميزة التي تعمل على تحسين أداء النموذج إلى أقصى حد. توقف عندما لا تساعد إضافة الميزات. قارن الميزات المحددة بنتائج RFE. أيهما أسرع؟ أيهما يعطي نتائج أفضل؟
2. **اختيار الاستقرار**: قم بتشغيل L1 لتحديد الميزة 50 مرة، في كل مرة على عينة فرعية عشوائية بنسبة 80% من البيانات، مع قيم ألفا مختلفة قليلاً. حساب عدد المرات التي يتم فيها تحديد كل ميزة. الميزات المحددة في أكثر من 80% من عمليات التشغيل تكون "مستقرة". قارن الميزات الثابتة مع تحديد L1 الذي يتم تشغيله مرة واحدة. وهو أكثر موثوقية؟
...
4. **اختيار الميزة pipeline**: عتبة تباين السلسلة، ومرشح المعلومات المتبادلة، وRFE في سطر pipe واحد. قم أولاً بإزالة ميزات التباين القريبة من الصفر، ثم احتفظ بأعلى 50% من خلال المعلومات المتبادلة، ثم قم بتشغيل RFE على الناجين. قارن سطر pipe هذا بتشغيل RFE بمفرده على كافة الميزات. هل خط pipe أسرع؟ هل هي دقيقة بنفس القدر؟
5. ** أهمية التقليب من الصفر **: تنفيذ أهمية التقليب. بالنسبة لكل ميزة، قم بتبديل قيمها 10 مرات، وقم بقياس متوسط ​​الانخفاض في درجة F1. قارن الترتيب بالأهمية القائمة على الشجرة. ابحث عن الحالات التي يختلفون فيها واشرح السبب (تلميح: الميزات المرتبطة).
## المصطلحات الرئيسية
...
| الطريقة المضمنة | "النموذج يختار الميزات أثناء التدريب" | اختيار الميزات الذي يحدث كجزء من ملاءمة النموذج، مثل تنظيم L1 الذي يقود الأوزان إلى الصفر |
...
## مزيد من القراءة
- [An Introduction to Variable and Feature Selection (Guyon & Elisseeff, 2003)](https://jmlr.org/papers/v3/guyon03a.html) -- الاستبيان التأسيسي حول طرق اختيار الميزات، والذي لا يزال يُشار إليه على نطاق واسع
- [scikit-learn Feature Selection Guide](https://scikit-learn.org/stable/modules/feature_selection.html) -- مرجع عملي للتصفية، والمغلف، والأساليب المضمنة مع أمثلة التعليمات البرمجية
- [Stability Selection (Meinshausen & Buhlmann, 2010)](https://arxiv.org/abs/0809.2932) -- يجمع بين أخذ العينات الفرعية واختيار الميزات للحصول على نتائج قوية وقابلة للتكرار
- [Beware Default Random Forest Importances (Strobl et al., 2007)](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/1471-2105-8-25) -- يوضح التحيز الأساسي في الأهمية المستندة إلى الشجرة ويقترح الأهمية المشروطة كبديل