...
**النوع:** تعلم
** اللغة: ** بايثون
**المتطلبات:** المرحلة الثانية، الدروس 01-09 (ML الأساسيات، الانحدار، التصنيف، التقييم)
**الوقت:** ~75 دقيقة
...
- شرح كيفية استخدام تقنيات التنظيم (L1، L2، التسرب، الإيقاف المبكر) لتداول التباين
...
لا يمكنك تصغير كليهما في نفس الوقت لسعة نموذج ثابتة. ادفع الانحياز للأسفل وسيرتفع التباين. دفع التباين إلى الأسفل والتحيز يرتفع. إن فهم هذه المقايضة هو المهارة التشخيصية الأكثر فائدة في التعلم الآلي. فهو يخبرك ما إذا كان يجب make نموذجك أكثر تعقيدًا أو أقل تعقيدًا، وما إذا كنت تريد الحصول على المزيد من البيانات أو هندسة ميزات أفضل، وما إذا كنت تريد تنظيمًا أكثر أو أقل.
...
- `f(x)` هي الدالة الحقيقية
- `f_hat(x)` هو توقع النموذج الخاص بك
- `E[...]` هو التوقع على مجموعات التدريب المختلفة
- `y` هي العلامة المرصودة (الوظيفة الحقيقية بالإضافة إلى الضوضاء)
...
|-----------|------|--------|-------------|
| منخفض جدًا | HIGH | LOW | HIGH (النقصان) |
| مجرد حق | معتدل | معتدل | LOWEST |
| عالية جدًا | LOW | HIGH | HIGH (التجاوز) |
...
- **L1 (Lasso):** يدفع بعض الأوزان بالضبط إلى الصفر. ينفذ اختيار الميزة.
...
| انحياز عالي، مسطح | عالية | عالية ومسطحة | صغيرة ومسطحة | المزيد من البيانات سوف NOT مساعدة | تغيير بنية النموذج |
...
- يبدأ خطأ التدريب منخفضًا ويزداد كلما زادت صعوبة حفظ البيانات
...
يتضمن الكود أيضًا `demo_regularization_sweep()`، الذي يعمل على إصلاح كثير الحدود عالي الدرجة (الدرجة 15) ويكتسح قوة تنظيم Ridge من 0.001 إلى 100. وهذا يوضح مقايضة التباين والتحيز من زاوية مختلفة: بدلاً من تغيير تعقيد النموذج، نقوم بتغيير قوة القيد.
...
مؤامرة `train_mse` و `val_mse` مقابل `train_sizes`. يخبرك الشكل بكل شيء عن النموذج الخاص بك.
...
3. أضف تسوية L2 (انحدار ريدج) إلى التجربة. للحصول على كثير حدود ثابت عالي الدرجة (الدرجة 15)، قم بمسح لامدا من 0 إلى 100. ارسم التحيز ^ 2 والتباين كوظائف لامدا.
...
- [ناكيران وآخرون، النسب المزدوج العميق (2019)](https://arxiv.org/abs/1912.02292) - النسب المزدوج من حيث العصر والعينة
- [سكوت فورتمان-رو: فهم مقايضة التحيز والتباين](http://scott.fortmann-roe.com/docs/BiasVariance.html) -- شرح مرئي واضح
