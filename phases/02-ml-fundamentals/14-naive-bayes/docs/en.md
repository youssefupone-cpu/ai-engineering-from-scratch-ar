...
يتعامل Naive Bayes مع هذا. إنه make افتراض خاطئ رياضيًا (أن كل ميزة مستقلة عن كل ميزة أخرى معطاة للفئة)، ولا يزال يتفوق على النماذج "الأكثر ذكاءً" في تصنيف النص، خاصة مع مجموعات التدريب الصغيرة. يتدرب في تمريرة واحدة من خلال البيانات. إنه يتسع لملايين الميزات. وتنتج تقديرات الاحتمالات (على الرغم من سوء معايرتها في كثير من الأحيان بسبب افتراض الاستقلال).
...
- `P(features | class)` -- احتمالية رؤية هذه الكلمات في مستندات هذا الفصل
- `P(class)` -- الاحتمالية السابقة للفئة (ما مدى شيوع البريد العشوائي بشكل عام؟)
- `P(features)` -- الأدلة واحدة لجميع الفئات، لذلك يمكننا تجاهلها عند المقارنة
الفصل الذي حصل على أعلى `P(class | features)` يفوز.
### افتراض الاستقلال الساذج
تتطلب عملية حساب `P(features | class)` بالضبط تقدير الاحتمالية المشتركة لجميع الميزات معًا. مع مفردات مكونة من 10000 كلمة، ستحتاج إلى تقدير التوزيع لأكثر من 2^10000 مجموعة محتملة. مستحيل.
...
3. **يلغي تكرار الميزة. ** توفر الميزات المرتبطة أدلة متكررة. يحسب المصنف هذا الدليل مرتين، لكنه يحصيه مرتين للفئة الصحيحة أيضًا. إذا كانت "الآلة" و"التعلم" تظهران معًا دائمًا، فإن كلاهما يقدم دليلاً على فئة "التكنولوجيا". NB يحسبهم مرتين، لكنه يحسبهم مرتين للفصل الصحيح.
...
البريد العشوائي يفوز بهامش كبير. إن ظهور كلمة "مجاني" مرتين هو دليل قوي على وجود البريد العشوائي. لاحظ أن عدم ظهور "الاجتماع" يساهم بصفر في مجموعي السجل (0 * سجل (P)) -- في متعدد الحدود NB، الكلمات الغائبة ليس لها أي تأثير. إن برنولي NB هو الذي يصمم بوضوح غياب الكلمات.
### ثلاثة خيارات
يأتي Naive Bayes بثلاث نكهات. كل نماذج `P(feature | class)` بشكل مختلف.
#### بايز الساذجة متعددة الحدود
نماذج كل ميزة كعدد. الأفضل للبيانات النصية حيث تكون الميزات عبارة عن تكرارات الكلمات أو قيم TF-IDF.
```
...
| برنولي | ثنائي (0/1) | نص قصير، ناقلات الميزة الثنائية | SMS البريد العشوائي، ميزات الحضور/الغياب |
### تجانس لابلاس
ماذا يحدث عندما تظهر كلمة في بيانات الاختبار ولكنها لم تظهر مطلقًا في بيانات التدريب الخاصة بفصل معين؟
بدون تجانس: `P(word | class) = 0/N = 0`. صفر مضروب في المنتج بأكمله makes `P(class | features) = 0`، بغض النظر عن جميع الأدلة الأخرى. إن كلمة واحدة غير مرئية تدمر التنبؤ بأكمله، بغض النظر عن عدد الأدلة الأخرى التي تدعمه.
يضيف تجانس لابلاس عددًا صغيرًا `alpha` (عادةً 1) إلى كل عدد من الميزات:
```
...
### ساذج بايز مقابل الانحدار اللوجستي
...
القاعدة الأساسية: ابدأ بـ Naive Bayes. إذا كان لديك ما يكفي من البيانات وNB من الاستقرار، فقم بالتبديل إلى الانحدار اللوجستي.
### خط أنابيب التصنيف
```mermaid
flowchart LR
...
## بنائها
الكود الموجود في `code/naive_bayes.py` يطبق كلاً من MultinomialNB وGaussianNB من البداية.
### متعدد الحدودNB
التنفيذ من الصفر:
...
يستخدم التنبؤ PDF الغوسي لكل ميزة، مضروبًا في المعالم (تمت إضافته في مساحة السجل).
### العرض التوضيحي: تصنيف النص
...
### العرض التوضيحي: الميزات المستمرة
...
- **تجربة حجم التدريب:** كيف تتحسن دقة NB مع نمو بيانات التدريب من 20 إلى 1600 عينة. NB يصل إلى دقة جيدة حتى مع عدد قليل جدًا من العينات -- وهذه هي ميزته الرئيسية.
- **مصفوفة الارتباك:** الدقة والتذكر ودرجة F1 لكل فصل لإظهار أماكن أخطاء NB makes.
### سرعة التنبؤ
...
- GaussianNB: تقييمات Gaussian PDF، كل منها أكثر من d = O(n * d * k)
وكلاهما خطي في كل البعد. قارن ذلك بـ KNN (الذي يتطلب حساب المسافة إلى جميع نقاط التدريب) أو SVM مع RBF kernel (الذي يتطلب تقييم kernel مقابل جميع متجهات الدعم). NB أسرع من حيث الحجم في وقت التنبؤ.
## استخدمه
...
قيم TF-IDF ليست سالبة، لذا فهي تعمل مع MultinomialNB. يعد الجمع بين TF-IDF + MultinomialNB أحد أقوى خطوط الأساس لتصنيف النص. وكثيرًا ما يتفوق على النماذج الأكثر تعقيدًا في مجموعات البيانات التي تحتوي على أقل من 10000 عينة تدريبية.
### BernoulliNB للنص القصير
...
### معايرة NB الاحتمالات
NB تمت معايرة الاحتمالات بشكل سيئ. عندما يقول NB P(spam) = 0.95، قد يكون الاحتمال الحقيقي 0.7. إذا كنت بحاجة إلى تقديرات احتمالية موثوقة (على سبيل المثال، لتعيين حد أو للدمج مع نماذج أخرى)، فاستخدم CalibratedClassifierCV الخاص بـ sklearn:
```python
...
يناسب هذا الانحدار اللوجستي أعلى النتائج الأولية لـ NB باستخدام التحقق المتبادل. الاحتمالات الناتجة أقرب بكثير إلى ترددات الطبقة الحقيقية.
### مسكتك المشتركة
...
4. ** تحجيم الميزات. ** لا يحتاج MultinomialNB إلى القياس (يعمل على الأعداد). لا يحتاج GaussianNB إلى القياس أيضًا (فإنه يقوم بتقدير إحصائيات كل ميزة). وهذه ميزة مقارنة بالانحدار اللوجستي وSVM، والتي تعتبر حساسة لمقاييس الميزات.
## اشحنها
ينتج هذا الدرس:
- `outputs/skill-naive-bayes-chooser.md` -- مهارة اتخاذ القرار لاختيار متغير NB المناسب
- `code/naive_bayes.py` -- MultinomialNB وGaussianNB من البداية، مع مقارنة sklearn
### عندما يفشل ساذج بايز
يفشل NB عندما يتسبب افتراض الاستقلال في تصنيفات غير صحيحة (وليس فقط احتمالات غير صحيحة). يحدث هذا عندما:
1. **تفاعلات الميزات القوية.** إذا كان الفصل يعتمد على مزيج من ميزتين ولكن ليس بمفردهما (أنماط تشبه XOR)، فسوف يفتقده NB تمامًا. لا توفر كل ميزة بمفردها أي دليل، ولا يمكن لـ NB دمجها بشكل غير خطي.
2. **الميزات المترابطة بشكل كبير مع الأدلة المتعارضة.** إذا كانت الميزة أ تقول "بريد عشوائي" والميزة ب تقول "ليس بريدًا عشوائيًا"، ولكن A وB مرتبطان تمامًا (هما يتفقان دائمًا في الواقع)، فسيرى NB أدلة متضاربة حيث لا يوجد أي منها.
...
2. **اختبار استقلالية الميزة.** خذ مجموعة بيانات نصية حقيقية. اختر كلمتين مرتبطتين بشكل واضح ("الآلة" و"التعلم"). احسب P(word1 | class) * P(word2 | class) وقارنه بـ P(word1 AND word2 | class). ما مدى خطأ افتراض الاستقلال؟ هل يؤثر ذلك على دقة التصنيف؟
...
4. **NB مقابل الانحدار اللوجستي.** تدريب كليهما على البيانات النصية. ابدأ بـ 100 عينة تدريبية وقم بزيادة العدد إلى 10000. دقة المؤامرة مقابل حجم مجموعة التدريب لكليهما. في أي نقطة يتفوق الانحدار اللوجستي على Naive Bayes؟
...
## مزيد من القراءة
- [scikit-learn Naive Bayes docs](https://scikit-learn.org/stable/modules/naive_bayes.html) -- جميع المتغيرات الثلاثة مع تفاصيل رياضية
- [McCallum and Nigam, A Comparison of Event Models for Naive Bayes Text Classification (1998)](https://www.cs.cmu.edu/~knigam/papers/multinomial-aaaiws98.pdf) -- المقارنة الكلاسيكية بين Multinomial و Bernoulli بالنسبة للنص
- [Rennie et al., Tackling the Poor Assumptions of Naive Bayes Text Classifiers (2003)](https://people.csail.mit.edu/jrennie/papers/icml03-nb.pdf) -- تحسينات على NB للنص
- [Ng and Jordan, On Discriminative vs. Generative Classifiers (2001)](https://ai.stanford.edu/~ang/papers/nips01-discriminativegenerative.pdf) -- يثبت أن NB يتقارب بشكل أسرع من LR ببيانات أقل