...
**الوقت:** ~75 دقيقة
## أهداف التعلم
- تنفيذ طرق الكشف عن شذوذ Z-score وIQR وIsolation Forest من البداية
...
### الخاضع للإشراف مقابل غير الخاضع للإشراف: المفاضلة
...
**ملاحظة عملية:** العامل 1.5 في IQR يتوافق مع الشعيرات الموجودة في مخطط الصندوق. النقاط الموجودة خارج الشعيرات هي قيم متطرفة محتملة. باستخدام 3.0 بدلاً من 1.5 makes، يكون الكاشف أكثر تحفظًا (إشارات أقل، وإيجابيات كاذبة أقل). يعتمد العامل الصحيح على قدرتك على تحمل الإنذارات الكاذبة.
### غابة معزولة
...
2. في كل node، اختر عنصرًا عشوائيًا وقيمة مقسمة عشوائيًا بين الحد الأدنى والحد الأقصى للعنصر
...
حيث `c(n)` هو طول المسار المتوقع لعدد n من العينات. النتيجة القريبة من 1 تعني حالة شاذة. النتيجة بالقرب من 0.5 يعني طبيعي. النتيجة القريبة من 0 تعني طبيعية جدًا (عميقة في التجمعات الكثيفة).
...
- `max_samples`: عدد العينات لكل شجرة. 256 هو الافتراضي في الورقة الأصلية. القيم الأصغر make الأشجار الفردية أقل دقة ولكنها تزيد من التنوع. المعاينة الفرعية هي ما تقوم به makes Isolation Forest بسرعة - حيث ترى كل شجرة جزءًا صغيرًا من البيانات.
- `contamination`: الجزء المتوقع من الحالات الشاذة. تستخدم فقط لتحديد العتبة. لا يؤثر على الدرجات نفسها.
### العامل الخارجي المحلي (LOF)
...
- LOF أكبر من 1.0 يعني كثافة أقل من الجيران (يحتمل أن تكون غير طبيعية)
...
| __المصطلح_1__ | المسافة ذات معنى | بطيء | سيئة | نعم |
### تحديات التقييم
...
- **AUROC مضلل.** في حالة عدم التوازن الشديد، يمكن أن يبدو AUROC جيدًا حتى عندما يتجاهل النموذج معظم الحالات الشاذة عند الحدود العملية.
- **مقاييس أفضل:** Precision@k (من بين العناصر التي تم وضع علامة عليها، كم عدد الحالات الشاذة الحقيقية)، AUPRC (المساحة الواقعة تحت منحنى الاسترجاع الدقيق)، والاستدعاء بمعدل إيجابي كاذب ثابت.
```mermaid
flowchart LR
...
السطر pipe لا يتم "إتمامه" أبدًا. وتتحول توزيعات البيانات، وتظهر أنواع جديدة من الشذوذ، وتحتاج العتبات إلى التعديل. تعامل مع اكتشاف الحالات الشاذة كنظام حي، وليس كنموذج لمرة واحدة.
## بنائها
...
يشمل الفصل `IsolationForest` عدة أشجار:
```python
...
ملاحظة `contamination` يعين الجزء المتوقع من الحالات الشاذة. إن ضبطه بشكل صحيح أمر مهم - فالمستوى المنخفض جدًا يخطئ في حدوث حالات شاذة، أما المستوى المرتفع جدًا فيؤدي إلى إنشاء إنذارات كاذبة.
يقارن الكود الموجود في `anomaly_detection.py` عمليات التنفيذ من البداية مع sklearn على نفس البيانات.
### معلمة التلوث sklearn
...
كلاهما ينتج نفس درجات الشذوذ. لكن `iso_5` يضع علامة على أعلى 5% بينما يشير `iso_10` إلى أعلى 10%. إذا كنت لا تعرف معدل الشذوذ الحقيقي (عادةً لا تعرفه)، فاضبط التلوث على "تلقائي" واعمل مع النتائج الأولية مباشرةً. قم بتعيين الحد الخاص بك بناءً على مقايضة التكلفة بين الإيجابيات الكاذبة والسلبيات الكاذبة.
### فئة واحدة SVM
...
تقارب المعلمة `nu` نسبة الحالات الشاذة. تعمل الفئة الواحدة SVM بشكل جيد على مجموعات البيانات الصغيرة إلى المتوسطة ولكنها لا تتوسع إلى بيانات كبيرة جدًا (تنمو مصفوفة النواة بشكل تربيعي).
### نهج التشفير التلقائي (معاينة)
...
1. تشغيل أجهزة كشف متعددة (درجة Z، IQR، غابة العزلة، LOF)
...
### اختيار العتبة
درجة الشذوذ مستمرة. أنت بحاجة إلى حد make للقرارات الثنائية. وهذا قرار تجاري وليس قرارًا تقنيًا.
...
3. **LOF من الصفر.** تنفيذ العامل الخارجي المحلي باستخدام أقرب جيران. قارن مع LocalOutlierFactor الخاص بـ sklearn على نفس البيانات. استخدم k=10 وk=50 - كيف يؤثر اختيار k على النتائج؟
...
5. **التقييم الواقعي.** خذ مجموعة بيانات تحتوي على حالات شاذة معروفة (عملية احتيال على بطاقة الائتمان من Kaggle، على سبيل المثال). قم بتقييم جميع الطرق الأربع باستخدام الدقة @ 100، والدقة @ 500، وAUPRC. ما هي الطريقة الأفضل؟ لماذا؟
## المصطلحات الرئيسية
...
| IQR | "المدى الربيعي" | Q3 - Q1، قياس انتشار الـ 50% الوسطى من البيانات، المستخدمة للكشف القوي عن القيم المتطرفة |
...
| __المصطلح_3__ | "المنطقة تحت منحنى الاسترجاع الدقيق" | مقياس يلخص أداء الاستدعاء الدقيق عبر كافة الحدود، وهو أفضل من AUROC للبيانات غير المتوازنة |
## مزيد من القراءة
- [Liu et al., Isolation Forest (2008)](https://cs.nju.edu.cn/zhouzh/zhouzh.files/publication/icdm08b.pdf) -- ورقة Isolation Forest الأصلية
- [Breunig et al., LOF: Identifying Density-Based Local Outliers (2000)](https://dl.acm.org/doi/10.1145/342009.335388) -- الورقة LOF الأصلية
- [scikit-learn Outlier Detection docs](https://scikit-learn.org/stable/modules/outlier_detection.html) -- نظرة عامة على جميع أجهزة كشف الشذوذ sklearn
- [Chandola et al., Anomaly Detection: A Survey (2009)](https://dl.acm.org/doi/10.1145/1541880.1541882) -- مسح شامل لطرق الكشف عن الشذوذ
- [Goldstein and Uchida, A Comparative Evaluation of Unsupervised Anomaly Detection Algorithms (2016)](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0152173) -- مقارنة تجريبية لـ 10 طرق على مجموعات بيانات حقيقية