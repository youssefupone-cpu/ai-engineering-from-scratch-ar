...
## المشكلة
لديك بيانات مرتبة حسب الوقت. المبيعات اليومية، درجة الحرارة بالساعة، لكل دقيقة استخدام CPU، أسعار الأسهم الأسبوعية. تريد التنبؤ بالقيمة التالية، الأسبوع المقبل، الربع القادم.
يمكنك الوصول إلى مجموعة أدوات ML القياسية الخاصة بك: تقسيم التدريب/الاختبار العشوائي، والتحقق المتبادل، وإدخال مصفوفة الميزات، والتنبؤ بالخروج. كل خطوة خاطئة.
تكسر السلاسل الزمنية الافتراضات التي يعتمد عليها المعيار ML. العينات ليست مستقلة - درجة الحرارة اليوم تعتمد على درجة حرارة الأمس. تؤدي الانقسامات العشوائية إلى تسرب المعلومات المستقبلية إلى الماضي. الميزات التي تبدو رائعة في الاختبار الخلفي تفشل في الإنتاج لأنها تعتمد على الأنماط التي تتغير بمرور الوقت.
...
يغطي هذا الدرس الأساسيات: ما هي الاختلافات في بيانات الوقت في make، وكيفية تقييم النماذج بأمانة، وكيفية تحويل السلسلة الزمنية إلى ميزات يمكن أن تستهلكها نماذج ML القياسية.
##المفهوم
...
في المعيار ML، تكون العينات قابلة للتبديل. خلطهم لا يغير شيئا. في السلاسل الزمنية، النظام هو كل شيء. الخلط يدمر الإشارة.
...
إذا لم تؤدي جولة واحدة من التفاضل إلى make إلى ثبات السلسلة، فقم بتطبيقها مرة أخرى (تفاضل من الدرجة الثانية). تحتاج معظم سلاسل العالم الحقيقي إلى جولتين على الأكثر.
**مثال:**
...
**الاختبار الرسمي:** اختبار ديكي فولر المعزز (ADF) هو الاختبار الإحصائي القياسي للثبات. الفرضية الصفرية هي أن "السلسلة غير ثابتة." القيمة p أقل من 0.05 تعني أنه يمكنك رفض القيمة الفارغة وإنهاء الثبات. نحن لا ننفذ ADF من الصفر (يتطلب جداول توزيع مقاربة)، ولكن نهج الإحصائيات المتداولة في الكود الخاص بنا يوفر فحصًا مرئيًا عمليًا.
### الارتباط التلقائي
يقيس الارتباط التلقائي مدى ارتباط القيمة في الوقت t بالقيمة في الوقت t-k (خطوات k في الماضي). وترسم دالة الارتباط الذاتي (ACF) هذا الارتباط لكل تأخر k.
**ACF يخبرك بما يلي:**
- إلى أي مدى يعود المسلسل إلى الذاكرة. إذا انخفض ACF إلى الصفر بعد التأخر 5، فلن تكون القيم التي مضت قبل أكثر من 5 خطوات غير ذات صلة.
- ما إذا كانت الموسمية موجودة. إذا ارتفع ACF في التأخر 12 (بيانات شهرية)، فهناك موسمية سنوية.
- كم عدد الميزات المتأخرة التي سيتم إنشاؤها. استخدم الفترات الفاصلة حتى يصبح ACF مهملاً.
**PACF (وظيفة الارتباط التلقائي الجزئي)** تزيل الارتباطات غير المباشرة. إذا كان اليوم يرتبط بـ 3 أيام مضت فقط لأن كلاهما يرتبط بالأمس، فإن PACF في التأخر 3 سيكون صفرًا بينما ACF في التأخر 3 لن يكون كذلك.
### ميزات التأخر: تحويل السلاسل الزمنية إلى تعلم خاضع للإشراف
تحتاج نماذج ML القياسية إلى مصفوفة ميزات X وهدف y. تمنحك السلسلة الزمنية عمودًا واحدًا من القيم. الجسر متخلف الميزات.
...
** كم عدد التأخرات؟ ** استخدم وظيفة الارتباط التلقائي. إذا كان ACF هامًا حتى التأخر 10، استخدم 10 تأخيرات على الأقل. إذا كانت هناك موسمية أسبوعية، فقم بتضمين التأخر 7 (وربما 14). المزيد من التأخير يمنح النموذج مزيدًا من التاريخ ولكن أيضًا المزيد من الميزات التي تناسبه، مما يزيد من خطر التجهيز الزائد.
...
### التحقق من صحة السير إلى الأمام
...
### ARIMA الحدس
ARIMA هو نموذج السلاسل الزمنية الكلاسيكية. لديها ثلاثة مكونات:
- **AR (الانحدار التلقائي):** التنبؤ من القيم السابقة. يستخدم AR(p) قيم p الأخيرة.
- **أنا (المتكامل):** التفاضل لتحقيق الثبات. I(d) يطبق جولات d من التفاضل.
- **MA (المتوسط ​​المتحرك):** توقع من أخطاء التنبؤ السابقة. MA(q) يستخدم أخطاء q الأخيرة.
ARIMA(p, d, q) يجمع بين الثلاثة. يمكنك اختيار p وd وq استنادًا إلى تحليل ACF/PACF أو البحث الآلي (auto-ARIMA).
لن نقوم بتنفيذ ARIMA من الصفر -- فهو يتطلب تحسينًا رقميًا يتجاوز نطاق هذا الدرس. الفكرة الأساسية هي فهم ما يفعله كل مكون حتى تتمكن من تفسير نتائج ARIMA ومعرفة متى تستخدمها.
### متى تستخدم ماذا
| النهج | الأفضل لـ | يعالج الموسمية | يعالج الميزات الخارجية |
|----------|--------|-------------------|--------|
| ميزات التأخر + ML | جدولي مع العديد من الميزات الخارجية | مع ميزات التقويم | نعم |
| __المصطلح_1__ | سلسلة وحيدة المتغير، قصيرة المدى | SARIMA البديل | لا (ARIMAX محدود) |
...
### الأخطاء الشائعة في السلاسل الزمنية
...
## بنائها
يقوم الكود الموجود في `code/time_series.py` بتنفيذ وحدات البناء الأساسية من البداية.
### صانع ميزة التأخر
```python
...
يؤدي هذا إلى تحويل سلسلة 1D إلى مصفوفة ميزات حيث يحتوي كل صف على قيم `n_lags` الأخيرة كميزات، والقيمة الحالية كهدف.
### التحقق من صحة السير للأمام
```python
...
### نموذج الانحدار التلقائي البسيط
نموذج AR الخالص هو مجرد انحدار خطي على ميزات التأخر:
```python
class SimpleAR:
...
بالنسبة لـ ARIMA، استخدم statsmodels:
```python
...
وهذا يعادل `walk_forward_split` من الصفر ولكنه مدمج في إطار عمل التحقق من الصحة لـ sklearn. يمكنك استخدامه مع `cross_val_score`:
```python
...
- `code/time_series.py` -- ميزات التأخر، والتحقق من صحة التقدم، ونموذج AR، وفحوصات الثبات
### الخطوط الأساسية التي يجب عليك التغلب عليها
...
إذا خسر نموذج ML الرائع الخاص بك خط الأساس الموسمي الساذج، فهذا يعني أن لديك خطأ. الأكثر شيوعًا: التسرب المستقبلي للميزات، أو طريقة التقييم الخاطئة، أو أن تكون السلسلة عشوائية حقًا ولا يمكن التنبؤ بها.
### نصائح عملية
...
6. **سلسلة منحرفة لتحويل السجل.** غالبًا ما تكون الإيرادات والأسعار والأعداد منحرفة نحو اليمين. يؤدي أخذ السجل إلى استقرار التباين وإضافة الأنماط المضاعفة make، والتي يمكن للنماذج الخطية التعامل معها. التنبؤ في مساحة السجل، ثم الأس للعودة إلى الوحدات الأصلية.
## تمارين
...
5. **التنبؤ متعدد الخطوات.** قم بتعديل نموذج AR للتنبؤ بخمس خطوات للأمام بدلاً من خطوة واحدة. قارن بين استراتيجيتين: (أ) توقع خطوة واحدة، واستخدم التنبؤ كمدخل للخطوة التالية (العودية)، و(ب) تدريب نماذج منفصلة لكل أفق (مباشر). أيهما أكثر دقة؟
## المصطلحات الرئيسية
...
| __المصطلح_2__ | "نموذج السلاسل الزمنية الكلاسيكية" | المتوسط ​​المتحرك المتكامل التلقائي: يجمع بين القيم السابقة (AR)، والاختلاف (I)، والأخطاء السابقة (MA) |
...
## مزيد من القراءة
- [Hyndman and Athanasopoulos, Forecasting: Principles and Practice (3rd ed.)](https://otexts.com/fpp3/) -- أفضل كتاب مدرسي مجاني حول التنبؤ بالسلاسل الزمنية
- [scikit-learn Time Series Split](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.TimeSeriesSplit.html) -- مقسم التقدم للأمام لـ sklearn
- [statsmodels ARIMA docs](https://www.statsmodels.org/stable/generated/statsmodels.tsa.arima.model.ARIMA.html) -- ARIMA التنفيذ مع التشخيص
- [Makridakis et al., The M5 Competition (2022)](https://www.sciencedirect.com/science/article/pii/S0169207021001874) -- منافسة تنبؤية واسعة النطاق تعرض أساليب ML مقابل الأساليب الإحصائية