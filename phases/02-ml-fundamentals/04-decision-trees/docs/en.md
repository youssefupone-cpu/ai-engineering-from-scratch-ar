...
> شجرة القرار هي مجرد مخطط انسيابي. لكن الغابة منها هي واحدة من أقوى الأدوات في ML.
**النوع:** بناء
** اللغة: ** بايثون
...
- قارن أهمية ميزة MDI بأهمية التقليب وحدد متى يكون MDI متحيزًا
## المشكلة
...
يقوم كل node داخلي باختبار ميزة مقابل حد معين. كل ورقة node make هي توقع. لتصنيف نقطة بيانات جديدة، عليك أن تبدأ من الجذر وتتبع الفروع حتى تصل إلى ورقة.
يتم إنشاء الشجرة من أعلى إلى أسفل عن طريق اختيار، في كل node، الميزة والعتبة التي تفصل البيانات بشكل أفضل. يتم تعريف "الأفضل" بمعيار الانقسام.
### معايير الانقسام: قياس النجاسة
في كل node، لدينا مجموعة من العينات. نريد تقسيمها بحيث تكون العناصر الفرعية الناتجة nodes "نقية" قدر الإمكان، مما يعني أن كل طفل يحتوي على فئة واحدة في الغالب.
**شوائب جيني** تقيس احتمالية أن يتم تصنيف العينة المختارة عشوائيًا بشكل خاطئ إذا تم تصنيفها وفقًا لتوزيع الفئة في ذلك node.
```
Gini(S) = 1 - sum(p_k^2)
...
بالنسبة لـ node خالص (كل فئة واحدة)، Gini = 0. بالنسبة للتقسيم الثنائي بفئات 50/50، Gini = 0.5. أقل هو أفضل.
```
...
**الإنتروبيا** تقيس محتوى المعلومات (الفوضى) في node. تمت تغطيته في المرحلة الأولى الدرس 09.
```
Entropy(S) = -sum(p_k * log2(p_k))
```

بالنسبة لـ node النقي، الإنتروبيا = 0. بالنسبة للتقسيم الثنائي بنسبة 50/50، الإنتروبيا = 1.0. أقل هو أفضل.
```
Example: 6 cats, 4 dogs
...
### كيفية عمل التقسيم
بالنسبة لمجموعة بيانات تحتوي على ميزات n وعينات m في node الحالي:
1. لكل ميزة j (j = 1 إلى n):
   - فرز العينات حسب الميزة ي
   - جرب كل نقطة وسط بين القيم المميزة المتتالية كعتبة
   - حساب كسب المعلومات لكل عتبة
...
4. العتاب على كل طفل
هذا النهج الجشع لا يضمن الشجرة المثالية عالميًا. العثور على الشجرة المثالية أمر صعب NP. لكن التقسيم الجشع يعمل بشكل جيد في الممارسة العملية.
### شروط التوقف
...
- التقليم المعقد من حيث التكلفة (المستخدم بواسطة scikit-learn): يضيف عقوبة تتناسب مع عدد الأوراق. زيادة العقوبة للحصول على أشجار أصغر
...
مصدران للعشوائية make تنوع الأشجار:
...
**متوسط ​​الانخفاض في الشوائب (MDI):** لكل ميزة، قم بجمع إجمالي الانخفاض في الشوائب عبر جميع الأشجار وجميع nodes حيث يتم استخدام هذه الميزة. تعد الميزات التي تنتج تخفيضات أكبر في الشوائب عند الانقسامات السابقة أكثر أهمية.
```
...
### عندما تتغلب الأشجار على الشبكات العصبية
...
## بنائها
### الخطوة 1: شوائب جيني والإنتروبيا
...
## استخدمه
باستخدام scikit-learn، يتكون تدريب الغابة العشوائية من ثلاثة أسطر:
```python
...
5. أهمية تنفيذ التقليب. قارنها بأهمية MDI في مجموعة بيانات حيث تكون إحدى الميزات عبارة عن ضوضاء عشوائية ولكنها ذات قيمة أساسية عالية. MDI سوف يصنف ميزة الضوضاء بدرجة عالية. سوف أهمية التقليب لا.
## المصطلحات الرئيسية
| مصطلح | ماذا يقول الناس | ماذا يعني في الواقع |
|------|----------------|----------------------|
| شجرة القرار | "مخطط انسيابي للتنبؤات" | نموذج يقوم بتقسيم المساحة إلى مناطق مستطيلة من خلال تعلم تسلسل تقسيمات if/else |
| نجاسة جيني | "ما مدى اختلاط node" | احتمال الخطأ في تصنيف عينة عشوائية في node. 0 = نقي، 0.5 = الحد الأقصى للشوائب للثنائي |
...
| أهمية التقليب | "خلط ورق اللعب والتحقق" | تنخفض الدقة عندما يتم تبديل قيم الميزة بشكل عشوائي. أكثر موثوقية من MDI بالنسبة للميزات المزعجة |
...
## مزيد من القراءة
- [Breiman: Random Forests (2001)](https://link.springer.com/article/10.1023/A:1010933404324) - ورقة الغابة العشوائية الأصلية
...
- [scikit-learn Decision Trees documentation](https://scikit-learn.org/stable/modules/tree.html) - دليل عملي بأدوات التصور
