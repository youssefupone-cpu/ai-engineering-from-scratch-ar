...
طرق المجموعة تفعل هذا بالضبط. إنها التقنية الأكثر موثوقية للفوز في مسابقات Kaggle على البيانات الجدولية، وهي تدعم معظم أنظمة الإنتاج ML، وتوضح مقايضة التحيز والتباين أثناء العمل. التعبئة تقلل من التباين. التعزيز يقلل من التحيز. يتعرف التراص على النماذج التي يجب الاعتماد عليها وعلى أي مدخلات.
##المفهوم
### لماذا تعمل الفرق؟
...
بالنسبة لـ 21 مصنفًا بدقة 60% لكل منها، تبلغ دقة تصويت الأغلبية حوالي 74%. وبوجود 101 مصنفًا ترتفع النسبة إلى 84%. يتم إلغاء الأخطاء عندما تكون النماذج make بها أخطاء مختلفة.
الشرط الأساسي هو **التنوع**. إذا كانت كافة النماذج make نفس الأخطاء، والجمع بينها لا يساعد شيئا. تعمل الفرق لأنها تنتج نماذج متنوعة من خلال:
...
### XGBoost: لماذا يهيمن على البيانات الجدولية
XGBoost (تعزيز التدرج الفائق) هو تعزيز التدرج من خلال التحسينات الهندسية التي make سريعة ودقيقة ومقاومة للتجاوز:
- **الهدف المنتظم:** عقوبات L1 وL2 على أوزان الأوراق تمنع الأشجار الفردية من أن تكون واثقة جدًا
...
- **بنية الكتلة المدركة لذاكرة التخزين المؤقت:** تم تحسين تخطيط الذاكرة لخطوط ذاكرة التخزين المؤقت CPU
...
## بنائها
### الخطوة 1: قرار اتخاذ القرار (المتعلم الأساسي)
الكود الموجود في `code/ensembles.py` ينفذ كل شيء من البداية. نبدأ بجذع القرار: شجرة ذات شق واحد.
```python
class DecisionStump:
    def __init__(self):
...
### الخطوة 3: تعزيز التدرج من الصفر
```python
...
### الخطوة 4: المقارنة مع sklearn
يتحقق الكود من أن تطبيقاتنا من البداية تنتج دقة مماثلة لـ `AdaBoostClassifier` و`GradientBoostingClassifier` الخاصين بـ sklearn، وتقارن جميع الطرق جنبًا إلى جنب.
## استخدمه
...
### حزمة الإنتاج للبيانات الجدولية
...
## مزيد من القراءة
- [Schapire & Freund: Boosting: Foundations and Algorithms](https://mitpress.mit.edu/9780262526036/) -- الكتاب من تأليف منشئي AdaBoost
- [Friedman: Greedy Function Approximation: A Gradient Boosting Machine (2001)](https://statweb.stanford.edu/~jhf/ftp/trebst.pdf) -- ورقة تعزيز التدرج الأصلية
- [Chen & Guestrin: XGBoost (2016)](https://arxiv.org/abs/1603.02754) -- ورقة XGBoost
- [Wolpert: Stacked Generalization (1992)](https://www.sciencedirect.com/science/article/abs/pii/S0893608005800231)--ورقة التجميع الأصلية
- [scikit-learn Ensemble Methods](https://scikit-learn.org/stable/modules/ensemble.html) -- مرجع عملي