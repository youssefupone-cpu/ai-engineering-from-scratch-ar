(function () {
  ...
  } else if (window.matchMedia && window.matchMedia('(يفضل نظام الألوان: داكن)').matches) {
  ...
  function renderPhases() {
    ...
    // أعد تطبيق التأخيرات المتدرجة لكل صف للصفوف التي تم إنشاؤها حديثًا.
    initStaggerIndex();

    // إذا تم بالفعل تهيئة مراقب الكشف (تم تعيين body.js-anim)،
    // يقوم IntersectionObserver بمراقبة الصفوف * الأصلية * فقط
    // تعطى عند بدء التشغيل. إعادة العرض عبر الداخليHTML يستبدل nodes بـ
    // العناصر الجديدة تمامًا التي يتم ملاحظتها NOT، لذا فإنها ستفعل خلاف ذلك
    // ابق مخفيًا إلى الأبد تحت `body.js-anim.toc-row { opacity: 0 }`.
    //
    // نظرًا لأن المستخدم قد شاهد بالفعل الرسوم المتحركة للكشف الأولي، فما عليك سوى وضع علامة عليها
    // الصفوف المعاد بناؤها تظهر على الفور (بدون تلاشي ثانٍ).
  ...
  function initModal() {
    ...
    if (resetBtn) {
      resetBtn.addEventListener('click', function () {
        ...
        var ok = window.confirm('هل تريد مسح كل تقدمك المحلي (إجابات الاختبار والدروس المكتملة)؟ لا يمكن التراجع عن هذا.');
        if (!ok) return;
  ...
  function renderModalLessons(p) {
    ...
    for (var i = 0; i < p.lessons.length; i++) {
      ...
      html += '<span class="modal-lesson-status ' + statusClass + '"' + (userComplete ? 'title="لقد أكملت هذا الدرس"' : '') + '></span>';
      if (l.url) {
      ...
      if ((l.status === 'complete' || userComplete) && lessonPath) {
        actionHtml = '<a href="lesson.html?path=' + lessonPath + '" class="modal-lesson-read">' + (userComplete ? 'راجع' : 'اقرأ') + '</a>';
      }
      var toggleHtml = '';
      if (hasProgress && lessonPath) {
        toggleHtml = '<button type="button" class="modal-lesson-toggle' + (userComplete ? ' done' : '') + '" مسار البيانات ="' + lessonPath + '" العنوان ="' + (userComplete ? 'تمييز كغير مكتمل' : 'تمييز كمكتمل') + '" aria-label ="' + (userComplete ? 'تمييز كغير مكتمل' : 'تمييز كمكتمل') + '">' + (userComplete ? '✓' : '+') + '</button>';
      }
  ...
  function initFadeObserver() {
    var prefersReduced = window.matchMedia && window.matchMedia('(يفضل-الحركة المنخفضة: يقلل)').matches;
    ...
    }, { threshold: 0.12, rootMargin: '0 بكسل 0 بكسل -40 بكسل 0 بكسل' });
  ...
  function initScrollExplode() {
    ...
    if (window.matchMedia && window.matchMedia('(يفضل-الحركة المنخفضة: يقلل)').matches) {
  ...
  function applyExplode(container, progress) {
    // يتم تحريك كل طبقة / تسمية على نافذتها الخاصة في [stagger_start, stagger_start + window].
    // الكشف المتسلسل: تنتظر الطبقة N حتى تستقر الطبقة N-1 في الغالب قبل البدء.
    var STAGGER_DENOM = 720; // أعلى → فجوات أوسع بين مداخل الطبقة
    var WINDOW = 0.55;       // مدة الرسوم المتحركة المحلية لكل طبقة تمثل جزءًا صغيرًا من التقدم العالمي
    ...
    function localProgress(staggerAttr) {
      ...
      if (local < 0) local = 0;
      if (local > 1) local = 1;
      // سهولة الخروج مكعب على الجزء المحلي
